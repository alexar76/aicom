...
def _guard_round_regression(product: dict, pid: str, qa_result: object, host) -> None:
    ...
    product["surrogate_repair_hint"] = revert_hint(int(previous), int(score), outstanding)
    product["reverted_round_count"] = int(product.get("reverted_round_count") or 0) + 1
    ...
    logger.error(
        ...
        "restored the tree QA last measured. Rounds reverted so far: %s",
        pid, previous, score, product["reverted_round_count"],
    )
                ...
                dev_task = {
                    ...
                    "input_data": {
                        ...
                        "qa_findings": [
                            ...
                            for issue in (live_gate_payload.get("issues") or [])[:20]
                        ],
                        "quality_gates_feedback": {
                            "passed": False,
                            "blocking_defects": list(live_gate_payload.get("issues") or []),
                        ...
                        "quality_repair_round": int(products[pid].get("quality_repair_round") or 0)
                        if pid in products
                        else 0,
                        "qa_gate_blocked": True,
