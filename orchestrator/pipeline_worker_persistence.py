...
class PipelineStatePersistence:
    ...
    def __init__(self, *, state_file: Path | None = None, use_sql_store: bool = False) -> None:
        ...
        # Single authoritative store per run: ``use_sql_store`` is captured once at
        # construction and never flipped mid-run, so load_async/save_async always hit the
        # same backend. This avoids the JSON↔SQL split-brain where one store reads stale
        # data the other already advanced (lost updates during migration).
        self.use_sql_store = use_sql_store
        self._async_store: Any | None = None
        logger.info(
            "Pipeline persistence authoritative store: %s",
            "SQL" if use_sql_store else f"JSON ({self.state_file})",
        )
    ...
    async def save_async(self, state: dict) -> bool:
        ...
        try:
            ...
            if use_dirty and not dirty_p and not dirty_t:
                return True
            if use_dirty:
                # Keep compound advances consistent: if a dirty task belongs to a product
                # that was NOT itself marked dirty, persist that product too. Otherwise a
                # dirty save could write the new next-task while leaving the product's
                # advanced state behind (or vice versa) — the O6 torn/lost-update case.
                if dirty_t:
                    dirty_task_pids = {
                        str(t.get("product_id") or "")
                        for t in task_queue
                        if t.get("id") in dirty_t and t.get("product_id")
                    }
                    dirty_p |= {pid for pid in dirty_task_pids if pid in products_map}
