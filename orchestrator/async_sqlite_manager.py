...
class AsyncSQLiteManager:
    ...
    async def initialize(self) -> None:
        ...
        try:
            await self._conn.execute("ALTER TABLE products ADD COLUMN extras TEXT")
        except Exception:
            logger.debug("products.extras alter skipped", exc_info=True)
        try:
            await self._conn.execute("ALTER TABLE products ADD COLUMN generic_metadata TEXT")
        except Exception:
            logger.debug("products.generic_metadata alter skipped", exc_info=True)
    ...
    def _rows_to_task_dicts(self, rows: list[dict]) -> list[dict]:
        ...
        for d in rows:
            ...
            d["timeout_sec"] = 30
            # Honor the configured retry ceiling (AIFACTORY_TASK_MAX_RETRIES,
            # default 7) — hardcoding 3 made the async worker retire tasks earlier
            # than the sync path and the configured limit.
            try:
    ...
    async def upsert_product(self, product: dict) -> None:
        ...
        extras_json = extras_to_json(extract_product_extras(product))
        # INSERT OR REPLACE rewrites the whole row, so generic_metadata MUST be
        # written too — omitting it nulled out the forward-compatible fields on
        # every async save.
        generic_json = extras_to_json(extract_generic_metadata(product))
        await self.execute(
            """INSERT OR REPLACE INTO products
               (id, workspace_id, idea, state, created_at, updated_at, spec, architecture, tags, category, monetization_scheme, evolution_history, error, current_task_id, extras, generic_metadata)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (
                product["id"],
