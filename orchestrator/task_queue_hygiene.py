...
def missing_forward_task(product: dict, task_queue: list) -> dict | None:
    ...
    if pstate == "HUMAN_REVIEW_PENDING":
        ...
        if kind in ("qa_repair_exhausted", "security_repair_exhausted", "qa_repair_stuck",
                    "live_mesh_payment_ops", "crash_loop_parked"):
            return None
...
def reopen_completed_with_critical_defects(products: dict, data_root=None) -> list[str]:
    ...
    for pid, product in list(products.items()):
        ...
        # Auto-recovery COMPLETED+locked+pipeline_on_hold left products stranded after this
        # flip: state became BUG_FOUND but hold/lock stayed, so idle-heal skipped them and
        # hygiene only logged "needs an operator reopen". Clear pause so Phase 4c can queue
        # developer / DEV_FIXING.
        ...
        try:
            ...
            set_product_improvement_on_hold(pid, False)
        except Exception:
            logger.debug("sharper reopen could not clear holds for %s", pid, exc_info=True)
        _REOPENED_FOR_RULES[pid] = SCORING_RULES_VERSION
        reopened.append(pid)
        logger.warning(
