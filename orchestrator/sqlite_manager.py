...
from .product_extras import (
    extract_generic_metadata,
    extract_product_extras,
    extras_from_json,
    extras_to_json,
    merge_generic_metadata,
...
class SQLiteManager:
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            log_suppressed(logger, "non-fatal (orchestrator/sqlite_manager.py)", exc_info=_suppressed_exc)
        try:
            self._conn.execute("ALTER TABLE products ADD COLUMN extras TEXT")
        except sqlite3.OperationalError as _suppressed_exc:
            log_suppressed(logger, "non-fatal (orchestrator/sqlite_manager.py)", exc_info=_suppressed_exc)
        # Forward-compatible catch-all: unknown top-level product fields not in a
        # dedicated column or the extras allow-list are persisted here as JSON, so
        # new product fields survive a round-trip without a schema/allow-list change.
        try:
    ...
    def _row_to_product_dict(row: sqlite3.Row) -> dict[str, Any]:
        ...
        d["metadata"] = metadata
        merge_product_extras(d, extras_from_json(d.pop("extras", None)))
        merge_generic_metadata(d, extras_from_json(d.pop("generic_metadata", None)))
        return d
    ...
    def _product_dict_to_sql_values(product: dict) -> dict[str, Any]:
        ...
        values: dict[str, Any] = {
            ...
            "extras": extras_to_json(extract_product_extras(product)),
            "generic_metadata": extras_to_json(extract_generic_metadata(product)),
        }
        return values
            ...
            self.conn.execute(
                """INSERT OR REPLACE INTO products
                   (id, workspace_id, idea, state, created_at, updated_at,
                    ...
                    error, current_task_id, extras, generic_metadata)
                   VALUES
                   (:id, :workspace_id, :idea, :state, :created_at, :updated_at,
                    :spec, :architecture, :tags, :category,
                    :monetization_scheme, :evolution_history,
    ...
    def delete_product(self, product_id: str) -> None:
        """Delete a product and its associated tasks (atomic — tasks and product together)."""
        with self.conn:
    ...
    def claim_pending_task(self, task_id: str, started_at: float) -> bool:
        ...
        picked one and saved it as running. Two workers listing at the same time both
        ...
        with self.conn:
            cursor = self.conn.execute(
                """UPDATE tasks SET status = 'running', started_at = ?
                   WHERE id = ? AND workspace_id = ? AND lower(status) = 'pending'""",
    ...
    def advance_product_and_enqueue(
        ...
        """Atomically persist a product state advance together with its follow-on tasks.

        The pipeline's "advance product + enqueue next task" step touches two tables
        (``products`` and ``tasks``). Writing them in two independent transactions can
        leave the store in a torn state if the worker dies between commits — e.g. a
        product advanced to the next state with no task queued to drive it (stall), or
        a next task queued against a product whose state never moved. Wrapping both in
        one transaction makes the compound update all-or-nothing.

        Args:
            product: A dict matching the Product.to_dict() structure.
            tasks: Zero or more task dicts (Task.to_dict() structure) to upsert in the
                same transaction (typically the freshly created next task).
        """
        product_values = self._product_dict_to_sql_values(product)
        task_values = [self._task_dict_to_sql_values(t) for t in (tasks or [])]
        with self.conn:
            self.conn.execute(
                """INSERT OR REPLACE INTO products
                   (id, workspace_id, idea, state, created_at, updated_at,
    ...
    def get_recent_failed_tasks_by_product(self, product_id: str, *, limit: int = 24) -> list[dict]:
        """Recent failed tasks only — for admin failure banners (not full history)."""
        lim = max(1, min(int(limit), 100))
        rows = self.conn.execute(
            """
            SELECT * FROM tasks
            WHERE workspace_id = ? AND product_id = ? AND LOWER(status) = 'failed'
            ORDER BY COALESCE(completed_at, started_at, created_at) DESC
            LIMIT ?
            """,
            (self.workspace_id, product_id, lim),
        ).fetchall()
        return [self._row_to_task_dict(r) for r in rows]
    ...
    def get_active_task_counts_by_product(self) -> tuple[dict[str, int], dict[str, int]]:
        """Pending/running counts per product — no full task history load."""
        rows = self.conn.execute(
            """
            SELECT product_id, LOWER(status) AS st, COUNT(*) AS cnt
            FROM tasks
            WHERE workspace_id = ? AND LOWER(status) IN ('pending', 'running')
            GROUP BY product_id, LOWER(status)
            """,
            (self.workspace_id,),
        ).fetchall()
    ...
    def clear_all(self) -> None:
        """Delete all rows from both tables (for testing) — atomic."""
        with self.conn:
