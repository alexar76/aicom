...
# NOTE: `extras` and `generic_metadata` MUST be written here. They carry
# delivery_profile, operator_locked, owner_email and every other product field
# without a dedicated column (mirrors SQLiteManager.upsert_product). Omitting
# them silently dropped all product extras on every Postgres save.
_PRODUCT_UPSERT = """
INSERT INTO products (
    id, workspace_id, idea, state, created_at, updated_at,
    spec, architecture, tags, category, monetization_scheme,
    evolution_history, error, current_task_id, extras, generic_metadata
) VALUES (
    ...
    %(evolution_history)s, %(error)s, %(current_task_id)s, %(extras)s, %(generic_metadata)s
)
ON CONFLICT (id) DO UPDATE SET
    ...
    generic_metadata = EXCLUDED.generic_metadata
"""
...
class PostgresManager:
    ...
    def connect(self) -> None:
        ...
        with self._pool.connection() as conn:
            ...
            # Backfill columns on pre-existing tables (CREATE TABLE IF NOT EXISTS
            # never adds columns to an already-present table).
            conn.execute("ALTER TABLE products ADD COLUMN IF NOT EXISTS extras TEXT")
            conn.execute("ALTER TABLE products ADD COLUMN IF NOT EXISTS generic_metadata TEXT")
            conn.commit()
    ...
    def _row_to_product_dict(row: dict[str, Any]) -> dict[str, Any]:
        ...
        d["metadata"] = metadata
        # Restore product extras + forward-compatible fields (mirrors SQLite).
