...
def _item_is_on_demand(item: dict[str, Any]) -> bool:
    """True when a batch item was explicitly human-requested (on-demand passthrough).

    Batch ideas are autonomous by default. An item may opt into on-demand semantics by
    carrying the same markers the rest of the factory uses (``on_demand`` /
    ``landing_fast_path`` / a ``guest-*`` tag); those keep building under a soft hold.
    """
    from core.product_origin import is_on_demand_product

    return is_on_demand_product(item)
...
def drain_batch_queue_into_state(
    ...
    Honors the factory soft hold (``general.factory_on_hold``): while held, autonomous
    batch ideas are NOT materialized (the backlog must not keep growing during a pause),
    but items explicitly flagged on-demand still pass through, consistent with the
    factory-wide on-demand passthrough invariant. The env hard stop is handled upstream
    by the worker, which skips the whole cycle.
    """
    ...
    for item in items:
        ...
        if str(item.get("status") or "") != "queued":
            continue
        # Soft hold: pause autonomous batch starts; let on-demand items through.
        if soft_hold and not _item_is_on_demand(item):
            continue
