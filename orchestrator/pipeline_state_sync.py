    ...
    *,
    ...
    for t in tasks:
        ...
        if agent == "__complete__" and status == "completed":
            # A completion marker is not evidence the product works. One was written
            # by a lifecycle branch entered in error, and this reconciler duly marked
            # a product COMPLETED whose backend did not boot, whose frontend did not
            # build, and whose QA report said gates failed. The product's own gates
            # outrank a marker that contradicts them.
            if quality_gates_failed(str(t.get("product_id") or "")):
                logger.warning(
                    "Ignoring __complete__ marker for %s: its QA gates did not pass",
                    t.get("product_id"),
                )
                continue
            try:
                ...
                if live_gate_from_saved_vercel_record(str(t.get("product_id") or "")):
                    logger.warning(
                        "Ignoring __complete__ marker for %s: Vercel --prod did not publish",
                        t.get("product_id"),
                    )
                    continue
            except Exception:
...
def quality_gates_failed(product_id: str) -> bool:
    """True when the product's own latest QA report says its gates did not pass.

    Read fresh from the QA artifact rather than trusted from task rows: the whole
    point is to disbelieve a completion marker that the gates contradict.
    """
    if not product_id:
        return False
    try:
        ...
        report = Path(resolve_data_root()) / "bugs" / str(product_id) / "qa_report.json"
    ...
    if not isinstance(result, dict):
        return False
    passed = result.get("quality_gates_all_passed")
...
def reconcile_product_state(
    ...
    if cur in _TERMINAL and cur != "FAILED":
        if cur == "COMPLETED" and product.get("operator_locked"):
            return False
        return False
