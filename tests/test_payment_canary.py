...
# ── every priced provider must be probed ──────────────────────────────────────────
# Added 2026-08-24. The canary picked one capability per provider and then truncated the list
# to MAX_PROBES=4 while six peers advertised prices, so two paywalls went untested on every
# run. An untested paywall is indistinguishable from a broken one, and a broken one is exactly
# how ATLAS's six priced capabilities were served free for as long as they were listed.

def _prices(*providers):
    return {"prices": [
        {"capability_id": f"cap.{i}@v1", "product_id": f"prod-{i}", "source_hub": hub,
         "price_usd": 0.01 * (i + 1)}
        for i, hub in enumerate(providers)]}


def test_priced_providers_is_the_denominator():
    from scripts.payment_canary import priced_providers
    prices = _prices("https://a.example", "https://b.example", "local")
    prices["prices"].append({"capability_id": "free@v1", "product_id": "p", 
                             "source_hub": "https://free.example", "price_usd": 0})
    assert priced_providers(prices) == {"https://a.example", "https://b.example", "local"}


def test_no_provider_is_dropped_silently():
    from scripts.payment_canary import MAX_PROBES, pick_priced_capabilities
    providers = [f"https://p{i}.example" for i in range(6)]
    picked = pick_priced_capabilities(_prices(*providers))
    assert len(picked) == 6, "a provider was dropped — the old ceiling truncated the list"
    assert MAX_PROBES >= 6, "MAX_PROBES is below the number of priced peers in production"


def test_an_unprobed_priced_provider_fails_the_run():
    from scripts.payment_canary import evaluate
    manifest = {"name": "hub", "payment_configured": True, "payment_testnet": False,
                "_priced_providers": ["https://a.example", "https://b.example"]}
    probes = [{"status": 402, "body": {}, "capability_id": "cap.0@v1",
               "source_hub": "https://a.example", "price_usd": 0.01}]
    checks = {c.name: c for c in evaluate(manifest, probes, {"service": "aimarket-hub-mcp",
                                                            "trial": "per-caller"})}
    check = checks["every_priced_provider_probed"]
    assert check.ok is False
    assert "https://b.example" in check.detail


def test_all_providers_probed_passes():
    from scripts.payment_canary import evaluate
    manifest = {"name": "hub", "payment_configured": True, "payment_testnet": False,
                "_priced_providers": ["https://a.example"]}
    probes = [{"status": 402, "body": {}, "capability_id": "cap.0@v1",
               "source_hub": "https://a.example", "price_usd": 0.01}]
    checks = {c.name: c for c in evaluate(manifest, probes, {"service": "aimarket-hub-mcp",
                                                            "trial": "per-caller"})}
    assert checks["every_priced_provider_probed"].ok is True
