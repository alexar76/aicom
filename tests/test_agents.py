...
class TestArtifactPersistence:
    ...
    def test_save_creates_directory(self, agent, tmp_path):
        """Saving creates the artifact directory that does not exist yet.

        This used to pass `"deep/path"` as the product id to prove intermediate
        directories are created. `_safe_product_dir` now refuses a product id that is
        not `[A-Za-z0-9._-]{1,128}` — a traversal guard — so the case asserted exactly
        what the guard exists to prevent. Both halves are pinned below.
        """
        assert not (tmp_path / "specs" / "prod-deep01").exists()
        agent._save_artifact("prod-deep01", "specs", {"a": 1}, "test.json")
        assert (tmp_path / "specs" / "prod-deep01" / "test.json").exists()

    def test_save_refuses_a_product_id_that_would_escape_the_artifact_root(self, agent, tmp_path):
        import pytest as _pytest

        for bad in ("deep/path", "../escape", "..", "a" * 129, ""):
            with _pytest.raises(ValueError):
                agent._save_artifact(bad, "specs", {"a": 1}, "test.json")
