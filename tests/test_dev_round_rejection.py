...
def test_score_counts_unresolvable_imports_and_unbound_names(tmp_path):
    """The score must MOVE with these defects. It is a sum over a growing detector set —
    two dozen of them now — so an exact total pinned here goes stale the moment a
    detector is added, and this test was asserting 3 against a score of 12 while the
    behaviour it guards was fine. What the round guard actually reads is the delta.
    """
    root = tmp_path / "code"
    ...
    with_defects = _tree_defect_score(root)
    assert with_defects is not None and with_defects >= 3, with_defects

    # Resolve the import and bind the names; nothing else about the tree changes.
    _write(root, "backend/app/security.py", "def hash_password(p):\n    return p\n")
    _write(root, "backend/app/models.py", "Column = Boolean = object\nx = Column\n")
    repaired = _tree_defect_score(root)
    assert repaired is not None
    assert with_defects - repaired >= 3, (with_defects, repaired)
...
def test_score_is_comparable_before_and_after_a_bad_edit(tmp_path):
    ...
    # Absolute totals are not the contract — the comparison is. A missing symbol
    # cascades through several detectors, so `after` is not 1 and pinning it here made
    # this test fail on a guard that was working exactly as designed.
