...
def test_patch_clear_admin(tmp_path):
    ...
    assert checkout_usdt_from_sales_file(pid, data_root=tmp_path, delivery_profile="marketing_landing") == 4.99


def test_delivery_profile_default_tiers():
    from web.backend.services.storefront_pricing import default_price_for_delivery_profile

    assert default_price_for_delivery_profile("marketing_landing") == 4.99
    assert default_price_for_delivery_profile("full_software") == 9.99
