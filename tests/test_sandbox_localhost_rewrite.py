...
def test_inject_base_href_uses_html_directory_for_spa_dist():
    """Vite ``./assets/…`` must resolve under frontend/dist, not the sandbox root."""
    html = "<html><head></head><body></body></html>"
    out = sb._inject_iframe_base_href(
        html, "sandbox-abc", file_path="frontend/dist/index.html"
    )
    assert 'href="/api/sandbox/file/sandbox-abc/frontend/dist/"' in out
    root = sb.sandbox_static_base_href("sandbox-abc", file_path="index.html")
    assert root.endswith("/api/sandbox/file/sandbox-abc/")
...
def test_inject_base_href_absolute_when_request_has_host():
    ...
    nested = sb._inject_iframe_base_href(
        html, "sandbox-abc", Req(), file_path="frontend/dist/index.html"
    )
    assert (
        'href="http://203.0.113.10:9080/api/sandbox/file/sandbox-abc/frontend/dist/"'
        in nested
    )
