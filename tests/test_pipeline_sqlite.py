...
class TestSQLiteFailTask:
    ...
    def test_fail_task_max_retries_exceeded(self, sqlite_state_machine, sqlite_product_with_task):
        product, task = sqlite_product_with_task
        task.max_retries = 1
        # `retry_count == max_retries` is the final PENDING attempt by design, so
        # max_retries=1 fails on the SECOND call, not the first.
        sqlite_state_machine.fail_task(task.id, "Retryable SQLite error")
        assert sqlite_state_machine._find_task(task.id).status == TaskStatus.PENDING
    ...
    # `pytest.ini` does not set `asyncio_mode = auto`, so pytest-asyncio runs in strict
    # mode and an UNMARKED `async def test_` is not executed — it is reported as an
    # error. This was the only unmarked async test in the suite outside the two
    # Playwright files (which skip themselves wholesale), so the retry/PENDING
    # behaviour it describes has not actually been run.
    @pytest.mark.asyncio
    async def test_retry_keeps_task_pending(self, sqlite_state_machine, sqlite_product_with_task):
        """Test that incrementing retry_count keeps task in PENDING state.

        Once it started running, its own setup turned out to be wrong too:
        `create_product` does not queue a task, so `get_next_task()` returned None.
        Every other case in this class takes the queued task from
        `sqlite_product_with_task`.
        """
        sm = sqlite_state_machine
        _product, queued = sqlite_product_with_task
