...
def test_an_unpublished_satellite_reaches_no_knowledge_base():
    ...
    The map is the mirror's INTENT list: a satellite is added to it before it is ever pushed, and
    `attested` sat in the map with github_published: false so bases must not invent GH links.
    Telling an agent about a component it cannot open is worse than silence — the agent then
