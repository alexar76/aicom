...
@pytest.mark.parametrize(
    ...
    [
        # `supply_chain_term` is matched as a STEM, not a literal: the glossary fixes the
        # terminology, not the grammatical case a sentence needs it in. Requiring
        # "цепочки поставок" made the test demand a nominative inside a prepositional
        # phrase ("по цепочке поставок"), i.e. it could only pass on ungrammatical Russian.
        ("ru", ("показание", "верификация", "квитанция"), "поставщиков", "цепочк.{0,2} поставок AI-агентов"),
...
def test_onboarding_satellites_use_the_localized_glossary(
    ...
    # Typography is not terminology: French prose uses the typographic apostrophe (U+2019)
    # and the glossary is written with the straight one, so compare them as the same
    # character rather than failing on a quote style.
    normalized = auditor_line.replace("\u2019", "'").replace("\u02bc", "'")
    assert re.search(supply_chain_term, normalized), (
        f"{lang}: {supply_chain_term!r} not found in {normalized!r}"
    )
...
def test_an_unpublished_satellite_reaches_no_knowledge_base():
    """What is not on GitHub is outside the knowledge base.

    The map is the mirror's INTENT list: a satellite is added to it before it is ever pushed.
    `github_published: false` entries must not invent GH links. Telling an agent about a
    component it cannot open is worse than silence — the agent then confidently hands a
    stranger a dead link.

    The first test in this file guarantees every published component IS present. This one
    guarantees the unpublished ones are NOT, which is the half that was missing.
    """
    unpublished = [s for s in ecosystem_knowledge.load_satellites()
                   if not ecosystem_knowledge.is_github_published(s)]
    assert unpublished, "nothing is marked unpublished — did the key get dropped from the map?"
    ids = {c["id"] for c in ecosystem_knowledge.components()}
    for sat in unpublished:
        assert str(sat["id"]) not in ids, f"{sat['id']} is unpublished but reached the fact table"
...
def test_publication_defaults_to_true_so_the_key_cannot_rot():
    """47 of 49 entries are published; demanding the key everywhere would rot into noise.

    The permissive default is deliberate, and the gap it leaves is closed by
    `scripts/verify_knowledge_base_publication.py`, which asks GitHub — something a unit test
    cannot do.
    """
    assert ecosystem_knowledge.is_github_published({"id": "x"}) is True
    assert ecosystem_knowledge.is_github_published({"id": "x", "github_published": True}) is True
    assert ecosystem_knowledge.is_github_published({"id": "x", "github_published": False}) is False
    # Only an explicit `false` excludes — a typo'd value must not silently hide a component.
    assert ecosystem_knowledge.is_github_published({"id": "x", "github_published": "no"}) is True


def test_factory_satellite_map_drops_unpublished_entries():
    """The GitHub factory snapshot must not list unpublished satellites."""
    src = (ROOT / "scripts" / "satellite-map.yaml").read_text(encoding="utf-8")
    out = sync.satellite_map_mirror_text("scripts/satellite-map.yaml", src)
    unpublished = [
        str(s["id"])
        for s in (yaml.safe_load(src) or {}).get("satellites", [])
        if isinstance(s, dict) and s.get("github_published") is False
    ]
    assert unpublished
    ...
    published = [
        str(s["id"])
        for s in (yaml.safe_load(src) or {}).get("satellites", [])
        if isinstance(s, dict) and s.get("github_published") is not False
    ]
    assert published
    for sid in published[:8]:
