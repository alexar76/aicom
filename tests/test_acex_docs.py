...
def test_ai_auditor_is_documented():
    poa = (ACEX / "protocol" / "proof-of-audit.md").read_text(encoding="utf-8")
    spec = (ACEX / "protocol" / "spec-capital-markets.md").read_text(encoding="utf-8")
    assert "ai-auditor:acex-v1" in poa
    assert "human_audit_override_forbidden" in poa
    assert "ai-auditor:acex-v1" in spec
    assert (ACEX / "integrations" / "ai_audit.py").is_file()
