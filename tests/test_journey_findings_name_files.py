...
_JOURNEY_LOOP = 'for line in journey.get("issues") or []:'


def _journey_finding_region() -> str:
    """The journey-issue loop up to the end of the finding it appends.

    Both callers used to take a flat 2600 characters from the loop header. The loop
    grew — the file/where wiring moved the `all_bugs.append(...)` past that mark — so
    the assertions started missing code that was right there, and read as "the finding
    does not carry the file" when it does. Anchor on the append instead of on a byte
    count.
    """
    qa = (Path(__file__).resolve().parents[1] / "agents" / "qa.py").read_text(encoding="utf-8")
    start = qa.index(_JOURNEY_LOOP)
    append = qa.index("all_bugs.append(", start)
    # Walk to the append's own closing paren. Counting braces does not work here: the
    # title is an f-string, so the first `}` belongs to `{str(line)[:120]}`.
    depth = 0
    end = append
    for offset in range(append + len("all_bugs.append"), len(qa)):
        char = qa[offset]
        if char == "(":
            depth += 1
        elif char == ")":
            ...
            if depth == 0:
                ...
                break
    assert end > append, "could not find the end of the all_bugs.append(...) call"
    return qa[start:end]
...
def test_the_finding_carries_the_file_and_says_so():
    region = _journey_finding_region()
