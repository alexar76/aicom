...
def test_five_language_supply_chain_admission_docs(lang: str):
    ...
    assert "$25" in text
    assert "federation/announce" in text
    join_suffix = "" if lang == "en" else f".{lang}"
    assert f"join-the-federation{join_suffix}.md" in text
...
@pytest.mark.parametrize("lang", LANGUAGES)
def test_five_language_join_federation_docs_name_the_guest_stake_door(lang: str):
    suffix = "" if lang == "en" else f".{lang}"
    path = ROOT / "docs" / f"join-the-federation{suffix}.md"
    text = path.read_text(encoding="utf-8")
    assert "$25" in text
    admission_suffix = "" if lang == "en" else f"-{lang}"
    assert f"supply-chain-admission{admission_suffix}.md" in text
