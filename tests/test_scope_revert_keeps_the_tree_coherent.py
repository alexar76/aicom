...
def test_the_breakdown_is_taken_before_the_rollback():
    ...
    # Ordering, checked by position in the file rather than inside a fixed-size window.
    # The window used to end at `system_prompt = build_developer_system_prompt`, a line
    # that no longer follows this block, so it fell back to 6000 characters — and the
    # `_breakdown_delta(...)` call is further down than that. The assertion then read as
    # "the log re-measures" about code that does exactly what it should.
    start = src.index("if patch_mode and before_score is not None:")
    capture = src.index("after_parts = _tree_defect_breakdown(code_root)", start)
    rollback = src.index("for rel, content in previous_content.items():", start)
    delta = src.index("_breakdown_delta(before_parts, after_parts)", start)
    assert capture < rollback, "the breakdown is measured after the tree was put back"
    assert capture < delta, (
