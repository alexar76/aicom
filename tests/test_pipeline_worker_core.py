...
@pytest.mark.asyncio
async def test_sandbox_preview_gc_runs_from_worker(monkeypatch):
    called: dict = {}

    def _gc(**kw):
        called["stats"] = {"containers": 2, "volumes": 9, "networks": 1}
        return called["stats"]

    monkeypatch.setattr(
        "web.backend.services.sandbox_docker.reap_stale_preview_resources",
        _gc,
    )
    worker = PipelineWorker()
    await worker._reap_sandbox_previews_once("test")
    assert called["stats"]["volumes"] == 9
