...
class TestReceiptVector:
    ...
    def test_an_unsigned_field_does_not_affect_verification(self, key):
        """`channel_id` rides along outside the v1 canonical. That is by design, and the
        vector carries one, so this pins that the canonical is a fixed field list rather
        than "whatever the object happens to hold"."""
        receipts = pytest.importorskip("aimarket_agent.receipts")
        receipt = dict(_vector("receipt-signed.json"))
        assert "channel_id" in receipt, "the vector no longer exercises this case"
