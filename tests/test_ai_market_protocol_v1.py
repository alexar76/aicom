...
def client(tmp_path, monkeypatch):
    ...
    # `AIFACTORY_CRYPTO_ENABLED` is the ecosystem-wide master switch and defaults to
    # OFF, which makes `/ai-market/channel/open` answer 400 `crypto_disabled`. This
    # file is the 402 → channel → invoke protocol, so with the switch unset there is no
    # channel to test and twelve cases here failed on the switch rather than on the
    # protocol — the sibling `test_ai_market_protocol_v1_admin.py` sets it, this one
    # never did.
...
def test_declared_capabilities_in_manifest(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("AIFACTORY_DATA_ROOT", str(tmp_path / "data"))
    monkeypatch.setenv("AIFACTORY_AI_MARKET_DEMO_PAYMENT", "1")
    monkeypatch.setenv("AIFACTORY_CRYPTO_ENABLED", "1")  # see the note on the `client` fixture
    monkeypatch.setenv("AICOM_PIPELINE_JSON", str(pipeline))
