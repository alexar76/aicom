...
''')
    ...
    assert "TypeError" in found[0]["detail"], "the detail must name the failure it predicts"
    assert "Fix by declaring" in found[0]["detail"], "a finding without a remedy is a nag"
...
def test_two_models_on_one_table_are_caught(tmp_path):
    ...
    assert "delete the other" in found[0]["detail"], "a finding without a remedy is a nag"
