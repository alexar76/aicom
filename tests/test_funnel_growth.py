...
def test_completion_asks_for_a_hero_screenshot(monkeypatch):
    """A launch post without a screenshot is how Sentinel shipped invisible.

    The funnel used to pass capture_screenshot=False, so even a marketing article
    that asked for a hero never got one — and a non-writable blog dir then ate
    the write. The capture flag must stay on; the write failure is a warning.
    """
    monkeypatch.setenv("AIFACTORY_FUNNEL_AUTO_HUB_LIST", "0")
    seen: dict = {}

    def _publish(pid, **kwargs):
        seen["pid"] = pid
        seen.update(kwargs)
        return True

    monkeypatch.setattr(
        "web.backend.services.product_blog.publish_product_launch_blog_post",
        _publish,
    )
    from web.backend.services.funnel_distribute import on_product_completed

    out = on_product_completed("prod-bdb1634806de", {"name": "Sentinel"})
    assert out["blog"] is True
    assert seen["pid"] == "prod-bdb1634806de"
    assert seen.get("capture_screenshot") is True
    assert seen.get("product") == {"name": "Sentinel"}
