...
def test_it_leads_the_blocking_list_and_the_health_gate():
    ...
    # Compare positions INSIDE the blocking-code tuple, not across the whole file.
    # `duplicate_tablename` also appears in `_BOOT_FATAL_SCOPE_CODES` near the top —
    # a different list, added later — and a bare `head.index(...)` found that one, so
    # this read as "missing_module is ranked last" while the ranking was correct.
    at = head.index('"missing_module"')
    tuple_start = head.rindex("in (", 0, at)
    tuple_end = head.index("):", at)
    blocking = head[tuple_start:tuple_end]
    assert '"duplicate_tablename"' in blocking, blocking[:200]
    assert blocking.index('"missing_module"') < blocking.index('"duplicate_tablename"'), (
