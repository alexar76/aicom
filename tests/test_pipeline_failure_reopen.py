...
def test_reopen_failed_product_sqlite(tmp_path, monkeypatch):
    ...
    assert int(product.get("quality_repair_round") or 0) == 0
    assert int(product.get("qa_repair_extensions") or 0) == 0
...
def test_reopen_resets_exhausted_repair_budget(tmp_path, monkeypatch):
    monkeypatch.setenv("USE_SQLITE", "1")
    db = tmp_path / "pipeline.db"
    monkeypatch.setenv("SQLITE_PATH", str(db))
    ...
    sm.connect()
    now = time.time()
    pid = "prod-fail-budget"
    sm.upsert_product(
        {
            "id": pid,
            "idea": "Relay",
            "state": "FAILED",
            "failure_reason": "Policy audit exhausted",
            "quality_repair_round": 42,
            "qa_repair_extensions": 2,
    ...
    res = reopen_failed_product(
        pid,
        "Factory fix: reopen Relay for QA after spa public/ + policy_audit HUMAN_REVIEW.",
        agent_type="qa",
    ...
    assert product.get("qa_repair_extensions") == 0
    assert not product.get("human_review_kind")
