...
#: `task_executor_agent.py` builds more than one `quality_gates_feedback` payload — the
#: QA-gates one and, added later and textually FIRST, the live-Vercel-gate one. Reading
#: only `str.index(...)` pinned whichever block happened to come first in the file, so
#: these tests started failing on a payload they were never about while the contract
#: they guard was intact. Check every block instead.
_MARKER = '"quality_gates_feedback": {'


def _feedback_blocks() -> list[str]:
    executor = (ROOT / "orchestrator" / "task_executor_agent.py").read_text(encoding="utf-8")
    blocks: list[str] = []
    start = executor.find(_MARKER)
    while start != -1:
        blocks.append(executor[start : start + 2000])
        start = executor.find(_MARKER, start + 1)
    assert blocks, "no quality_gates_feedback payload found — the walk is broken"
    return blocks
...
def test_the_keys_dev_reads_are_passed_through():
    ...
    for subset in _feedback_blocks():
...
def test_the_detector_gates_are_passed_through():
    ...
    blocks = _feedback_blocks()
    for gate in ("module_health", "frontend_build", "api_contract", "demo_journey"):
        assert any(f'"{gate}"' in subset for subset in blocks), (
            f"{gate} findings never reach the round"
        )
