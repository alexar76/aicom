...
def test_policy_audit_exhausted_goes_to_human_review_not_failed(tmp_path, monkeypatch):
    monkeypatch.setenv("AIFACTORY_MARKETPLACE_QUALITY_GATE", "1")
    monkeypatch.setenv("AIFACTORY_MARKETPLACE_MIN_SPEC_COVERAGE", "0")
    monkeypatch.setenv("AIFACTORY_MAX_QUALITY_LOOPS", "2")
    monkeypatch.setenv("AIFACTORY_QA_REPAIR_EXTENSIONS", "0")

    root = tmp_path
    pid = "prod-audit-exhausted"
    _write_manifest(root / "data" / "code" / pid, ["index.html"])
    (root / "data" / "code" / pid / "index.html").write_text(
    ...
    spec_path.mkdir(parents=True)
    (spec_path / "specification.json").write_text(
        json.dumps({"specification": {"product_name": "X", "description": "y", "core_features": []}}),
        encoding="utf-8",
    )
    ...
    changed = apply_policy_audit(products, task_queue, 1_700_000_000.0, data_root=str(root / "data"))
    assert changed is True
    assert products[pid]["state"] == "HUMAN_REVIEW_PENDING"
    assert products[pid].get("human_review_kind") == "qa_repair_exhausted"
    assert products[pid]["state"] != "FAILED"
    assert len(task_queue) == 0


def test_policy_audit_skips_operator_locked(tmp_path, monkeypatch):
    monkeypatch.setenv("AIFACTORY_MARKETPLACE_QUALITY_GATE", "1")
    root = tmp_path
    pid = "prod-locked"
    _write_manifest(root / "data" / "code" / pid, ["index.html"])
    (root / "data" / "code" / pid / "index.html").write_text("<html></html>", encoding="utf-8")
