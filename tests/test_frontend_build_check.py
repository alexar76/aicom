...
def test_resolve_frontend_build_output_dir_prefers_dist_then_vercel_public(tmp_path):
    code = tmp_path / "code" / "prod-x"
    front = code / "frontend"
    front.mkdir(parents=True)
    (front / "dist").mkdir()
    (front / "dist" / "index.html").write_text("<html></html>", encoding="utf-8")
    (code / "public").mkdir()
    (code / "public" / "index.html").write_text("<html></html>", encoding="utf-8")
    assert resolve_frontend_build_output_dir(code, front) == front / "dist"

    (front / "dist" / "index.html").unlink()
    assert resolve_frontend_build_output_dir(code, front) == code / "public"

    (code / "public" / "index.html").unlink()
    (code / "dist").mkdir()
    (code / "dist" / "index.html").write_text("<html></html>", encoding="utf-8")
    assert resolve_frontend_build_output_dir(code, front) == code / "dist"
