...
def test_working_app_publish_uses_compose_view_url(tmp_path, monkeypatch):
    ...
    def fake_start(pid, storefront=False, wait_for_bootstrap=False):
        assert pid == "prod-x"
        assert storefront is False
        # The publisher decides publishability from compose_preview/preview_api, which are
        # only populated once the stack is up — so it must be the caller that waits.
        assert wait_for_bootstrap is True
        return {
