...
def test_skopos_healthz_producer_consumer_contract():
    """The public `/healthz` payload and the Monitor's consumer must agree.

    SKOPOS deliberately split its status: `/healthz` is unauthenticated and slim
    (`ok`, `service`, `version`, `database`) because publishing which hosts are watched
    publishes fleet posture; `servers_monitored` / `requests_total` / `log_parsers` /
    `security_score` moved to `build_fleet_status` behind authentication.

    This test used to demand the fleet keys from `build_status` — against a nonexistent
    config, so it also failed on `ok` — and while it was red nobody saw the actual
    consequence: `skopos_status.apply_skopos_to_nodes` still read those keys off
    `/healthz` and coerced each absence to 0, so the live panel showed a fleet of 0
    servers and security score 0, and the node never left "idle".
    """
    _skopos_path()
    from skopos.public_status import build_fleet_status, build_status  # noqa: E402

    public = build_status()
    assert public.get("service") == "skopos"
    for key in ("ok", "service", "version", "database"):
        assert key in public, key

    fleet_only = ("servers_monitored", "requests_total", "log_parsers")
    for key in fleet_only:
        assert key not in public, f"{key} must not be on the unauthenticated endpoint"
    fleet = build_fleet_status()
    for key in fleet_only:
        assert key in fleet, f"{key} belongs to the authenticated fleet payload"
    ...
    nodes = [{"id": "skopos", "metrics": {}}]
    apply_skopos_to_nodes(nodes, {"ok": True, "service": "skopos", "database": "postgresql"})
    live = nodes[0]["skopos_live"]
    assert live["database"] == "postgresql"
    assert "servers_monitored" not in live, "an unpublished count must not read as 0"
    assert nodes[0]["metrics"] == {}, nodes[0]["metrics"]
    assert nodes[0]["status"] == "active", "the service answered; that much is known"

    nodes = [{"id": "skopos", "metrics": {}}]
    apply_skopos_to_nodes(
        nodes,
        {"ok": True, "database": "sqlite", "servers_monitored": 3, "requests_total": 120},
    )
    assert nodes[0]["metrics"]["servers"] == 3
    assert nodes[0]["metrics"]["requests_total"] == 120
...
def test_skopos_agent_portal_fab_overlay_contract():
    """The overlay is portalled to document.body, and torn down before login.

    The two halves live in different modules since the widget was split:
    `agent_widget` builds and mounts the FAB overlay, `agent_portal` is the tiny
    script that runs on EVERY page (login gate included) and removes it. This test
    asserted `findChatButton` — a name that only ever existed in the widget — against
    the teardown module, so it failed for a reason that had nothing to do with either
    contract, and it hid a second problem: importing `agent_portal` pulled in
    Streamlit, which the factory does not install.
    """
    _skopos_path()
    from skopos.agent_portal import _portal_script  # noqa: E402

    teardown = _portal_script()
    assert "skopos-agent-fab-overlay" in teardown
    assert "skopos-login-page-marker" in teardown, "the assistant must not show pre-auth"
    assert "teardownLegacy" in teardown

    # The widget half is read as source rather than imported: `skopos.agent_widget`
    # pulls in Streamlit and the SKOPOS theme stack, neither of which the factory
    # installs. What this asserts is the JS payload, and that is text either way.
    widget = (ROOT / "skopos" / "skopos" / "agent_widget.py").read_text(encoding="utf-8")
    assert "skopos-agent-fab-overlay" in widget
    assert "doc.body.appendChild" in widget, "the overlay must escape section.main"
