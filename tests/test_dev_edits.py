...
def test_a_large_file_is_truncated_rather_than_dropped(tmp_path, monkeypatch):
    """A truncated tail still lets an append anchor on what is visible; nothing at all does not.

    The cap is `attach_file_chars()` now, not the `MAX_ATTACHED_FILE_CHARS` floor: it is
    derived from the active model's context window and reaches 200k on the deployed
    provider. Measuring against the floor meant a 25k file was attached whole and this
    read as "nothing is truncated". Pin the cap through its own env knob instead.
    """
    from core.repair_batches import attach_file_contents

    cap = 20_000
    monkeypatch.setenv("AIFACTORY_ATTACH_FILE_CHARS", str(cap))
    code = tmp_path / "code"
    (code / "frontend").mkdir(parents=True)
    (code / "frontend" / "big.tsx").write_text("x" * (cap + 5000), encoding="utf-8")
    contents = attach_file_contents({"files": ["frontend/big.tsx"]}, code)
    body = contents["frontend/big.tsx"]
    assert len(body) < cap + 200
    assert "truncated" in body


def test_a_file_inside_the_window_arrives_whole(tmp_path, monkeypatch):
    """The other half of the same contract: do not truncate what fits."""
    from core.repair_batches import attach_file_contents

    monkeypatch.setenv("AIFACTORY_ATTACH_FILE_CHARS", "20000")
    code = tmp_path / "code"
    (code / "frontend").mkdir(parents=True)
    (code / "frontend" / "small.tsx").write_text("y" * 19_000, encoding="utf-8")
    body = attach_file_contents({"files": ["frontend/small.tsx"]}, code)["frontend/small.tsx"]
    assert len(body) == 19_000
