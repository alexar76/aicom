...
def test_pricing_audit_overlay():
    ...
    assert body["audit_standard"] == "ai-auditor:acex-v1"


def test_pricing_overlay_only_federated_listing():
    """Hub-federated products (atlas.products) are not in factory catalog."""
    body = build_pricing_snapshot(
        [{"product_id": "prod-factory", "capability_id": "x", "price_per_call_usd": 0.1}],
        ipo_overlay={
            "atlas.products": {
                "listing_id": "atlas.products",
                "status": "approved",
                "symbol": "CAPATLAS",
                ...
                "audit_pack_sha256": "abc",
                "revenue": {},
            }
        },
        limit=10,
    )
    ids = [x["listing_id"] for x in body["listings"]]
    assert ids[0] == "atlas.products"
    listed = body["listings"][0]
    assert listed["acex_listed"] is True
    assert listed["nav_source"] == "unpriced_until_pool"
    assert listed["auditor_id"] == "ai-auditor:acex-v1"
    assert body["acex_listings_live"] == 1
