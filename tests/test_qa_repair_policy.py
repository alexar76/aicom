...
def test_stuck_kind_is_a_repair_resume_not_a_sales_gate():
    """A plateau park must not be classified as post-devops, or approve would skip to sales."""
    from web.backend.services.human_pipeline import _human_review_kind

    assert _human_review_kind({"human_review_kind": "qa_repair_stuck"}) == "qa_repair_stuck"
    assert _human_review_kind({"human_review_kind": "qa_repair_exhausted"}) == "qa_repair_exhausted"
    assert _human_review_kind({"human_review_kind": "live_mesh_payment_ops"}) == "live_mesh_payment_ops"
    assert _human_review_kind({}) == "post_devops"
