...
def test_qa_peer_review_force_proceed_after_max_iterations(monkeypatch):
    monkeypatch.setenv("AIFACTORY_QA_PEER_REVIEW_MAX_ITERS", "2")
    engine = PeerReviewEngine(_priority)
    product_state = {
        "qa_peer_review_iterations": 2,
        "peer_reviews": {
            "qa": {"recommended": "block", "blockers": ["focus-visible"], "notes": "fix a11y"}
        },
    }
    queue = []
    blocked = engine.apply_block(
        {"product_id": "prod-1", "agent_type": "qa", "state": "QA_TESTING"},
        product_state,
        queue,
        {"idea": "x"},
    )
    assert blocked is False
    assert queue == []
    assert "qa_review_forced_proceed" in product_state
