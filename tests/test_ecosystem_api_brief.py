...
def test_the_envelope_names_the_two_legal_fields_and_the_traps():
    ...
    assert "/ai-market/v2/invoke" in INVOKE_ENVELOPE
    assert "/aimarket/invoke" in INVOKE_ENVELOPE  # named as the trap


def test_the_participant_contract_teaches_payment_not_agent_key():
    from core.ecosystem_api_brief import PARTICIPANT_CONTRACT

    brief = render_brief(capability_ids=["atlas.situation.brief@v1"])
    assert "X-Payment-Channel" in brief and "X-AIMarket-Sandbox-Visitor" in brief
    assert "demo-atlas-key" in brief or "X-Agent-Key" in PARTICIPANT_CONTRACT
    assert "AIMARKET_HUB_URL" in brief or "modelmarket.dev" in brief
    assert PARTICIPANT_CONTRACT in brief
...
def test_aimarket_native_agent_prompt_is_loadable():
    from agents.prompts.load_prompt import load_prompt

    text = load_prompt("aimarket_native_agent.md")
    assert "X-Payment-Channel" in text
    assert "/ai-market/v2/invoke" in text
    assert "AIMARKET_HUB_URL" in text
