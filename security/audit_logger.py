...
def _audit_log_files_chrono(log_dir: Path, *, reverse: bool = False) -> list[Path]:
    ...
    def sort_key(p: Path) -> tuple:
        ...
        suf_i = 0 if suf is None else int(suf)
        try:
...
class AuditLogger:
    ...
    def _rotate(self) -> None:
        ...
        while candidate.exists():
            suffix += 1
            candidate = self.log_dir / f"audit-{base}-{suffix:03d}.jsonl"
    ...
    def _get_last_hash(self) -> str:
        """Return the hash at the tip of the persisted chain (all files, chrono order)."""
        tip = ""
