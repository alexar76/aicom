...
def _is_production() -> bool:
    """Production marker — honours both AIFACTORY_PROD and AIFACTORY_PRODUCTION."""
    for name in ("AIFACTORY_PROD", "AIFACTORY_PRODUCTION"):
...
class SandboxIsolation:
    ...
    def __init__(
        ...
        # Container isolation is mandatory in production: process mode has no kernel
        # boundary, so generated code would run with the factory's own privileges.
        # AIFACTORY_SANDBOX_REQUIRE_CONTAINER defaults to ON in prod and can only be
        # explicitly disabled (=0) by an operator who accepts the risk.
        if require_container is None:
            self.require_container = _env_truthy(
                "AIFACTORY_SANDBOX_REQUIRE_CONTAINER", default=_is_production()
            )
        else:
            self.require_container = bool(require_container)
        if self.require_container and self.execution_mode != "container":
            self.execution_mode = "container"
        
        self.sandboxes: dict[str, Sandbox] = {}
        self._allocated_ports: set = set()
        # Serialise port allocation so two sandboxes created concurrently can never
        # observe the same port as free and both claim it (TOCTOU).
    ...
    def _resolve_execution_mode(explicit: str | None) -> str:
        ...
        if env_mode in ("process", "container"):
            return env_mode
        # Default to container whenever container isolation is required — in prod
        # AIFACTORY_SANDBOX_REQUIRE_CONTAINER defaults ON, so prod gets container mode.
        if _env_truthy("AIFACTORY_SANDBOX_REQUIRE_CONTAINER", default=_is_production()):
            return "container"
        return "process"
    ...
    def start_sandbox(self, sandbox_id: str, command: list[str] | None = None) -> Sandbox:
        ...
        try:
            ...
            # Process mode has NO kernel isolation — generated code runs with the
            # factory's own privileges. Refuse it whenever container isolation is
            # required (always the case in production); only fall through in dev,
            # and then only with a loud warning.
            if self.require_container:
                sandbox.status = SandboxStatus.FAILED
                sandbox.error = (
                    "Process-mode sandbox refused: container isolation is required "
                    "(AIFACTORY_SANDBOX_REQUIRE_CONTAINER). Provide a Docker daemon "
                    "(DOCKER_HOST) so the sandbox can run in container mode."
                )
                logger.error("Sandbox %s: %s", sandbox_id, sandbox.error)
                self._save_state()
                return sandbox

            logger.warning(
                "Sandbox %s starting in PROCESS mode — no kernel isolation, generated "
                "code runs with factory privileges. Use container mode "
                "(AIFACTORY_SANDBOX_REQUIRE_CONTAINER=1) outside local development.",
                sandbox_id,
            )
    ...
    def _start_container_sandbox(self, sandbox: Sandbox, command: list[str]) -> bool:
        ...
        base = hardened_docker_run_args(
            ...
            cpus="0.5",
            workdir="/workspace",
            # Always mount the code dir read-only so a compromised workload cannot
            # overwrite the generated source and persist back onto the host. Writable
            # scratch space is provided separately via a private tmpfs at /work (and
            # the hardened /tmp tmpfs) rather than the code mount.
            volume_mount=f"{sandbox.work_dir}:/workspace:ro",
            writable_tmpfs=["/work"],
            publish_port=int(sandbox.port) if sandbox.port else None,
            read_only_root=not self.enable_network,
        )
        # Pass the command as argv (never a shell-joined string) so container
        # commands are exec'd directly and never re-interpreted by a shell.
        docker_cmd = append_image_and_command(base, self.container_image, command)
        try:
    ...
    def _allocate_port(self) -> int:
        ...
        The free-port probe and the bookkeeping ``add`` are guarded by a lock so
        two concurrent ``create_sandbox`` calls cannot both pass the availability
        check and claim the same port (check-then-act TOCTOU).
        """
        import socket
