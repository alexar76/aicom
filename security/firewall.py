...
_FIREWALL_ENFORCE_WARNED = False


def _warn_firewall_not_enforced_in_prod() -> None:
    """Log once if the default-deny ACL is not enforced while running in production."""
    global _FIREWALL_ENFORCE_WARNED
    if _FIREWALL_ENFORCE_WARNED:
        return
    if (os.environ.get("AIFACTORY_PROD") or "").strip() == "1":
        _FIREWALL_ENFORCE_WARNED = True
        logger.warning(
            "Firewall ACL not enforced (AIFACTORY_FIREWALL_ENFORCE unset/0) — HTTP "
            "requests are allowed by default. Set AIFACTORY_FIREWALL_ENFORCE=1 and "
            "define allow rules to enable the default-deny ACL in production."
        )
...
class FirewallManager:
    ...
    def http_request_allowed(self, ip: str, port: int = 8081) -> tuple[bool, str]:
        ...
        # ACL not enforced — allow through (the app usually runs behind a reverse proxy
        # that owns network ACLs). Surface the open default once in production so it is
        # a deliberate operator choice rather than a silent gap.
        _warn_firewall_not_enforced_in_prod()
        return True, "ok"
