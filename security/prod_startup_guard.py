...
# Addresses from the standard Anvil/Hardhat dev mnemonic ("test test … junk").
# Their PRIVATE KEYS ARE PUBLIC — printed by every local node on boot and copied
# into thousands of tutorials — so anyone can sweep whatever lands there. They are
# not placeholders that merely fail; they are live addresses that silently work and
# hand the money to the public. A local .env that walked to a production host is the
# normal way one of these ends up as AIMARKET_PAYMENT_RECIPIENT, which is exactly
# how it must fail closed. Contract addresses from the deterministic Hardhat deploy
# sequence are included too: on any real chain they hold no code, so verification
# and escrow would silently target nothing.
_WELL_KNOWN_DEV_ADDRESSES: frozenset[str] = frozenset(
    a.lower()
    for a in (
        # Anvil/Hardhat accounts #0–#9 (dev mnemonic)
        ...
        # Deterministic Hardhat/Anvil contract deploy addresses
...
# Spellings that count as "the operator switched this on". Deliberately WIDER than the
# runtime reads it guards (supply_security and channels both test `== "1"`): a host set to
# `RELAXED=true` does not actually relax anything, but the operator plainly meant it to, and
# refusing to boot is the right answer to that intent either way. Fail closed on the intent,
# not on the spelling.
_TRUTHY_FLAGS: frozenset[str] = frozenset({"1", "true", "yes", "on"})


def _flag_on(name: str) -> bool:
    return os.environ.get(name, "").strip().lower() in _TRUTHY_FLAGS
...
def _is_placeholder_address(address: str) -> bool:
    """True when ``address`` is empty, all-zero, a template value, or a dev-chain address.

    Guards against shipping the ``payment.py`` defaults (zero address), copy-paste
    template values (``0xYour...``, ``SET_ME``, ...), or an Anvil/Hardhat dev address
    (public private key — see ``_WELL_KNOWN_DEV_ADDRESSES``) into a live deployment.
    """
    ...
    if hex_body and set(hex_body) <= {"0"}:
        return True
    if addr in _WELL_KNOWN_DEV_ADDRESSES:
        return True
...
def production_startup_issues() -> list[str]:
    ...
    # Supply chain: RELAXED removes the publisher collateral requirement entirely
    # (supply_security.py), so anyone may list a capability on a hub that advertises
    # itself as stake-gated. It is a dev convenience, and independent/'s compose ships
    # it hardcoded to "1" — which is exactly how it reaches a host that later turns
    # production mode on.
    if _flag_on("AIMARKET_SUPPLY_SECURITY_RELAXED"):
        issues.append(
            "AIMARKET_SUPPLY_SECURITY_RELAXED is on in production mode — publisher "
            "collateral is not required, so any caller can list capabilities on a hub "
            "that claims to be stake-gated. Set AIMARKET_SUPPLY_SECURITY_RELAXED=0 "
            "before running with AIFACTORY_PROD=1."
        )

    # Channels: crediting a deposit without proof the caller CONTROLS the paying wallet
    # leaves deposit front-running wide open — the payer address is public in the very
    # transaction being quoted, and whoever calls open() walks away with the
    # channel_secret. channels._allow_unproven_payer documents it as a migration hatch
    # for the window before the HTTP layer forwarded the signature; that window is not
    # production.
    if _flag_on("AIMARKET_CHANNEL_ALLOW_UNPROVEN_PAYER"):
        issues.append(
            "AIMARKET_CHANNEL_ALLOW_UNPROVEN_PAYER is on in production mode — a payment "
            "channel is credited without proof of control over the paying wallet, so a "
            "front-runner who reads the deposit off-chain claims the victim's channel "
            "and spends the whole balance. Unset it before running with AIFACTORY_PROD=1."
        )

    # Sandbox: the stub answers with canned demo output whenever the factory is
    # unreachable (sandbox_trials.py), so a trial "succeeds" with nothing executed.
    # Correct offline and in CI, a lie to a buyer evaluating the product.
    if _flag_on("AIMARKET_SANDBOX_STUB_INVOKE"):
        issues.append(
            "AIMARKET_SANDBOX_STUB_INVOKE is on in production mode — sandbox trials "
            "return canned demo output instead of executing, so a buyer evaluates a "
            "stub and believes it was the product. Set AIMARKET_SANDBOX_STUB_INVOKE=0 "
            "before running with AIFACTORY_PROD=1."
        )
