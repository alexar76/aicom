...
class SecretsManager:
    ...
    def _write_master_key(self, key: bytes, *, overwrite: bool = False) -> None:
        """Write the master key with owner-only perms, never exposing plaintext.

        New keys are created with ``O_CREAT|O_EXCL`` and mode ``0o600`` so the file
        is never world-readable for any window (the previous create-then-``chmod``
        sequence left a race where the plaintext key was briefly readable). Rotation
        passes ``overwrite=True`` to replace an existing key in place; perms are
        still enforced on the open fd before any bytes are written.
        """
        flags = os.O_CREAT | os.O_WRONLY | (os.O_TRUNC if overwrite else os.O_EXCL)
        fd = os.open(str(self.master_key_file), flags, 0o600)
        try:
            # Re-assert perms in case a permissive umask widened the create mode,
            # or the pre-existing file (on rotation) had looser perms.
            os.fchmod(fd, 0o600)
            os.write(fd, key)
        finally:
            os.close(fd)
    ...
    def rotate_master_key(self) -> None:
        ...
        # Save new key (atomic, owner-only — no plaintext-exposure race)
        self._write_master_key(new_key, overwrite=True)
