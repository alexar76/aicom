...
export type MarketingLocale = 'en' | 'ru' | 'es' | 'fr' | 'zh';
...
export type MarketingStrings = {
  ...
  /** LOGOS federation analytics callout (home) */
  logosEyebrow: string;
  logosTitle: string;
  logosBody: string;
  logosReadOnly: string;
  logosDashboard: string;
  logosSource: string;
  logosDocs: string;
};
...
const EN: MarketingStrings = {
  ...
  logosEyebrow: 'Federation intelligence · live product',
  logosTitle: 'LOGOS turns the whole ecosystem into one observatory.',
  logosBody:
    'It polls the Hub, MOMUS, SKOPOS, and Treasury; stores real snapshots; detects rolling z-score anomalies; correlates security, latency, reputation, and economy signals; and answers natural-language questions through a guarded assistant.',
  logosReadOnly: 'Read-only by construction — never scans, remediates, pays, or deploys',
  logosDashboard: 'Open live LOGOS',
  logosSource: 'Source',
  logosDocs: 'How the analytics work',
};
...
export function getMarketingStrings(locale?: string | null): MarketingStrings {
  ...
  if (raw.startsWith('fr')) return MARKETING_FR;
  if (raw.startsWith('zh')) return MARKETING_ZH;
  return EN;
}
