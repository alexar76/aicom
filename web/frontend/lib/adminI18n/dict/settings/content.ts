import type { I18nDict } from '../../types';

export const CONTENT_SETTINGS_DICT: I18nDict = {
  'settings.section.neuralUi': {
    en: 'Neural UI reference pool',
    ru: 'Пул Neural UI‑референсов',
    es: 'Pool de referencias Neural UI',
    fr: 'Pool de références Neural UI',
    zh: 'Neural UI 参考池',
  },
  'settings.neuralUi.intro': {
    en: 'Optionally inject a generated vanilla HTML/CSS/JS shell into the Developer (and Hardening) prompt so new products mirror motion, tokens, and layout polish. Build the pool offline: python scripts/generate_reference_templates.py --data-root ./data — outputs under data/reference_templates/ plus manifest.json. Style presets live in reference_templates/style_presets.json. Environment variables AIFACTORY_REFERENCE_* override these saved values on the worker.',
    ru: 'По желанию вставляйте сгенерированную оболочку HTML/CSS/JS без фреймворков в промпт Developer (и Hardening), чтобы новые продукты повторяли motion, токены и вёрстку. Соберите пул офлайн: python scripts/generate_reference_templates.py --data-root ./data — вывод в data/reference_templates/ и manifest.json. Стилевые пресеты: reference_templates/style_presets.json. Переменные AIFACTORY_REFERENCE_* в env переопределяют сохранённые значения на воркере.',
    es: 'Opcionalmente inyecta un shell vanilla HTML/CSS/JS generado en el prompt de Developer (y Hardening) para que los productos reflejen motion, tokens y pulido de layout. Construye el pool offline: python scripts/generate_reference_templates.py --data-root ./data — salida en data/reference_templates/ y manifest.json. Presets en reference_templates/style_presets.json. Las variables AIFACTORY_REFERENCE_* anulan estos valores guardados en el worker.',
    fr: 'Injectez éventuellement un shell HTML/CSS/JS vanilla généré dans le prompt Developer (et Hardening) pour que les nouveaux produits reprennent le motion, les tokens et le soin de la mise en page. Construisez le pool hors ligne : python scripts/generate_reference_templates.py --data-root ./data — sortie dans data/reference_templates/ plus manifest.json. Les presets de style vivent dans reference_templates/style_presets.json. Les variables d\'environnement AIFACTORY_REFERENCE_* remplacent ces valeurs enregistrées sur le worker.',
    zh: '可选：将生成的原生 HTML/CSS/JS 外壳注入 Developer（及 Hardening）提示，使新产品沿用其动效、tokens 与布局细节。离线构建参考池：python scripts/generate_reference_templates.py --data-root ./data — 输出至 data/reference_templates/ 及 manifest.json。样式预设位于 reference_templates/style_presets.json。环境变量 AIFACTORY_REFERENCE_* 会在 worker 上覆盖这些已保存的值。',
  },
  'settings.neuralUi.injectToggle': {
    en: 'Inject reference shell into Developer prompt',
    ru: 'Вставлять референс‑оболочку в промпт Developer',
    es: 'Inyectar shell de referencia en el prompt de Developer',
    fr: 'Injecter le shell de référence dans le prompt Developer',
    zh: '将参考外壳注入 Developer 提示',
  },
  'settings.neuralUi.injectHelp': {
    en: 'Web deliverables only. Requires a generated pool (manifest + template folders on disk).',
    ru: 'Только веб‑артефакты. Нужен сгенерированный pool (manifest + папки шаблонов на диске).',
    es: 'Solo entregables web. Requiere pool generado (manifest + carpetas en disco).',
    fr: 'Livrables web uniquement. Nécessite un pool généré (manifest + dossiers de templates sur le disque).',
    zh: '仅限 web 交付物。需要已生成的 pool（磁盘上的 manifest 与模板文件夹）。',
  },
  'settings.neuralUi.templatesDir': {
    en: 'Templates directory (optional)',
    ru: 'Каталог шаблонов (необязательно)',
    es: 'Directorio de plantillas (opcional)',
    fr: 'Répertoire des templates (optionnel)',
    zh: '模板目录（可选）',
  },
  'settings.neuralUi.templatesDirPlaceholder': {
    en: 'Empty → <data_root>/reference_templates',
    ru: 'Пусто → <data_root>/reference_templates',
    es: 'Vacío → <data_root>/reference_templates',
    fr: 'Vide → <data_root>/reference_templates',
    zh: '留空 → <data_root>/reference_templates',
  },
  'settings.neuralUi.selectionMode': {
    en: 'Selection mode',
    ru: 'Режим выбора',
    es: 'Modo de selección',
    fr: 'Mode de sélection',
    zh: '选择模式',
  },
  'settings.neuralUi.mode.random': {
    en: 'random (stable per product id)',
    ru: 'random (стабильно на product id)',
    es: 'random (estable por product id)',
    fr: 'random (stable par product id)',
    zh: 'random（每个 product id 稳定）',
  },
  'settings.neuralUi.mode.roundRobin': { en: 'round_robin', ru: 'round_robin', es: 'round_robin', fr: 'round_robin', zh: 'round_robin' },
  'settings.neuralUi.mode.fixed': { en: 'fixed', ru: 'fixed', es: 'fixed', fr: 'fixed', zh: 'fixed' },
  'settings.neuralUi.mode.matchSpec': {
    en: 'match_spec (keyword overlap with spec)',
    ru: 'match_spec (пересечение ключевых слов со спекой)',
    es: 'match_spec (solapamiento de palabras con la spec)',
    fr: 'match_spec (recoupement de mots-clés avec la spec)',
    zh: 'match_spec（与 spec 的关键词重叠）',
  },
  'settings.neuralUi.templatesDetected': {
    en: 'Templates detected on disk:',
    ru: 'Шаблонов на диске:',
    es: 'Plantillas detectadas en disco:',
    fr: 'Templates détectés sur le disque :',
    zh: '磁盘上检测到的模板：',
  },
  'settings.neuralUi.emptyPoolHint': {
    en: '— generate the pool (see command above), upload below, or check the templates directory path.',
    ru: '— сгенерируйте pool (см. команду выше), загрузите ниже или проверьте путь к каталогу.',
    es: '— genera el pool (ver comando arriba), sube abajo o revisa la ruta del directorio.',
    fr: '— générez le pool (voir la commande ci-dessus), téléversez ci-dessous ou vérifiez le chemin du répertoire des templates.',
    zh: '— 生成 pool（见上方命令）、在下方上传，或检查模板目录路径。',
  },
  'settings.neuralUi.installedTitle': {
    en: 'Installed templates',
    ru: 'Установленные шаблоны',
    es: 'Plantillas instaladas',
    fr: 'Templates installés',
    zh: '已安装的模板',
  },
  'settings.neuralUi.remove': {
    en: 'Remove',
    ru: 'Удалить',
    es: 'Quitar',
    fr: 'Supprimer',
    zh: '移除',
  },
  'settings.neuralUi.addCustomTitle': {
    en: 'Add custom template',
    ru: 'Добавить свой шаблон',
    es: 'Añadir plantilla personalizada',
    fr: 'Ajouter un template personnalisé',
    zh: '添加自定义模板',
  },
  'settings.neuralUi.addCustomBody': {
    en: 'Saves under your reference templates directory as a new folder with index.html (required) and optional style.css / app.js. Slug: lowercase letters, digits, hyphen, underscore.',
    ru: 'Сохраняется в каталоге референс‑шаблонов как новая папка с index.html (обязательно) и при необходимости style.css / app.js. Slug: строчные буквы, цифры, дефис, подчёркивание.',
    es: 'Se guarda en el directorio de plantillas como carpeta nueva con index.html (obligatorio) y opcional style.css / app.js. Slug: minúsculas, dígitos, guión, guión bajo.',
    fr: 'Enregistré dans votre répertoire de templates de référence sous forme de nouveau dossier avec index.html (requis) et, en option, style.css / app.js. Slug : lettres minuscules, chiffres, trait d\'union, tiret bas.',
    zh: '保存到您的参考模板目录中，成为一个包含 index.html（必需）以及可选 style.css / app.js 的新文件夹。Slug：小写字母、数字、连字符、下划线。',
  },
  'settings.neuralUi.slugLabel': {
    en: 'Folder id (slug)',
    ru: 'ID папки (slug)',
    es: 'ID de carpeta (slug)',
    fr: 'Id de dossier (slug)',
    zh: '文件夹 id（slug）',
  },
  'settings.neuralUi.slugPlaceholder': {
    en: 'my-brand-shell',
    ru: 'my-brand-shell',
    es: 'my-brand-shell',
    fr: 'my-brand-shell',
    zh: 'my-brand-shell',
  },
  'settings.neuralUi.displayTitle': {
    en: 'Display title (optional)',
    ru: 'Отображаемое название (необязательно)',
    es: 'Título visible (opcional)',
    fr: 'Titre affiché (optionnel)',
    zh: '显示标题（可选）',
  },
  'settings.neuralUi.displayPlaceholder': {
    en: 'My brand shell',
    ru: 'My brand shell',
    es: 'My brand shell',
    fr: 'My brand shell',
    zh: 'My brand shell',
  },
  'settings.neuralUi.indexHtml': {
    en: 'index.html *',
    ru: 'index.html *',
    es: 'index.html *',
    fr: 'index.html *',
    zh: 'index.html *',
  },
  'settings.neuralUi.indexPlaceholder': {
    en: '<!DOCTYPE html>...',
    ru: '<!DOCTYPE html>...',
    es: '<!DOCTYPE html>...',
    fr: '<!DOCTYPE html>...',
    zh: '<!DOCTYPE html>...',
  },
  'settings.neuralUi.styleCss': {
    en: 'style.css (optional)',
    ru: 'style.css (необязательно)',
    es: 'style.css (opcional)',
    fr: 'style.css (optionnel)',
    zh: 'style.css（可选）',
  },
  'settings.neuralUi.appJs': {
    en: 'app.js (optional)',
    ru: 'app.js (необязательно)',
    es: 'app.js (opcional)',
    fr: 'app.js (optionnel)',
    zh: 'app.js（可选）',
  },
  'settings.neuralUi.saveTemplate': {
    en: 'Save template',
    ru: 'Сохранить шаблон',
    es: 'Guardar plantilla',
    fr: 'Enregistrer le template',
    zh: '保存模板',
  },
  'settings.neuralUi.hint.random': {
    en: 'Same product always gets the same reference (hash of product id over the pool).',
    ru: 'Один и тот же продукт всегда получает тот же референс (хеш product id по пулу).',
    es: 'El mismo producto siempre recibe la misma referencia (hash del id sobre el pool).',
    fr: 'Un même produit reçoit toujours la même référence (hash de l\'id produit sur le pool).',
    zh: '同一产品始终获得相同的参考（对 pool 取 product id 的哈希）。',
  },
  'settings.neuralUi.hint.roundRobin': {
    en: 'Each new Developer run advances to the next template in order (state file on disk).',
    ru: 'Каждый новый запуск Developer переходит к следующему шаблону по порядку (файл состояния на диске).',
    es: 'Cada nueva ejecución de Developer avanza al siguiente template en orden (estado en disco).',
    fr: 'Chaque nouvelle exécution de Developer passe au template suivant dans l\'ordre (fichier d\'état sur le disque).',
    zh: '每次新的 Developer 运行按顺序前进到下一个模板（磁盘上的状态文件）。',
  },
  'settings.neuralUi.hint.matchSpec': {
    en: 'Chooses the preset whose keywords best overlap with the specification and admin brief; falls back to random if there is no overlap.',
    ru: 'Выбирает пресет с лучшим пересечением ключевых слов со спекой и админским брифом; при отсутствии пересечения — random.',
    es: 'Elige el preset cuyas palabras mejor solapan con la spec y el brief; si no hay solape, vuelve a random.',
    fr: 'Choisit le preset dont les mots-clés recoupent le mieux la spécification et le brief admin ; revient à random en l\'absence de recoupement.',
    zh: '选择关键词与规格及管理员简报重叠度最高的预设；若无重叠则回退到 random。',
  },
  'settings.neuralUi.templateLabel': {
    en: 'Template',
    ru: 'Шаблон',
    es: 'Plantilla',
    fr: 'Template',
    zh: '模板',
  },
  'settings.neuralUi.selectTemplate': {
    en: '— Select template —',
    ru: '— Выберите шаблон —',
    es: '— Seleccionar plantilla —',
    fr: '— Sélectionner un template —',
    zh: '— 选择模板 —',
  },
  'settings.neuralUi.unknownTemplateWarning': {
    en: 'Current id “{id}” is not in the catalog — pick above or fix the path.',
    ru: 'Текущий id «{id}» нет в каталоге — выберите выше или исправьте путь.',
    es: 'El id «{id}» no está en el catálogo — elige arriba o corrige la ruta.',
    fr: 'L\'id actuel « {id} » n\'est pas dans le catalogue — choisissez ci-dessus ou corrigez le chemin.',
    zh: '当前 id “{id}” 不在目录中 — 在上方选择或修正路径。',
  },
  'settings.neuralUi.fixedIdLabel': {
    en: 'Fixed template id (folder name)',
    ru: 'Фиксированный id шаблона (имя папки)',
    es: 'Id fijo de plantilla (carpeta)',
    fr: 'Id de template fixe (nom du dossier)',
    zh: '固定模板 id（文件夹名）',
  },
  'settings.neuralUi.fixedIdPlaceholder': {
    en: 'e.g. aurora-glass',
    ru: 'напр. aurora-glass',
    es: 'p. ej. aurora-glass',
    fr: 'p. ex. aurora-glass',
    zh: '例如 aurora-glass',
  },
  'settings.neuralUi.maxPromptChars': {
    en: 'Max prompt chars:',
    ru: 'Макс. символов в промпте:',
    es: 'Máx. caracteres en el prompt:',
    fr: 'Caractères max du prompt :',
    zh: '提示最大字符数：',
  },
  'settings.section.publicSite': {
    en: 'Public site URL',
    ru: 'URL публичной витрины',
    es: 'URL del sitio público',
    fr: 'URL du site public',
    zh: '公开站点 URL',
  },
  'settings.publicSite.intro': {
    en: 'Canonical HTTPS origin for “Made with AI-Factory” watermarks on free-tier builds, referral share links, and embed badges. Overrides legacy aifactory.dev when the pipeline runs.',
    ru: 'Канонический HTTPS-адрес для водяного знака «Made with AI-Factory» (free), реферальных ссылок и embed-бейджа. Заменяет старый aifactory.dev при следующей сборке.',
    es: 'Origen HTTPS canónico para la marca “Made with AI-Factory” (plan free), enlaces de referidos y la insignia embed. Sustituye aifactory.dev en la próxima build.',
    fr: 'Origine HTTPS canonique pour les filigranes « Made with AI-Factory » des builds free, les liens de parrainage et les badges embed. Remplace l\'ancien aifactory.dev lorsque le pipeline s\'exécute.',
    zh: '用于 free 版构建的 “Made with AI-Factory” 水印、推荐分享链接和嵌入徽章的规范 HTTPS 源。管线运行时覆盖旧的 aifactory.dev。',
  },
  'settings.publicSite.urlLabel': {
    en: 'Public storefront URL (HTTPS)',
    ru: 'URL витрины (HTTPS)',
    es: 'URL de la tienda (HTTPS)',
    fr: 'URL de la vitrine publique (HTTPS)',
    zh: '公开店面 URL（HTTPS）',
  },
  'settings.publicSite.urlPlaceholder': {
    en: 'https://magic-ai-factory.com',
    ru: 'https://magic-ai-factory.com',
    es: 'https://magic-ai-factory.com',
    fr: 'https://magic-ai-factory.com',
    zh: 'https://magic-ai-factory.com',
  },
  'settings.section.siteBadge': {
    en: '“Built with AI-Factory” badge',
    ru: 'Бейдж «Built with AI-Factory»',
    es: 'Insignia “Built with AI-Factory”',
    fr: 'Badge « Built with AI-Factory »',
    zh: '“Built with AI-Factory” 徽章',
  },
  'settings.badge.intro': {
    en: 'After each developer build, inject a small fixed-corner link on every generated *.html file. Point it at your public repo (e.g. GitHub) so visitors can star or fork the factory.',
    ru: 'После каждой сборки Developer вставляйте маленькую ссылку в угол каждого *.html. Укажите на публичный репозиторий (например GitHub), чтобы гости могли поставить звезду или форкнуть фабрику.',
    es: 'Tras cada build de Developer, inyecta un enlace pequeño en la esquina de cada *.html. Apunta a tu repo público (p. ej. GitHub) para que los visitantes puedan dar estrella o hacer fork.',
    fr: 'Après chaque build developer, injecte un petit lien fixé dans un coin de chaque *.html généré. Pointez-le vers votre dépôt public (p. ex. GitHub) pour que les visiteurs puissent star ou forker la factory.',
    zh: '每次 developer 构建后，在每个生成的 *.html 文件角落注入一个固定的小链接。指向你的公开仓库（例如 GitHub），让访客可以给 factory 加星或 fork。',
  },
  'settings.badge.enable': {
    en: 'Enable badge on generated sites',
    ru: 'Включить бейдж на сгенерированных сайтах',
    es: 'Activar insignia en sitios generados',
    fr: 'Activer le badge sur les sites générés',
    zh: '在生成的站点上启用徽章',
  },
  'settings.badge.enableHelp': {
    en: 'Runs when Developer completes (needs HTTPS URL below).',
    ru: 'Срабатывает по завершении Developer (нужен HTTPS URL ниже).',
    es: 'Se aplica al terminar Developer (requiere URL HTTPS abajo).',
    fr: 'S\'exécute à la fin de Developer (nécessite l\'URL HTTPS ci-dessous).',
    zh: '在 Developer 完成时运行（需要下方的 HTTPS URL）。',
  },
  'settings.badge.urlLabel': {
    en: 'Badge link URL (HTTPS)',
    ru: 'URL ссылки бейджа (HTTPS)',
    es: 'URL del enlace de la insignia (HTTPS)',
    fr: 'URL du lien du badge (HTTPS)',
    zh: '徽章链接 URL（HTTPS）',
  },
  'settings.badge.urlPlaceholder': {
    en: 'Leave empty to use Public site URL above',
    ru: 'Пусто — использовать URL витрины выше',
    es: 'Vacío — usar la URL pública de arriba',
    fr: 'Laisser vide pour utiliser l\'URL du site public ci-dessus',
    zh: '留空以使用上方的公开站点 URL',
  },
  'settings.section.headSnippet': {
    en: 'Head snippet on generated sites',
    ru: 'Сниппет в <head> сгенерированных сайтов',
    es: 'Snippet en <head> de sitios generados',
    fr: 'Snippet dans le <head> des sites générés',
    zh: '生成站点 <head> 中的片段',
  },
  'settings.headSnippet.intro': {
    en: 'Raw HTML inserted before </head> on every *.html when Developer finishes (site-verification meta, first-party scripts). Leave empty to disable. Trusted admin content only. Google Analytics / gtag snippets are stripped and are not loaded on this storefront.',
    ru: 'Сырой HTML вставляется перед </head> в каждом *.html по завершении Developer (meta‑верификация, свои скрипты). Пусто — отключено. Только доверенный контент админа. Google Analytics / gtag вырезаются и на витрину Next.js не подключаются.',
    es: 'HTML crudo antes de </head> en cada *.html al terminar Developer (meta de verificación, scripts propios). Vacío para desactivar. Solo contenido de admin de confianza. Los snippets de Google Analytics / gtag se eliminan y no se cargan en esta tienda Next.js.',
    fr: 'HTML brut inséré avant </head> dans chaque *.html à la fin de Developer (balises meta de vérification, scripts internes). Laisser vide pour désactiver. Contenu admin de confiance uniquement. Les snippets Google Analytics / gtag sont retirés et ne sont pas chargés sur cette vitrine Next.js.',
    zh: '在 Developer 完成时，向每个 *.html 的 </head> 之前插入的原始 HTML（站点验证 meta、自有脚本）。留空以禁用。仅限可信的管理员内容。Google Analytics / gtag 片段会被剔除，不会加载到此 Next.js 店面。',
  },
  'settings.headSnippet.label': {
    en: 'HTML / scripts for <head>',
    ru: 'HTML / скрипты для <head>',
    es: 'HTML / scripts para <head>',
    fr: 'HTML / scripts pour <head>',
    zh: '用于 <head> 的 HTML / 脚本',
  },
  'settings.headSnippet.placeholder': {
    en: '<!-- google-site-verification example -->\n<meta name="google-site-verification" content="…" />',
    ru: '<!-- пример google-site-verification -->\n<meta name="google-site-verification" content="…" />',
    es: '<!-- ejemplo google-site-verification -->\n<meta name="google-site-verification" content="…" />',
    fr: '<!-- exemple google-site-verification -->\n<meta name="google-site-verification" content="…" />',
    zh: '<!-- google-site-verification 示例 -->\n<meta name="google-site-verification" content="…" />',
  },
  'settings.demo.readonlyBanner': {
    en: 'Public demo mode: most Settings are read-only (backup, password, head snippet, etc.). Exception: Factory hold at the top — pause or resume the pipeline on the shared demo host.',
    ru: 'Публичное демо: большинство «Настроек» только для чтения (бэкап, пароль, сниппет в head и т.д.). Исключение: «Пауза фабрики» вверху — можно остановить или запустить пайплайн на общем демо-хосте.',
    es: 'Demo público: la mayoría de Ajustes son solo lectura (copia, contraseña, snippet de head, etc.). Excepción: Pausa de la fábrica arriba — pausar o reanudar el pipeline en el host demo compartido.',
    fr: 'Mode démo public : la plupart des Réglages sont en lecture seule (sauvegarde, mot de passe, snippet head, etc.). Exception : la Pause de la factory en haut — mettre en pause ou reprendre le pipeline sur l\'hôte de démo partagé.',
    zh: '公开演示模式：大多数设置为只读（备份、密码、head 片段等）。例外：顶部的工厂暂停 — 可在共享演示主机上暂停或恢复管线。',
  },
  'settings.demo.settingsSaveBlocked': {
    en: 'Demo mode: settings are not saved. Self-host without demo readonly to edit them here.',
    ru: 'Демо-режим: настройки не сохраняются. Поднимите свой инстанс без demo readonly.',
    es: 'Modo demo: los ajustes no se guardan. Autoaloje sin demo readonly.',
    fr: 'Mode démo : les réglages ne sont pas enregistrés. Auto-hébergez sans le readonly de démo.',
    zh: '演示模式：设置不会保存。自托管且不启用 demo readonly。',
  },
  'settings.demo.autoGenBlocked': {
    en: 'Autonomous development cannot be enabled in public demo mode. Set AIFACTORY_DEMO_READONLY=0 in .env on your own instance.',
    ru: 'Автономная разработка недоступна в публичном демо. На своём инстансе установите AIFACTORY_DEMO_READONLY=0 в .env.',
    es: 'El desarrollo autónomo no está disponible en demo público. En su instancia use AIFACTORY_DEMO_READONLY=0 en .env.',
    fr: 'Le développement autonome ne peut pas être activé en mode démo public. Définissez AIFACTORY_DEMO_READONLY=0 dans .env sur votre propre instance.',
    zh: '公开演示模式下无法启用自主开发。在你自己的实例的 .env 中设置 AIFACTORY_DEMO_READONLY=0。',
  },
  'settings.demo.autoGenToggleHint': {
    en: 'Demo protection is on — click the toggle to see why saving is blocked.',
    ru: 'Защита демо включена — нажмите переключатель, чтобы увидеть причину блокировки.',
    es: 'Protección demo activa — pulse el interruptor para ver por qué está bloqueado.',
    fr: 'La protection démo est activée — cliquez sur le commutateur pour voir pourquoi l\'enregistrement est bloqué.',
    zh: '演示保护已开启 — 点击开关查看保存被阻止的原因。',
  },
  'settings.demo.autoGenModalTitle': {
    en: 'Demo protection active',
    ru: 'Активна защита демо-режима',
    es: 'Protección demo activa',
    fr: 'Protection démo active',
    zh: '演示保护已激活',
  },
  'settings.demo.autoGenModalBanner': {
    en: 'AIFACTORY_DEMO_READONLY=1 is set on this host. Platform settings cannot be saved here — autonomous development cannot be enabled. On your own instance set AIFACTORY_DEMO_READONLY=0 in .env and restart the app container.',
    ru: 'На этом хосте включён AIFACTORY_DEMO_READONLY=1. Сохранение настроек заблокировано — автономную разработку включить нельзя. На своём инстансе установите AIFACTORY_DEMO_READONLY=0 в .env и перезапустите контейнер app.',
    es: 'En este host está AIFACTORY_DEMO_READONLY=1. No se pueden guardar ajustes — no se puede activar desarrollo autónomo. En su instancia use AIFACTORY_DEMO_READONLY=0 en .env y reinicie el contenedor app.',
    fr: 'AIFACTORY_DEMO_READONLY=1 est défini sur cet hôte. Les réglages de la plateforme ne peuvent pas être enregistrés ici — le développement autonome ne peut pas être activé. Sur votre propre instance, définissez AIFACTORY_DEMO_READONLY=0 dans .env et redémarrez le conteneur app.',
    zh: '此主机上设置了 AIFACTORY_DEMO_READONLY=1。此处无法保存平台设置 — 无法启用自主开发。在你自己的实例的 .env 中设置 AIFACTORY_DEMO_READONLY=0，并重启 app 容器。',
  },
  'settings.modal.close': {
    en: 'Close',
    ru: 'Закрыть',
    es: 'Cerrar',
    fr: 'Fermer',
    zh: '关闭',
  },
  'settings.modal.cancelSaving': {
    en: 'Cancel',
    ru: 'Отмена',
    es: 'Cancelar',
    fr: 'Annuler',
    zh: '取消',
  },
  'settings.demo.headSnippetBlocked': {
    en: 'This field cannot be saved on the shared demo. Self-host to edit and save the snippet here.',
    ru: 'На общем демо поле не сохраняется. Свой инстанс без demo readonly.',
    es: 'En el demo compartido no se guarda. Autoaloje sin demo readonly.',
    fr: 'Ce champ ne peut pas être enregistré sur la démo partagée. Auto-hébergez pour modifier et enregistrer le snippet ici.',
    zh: '此字段无法在共享演示上保存。请自托管以在此编辑并保存该片段。',
  },
  'settings.headSnippet.footer': {
    en: 'Max 100,000 characters (server truncates beyond that). Already-built pages are not rewritten; run Developer again or edit HTML on disk to apply changes retroactively. This field autosaves a few seconds after edits — blur the field to save immediately.',
    ru: 'Макс. 100 000 символов (сервер обрежет). Уже собранные страницы не переписываются; перезапустите Developer или правьте HTML на диске для ретроактивных изменений. Поле автосохраняется через несколько секунд — уберите фокус для немедленного сохранения.',
    es: 'Máx. 100 000 caracteres (el servidor trunca). Las páginas ya construidas no se reescriben; ejecuta Developer o edita el HTML en disco para cambios retroactivos. Este campo se autoguarda tras unos segundos — pierde el foco para guardar al instante.',
    fr: 'Max 100 000 caractères (le serveur tronque au-delà). Les pages déjà construites ne sont pas réécrites ; relancez Developer ou modifiez le HTML sur le disque pour appliquer les changements rétroactivement. Ce champ s\'enregistre automatiquement quelques secondes après les modifications — retirez le focus du champ pour enregistrer immédiatement.',
    zh: '最多 100,000 个字符（超出部分服务器会截断）。已构建的页面不会被重写；重新运行 Developer 或在磁盘上编辑 HTML 以追溯应用更改。此字段在编辑几秒后自动保存 — 让字段失去焦点可立即保存。',
  },
  'settings.headSnippet.charCount': {
    en: '{n} / 100,000',
    ru: '{n} / 100 000',
    es: '{n} / 100.000',
    fr: '{n} / 100 000',
    zh: '{n} / 100,000',
  },
  'settings.toast.refSlugRequired': {
    en: 'Enter a folder id (slug), e.g. my-brand-shell',
    ru: 'Введите id папки (slug), напр. my-brand-shell',
    es: 'Introduce un id de carpeta (slug), p. ej. my-brand-shell',
    fr: 'Saisissez un id de dossier (slug), p. ex. my-brand-shell',
    zh: '输入文件夹 id（slug），例如 my-brand-shell',
  },
  'settings.toast.refHtmlRequired': {
    en: 'index.html content is required',
    ru: 'Нужно содержимое index.html',
    es: 'Se requiere contenido de index.html',
    fr: 'Le contenu de index.html est requis',
    zh: '需要 index.html 的内容',
  },
  'settings.toast.refTemplateSaved': {
    en: 'Template “{slug}” saved',
    ru: 'Шаблон «{slug}» сохранён',
    es: 'Plantilla “{slug}” guardada',
    fr: 'Template « {slug} » enregistré',
    zh: '模板 “{slug}” 已保存',
  },
  'settings.toast.uploadFailed': {
    en: 'Upload failed',
    ru: 'Загрузка не удалась',
    es: 'Error al subir',
    fr: 'Échec du téléversement',
    zh: '上传失败',
  },
  'settings.confirm.deleteTemplate': {
    en: 'Remove reference template folder “{path}” from disk? This cannot be undone.',
    ru: 'Удалить папку референс‑шаблона «{path}» с диска? Это необратимо.',
    es: '¿Quitar la carpeta de plantilla «{path}» del disco? No se puede deshacer.',
    fr: 'Supprimer le dossier de template de référence « {path} » du disque ? Cette action est irréversible.',
    zh: '从磁盘移除参考模板文件夹 “{path}”？此操作无法撤销。',
  },
  'settings.toast.templateRemoved': {
    en: 'Template removed',
    ru: 'Шаблон удалён',
    es: 'Plantilla eliminada',
    fr: 'Template supprimé',
    zh: '模板已移除',
  },
  'settings.toast.deleteFailed': {
    en: 'Delete failed',
    ru: 'Не удалось удалить',
    es: 'Error al eliminar',
    fr: 'Échec de la suppression',
    zh: '删除失败',
  },
};
