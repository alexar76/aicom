...
/**
 * Origins the client is allowed to send credentials (session cookie, CSRF
 * token, customer Bearer) to. Same-origin is always trusted; an optional
 * NEXT_PUBLIC_API_ORIGIN lets a split deployment point the SPA at a known API
 * host. Anything else is treated as cross-origin and never receives creds.
 */
function buildTrustedOrigins(): Set<string> {
  ...
  if (configured) {
    try {
      ...
      /* ignore malformed env value — fall back to same-origin only */
    }
  }
  return origins;
}
...
/**
 * Resolve a request URL against the current document and confirm its origin is
 * in {@link TRUSTED_ORIGINS}. Relative URLs (the common `/api/...` case) are
 * same-origin by definition and always allowed. Returns false for any
 * cross-origin target so the caller can refuse to attach credentials.
 */
function isTrustedRequestUrl(url: string): boolean {
  if (typeof window === 'undefined') {
    // SSR/build: no document origin to leak credentials cross-origin from.
    return true;
  }
  try {
    const resolved = new URL(url, window.location.href);
    return TRUSTED_ORIGINS.has(resolved.origin);
  } catch {
    return false;
  }
}

/** A per-caller copy, so two coalesced callers cannot see each other's mutations. */
function copyForCaller<T>(value: T): T {
  if (value === null || typeof value !== 'object') return value;
  try {
    return structuredClone(value);
  } catch {
    // Old engine, or a value structuredClone refuses (it never should here — these are
    // JSON bodies). Sharing the object is still better than a second round trip.
    return value;
  }
}
...
export interface AdminMeResponse {
  ...
  /** AIFACTORY_DEMO_READONLY=1 — shared demo host (passwordless admin login). */
  public_demo?: boolean;
...
class ApiClient {
  ...
  /** GET requests currently in flight, keyed by URL + timeout. See `request`. */
  ...
  /**
   * Coalesce identical GETs that are already in flight.
   *
   * The admin console loads the same endpoints from several independent places —
   * `loadAdminDashboardLayers` alone is called from DashboardTab, useMonitorMetrics and
   * prefetchAdminDashboard — so one dashboard render measured 18 requests for 11 distinct
   * endpoints: /admin/dashboard four times, /admin/auth/me, /admin/settings,
   * /admin/dashboard/pipeline-summary and /admin/pipeline/focus-mode twice each. That is not
   * merely wasteful: the HTTP firewall allows 100 requests per minute per IP across every
   * route, so roughly ten tab switches exhausted the budget and the console started answering
   * 403 to everything, /admin/auth/me included — which the UI reads as a lost session.
   *
   * Deduplicating in the client fixes every caller at once and is the half of that problem
   * that does not require weakening a brute-force guard.
   *
   * Deliberately narrow:
   *   - GET/HEAD only. Coalescing a mutation would silently drop one caller's write.
   *   - Skipped when the caller supplies its own `signal`: with a shared promise, one
   *     caller's abort would cancel the other's request too.
   *   - `clientTimeoutMs` is part of the key, so a caller that asked for a short deadline
   *     never inherits a long one.
   *   - The entry is removed as soon as the promise settles, so nothing is cached and a
   *     failure is never replayed. This is request coalescing, not a response cache.
   *   - Each caller gets its own copy of the parsed body, preserving today's behaviour where
   *     one caller mutating the result cannot affect another.
   */
  private async request<T>(
    ...
    const key = `${method} ${this.baseUrl}${endpoint} t=${options.clientTimeoutMs ?? ''}`;
    const existing = this.inFlight.get(key);
    if (existing) {
      return copyForCaller(await (existing as Promise<T>));
    }

    const pending = this.requestUncoalesced<T>(endpoint, options);
    this.inFlight.set(key, pending as Promise<unknown>);
    try {
      return copyForCaller(await pending);
    } finally {
      this.inFlight.delete(key);
    }
  }

  private async requestUncoalesced<T>(
    endpoint: string,
    options: RequestInit & { clientTimeoutMs?: number; publicStorefront?: boolean } = {}
  ): Promise<T> {
    ...
    // F3: never ship credentials (session cookie, CSRF, Bearer) to a host that
    // is not same-origin / the configured API origin. A relative `/api/...`
    ...
    const doFetch = (hdrs: Record<string, string>) =>
      fetch(requestUrl, {
        ...fetchInit,
        headers: hdrs,
        // Only attach the session cookie when the target origin is trusted.
        credentials: trustedTarget ? 'include' : 'omit',
        ...(signal ? { signal } : {}),
      });
    ...
    if (!response.ok) {
      if (response.status === 401 && typeof window !== 'undefined') {
        if (isAdminSessionEndpoint(endpoint)) {
          // Defensively drop any legacy raw token still lingering in storage.
          localStorage.removeItem('admin_token');
  ...
  async getStorefrontCount(): Promise<{
    count: number | null;
    pending: boolean;
    stale: boolean;
  }> {
    const result = await this.request<{
      ...
      pending?: boolean;
      stale?: boolean;
    }>('/products/storefront-count', { publicStorefront: true, clientTimeoutMs: 15_000 });
    const count =
  ...
  async getCategories(): Promise<{
    ...
    const result = await this.request<{
      ...
      pending?: boolean;
      stale?: boolean;
    }>('/products/categories', { publicStorefront: true, clientTimeoutMs: 60_000 });
  ...
  async publicDemoConfig(): Promise<{ public_demo: boolean; passwordless_admin: boolean }> {
    return this.request('/public/demo-config');
  }
  ...
  async getMyAdminActionLog(limit = 100): Promise<AdminActionLogResponse> {
    return this.request(`/admin/action-log/me?limit=${limit}`);
  }

  async getAdminUserActionLog(userId: string, limit = 100): Promise<AdminActionLogResponse> {
    return this.request(`/admin/action-log/users/${encodeURIComponent(userId)}?limit=${limit}`);
  }
  ...
  async getMe(options?: { clientTimeoutMs?: number }): Promise<AdminMeResponse> {
    return this.request('/admin/auth/me', {
      clientTimeoutMs: options?.clientTimeoutMs ?? 15000,
    });
  }
  ...
  async uploadDemoReplayVideo(file: File): Promise<DemoReplayAdminConfig> {
    // Auth via the HttpOnly admin session cookie (credentials:'include').
  ...
  async getPipelineFocusMode(): Promise<{
  ...
  async setPipelineFocusMode(body: {
    focus_product_id?: string;
    auto_select?: boolean;
    resume_factory?: boolean;
    clear_focus?: boolean;
  }): Promise<{
    focus_product_id: string | null;
  ...
  async getAdminProductFiles(
    productId: string,
    opts?: { clientTimeoutMs?: number },
  ): Promise<{
    product_id: string;
    files: Array<Record<string, unknown>>;
    count: number;
    truncated_by_category?: Record<string, boolean>;
  }> {
    return this.request(`/admin/products/${encodeURIComponent(productId)}/files`, {
  ...
  async getAdminSettings(): Promise<{
    factory_on_hold?: boolean;
    factory_focus_product_id?: string | null;
    autonomy_mode?: 'supervised' | 'full';
    ...
    product_catalog_allowlist: string;
    product_catalog_require_github_house: boolean;
  ...
  async updateAdminSettings(
    ...
    options: { clientTimeoutMs?: number; signal?: AbortSignal } = {},
  ...
  async getPublicBenchmark(): Promise<{
    ...
    investor_metrics: {
      ...
      trend_vs_7d: number;
      confidence_interval_95: { low: number; high: number; n: number };
      production_readiness_index: number | null;
    };
  }> {
    const empty = {
      scorecard: {},
      alerts_count: 0,
      degraded: true,
      investor_metrics_source: 'benchmark_scorecard' as const,
      investor_metrics: {
        rolling_24h_pass_rate: null,
        rolling_7d_pass_rate: null,
        latest_pass_rate: null,
        trend_vs_7d: 0,
        confidence_interval_95: { low: 0, high: 0, n: 0 },
        production_readiness_index: null,
      },
    };
    try {
      return await this.request('/benchmark', { publicStorefront: true, clientTimeoutMs: 30_000 });
    } catch {
      return empty;
    }
  }
