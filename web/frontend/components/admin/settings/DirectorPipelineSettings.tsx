        ...
        <div className="space-y-4">
          <label className="flex cursor-pointer flex-col gap-3 rounded-xl bg-white/5 p-3 transition-colors hover:bg-white/10 sm:flex-row sm:items-center sm:justify-between">
            <div className="min-w-0 flex-1">
              ...
              <div className="text-xs text-gray-400 mt-0.5">{t(locale, 'settings.toggle.autonomousDev.help')}</div>
            </div>
            <button
              type="button"
          ...
          >
            ...
            <label className="block text-xs text-gray-400 mb-1">{t(locale, 'settings.label.customIntervalMinutes')}</label>
            <input
              type="number"
              min={15}
              max={10080}
            ...
            <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
              <Button
                size="sm"
                variant="ghost"
                onClick={() => !autoGenSaving && setAutoGenModalOpen(false)}
