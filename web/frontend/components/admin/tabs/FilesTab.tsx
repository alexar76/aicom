...
function filesSandboxStartError(err: unknown): string {
  if (err instanceof ApiRequestError) {
    const code = err.detailPayload?.code;
    if (code === 'sandbox_busy') {
      return 'All live preview slots are in use. Retry in a few seconds.';
    }
    if (code === 'sandbox_start_failed') {
      return err.message || 'Preview could not be started. Retry in a few seconds.';
    }
    if (err.status === 500 && !err.detailPayload) {
      return 'Factory API was unreachable (often a restart). Retry in a few seconds.';
    }
    return err.message;
  }
...
function ProductArtifactsPanel({
  ...
  // `sandboxLoading` covers starting the sandbox and polling it ready — it goes false the
  // moment the src is handed over. But the frame then has to fetch the viewer and boot the
  // product's own SPA, and an iframe painting nothing over a dark panel reads as a broken
  // preview, not a loading one. Hold the overlay until the frame's own load event fires.
  const [frameBooting, setFrameBooting] = useState(false);
  useEffect(() => {
    setFrameBooting(Boolean(sandboxIframeSrc));
  }, [sandboxIframeSrc, sandboxReloadKey]);
  const previewBusy = sandboxLoading || frameBooting;
  ...
  return (
    <>
        ...
        {sandboxLoading && !sandboxIframeSrc ? (
          ...
          <div className="relative aspect-video h-[min(40vh,320px)] w-full sm:h-[min(45vh,380px)]">
            <iframe
              ...
              onLoad={() => setFrameBooting(false)}
            />
            {previewBusy ? (
              <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-black/70 backdrop-blur-[1px] px-6 text-center">
                <Loader2 className="h-8 w-8 animate-spin text-white" />
                <span className="text-xs text-gray-300">
                  {sandboxProgress?.label ?? 'Opening preview…'}
                </span>
                {sandboxProgress ? (
                  <ProgressBar
                    value={sandboxProgress.percent}
                    max={100}
                    className="w-full max-w-xs h-1.5"
                  />
                ) : null}
              </div>
            ) : null}
          ...
          <div className="flex aspect-video min-h-[200px] flex-col items-center justify-center gap-3 px-4 text-center text-sm text-gray-500">
            {fileLoading ? (
              ...
              <div className="flex flex-col items-center gap-3">
                <span className="max-w-md text-amber-400">{filesError}</span>
                <Button type="button" variant="secondary" size="sm" onClick={onRetryFiles}>
                  Retry
                </Button>
              </div>
            ) : files.length === 0 ? (
      ...
      {sandboxModalOpen && sandboxIframeSrc ? (
        ...
        >
          ...
          <div className="relative flex min-h-0 flex-1 flex-col">
            ...
            />
            {previewBusy ? (
              <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-black/60 backdrop-blur-[1px] text-center">
      ...
      {fileLoading ? (
        ...
        <div className="py-6 text-center text-sm text-amber-400">{filesError}</div>
      ) : files.length === 0 ? (
...
export function FilesTab({ locale }: { locale: AdminLocale }) {
  ...
  // Five attempts with a 400ms-doubling backoff gave up after ~6 seconds. The backend restarts in
  // 30-60, and during a deploy every one of those attempts hits ECONNREFUSED through the Next
  // proxy — which is exactly the window in which a user is most likely to be clicking. The
  // request itself is cheap (0.9 MB, under a second for 1661 files), so waiting is the cheaper
  // error.
  const FILES_ATTEMPTS = 7;
  const FILES_BACKOFF_CAP_MS = 15_000;
  ...
  const loadFiles = async (productId: string) => {
    ...
    for (let attempt = 0; attempt < FILES_ATTEMPTS; attempt++) {
      try {
        // Cookie session via api.ts — never attach leftover localStorage JWT.
        // A stale Bearer used to 401 this call while the catalog (cookie-only) still loaded.
        const data = await api.getAdminProductFiles(productId, { clientTimeoutMs: FILES_FETCH_MS });
        ...
        const retriable =
          status !== 401 &&
          status !== 403 &&
          !/not authenticated|unauthorized/i.test(msg);
        if (!retriable) break;
        if (attempt < FILES_ATTEMPTS - 1) {
          await new Promise((r) => setTimeout(r, Math.min(FILES_BACKOFF_CAP_MS, 400 * 2 ** attempt)));
        }
      }
    }
    setFiles([]);
    ...
    // "No files to preview" is a statement about the product; a failed request is a statement
    // about us. Conflating them told a user with 1661 files on disk that the product had none,
    // every time the backend was briefly unreachable — and the toast that said otherwise was gone
    // seconds later. The panel now keeps the real reason on screen.
    setFilesError(
      /401|403|not authenticated|unauthorized|invalid or expired token|expired/i.test(msg)
        ? 'Your admin session expired — sign in again at /admin/login, then reopen this tab.'
        : `Could not load the file list: ${msg}`,
    );
  ...
  const refreshSandbox = useCallback(async () => {
    ...
    try {
      ...
      setSandboxIframeSrc(null);
      const msg = filesSandboxStartError(e);
      setSandboxError(msg);
      toast.error(msg);
    } finally {
      setSandboxLoading(false);
  ...
  }, [selectedProduct, setOwnerZipBusy]);
  ...
  const artifactsPanel =
    selectedProduct != null ? (
      <ProductArtifactsPanel
        ...
        onRetryFiles={() => selectedProduct && void loadFiles(selectedProduct)}
