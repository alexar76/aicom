...
export function DirectorTab({ locale }: { locale: AdminLocale }) {
  ...
  const [decisions, setDecisions] = useState<{ pending: any[]; applied: any[]; pending_count: number; total_count: number }>({ pending: [], applied: [], pending_count: 0, total_count: 0 });
  ...
  const handleApprove = async (decisionId: string) => {
    ...
    try {
      ...
      const updated = await api.getDirectorDecisions();
  ...
  const handleTriggerBenchmark = async () => {
    ...
    try {
      ...
      const updated = await api.getBenchmarkScorecard();
  ...
  const loadCockpit = async () => {
    ...
    try {
      const data = await api.getReleaseCockpit(pid);
      setCockpit(data);
    } catch (e: any) {
