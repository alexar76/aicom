...
import { PipelineFocusPanel } from '@/components/admin/pipeline/PipelineFocusPanel';
  ...
  return (
    <motion.div className="space-y-4">
      ...
      <FilterControlsPanel
        ...
        gridClassName="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-8 gap-2"
      >
        <Input
          value={productSearch}
        ...
        <FilterSelect
          value={deliveryProfileFilter}
          onChange={(e) => setDeliveryProfileFilter(e.target.value as typeof deliveryProfileFilter)}
          className="px-3 py-2"
        >
          <option value="all">{t(locale, 'pipeline.filter.deliveryAll')}</option>
          <option value="full_software">{t(locale, 'pipeline.filter.deliveryFullSoftware')}</option>
          <option value="marketing_landing">{t(locale, 'pipeline.filter.deliveryMarketingLanding')}</option>
          <option value="landing_fast">{t(locale, 'pipeline.filter.deliveryLandingFast')}</option>
        </FilterSelect>
        <FilterSelect
          value={pipelineWorkFilter}
          onChange={(e) => setPipelineWorkFilter(e.target.value as typeof pipelineWorkFilter)}
          className="px-3 py-2"
        >
          <option value="all">{t(locale, 'pipeline.filter.workAll')}</option>
          <option value="active">{t(locale, 'pipeline.filter.workActive')}</option>
          <option value="paused">{t(locale, 'pipeline.filter.workPaused')}</option>
          <option value="focus">{t(locale, 'pipeline.filter.workFocus')}</option>
        </FilterSelect>
        <FilterSelect
          value={repairRoundMin}
          onChange={(e) => setRepairRoundMin(e.target.value as typeof repairRoundMin)}
          className="px-3 py-2"
        >
          <option value="all">{t(locale, 'pipeline.filter.repairAll')}</option>
          <option value="1">{tVars(locale, 'pipeline.filter.repairMin', { n: 1 })}</option>
          <option value="2">{tVars(locale, 'pipeline.filter.repairMin', { n: 2 })}</option>
          <option value="3">{tVars(locale, 'pipeline.filter.repairMin', { n: 3 })}</option>
        </FilterSelect>
        <FilterSelect
