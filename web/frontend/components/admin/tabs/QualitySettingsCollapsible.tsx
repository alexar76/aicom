...
export type QualitySettingsState = {
  ...
  max_pipeline_repair_rounds_landing: number;
  monitoring_dev_refresh_enabled: boolean;
...
export const OPTIMIZED_QUALITY_PRESETS: Pick<
  QualitySettingsState,
  | 'max_pipeline_cost_usd'
  | 'max_pipeline_repair_rounds'
  | 'max_pipeline_repair_rounds_landing'
  | 'monitoring_dev_refresh_enabled'
> = {
  max_pipeline_cost_usd: 5,
  max_pipeline_repair_rounds: 10,
  max_pipeline_repair_rounds_landing: 8,
  monitoring_dev_refresh_enabled: false,
};

export const LEGACY_QUALITY_PRESETS: Pick<
  QualitySettingsState,
  | 'max_pipeline_cost_usd'
  | 'max_pipeline_repair_rounds'
  | 'max_pipeline_repair_rounds_landing'
        ...
        <div className="space-y-5 border-t border-white/10 px-4 pb-4 pt-3">
          ...
          <div>
            ...
            <div className="space-y-3">
              ...
              />
              <NumberRow
                label={t(locale, 'settings.quality.maxRepairLanding.label')}
                description={t(locale, 'settings.quality.maxRepairLanding.desc')}
                value={quality.max_pipeline_repair_rounds_landing}
                min={1}
                max={100}
                onChange={(v) => onChange('max_pipeline_repair_rounds_landing', v)}
                disabled={disabled}
              />
            </div>
          </div>
          ...
          <div>
            <h4 className="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-500">
              {t(locale, 'settings.quality.section.postShip')}
            </h4>
            <div className="space-y-3">
              <ToggleRow
                label={t(locale, 'settings.quality.monitoringDevRefresh.label')}
                description={t(locale, 'settings.quality.monitoringDevRefresh.desc')}
                checked={quality.monitoring_dev_refresh_enabled}
                onChange={(v) => onChange('monitoring_dev_refresh_enabled', v)}
                disabled={disabled}
              />
            </div>
          </div>
