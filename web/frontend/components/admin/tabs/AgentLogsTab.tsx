...
export function AgentLogsTab() {
  ...
  const agentTypes = [
    'pm', 'methodologist', 'architect', 'developer', 'devops', 'qa',
    'security', 'marketing', 'sales', 'evolution_analyst', 'audit',
  ];

  const loadLogs = async (agent?: string) => {
    setLoading(true);
    try {
      const filter = agent && agent !== 'all' ? agent : undefined;
      const data = await api.getAgentLogs(filter, 500);
      setLogs(data.logs || []);
  ...
  return (
    <div className="space-y-4">
      ...
      <FilterResetSummary
        ...
        summary={`Showing ${filteredLogs.length} filtered / ${logCount} loaded / ${logTotal} total log entries`}
      />
