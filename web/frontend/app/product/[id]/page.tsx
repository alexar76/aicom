...
import api, { ApiRequestError, Product } from '@/lib/api';
...
import { formatSpecFeature, formatUserStory, labelTechStackKey, formatDataModelFields, formatDataModelRelationships } from '@/lib/product-spec';
import { CATEGORY_EMOJIS, CATEGORY_LABELS } from '@/lib/categories';
import { buildProductShareUrl, trackEvent } from '@/lib/analytics';
import { StorefrontRouteLoader } from '@/components/storefront/StorefrontRouteLoader';
...
export default function ProductDetailPage() {
  ...
  const [storefrontLocale, setStorefrontLocale] = useState<MarketingLocale>('en');
  ...
  const sandboxStartErrorMessage = (err: unknown): string => {
    if (err instanceof ApiRequestError) {
      const code = err.detailPayload?.code;
      if (code === 'sandbox_busy') {
        return 'All live preview slots are in use. Please wait a minute and try again.';
      }
      if (code === 'host_disk_full') {
        return 'The server does not have enough disk space to start a preview right now.';
      }
      if (code === 'resources_insufficient') {
        return 'Not enough server resources for a full preview. Try again later.';
      }
      return err.message;
    }
  ...
  const handleStartSandbox = async () => {
    ...
    try {
      ...
      const msg = sandboxStartErrorMessage(err);
      setSandboxError(msg);
  ...
  if (loading) {
    return <StorefrontRouteLoader label="Loading product…" />;
  }
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
                  ...
                  <div className="px-4 pb-4 space-y-3 border-t border-white/5 pt-3">
                    {product.architecture.data_models.map((dm: any, i: number) => (
                      <div key={i} className="text-sm">
                        ...
                        {dm.fields && Array.isArray(dm.fields) && formatDataModelFields(dm.fields) && (
                          <p className="text-xs text-gray-500 mt-1 font-mono">{formatDataModelFields(dm.fields)}</p>
                        )}
                        {dm.relationships && formatDataModelRelationships(dm.relationships) && (
                          <p className="text-xs text-gray-500 mt-0.5">{formatDataModelRelationships(dm.relationships)}</p>
                        )}
                      </div>
                    ))}
                  </div>
        ...
        >
          ...
          {sandboxError && (
            <div
              role="alert"
              className="w-full flex items-start gap-2 rounded-lg border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm text-red-200"
            >
