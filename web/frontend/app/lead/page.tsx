...
export default function LeadPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await submitLead({ email, idea, name: name || undefined, company: company || undefined });
      trackEvent('lead_submit', { source: 'lead_page' });
      setDone(true);
    } catch (err: unknown) {
  ...
  return (
    <div className="min-h-screen px-4 py-16 pt-24 max-w-xl mx-auto">
      ...
      <p className="text-gray-400 mb-8 text-sm">
        Tell us what you want built. We log requests for review and follow-up (no spam automation).
      </p>

      {done ? (
        <GlassCard className="text-center py-10">
          <CheckCircle2 className="w-12 h-12 text-emerald-400 mx-auto mb-4" />
          <p className="text-white font-medium mb-2">Received</p>
          <p className="text-gray-400 text-sm mb-6">
            Thanks — your idea was saved. We’ll get back via email when appropriate.
          </p>
        ...
        <GlassCard>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button type="submit" disabled={loading} icon={loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />}>
