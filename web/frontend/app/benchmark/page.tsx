...
export default function BenchmarkPage() {
  ...
  useEffect(() => {
    api
      ...
      .then((payload) => {
        setData(payload);
        setErr(payload.degraded ? 'Metrics are temporarily unavailable — showing placeholders.' : null);
      })
      .catch(() => setErr('Metrics are temporarily unavailable — showing placeholders.'));
  }, []);
  ...
  return (
    <main className="min-h-screen px-4 py-12 max-w-4xl mx-auto">
      ...
      {err && <p className="text-amber-400/90 text-sm mb-4">{err}</p>}
