...
export default function DocsPage({ initialLocale }: { initialLocale: DocLocale }) {
  // Resolved on the server (app/docs/page.tsx) so both render passes agree; see HomePage.
  const [locale, setLocale] = useState<DocLocale>(initialLocale);

  // docs-locale and marketing_locale in localStorage are choices the server cannot see.
  useEffect(() => {
    const stored = detectStoredDocLocale();
    if (!stored) return;
    setLocale(stored);
