...
class SecurityManager:
    ...
    def log_audit(
        ...
        *,
        ...
        """Public audit entry point (hash-chain preferred)."""
        entry: dict = {
            "action": action,
            "username": actor,
            "resource": resource,
...
async def get_current_admin(
    ...
    Validates JWT from Authorization header, then cookies (stale Bearer does not
    hide a valid session cookie).
    """
    ...
    # Header first, then cookies. A stale ``Authorization: Bearer`` from leftover
    # localStorage must not mask a valid HttpOnly ``aif_admin_session`` cookie —
    # Admin → Files used to fail with "Invalid or expired token" while the rest
    # of the dashboard (cookie-only api.ts) still worked.
    candidates: list[str] = []
    if credentials and credentials.credentials:
        candidates.append(credentials.credentials)
    for cookie_name in ("access_token", "aif_admin_session"):
        ck = request.cookies.get(cookie_name)
