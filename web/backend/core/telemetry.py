...
class TelemetryCollector:
    ...
    def record_event(
        ...
        # Save to product telemetry file. The id reaches us from a PUBLIC route, so it is
        # confined here as well as validated at the schema: a caller must not be able to
        # place a directory or an appended JSONL line outside the telemetry root.
        root = self.data_root.resolve()
        product_dir = (root / product_id).resolve()
        if product_dir != root and root not in product_dir.parents:
            raise ValueError(f"product_id escapes the telemetry root: {product_id!r}")
