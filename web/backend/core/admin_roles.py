...
Legacy JWTs without a `role` claim are treated as **admin** (not super_admin) so
existing installs keep pipeline access without automatic user-management privilege.
Invalid role strings fail closed to **viewer**.
"""
...
VIEWER_BLOCKED_GET_PREFIXES: tuple[str, ...] = (
    ...
    # Config plane: status names the pipeline database (masked, but still topology).
...
OPERATOR_WRITE_DENIED_PREFIXES: tuple[str, ...] = (
    ...
    "/api/admin/feedback",
    "/api/admin/factory-backup",
    # NOT just "copy the pipeline database": POST test-connection makes the server dial a
    # caller-supplied database_url (SSRF), and POST migrate-sqlite-to-postgres WRITES the
    # pipeline database into one. That is a data-exfiltration primitive, and the operator
    # role is documented as "runs the factory", not "moves the factory's data off-box".
    "/api/admin/pipeline-database",
)
