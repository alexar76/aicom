...
<head>
  ...
  <!-- Root-relative on purpose: served from the factory host this resolves to its own
       public/fonts, and if the generated product is lifted onto someone else's host the
       font-family chains below fall through to system-ui. A Google URL here would instead
       have made every page the factory generates disclose its visitors to a third party. -->
...
def resolve_sandbox_index_html(
    ...
    *,
    ...
    if is_factory_boilerplate_index(on_disk_html):
        ...
        from web.backend.services.sandbox_static_entry import is_unbuilt_spa_dev_shell

        if is_unbuilt_spa_dev_shell(on_disk_html):
            built = build_spec_landing_html(product_id)
            html_out = built if built else on_disk_html
        else:
