...
def inject_preview_api_fetch_shim(
    ...
    *,
    ...
    Forces ``credentials: 'omit'`` so an opaque-origin iframe (no ``allow-same-origin``) never
    attaches the operator's factory session cookies to preview API calls — auth is the
    ``X-Sandbox-Preview-Token`` only. Opaque frames also send ``Origin: null``; the factory
    answers those sandbox paths with ``Access-Control-Allow-Origin: null`` (see
    ``web.backend.middleware.sandbox_opaque_cors``).
    """
    import json
    ...
    shim = (
        ...
        "function rewrite(u){if(typeof u!=='string')return u;"
        "if(u.indexOf(BASE)===0)return u;if(u.startsWith('/api/'))return BASE+u;return u;}"
        "function withAuth(init){init=init?Object.assign({},init):{};"
        # Opaque iframe + operator cookies must never meet: token is the only credential.
        "init.credentials='omit';"
        "if(!PT)return init;"
        "var h=new Headers(init.headers||{});if(!h.has('X-Sandbox-Preview-Token'))"
        "h.set('X-Sandbox-Preview-Token',PT);init.headers=h;return init;}"
        ...
        "XMLHttpRequest.prototype.open=function(m,u){if(typeof u==='string')arguments[1]=rewrite(u);"
        "this.__aicomCreds=false;try{this.withCredentials=false;}catch(e){}"
        "return XO.apply(this,arguments);};"
        "XMLHttpRequest.prototype.send=function(){if(PT)try{this.setRequestHeader('X-Sandbox-Preview-Token',PT);}catch(e){}"
...
def sandbox_static_base_href(
    ...
    *,
    ...
    Must include the HTML file's directory so Vite SPA assets like ``./assets/index-….js``
    resolve to ``…/frontend/dist/assets/…`` rather than the sandbox root (404).
    """
    rel = (file_path or "").replace("\\", "/").lstrip("/")
    if rel and "/" in rel:
        dir_part = rel.rsplit("/", 1)[0].rstrip("/") + "/"
    else:
        dir_part = ""
    path = f"/api/sandbox/file/{sandbox_id}/{dir_part}"
...
def _inject_iframe_base_href(
    ...
    *,
    ...
    return inject_html_base_href(
        html,
        sandbox_static_base_href(sandbox_id, request, file_path=file_path),
    )
...
#: Permissions the preview needs that cost nothing.
_SANDBOX_IFRAME_BASE = (
    "allow-scripts allow-forms allow-popups allow-downloads "
    "allow-modals allow-pointer-lock"
)


def sandbox_iframe_sandbox_attr() -> str:
    """The iframe ``sandbox`` attribute for generated preview content.

    ``allow-same-origin`` is NOT included by default, and that omission is the security
    property. Paired with ``allow-scripts`` on a document served from our own origin — which
    is what ``sandbox_public_url`` produces — the HTML sandbox spec leaves the framed
    document in its REAL origin, so the attribute isolates nothing: the generated script can
    read and write same-origin storage and call the factory's own API with whatever cookies
    the operator's browser is carrying. And the script's author need not be the operator: an
    anonymous caller can queue a build through POST /api/public/generate-landing, which the
    operator then opens in Admin -> Preview.

    Without it the frame gets an opaque origin, which is what a sandbox is for. Preview
    ...
    if os.environ.get("AIFACTORY_SANDBOX_PREVIEW_ALLOW_SAME_ORIGIN", "").strip().lower() in (
        "1", "true", "yes",
    ):
        return _SANDBOX_IFRAME_BASE + " allow-same-origin"
    return _SANDBOX_IFRAME_BASE


#: Back-compat for importers; prefer the function, which honours the env at render time.
SANDBOX_IFRAME_SANDBOX_ATTR = sandbox_iframe_sandbox_attr()
