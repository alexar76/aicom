...
def try_publish_working_app(product_id: str) -> dict[str, Any]:
    ...
    try:
        # WAIT here, unlike the HTTP routes: the decision below is made from
        # `compose_preview.status` / `preview_api.enabled`, which are only populated once the
        # bootstrap has actually brought the stack up. This runs in a background job, so a
        # multi-minute call is fine — an HTTP request is not (see _start_sandbox_for_product).
        result = start(product_id, storefront=False, wait_for_bootstrap=True)
    except Exception as exc:
