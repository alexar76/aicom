...
def _wants_screenshot(blog: dict[str, Any]) -> bool:
    """Hero shot unless Marketing explicitly opted out (CLI / API-only products)."""
    if blog.get("include_screenshot") is False:
        return False
    return True
...
def publish_product_launch_blog_post(
    ...
    *,
    ...
    if not post:
        logger.warning("blog: skip %s — no marketing content", product_id)
        return False
    ...
    try:
        ...
        logger.exception("blog: cannot write launch post %s for %s", slug, product_id)
        return False
