...
def analyze_visual_quality(
    *,
    ...
    tailwindish = bool(
        re.search(r"@tailwind\b|tailwindcss|@theme\b", css_bundle, re.I)
        or re.search(r"\b(?:sm|md|lg|xl):", combined)
    )
    if n_vars < 3 and not tailwindish:
        issues.append(
            {
    ...
    # Landmarks — Vite shell has no <main>; the React tree does.
    ...
    # Responsive media queries — Tailwind breakpoints live as sm:/md: in TSX, not @media in source CSS.
    if "@media" not in css_bundle and not re.search(r"\b(?:sm|md|lg|xl):", combined):
        issues.append(
            {
    ...
    # Mobile nav pattern. Bare `md:hidden` on a card is not a menu; require a
    # toggle token or a Tailwind hide next to nav/header/menu.
    has_nav_toggle = bool(
        re.search(
        ...
        re.search(
            r"(?:<nav\b|<header\b|role=[\"']navigation[\"'])[\s\S]{0,1200}(?:md|lg):hidden"
            r"|(?:md|lg):hidden[\s\S]{0,400}(?:\bnav\b|menu|drawer)",
            lower,
        )
    )
    ...
    if app_like:
        has_page_shell = bool(
            re.search(
                r"page-shell|padding-inline|padding:\s*0\s+24px|padding:\s*0\s+1\.5rem|"
                r"aicom-visual-gate-autofix page shell",
                css_bundle,
                re.I,
            )
        )
        bare_main = bool(
            re.search(r"<main\s+className\s*=\s*['\"]\s*['\"]", combined)
            or re.search(r"<main\s+className=\{['\"]\s*['\"]\}", combined)
        )
        if bare_main and not has_page_shell:
            issues.append(
                {
                    "code": "visual_flush_page_edge",
                    "detail": "App pages use bare <main> without horizontal inset — add .page-shell or .container (16–24px padding-inline, max-width).",
                }
            )

        skel = bool(
            re.search(
                r"(skeleton|shimmer|animate-pulse|aria-busy|data-loading|loading-state|"
                r"\.pulse\b|isloading|is_loading|setloading|\{loading|loading\s*&&|spinner)",
                lower,
            )
        )
        if not skel:
        ...
        empty_ok = bool(
            re.search(
                ...
                r"no results|isempty|is_empty|length\s*===\s*0|length\s*==\s*0)",
                lower,
            )
        )
        if not empty_ok:
        ...
        err_ok = bool(
            re.search(
                r"(role=[\"']alert[\"']|toast|snackbar|error-state|data-toast|validation-error|"
                r"\.toast\b|seterror|set_error|\{error|error\s*&&)",
                lower,
            )
        )
        if not err_ok:
