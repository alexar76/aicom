...
def spa_routes_from_source(code_dir: Path) -> list[str]:
    """React Router routes are not <a href>s. A BFS over links therefore never left `/`.

    Measured on Sentinel: pages_visited=1, spec_alignment_llm_failed because /login,
    /operator and /analytics exist in App.tsx and nowhere as markup links.
    """
    found: list[str] = []
    seen: set[str] = set()
    roots = [code_dir / "frontend" / "src", code_dir / "src"]
    for root in roots:
        if not root.is_dir():
            continue
        for path in sorted(root.rglob("*")):
            ...
            try:
                ...
                continue
            for match in _SPA_ROUTE_RE.finditer(text):
                route = (match.group(1) or "").strip()
                if not route or route == "/" or "*" in route:
                    continue
...
def exception_in_product_output(text: str) -> str:
    """A Python exception that leaked into a 200 body or the visible UI.

    Honesty-policy products wrap ``except Exception`` as ``{"level": "UNKNOWN", "reason": ...}``.
    Every sandbox gate saw 200. A visitor sees ``got an unexpected keyword argument 'west'``.
    """
    if not text:
        return ""
    low = text.lower()
    ...
    for needle in needles:
        ...
        if idx < 0:
            continue
        start = max(0, idx - 48)
        return " ".join(text[start : start + 240].split())
    return ""


def e2e_credentials() -> dict[str, str]:
    """Login the crawl uses. Must be the same identity the sandbox preview seeds.

    Default used to be ``e2e-user@example.invalid`` / ``e2e-password-change-me``.
    Preview seeds ``SANDBOX_DEMO_*``. Every /operator and /analytics visit then
    401'd, spec_alignment_llm concluded the consoles did not exist, and the
    developer was sent to polish the public widget for ten hours.
    ``AIFACTORY_E2E_*`` still overrides when an operator sets them.
    """
    ...
    email = (os.environ.get("AIFACTORY_E2E_EMAIL") or "").strip() or sandbox_demo_email()
    password = (
        os.environ.get("AIFACTORY_E2E_PASSWORD") or ""
    ).strip() or effective_sandbox_demo_password_for_compose()
    username = (os.environ.get("AIFACTORY_E2E_USERNAME") or "").strip() or email.split("@", 1)[0]
    full_name = (os.environ.get("AIFACTORY_E2E_FULL_NAME") or "").strip() or "Sandbox Demo"
    return {
        "email": email,
        "username": username,
        "password": password,
...
def _layout_defects(page: Any, limit: int = 12) -> dict[str, Any]:
    """Elements whose own content does not fit inside them, measured in the browser.

    The pipeline shipped a landing whose section icons were cut off by the squares
    they sat in, and every gate passed it. Nothing here could have seen it: the HTTP
    status was 200, the text snippet was unchanged, the quality score is a number
    about content, and `llm/visual_quality_system.py` is a PROMPT — an instruction to
    the generator, not a check on what it produced. Screenshots were taken and filed,
    but the spec-alignment reviewer is explicitly a text-only LLM and receives their
    PATHS, so no one ever looked at them.

    A clipped icon, a headline running out of its column, a table forcing the page
    sideways — all of these are one measurable fact: rendered content wider than the
    box that holds it. This asks the page, which costs one evaluate() per crawled URL
    and needs no vision model.

    Deliberately conservative, because a false alarm here fails somebody's build:
      * only leaf elements — a clipped child would otherwise also indict every
        ancestor that contains it;
      * only boxes that actually clip (`overflow: hidden/clip/auto/scroll` do so on
        purpose, and are skipped — a scroll container is meant to scroll);
      * a 3px tolerance for sub-pixel rounding;
      * elements with no text are ignored, so decorative spacers stay quiet.
    """
    try:
        found = page.evaluate(
            """(lim) => {
            ...
            for (const el of document.querySelectorAll('body *')) {
                ...
                // A box that declares it clips or scrolls is doing so on purpose.
                ...
                if (over <= 3) continue;
                const key = el.tagName + '|' + (el.className || '') + '|' + text.slice(0, 24);
                if (seen.has(key)) continue;                      // one row per repeated card
                seen.add(key);
                out.push({
                    tag: el.tagName.toLowerCase(),
                    cls: String(el.className || '').slice(0, 60),
                    overflow_px: over,
                    box_px: el.clientWidth,
            ...
            return {
                clipped: out,
                // The page itself scrolling sideways is the same defect one level up.
                page_overflow_px: Math.max(0, doc.scrollWidth - doc.clientWidth),
            };
        }""",
            limit,
        )
    ...
    return {"clipped": [], "page_overflow_px": 0}
            ...
            try:
                ...
                for ii in range(icount):
                    ...
                    try:
                        ...
                        elif any(k in label for k in ("lat", "latitude")):
                            # Empty required lat/lon is how the Vercel gate clicked Get Safety
                            # Status and certified a product whose TypeError never ran.
                            el.fill("52.52", timeout=2_000)
                        elif any(k in label for k in ("lon", "lng", "longitude")):
                            el.fill("13.40", timeout=2_000)
...
def run_deep_crawl(
    ...
    *,
    ...
    ``seed_urls`` are SPA routes taken from source (React Router) so the crawl is not stuck on `/`.
    """
    ...
    for seed in seed_urls or []:
        ...
        if not seed:
            continue
        if seed.startswith("/"):
            seed = urljoin(base_origin.rstrip("/") + "/", seed.lstrip("/"))
        if same_origin(seed, base_origin) or seed.startswith(base_origin):
            queue.append((seed, 0))
    ...
    while queue and len(visited) < max_pages:
        ...
        # Fill then submit first. Clicking the primary button on an empty required form is a
        # no-op (HTML5 validation), which is how the live gate certified Sentinel while
        # AtlasClient.get_situation_brief(west=...) never ran.
...
def _http_error_is_auth_wall(url: str, status: int, snippet: str) -> bool:
    """401/403 on /api/* or a login form is a wall, not a missing page."""
    ...
    return any(w in low for w in ("login", "password", "sign in", "unauthorized", "access token"))
...
def deep_crawl_gate_issues(summary: dict[str, Any]) -> list[str]:
    ...
    for p in summary.get("pages") or []:
        ...
        if isinstance(st, int) and st >= 400:
            if _http_error_is_auth_wall(str(p.get("url") or ""), st, str(p.get("text_snippet") or "")):
                continue
            issues.append(f"deep_http_{st}:{p.get('url','')}")
        err = p.get("error")
        if err:
            issues.append(f"deep_page_error:{p.get('url','')}:{err}")
        leak = exception_in_product_output(str(p.get("text_snippet") or ""))
        if leak:
            issues.append(
                f"deep_exception_in_ui:{p.get('url','')}:{leak}. A 200 page that prints a "
                "Python TypeError is a call/signature mismatch hidden as UNKNOWN — not missing "
                "sensor data. Fix the call to match the method; do not swallow Exception."
            )
        # Content that does not fit the box holding it. This is the class of defect
        # that produced a shipped landing with its section icons cut in half: a 200
        # page, unchanged text, a passing score, and a screenshot nobody read.
        layout = p.get("layout") or {}
        for c in (layout.get("clipped") or [])[:6]:
            issues.append(
                f"deep_layout_clipped:{p.get('url','')}:<{c.get('tag','?')} "
                f"class={c.get('cls','')}> overflows its {c.get('box_px','?')}px box by "
                f"{c.get('overflow_px','?')}px — text {c.get('text','')!r} is being cut off. "
                "Fix the box or the type size; do not add overflow:hidden, which hides "
                "the clipping instead of removing it."
            )
        page_over = layout.get("page_overflow_px") or 0
        if isinstance(page_over, (int, float)) and page_over > 3:
            issues.append(
                f"deep_layout_page_overflow:{p.get('url','')}:the page scrolls sideways by "
                f"{int(page_over)}px — something inside is wider than the viewport."
            )
        if strict_ui:
