...
def load_security_report(product_id: str, *, data_root: str | Path | None = None) -> Optional[dict[str, Any]]:
    ...
    # SECURITY: reject path traversal attempts (../, /, etc.)
    _bad = {"..", "/", "\\"}
    if any(c in product_id for c in _bad) or not product_id.strip():
        logger.warning("Rejected suspicious product_id in security report lookup: %r", product_id)
        return None
