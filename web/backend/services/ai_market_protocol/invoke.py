...
def reseller_label(presented: str | None) -> str:
    """Which declared reseller this key belongs to, or "" for everyone else.

    ``AIFACTORY_RESELLER_KEYS`` is ``label=key`` pairs, comma separated. Compared with
    ``compare_digest`` because an ``==`` on a secret leaks its prefix by timing, and
    guarded against non-ASCII: HTTP headers arrive latin-1 decoded, and one high byte
    would turn a rejection into a 500.
    """
    import hmac
    import os

    # Type-checked, not just falsy-checked: a caller that forwards a FastAPI
    # `Header(...)` default instead of a resolved value hands this a Header OBJECT,
    # which is truthy — `(presented or "")` keeps it and `.strip()` raises, turning an
    # anonymous invoke into a 500. The route that did that is fixed; this makes the
    # same slip a rejection rather than an outage, which is the rule the rest of this
    # function already follows.
    key = presented.strip() if isinstance(presented, str) else ""
    if not key or not key.isascii():
        return ""
    raw = os.environ.get("AIFACTORY_RESELLER_KEYS", "").strip()
    if not raw:
        return ""
    for pair in raw.split(","):
        label, _, value = pair.partition("=")
        value = value.strip()
        if value and hmac.compare_digest(value, key):
            return label.strip()[:60] or "reseller"
    return ""
...
async def invoke_capability_v1(
    *,
    ...
    reseller = reseller_label(x_api_key)

    if reseller:
        # A declared reseller has already taken the buyer's money on its own rail and is
        # the seller of record for this call — charging here would bill the same invoke
        # twice. Without this, a hub that lists our capabilities cannot complete a single
        # sale: it collects, forwards, and gets our 402 back for a buyer who has no
        # account here. The key is issued by this operator, per reseller, and is the
        # entire trust statement; nothing about the request body can grant it.
        paid = True
        payment_kind = "reseller"
        payment_ref = reseller
    ...
    uni_receipt = None
    # A reseller sale was settled in USD on the reseller's rail; the buyer is their
    # customer, not ours, and has no wallet here. Charging a UNI wallet for it would be
    # double-accounting against a balance that never received the money — and, measured on
    # 2026-08-31, it simply fails with `insufficient_balance`, flipping `success` to false
    # over a delivered result. The consumption is recorded in the stat below with its
    # payment_kind, which is what the operator reconciles the reseller's invoice against.
    if success and paid and payment_kind != "reseller":
