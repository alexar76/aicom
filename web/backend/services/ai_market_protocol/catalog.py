...
def list_factory_capabilities(*, exclude_smoke_demos: bool = True) -> list[dict[str, Any]]:
    ...
    try:
        ...
        logger.warning(
            "list_factory_capabilities: could not import products map "
            "(web.backend.api.products); falling back to shipped-only demo "
            "capabilities from pipeline.json — these may not reflect real "
            "on-disk products",
            exc_info=True,
        )
