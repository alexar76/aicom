...
def _evaluate_methodology(
    ...
    # A QA report's methodology_review is a historical artifact. Preferring it over a
    # live scan froze Sentinel on domain_api_endpoint_missing after the tree already
    # served those routes under a router prefix.
    ...
    if not code_dir.is_dir() and isinstance(qa_report, dict):
        qa_result = qa_report.get("qa_result")
...
def _qa_release_score(
    ...
    *,
    ...
    if not isinstance(qa_report, dict):
        return None
    if prefer_refresh:
        refresh = qa_report.get("storefront_refresh_result")
        if isinstance(refresh, dict) and isinstance(refresh.get("release_score"), (int, float)):
            return int(refresh["release_score"])
    qa_result = qa_report.get("qa_result")
    if not isinstance(qa_result, dict):
        return None
...
def evaluate_marketplace_quality(
    ...
    *,
    ...
    release_score = _qa_release_score(qa_report, prefer_refresh=not require_full_qa)
