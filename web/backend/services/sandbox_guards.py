...
def pop_expired_sandboxes(active: dict[str, Any]) -> list[tuple[str, dict[str, Any]]]:
    """Drop expired / non-running rows; return the expired ones that were still ``running``.

    Those still hold real resources — a uvicorn child, its Postgres in DinD, a compose stack,
    ~180 MB of venv on disk — and the caller releases them. Rows already ``stopped`` were
    released when they were stopped, so they are only dropped.
    """
    now = time.time()
...
def prune_expired_sandboxes(
    active: dict[str, Any],
    *,
    teardown: Callable[[str, dict[str, Any]], None] | None = None,
) -> int:
    """Drop expired / non-running registry rows so concurrency limits stay honest.

    Without ``teardown`` this only edits the dict — which is all any caller did until
    2026-09-11, so an expired storefront preview kept its uvicorn, venv and Postgres for as
    long as the app process lived: 70 leftover preview trees (9.4 GB) on the factory host and
    "running" rows whose processes were long dead. Pass the API's teardown to release them.
    """
    before = len(active)
    expired = pop_expired_sandboxes(active)
    removed = before - len(active)
    if teardown is not None:
        for sid, sb in expired:
            try:
                teardown(sid, sb)
            except Exception as exc:  # one broken teardown must not keep the rest alive
                logger.warning("sandbox %s: expiry teardown failed: %s", sid[:24], exc)
    if removed:
        logger.info("sandbox registry: pruned %d expired/stale entries", removed)
    return removed


def mark_orphaned_uvicorn_previews(active: dict[str, Any]) -> int:
    """Reap previews whose process died with the previous app process.

    uvicorn previews are children of this process, so anything still marked
    ``running`` in a registry read from disk at startup is already dead. Leaving
    them counted keeps the concurrency limiter at its cap and every new preview
    gets "Too many live previews running" until the entries expire hours later.
    Compose previews are containers and do outlive a restart, so they are kept.

    A bootstrap interrupted by restart looks like ``startup_phase=bootstrapping``
    with no port — also dead, and it held the admin Files-tab slot (Sentinel:
    HTTP 500 / sandbox_busy while a second product's static preview filled cap 2).
    Static ``ready`` previews have no port on purpose and must survive.
    """
    reaped = 0
    for entry in active.values():
        if not isinstance(entry, dict) or entry.get("status") != "running":
            continue
        if entry.get("compose_proxy_port"):
            continue
        phase = str(entry.get("startup_phase") or "")
        if not entry.get("backend_preview_port"):
            if phase in ("starting", "bootstrapping"):
                entry["status"] = "stopped"
                entry["stopped_reason"] = "orphaned_bootstrap"
                reaped += 1
            continue
        entry["status"] = "stopped"
        entry["stopped_reason"] = "orphaned_by_restart"
        reaped += 1
    return reaped
