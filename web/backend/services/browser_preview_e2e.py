...
**Preview runtime (sessions / real app, not only static files):**
  ...
  FastAPI products with ``frontend/dist`` get a factory SPA overlay so Playwright ``/`` is the widget,
  not ``{"message":"... API"}``. Disable with ``AIFACTORY_PREVIEW_SPA_FALLBACK=0``.
...
def run_browser_preview_e2e(
    ...
    **Serve modes:** static ``index.html``, **FastAPI** (uvicorn on loopback, cookies/sessions work), or **Docker**
    ...
    from web.backend.services.e2e_isolate import browser_e2e_memory_ok

    mem_ok, mem_detail = browser_e2e_memory_ok()
    if not mem_ok:
        logger.warning("Browser E2E skipped (low cgroup memory): %s", mem_detail)
        return {
            "passed": True,
            "skipped": True,
            "reason": "low_cgroup_memory",
            "detail": mem_detail,
        }
...
def _playwright_check(
    ...
    *,
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            def on_response(response) -> None:
                # The console says "Failed to load resource: … 500" and NOTHING about which
                # resource. Two rounds were spent guessing in App.tsx on exactly this shape of
                # finding, and a 405 costume of the same bug misdirected two more. The response
                # event is where the method, the path and the status actually are.
                try:
                    ...
                    log_suppressed(
                        logger,
                        "non-fatal (web/backend/services/browser_preview_e2e.py)",
            ...
            try:
                page.wait_for_selector("h1, [role='heading']", timeout=4_000)
            except Exception:
                pass
            # Hydrated markup. urllib on the same URL sees the Vite shell (`<div id="root">`
            # and no <h1>), which is how a11y_missing_h1 stayed red while PublicWidget
            # already rendered <h1>Sentinel</h1>.
            ...
            if deep_env_bool("AIFACTORY_BROWSER_DEEP_CRAWL", True):
                ...
                seed_urls = [
                    urljoin(base_origin.rstrip("/") + "/", route.lstrip("/"))
    ...
    # A failing request named by method, path and status — and, where the route table can find it,
    # by the file that serves it. This is the same finding the console reports as "Failed to load
    # resource: 500" with no path, which is unexecutable by construction.
    _seen_failures: set[tuple[str, str, int]] = set()
    for fr in failed_requests:
        key = (fr["method"], fr["path"], fr["status"])
        ...
        try:
            ...
            handler = route_handler_file(code_dir, fr["path"]) or ""
        except Exception as _suppressed_exc:
            log_suppressed(
                logger,
                "non-fatal (web/backend/services/browser_preview_e2e.py)",
        ...
        issues.append(
            f"browser_http_{fr['status']}:{fr['method']} {fr['path']}:{fr['status']}"
            + (f" — the handler for this route lives in {handler}" if handler else "")
            + (
                " (the browser loads this at page load, so the console error with no path is this "
                "request)"
            )
        )
        if len(_seen_failures) >= 8:
            break
    ...
    if console_errors:
        passed = False
        _paths = ", ".join(
            f"{f['method']} {f['path']} -> {f['status']}" for f in failed_requests[:4]
        )
        for ce in console_errors[:12]:
            t = ce.get("text", "")[:400]
            if "Failed to load resource" in t and _paths:
                t = f"{t} [the failing request(s) this round: {_paths}]"
    ...
    # Basic a11y checks against the hydrated landing, not the Vite shell urllib would fetch.
    try:
        import re

        html = landing_html or ""
        stripped = html.lstrip()
        if stripped.startswith("{") or stripped.startswith("["):
            passed = False
            issues.append(
                "a11y_preview_served_api_json: Playwright hit the FastAPI JSON origin, "
                "not the SPA. This is a factory preview-serve defect — do not add <h1> to index.html."
            )
        else:
            lower = html.lower()
            if "<form" in lower:
            ...
            has_h1 = "<h1" in lower or 'aria-level="1"' in lower or "aria-level='1'" in lower
            if not has_h1:
                try:
                    has_h1 = bool(
                        page.evaluate(
                            """() => {
                              ...
                              if (!h) return false;
                              const lv = h.getAttribute('aria-level');
                              return !lv || lv === '1';
                            }"""
                        )
                    )
