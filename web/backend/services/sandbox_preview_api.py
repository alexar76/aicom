...
def exited_preview_sandboxes() -> list[str]:
    """Sandbox ids whose uvicorn child exited on its own (crash, OOM kill, operator kill).

    The registry still says ``running`` for them, so they hold a concurrency slot and their
    Postgres/venv until someone notices. ``mark_orphaned_uvicorn_previews`` covers only the
    app-restart case; this covers a process that died while the app kept running.
    """
    dead: list[str] = []
    for sid, proc in list(_preview_procs.items()):
        try:
