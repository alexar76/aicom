...
def _iter_code_artifacts(code_dir: Path) -> Iterable[tuple[str, str]]:
    ...
    for p in sorted(code_dir.rglob("*")):
        if not p.is_file():
            continue
        if should_skip_code_path(p) or any(part in _SKIP_PARTS for part in p.parts):
            continue
...
def _path_placeholder_re(path_pattern: str) -> str:
    """Escape a pack path and loosen ``{id}`` / ``{name}`` placeholders."""
    return (
...
def _eval_api(raw_text: str, endpoints: tuple[ApiEndpoint, ...]) -> tuple[list[str], list[str]]:
    ...
    * a literal occurrence of the path pattern inside a string / call site.
    ...
    for e in endpoints:
        path_re = _path_placeholder_re(e.path_pattern)
        # A pack path is a shape, not a mount contract. The live product served the analytics
        # resources at /api/analytics/dashboards — a router prefix, nothing more — and this check
        # demanded the literal /api/dashboards, so a conforming implementation failed the gate on
        # every run of the night. Up to two path segments are allowed between /api and the resource
        # tail; the tail itself still has to match in full, method and all.
        if path_re.startswith(re.escape("/api/")):
            path_re = re.escape("/api") + r"(?:/[\w-]+){0,2}" + path_re[len(re.escape("/api")):]
        ...
        # FastAPI mounts the resource under include_router(prefix="/api/…"), so the
        # decorator is `@router.post("/datasets")` rather than `@router.post("/api/datasets")`.
        decorator_rel_re = None
        if e.path_pattern.startswith("/api/"):
            tail_re = _path_placeholder_re(e.path_pattern[len("/api"):])
            decorator_rel_re = re.compile(
                rf'@\w+\.{method.lower()}\s*\(\s*["\']{tail_re}["\']',
                re.I,
            )
        if (
            method_with_verb.search(raw_text)
            or decorator_re.search(raw_text)
            or method_re.search(raw_text)
            or (decorator_rel_re is not None and decorator_rel_re.search(raw_text))
        ):
