...
def resolve_static_preview_relpath(code_root: Path) -> Optional[str]:
    """HTML entry relative to the product code root.

    Path order is the default. When a later candidate's *currently linked*
    CSS matches its class names by a clear margin, prefer that page so
    sandbox preview is never a naked HTML shell sitting next to a styled twin.
    """
    if not code_root.is_dir():
        return None
    usable = [rel for rel in _STATIC_PREVIEW_CANDIDATES if static_html_preview_usable(code_root, rel)]
    if not usable:
        return None
    if len(usable) == 1:
        return usable[0].replace("\\", "/")

    from web.backend.services.preview_stylesheet_heal import (
        PREVIEW_FIT_OVERRIDE_MARGIN,
        preview_fit_score,
    )

    best_rel = usable[0]
    best_fit = preview_fit_score(code_root, best_rel)
    for rel in usable[1:]:
        fit = preview_fit_score(code_root, rel)
        if fit >= best_fit + PREVIEW_FIT_OVERRIDE_MARGIN:
            best_rel = rel
            best_fit = fit
    return best_rel.replace("\\", "/")
