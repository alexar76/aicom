...
def npm_env() -> dict[str, str]:
    """Environment npm can actually write to — and nothing more.

    ``npm install`` on a model-authored ``package.json`` executes that package's
    ``preinstall``/``postinstall`` scripts, and every dependency's. Generated product code
    is untrusted (docs/sandbox-trust-model.md), so this used to be ``os.environ.copy()``:
    one lifecycle script, in the QA frontend gate or on the Vercel publish path, could read
    the factory's provider keys, JWT signing secret, publish tokens and Docker socket.
    Scrubbed the same way the uvicorn preview and the runtime-test runner are — see
    core/child_env.py. A private-registry auth token is scrubbed too; if you need one, add
    it to the explicit `keep` list here and accept that untrusted build scripts will see it.
    """
...
def resolve_frontend_build_output_dir(product_code: Path, frontend_base: Path) -> Path | None:
    """Directory containing index.html after a successful npm run build.

    Most SPAs emit ``frontend/dist``; Vercel fullstack layouts often set vite
    ``outDir: '../public'`` so the static bundle lands at the product root.
    """
    candidates = [
        frontend_base / "dist",
        frontend_base / "build",
    ...
    return next((d for d in candidates if (d / "index.html").is_file()), None)
...
def run_frontend_build_check(
    ...
    if build.returncode == 0:
        dist = resolve_frontend_build_output_dir(product_code, base)
        if dist is None:
            return {
                **report,
                "passed": False,
                "issues": [
                    ...
                    "but no index.html was found under frontend/dist, dist/, or public/ "
                    "(Vercel fullstack often uses vite outDir '../public') — the preview "
                    "and the Vercel bundle have nothing to serve."
                ],
            }
