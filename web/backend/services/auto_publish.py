...
def _product_delivery_profile(product_id: str) -> str:
    """Read delivery_profile from pipeline.json for auto-publish gating."""
    import json
    ...
    try:
        ...
        return str(p.get("delivery_profile") or spec.get("delivery_profile") or "")
    except Exception as e:
        logger.debug("auto_publish: delivery_profile lookup failed for %s: %s", product_id, e)
        return ""
...
def try_publish_after_devops(product_id: str) -> dict[str, Any]:
    ...
    if landing_only:
        ...
        if normalize_delivery_profile(dp) != MARKETING_LANDING:
            return {
                "ok": False,
                "skipped": True,
