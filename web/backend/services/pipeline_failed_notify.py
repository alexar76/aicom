...
def _dedupe_path() -> Path:
    """Where the "already told them" marker lives, resolved per call.

    It was a module-level constant built from ``data_root()`` at IMPORT time, which
    freezes it to whatever the data root was the first time anything imported this
    module. A process that resolves its data root later — or a test that sets
    ``AIFACTORY_DATA_ROOT`` after some earlier import — kept writing the marker to the
    old location, so the dedupe silently consulted a file for a different deployment.
    """
    return Path(data_root()) / "state" / "pipeline_failed_telegram_sent.json"
