...
def _sqlite_url_for_sandbox(code_dir: Path, sandbox_id: str, *, async_driver: bool) -> str:
    db_path = sandbox_sqlite_file(code_dir, sandbox_id)
    if async_driver:
        return f"sqlite+aiosqlite:///{db_path}"
    return f"sqlite:///{db_path}"


def sandbox_sqlite_file(code_dir: Path, sandbox_id: str) -> Path:
    """Absolute path to the isolated preview SQLite file for this sandbox id."""
    root = code_dir / ".aicom_sandbox" / sandbox_id[:40]
    root.mkdir(parents=True, exist_ok=True)
    return root / "preview.db"


def reset_stale_demo_sqlite(code_dir: Path, sandbox_id: str) -> list[str]:
    """Drop local SQLite files that still hold a demo user from a previous password.

    Measured on Relay: the app ignores ``DATABASE_URL`` and keeps operators in
    ``./.data/relay.db`` (via ``RELAY_DB_PATH``). Journey reuses
    ``journey-{product_id}``, so the first seed with ``RelayDemo!2025`` survived
    forever while QA logged in as ``sandbox.demo@…`` + the factory password →
    perpetual ``demo_login_failed`` 401. Wipe before boot so seed recreates the
    operator from the current ``SANDBOX_DEMO_*`` env.
    """
    removed: list[str] = []
    candidates = [
        sandbox_sqlite_file(code_dir, sandbox_id),
        code_dir / "backend" / ".data" / "relay.db",
        code_dir / ".data" / "relay.db",
        code_dir / "relay.db",
        code_dir / "backend" / "relay.db",
    ]
    for path in candidates:
        if path.is_file():
            try:
                path.unlink()
                try:
                    removed.append(str(path.relative_to(code_dir)))
                except ValueError:
                    removed.append(str(path))
            except OSError as exc:
                logger.debug("reset_stale_demo_sqlite unlink %s: %s", path, exc)
    return removed


def _bind_product_db_env(env: dict[str, str], db_path: Path) -> None:
    """Point product-specific SQLite settings at the sandbox file, not cwd defaults."""
    abs_db = str(db_path.resolve())
    env["DATABASE_URL"] = f"sqlite:///{abs_db}"
    # Generated apps often ignore DATABASE_URL and read a private path alias.
    for key in (
        "RELAY_DB_PATH",
        "SQLITE_PATH",
        "SQLITE_DB_PATH",
...
def pip_env() -> dict[str, str]:
    """Environment pip may see — and nothing more.

    ``pip install`` on a model-authored ``requirements.txt`` (or the product's own
    ``pyproject.toml``) executes that package's build hooks: ``setup.py`` runs, or a PEP 517
    backend does, at INSTALL time, as this process's user. Generated product code is
    untrusted (docs/sandbox-trust-model.md), so an inherited ``os.environ`` hands one
    ``setup.py`` the factory's provider keys, JWT signing secret, publish tokens, database
    URL and ``DOCKER_HOST``.

    ``npm_env()`` in frontend_build_check.py made exactly this argument about
    ``preinstall``/``postinstall`` scripts, and the uvicorn preview, the QA runner and the
    automated-verify runner were all scrubbed — pip was the twin nobody came back to, even
    though it runs FIRST, before the preview whose environment was so carefully cleaned.

    A private-index credential is scrubbed too: add it to an explicit ``keep`` here if you
    need one, and accept that untrusted build hooks will then see it.
    """
...
def build_fastapi_preview_env(
    *,
    ...
    """
    # Generated product code is UNTRUSTED (docs/sandbox-trust-model.md) and this module
    # RUNS it: pip installs its requirements (build hooks execute) and uvicorn imports
    # its modules (module-level code executes). So the child gets a SCRUBBED copy of the
    # factory environment -- core/child_env.py says what is dropped and why. The
    # per-sandbox venv isolates packages; it never isolated privilege.
    env = scrub_child_env(base_env or os.environ)
    ...
    # Force — product .env / prior seed must not keep a stale demo password.
    env["SANDBOX_DEMO_PASSWORD"] = effective_sandbox_demo_password_for_compose()
    ...
    if needs_pg:
        ...
        if not docker_available():
            ...
            _bind_product_db_env(env, sandbox_sqlite_file(code_dir, sandbox_id))
        ...
        sync_url = _sqlite_url_for_sandbox(code_dir, sandbox_id, async_driver=False)
        _bind_product_db_env(env, sandbox_sqlite_file(code_dir, sandbox_id))
        # Keep async URL for apps that prefer aiosqlite.
