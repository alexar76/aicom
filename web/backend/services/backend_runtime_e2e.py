...
def _fetch_openapi_paths(base: str) -> list[str]:
    """Read the booted app's own route table (authoritative for the contract gate)."""
    # FastAPI apps often relocate the schema (`openapi_url=f"{API_V1_STR}/openapi.json"`).
    for path in (
        "/openapi.json",
        "/api/openapi.json",
        "/api/v1/openapi.json",
        ...
        try:
            with urlopen(Request(url=f"{base}{path}", method="GET"), timeout=4.0) as r:
...
def _probe_full_software_endpoints(base: str, routes: list[dict[str, str]]) -> list[str]:
    ...
    demo_body_reg = {
        "email": "runtime.probe@magic-ai-factory.com",
        "password": "RuntimeProbe!2026",
        "name": "Runtime Probe",
    }
    demo_body_login = {"email": "runtime.probe@magic-ai-factory.com", "password": "RuntimeProbe!2026"}
...
def run_backend_runtime_e2e(product_id: str, data_root: str | Path | None = None) -> dict[str, Any]:
    ...
    py_files = list(iter_product_files(code_dir, "*.py"))
    if not py_files:
    ...
    # Preferred path: boot the app exactly like the live preview does — right
    # interpreter, prepared venv, SQLite fallback, and the real ASGI module.
    # The old path ran `python <first .py file>` from the repo root and expected
    # port 8000, which can never work for a `backend/app/main.py` layout: it
    # reported "did_not_bind_port_8000" on every well-structured product and so
    # vetoed the QA gate permanently.
    proc = None
    port = 0
    preview_info = detect_fastapi_backend(code_dir)
    if preview_info is not None:
        port, proc, boot_status = start_fastapi_preview(
            sandbox_id=f"e2e-{product_id}", code_dir=code_dir
        )
        details["boot"] = {"mode": "preview", "status": boot_status, "module": preview_info["module"]}
        if not port:
            issues.append(f"backend_boot_failed:{boot_status}")
        ...
        if entry is None:
            entry = py_files[0]
        port = 8000
        details["boot"] = {"mode": "direct", "entrypoint": str(entry)}
        # Direct entrypoints are generated code just like the preferred uvicorn
        # preview path, and therefore require the same credential boundary.
    ...
    try:
        if not _wait_for_port(host, port, timeout_sec=15.0):
            issues.append(f"backend_did_not_bind_port_{port}")
            return {"passed": False, "skipped": False, "issues": issues, "details": details}
        ...
        # Live route inventory beats source parsing — the API contract gate uses it.
        details["openapi_paths"] = _fetch_openapi_paths(base)

        # Probe one non-health business route if present. Prefer the live route
        # table: source decorators carry no router prefix, so probing them hits
        # paths the app does not serve.
        routes = _extract_routes(py_files)
        served = details.get("openapi_paths") or []
        if served:
            routes = [
                {"method": "GET", "path": p}
                for p in served
                if "{" not in p and not p.endswith(("/openapi.json", "/docs", "/redoc"))
            ] or routes
