...
_DESKTOP = re.compile(
    r"\b(desktop\s*app|electron|tauri|flutter\s*desktop|native\s*client|system\s*tray|installable\s*app)\b",
    re.I,
)
...
def suggest_delivery_profile(idea: str) -> str | None:
    ...
    if len(t) < 12:
        return None
    if _DESKTOP.search(t) and not _LANDING.search(t):
        return "desktop_app"
    if _LANDING.search(t) and not _APP.search(t) and not _DESKTOP.search(t):
