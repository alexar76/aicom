...
def is_product_pipeline_on_hold(product_id: str) -> bool:
    """When True, worker skips all pipeline agent work for this product (pre-ship and repair)."""
    raw = read_followup(product_id)
    return bool(raw and raw.get(PIPELINE_ON_HOLD_KEY))


def set_product_pipeline_on_hold(product_id: str, on_hold: bool) -> dict[str, Any]:
    cur = read_followup(product_id) or {}
    cur[PIPELINE_ON_HOLD_KEY] = bool(on_hold)
    if on_hold:
        cur[PIPELINE_ON_HOLD_AT_KEY] = time.time()
    else:
        cur.pop(PIPELINE_ON_HOLD_AT_KEY, None)
    write_followup(product_id, cur)
    return normalize_pipeline_followup(cur)
