...
from core.code_discovery import PRODUCT_CODE_SKIP_DIR_NAMES, should_skip_code_path
...
def _read_index_html(code_dir: Path) -> Optional[str]:
    ...
    if p is None:
        # static_preview_file skips Vite dev shells (module script → src/*.tsx).
        # That is right for a sandbox iframe and wrong for this scorer: the shell
        # is still the mount point, and the UI lives in frontend/src. Returning
        # None here produced no_index_html / score 10 against a complete React app.
        for rel in ("index.html", "frontend/index.html"):
            cand = code_dir / rel
            if cand.is_file():
                try:
...
def _read_stylesheet(code_dir: Path) -> str:
    """CSS actually applied to the sandbox preview HTML, not a random root file."""
    chunks: list[str] = []
    seen: set[Path] = set()
    html_path = resolve_main_html_path(code_dir)
    if html_path is not None:
        try:
            from web.backend.services.preview_stylesheet_heal import linked_local_css_paths

            for css_path in linked_local_css_paths(html_path, code_dir):
                resolved = css_path.resolve()
                if resolved in seen:
                    continue
                try:
                    chunks.append(css_path.read_text(encoding="utf-8", errors="replace"))
                    seen.add(resolved)
                except OSError:
                    continue
        except Exception:
            pass
    for fallback in (code_dir / "style.css", code_dir / "frontend" / "style.css"):
        ...
        if resolved in seen:
            continue
        try:
            chunks.append(fallback.read_text(encoding="utf-8", errors="replace"))
            seen.add(resolved)
        except OSError:
            continue
    # Vite/React does not <link> CSS from index.html — it imports it from JS.
    # Scoring only the shell left Sentinel at demo_quality F with ~10 visual
    # codes while frontend/src/styles/index.css already had :root tokens and
    # @media rules that the gate never opened.
    chunks.append(_read_spa_css(code_dir, seen))
...
def _extract_foreground_rgb(decl_body: str) -> tuple[float, float, float] | None:
    # Lookbehind so ``border-color`` / ``outline-color`` / ``background-color`` are not
    # text color. Measured on Sentinel: a hover rule with only
    # ``background-color`` + ``border-color`` (same teal) read as fg==bg, ratio 1.0,
    # and held ``ux_low_contrast_cta`` critical while the resting button was ~10:1.
    m = re.search(r"(?<![\w-])color\s*:\s*([^;]+)", decl_body, flags=re.IGNORECASE)
    if not m:
        return None
...
def _iter_spa_source_files(code_dir: Path, suffixes: tuple[str, ...]):
    for root_name in ("frontend/src", "src"):
        root = code_dir / root_name
        if not root.is_dir():
            continue
        for path in sorted(root.rglob("*")):
            if path.suffix.lower() not in suffixes:
                continue
            if any(part in {"node_modules", "dist", "build", ".aicom_sandbox"} for part in path.parts):
                continue
            yield path


def _read_spa_css(code_dir: Path, seen: set[Path] | None = None) -> str:
    seen = seen if seen is not None else set()
    chunks: list[str] = []
    total = 0
    for path in _iter_spa_source_files(code_dir, (".css",)):
...
def _read_app_js(code_dir: Path) -> str:
    ...
    if p.is_file():
        try:
            ...
            pass
    for path in _iter_spa_source_files(code_dir, (".tsx", ".ts", ".jsx", ".js")):
        try:
...
def _is_bundler_root_path(url: str) -> bool:
    path = (url or "").split("?", 1)[0].split("#", 1)[0]
    if path.startswith(("/src/", "/assets/", "/@", "/static/", "/frontend/")):
        return True
    return path in ("/favicon.ico", "/vite.svg", "/manifest.json")


def _root_absolute_sandbox_breakers(index_html: str) -> bool:
    for match in re.finditer(r'(?:src|href)\s*=\s*["\'](/[^"\']+)["\']', index_html, re.I):
        if not _is_bundler_root_path(match.group(1)):
            return True
    return False
...
_SKIP_ARTIFACT_PARTS = PRODUCT_CODE_SKIP_DIR_NAMES | frozenset(
    {
        "dist-info",
        "htmlcov",
        ".ruff_cache",
...
#: Built output. Skipped for most checks (it is generated, and large), but it is the
#: only place a baked-in URL can be seen: the browser loads `dist/assets/*.js`, not the
#: source it came from. `_detect_loopback_artifact_urls` therefore opts back into it.
_BUILT_OUTPUT_DIR_NAMES = frozenset({"dist", "build", ".next"})


def _iter_text_artifacts(code_dir: Path, *, include_built: bool = False):
    """Yield text files under product code, pruning huge sandbox/vendor trees during walk.

    ``Path.rglob(\"*\")`` still descends into skipped dirs before filters run — that hung
    storefront ``GET /api/products/<id>`` on products with multi-GB ``.aicom_sandbox/``.

    ``include_built`` keeps ``dist``/``build``/``.next`` in the walk. Off by default:
    generated output is noise for most checks. On for the loopback scan, because
    `http://localhost:5173/api` compiled into a bundle exists nowhere else, and the
    gate that was meant to catch it walked straight past ``frontend/dist/``.
    """
    if not code_dir.is_dir():
        return
    skip = _SKIP_ARTIFACT_PARTS
    if include_built:
        skip = skip - _BUILT_OUTPUT_DIR_NAMES
    for dirpath, dirnames, filenames in os.walk(code_dir, topdown=True, followlinks=False):
        # Prune in-place so os.walk never enters junk trees.
        dirnames[:] = [d for d in dirnames if d not in skip]
        root = Path(dirpath)
        for name in filenames:
            p = root / name
            if should_skip_code_path(p) and not (
                include_built
                and set(p.parts) & _BUILT_OUTPUT_DIR_NAMES
                and not (set(p.parts) & (PRODUCT_CODE_SKIP_DIR_NAMES - _BUILT_OUTPUT_DIR_NAMES))
            ):
                continue
...
def _detect_loopback_artifact_urls(code_dir: Path) -> list[str]:
    """Loopback URLs anywhere the browser can reach, built output included.

    Bounded the same way it always was — it stops at five offenders — so opting into
    ``dist``/``build`` costs a walk, not an unbounded read.
    """
    offenders: list[str] = []
    for rel, _p, text in _iter_text_artifacts(code_dir, include_built=True) or []:
...
_COVERAGE_STOPWORDS = frozenset(
    """
    a an the and or for with from that this have will into onto over under as is
    are was were be by to of on in at it its their they them you your not but if
    when than then also such into onto over under about into default optional
    single using used must should shall via per
    """.split()
)


def _coverage_keyword_hit(index_lower: str, kw: str) -> bool:
    """A PM sentence is not a UI string. Count a hit when distinctive tokens land."""
    needle = (kw or "").strip().lower()
    if len(needle) < 5:
        return False
    if needle in index_lower:
        return True
    toks = [
        w
        for w in re.findall(r"[a-z][a-z0-9_.@-]{4,}", needle)
        if w not in _COVERAGE_STOPWORDS
    ]
    if not toks:
        return False
    return any(t in index_lower for t in toks)


def _coverage_score(index_lower: str, keywords: list[str]) -> int:
    if not keywords:
        return 0
    scored = [kw for kw in keywords if len(kw) >= 5]
    if not scored:
        return 0
    hits = sum(1 for kw in scored if _coverage_keyword_hit(index_lower, kw))
    return int(min(100, round(100 * hits / max(1, len(scored)))))
...
def assess_product_demo(
    ...
    spa_source = _read_app_js(code_dir)
    ...
    # Structure / CTA / spec coverage must see the SPA, not only the Vite shell.
    # index.html is `<div id="root">` plus a module script; the product lives in
    # frontend/src. Scoring the shell produced ux_structure_thin / ux_missing_cta
    # / low_spec_alignment / visual_* against a working React tree.
    lower = (index_html + "\n" + css_text + "\n" + spa_source).lower()
    html_lower = index_html.lower()
    for snippet in BANNED_SNIPPETS:
        if snippet.lower() in html_lower:
            issues.append(
                {
    ...
    # Vite's `/src/main.tsx` and hashed `/assets/…` are the bundler contract, not a
    # sandbox bug — flagging them froze demo_quality at D while the SPA was fine.
    if _root_absolute_sandbox_breakers(index_html):
        issues.append(
            {
    ...
    has_main = bool(re.search(r"<main\b|role=[\"']main[\"']", lower))
    has_cta = bool(
        re.search(
            r"<button\b|type=[\"']submit[\"']|cta|start free|try free|get started|get safety|book demo|sign up",
            lower,
        )
    )
    has_form = "<form" in lower
    if section_count < 1 and not has_main:
        issues.append(
            {
