...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        ds = sync_provider_at_startup(reset_circuit=True)
        if ds.get("ok"):
            logger.info("LLM provider synced at startup: %s", ds)
    except Exception as e:
        logger.warning("LLM provider sync skipped: %s", e)
    ...
    # Sandbox previews expire after an hour; until 2026-09-11 nothing released what they held
    # (uvicorn child, Postgres in DinD, ~180 MB venv per preview). The sweeper does, every minute.
    try:
        if sandbox.start_expiry_sweeper():
            logger.info("sandbox expiry sweeper started")
    except Exception:
        logger.warning("sandbox expiry sweeper failed to start", exc_info=True)
...
# /agents is a real HTML page (inline CSS/JS + fonts) — same trap as sandbox if we stamp
# default-src 'none' on it (unstyled Times New Roman + forever "loading…").
...
_HTML_PAGE_CSP_OWNED_PATHS = frozenset({"/agents"})
...
def _html_page_owns_csp(path: str) -> bool:
    return path.rstrip("/") in _HTML_PAGE_CSP_OWNED_PATHS or path in _HTML_PAGE_CSP_OWNED_PATHS
...
async def add_security_headers(request: Request, call_next):
    ...
    if (
        ...
        and not _html_page_owns_csp(path)
...
# Registered LAST, so it is the OUTERMOST http middleware: the RuntimeError a client
# disconnect provokes is raised by the innermost BaseHTTPMiddleware and re-raised by every
# layer above it, so only the outermost one can stop it becoming a 500.
from web.backend.middleware.client_disconnect import client_disconnect_middleware
...
from web.backend.api import agents_page as agents_page_api

app.include_router(agents_page_api.router)
...
# The SKOPOS remediation conductor's patch-authoring endpoint. Mounted ONLY where
# AIFACTORY_REMEDIATION_FIX_ENABLED=1, which is deliberately not the same as "off but present":
#
#   * On the PUBLIC instance the flag is unset, so the route does not exist at all. That matters
#     because web/frontend/next.config.js rewrites /api/:path* to this API, so a merely-disabled
#     route would still be a reachable public endpoint answering 403 — new surface for no reason.
#   * On the PRIVATE remediation instance the flag is set and the route is served.
#
# One consequence worth knowing when you change this code: only the instance that SERVES the route
# needs redeploying. The public factory does not run remediation, so it is unaffected.
if os.environ.get("AIFACTORY_REMEDIATION_FIX_ENABLED", "").strip().lower() in ("1", "true", "yes", "on"):
...
def _sanitize_published_site_head_html(raw: Any) -> str:
    ...
    """
    snippet = strip_google_analytics_html(str(raw or ""))[:PUBLISHED_SITE_HEAD_MAX_CHARS]
...
async def get_admin_settings(_admin: dict = Depends(require_admin_with_rbac)):
    ...
    from core.github_catalog import github_pat_configured
    ...
    return {
        ...
        "product_catalog_enabled": bool(config.get("general.product_catalog_enabled", False)),
        "product_catalog_allowlist": str(config.get("general.product_catalog_allowlist") or "").strip(),
        "product_catalog_require_github_house": bool(
            config.get("general.product_catalog_require_github_house", True)
        ),
        ...
        "published_site_head_html": strip_google_analytics_html(
            str(config.get("general.published_site_head_html") or "")
        ),
        ...
        if key in body:
            ...
            if key == "pipeline_cost_optimized" and val is not None:
                ...
                if qnorm is not None:
                    ...
                    bump_quality_cache_after_config_write()
            if key == "product_catalog_enabled" and val is not None:
                val = bool(val)
                if val:
                    from core.github_catalog import github_pat_configured as _gh_pat_ok

                    if not _gh_pat_ok():
                        raise HTTPException(
                            status_code=400,
                            detail=(
                                "Cannot enable product catalog: set GH_PAT or GITHUB_TOKEN "
                                "on the factory host first (alexar76 publish identity)."
                            ),
                        )
