...
async def get_pipeline_products(
    ...
    try:
        ...
        for pid, product in window:
            ...
            focus_id = get_factory_focus_product_id()
            pipeline_focus_active = bool(focus_id and str(pid) == str(focus_id))
            ...
            shipped = is_shipped_pipeline_product_state(product.get("state"))
            storefront_marketing_copy: dict[str, Any] = {}
            # Light catalog skips heavy disk hydration, but shipped rows need storefront copy
            # for the admin Marketplace copy / pricing panel (marketing_content.json).
            if not light or shipped:
            ...
            row: dict[str, Any] = {
                ...
                "delivery_profile": delivery_profile,
                "pipeline_focus_active": pipeline_focus_active,
            ...
            mg = product.get("metis_gate")
            if isinstance(mg, dict) and mg.get("at") is not None:
                row["metis_gate"] = {
                    "stage": mg.get("stage"),
                    "ok": mg.get("ok"),
                    "status": mg.get("status"),
                    "verify_score": mg.get("verify_score"),
                    "verified": mg.get("verified"),
                    "route": mg.get("route"),
                    "blocked": mg.get("blocked"),
                    "at": mg.get("at"),
                }
        ...
        raise HTTPException(status_code=500, detail="Failed to get pipeline products")
...
async def patch_pipeline_product_followup(product_id: str, body: StorefrontFollowupPatch):
    ...
    try:
        ...
        if body.pipeline_on_hold is not None:
            record = set_product_pipeline_on_hold(product_id, bool(body.pipeline_on_hold))
...
@router.get("/pipeline/focus-mode")
async def get_pipeline_focus_mode(request: Request):
    """Current focus product (if any) and suggested complex in-progress target."""
    import asyncio

    from web.backend.services.pipeline_focus import focus_mode_status

    cfg = request.app.state.config.get_all()
    return await asyncio.to_thread(focus_mode_status, config=cfg)


@router.post("/pipeline/focus-mode")
async def post_pipeline_focus_mode(request: Request, body: PipelineFocusModeBody):
    """
    Enable focus mode: resume factory (optional) and pause pipeline work on all products
    except ``focus_product_id``. Use ``auto_select`` to pick the heaviest in-progress SKU.
    """
    import asyncio

    from web.backend.api.admin.dashboard.helpers import _load_pipeline_products_for_metrics
    from web.backend.services.pipeline_focus import apply_pipeline_focus_mode, suggest_focus_product

    config = request.app.state.config
    products = await asyncio.to_thread(_load_pipeline_products_for_metrics)

    if body.clear_focus:
        result = apply_pipeline_focus_mode(
            config,
            focus_product_id=None,
            resume_factory=body.resume_factory,
            products=products,
        )
        return result

    focus_id = body.focus_product_id
    if body.auto_select or not focus_id:
        focus_id = suggest_focus_product(products)
        if not focus_id:
            raise HTTPException(status_code=400, detail="No in-progress product available for focus mode")

    try:
        return apply_pipeline_focus_mode(
            config,
