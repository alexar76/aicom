        ...
        try:
            ...
            try:
                ...
                for row in rows:
                    ...
                    if not product:
                        continue
                    tasks = sm.get_recent_failed_tasks_by_product(pid)
...
def _load_pipeline_products_for_metrics() -> dict[str, Any]:
    """Products keyed by id — does not load ~20k historical task rows."""
    products, _ = _load_pipeline_snapshot_for_metrics(include_tasks=False)
    return products


def _load_pipeline_snapshot_for_metrics(
    *,
    include_tasks: bool = True,
) -> tuple[dict[str, Any], list[dict]]:
    """
    Products keyed by id and optional global task list for dashboard counts.
    ...
    Callers that only need product rows must pass ``include_tasks=False`` so
    admin HTTP handlers do not block the single uvicorn worker for tens of seconds.
    """
    if _admin_use_sqlite_pipeline() and _admin_sql_store_available():
        try:
            from core.pipeline_database import create_sync_pipeline_manager
            ...
            try:
                ...
                tlist = sm.get_all_tasks() if include_tasks else []
            finally:
                sm.close()
