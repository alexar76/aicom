...
def build_storefront_categories_response() -> dict[str, Any]:
    ...
    for pid, product in products.items():
        ...
        product_category = (
            LANDINGS_LISTING_SLUG
            if is_landing
            else _canonical_marketplace_category(marketing, product)
        )
    ...
    total_visible = len(listings)
    return {"categories": result, "total_count": total_visible, "listings": listings}


@router.get("/storefront-count")
async def storefront_count():
    """Fast storefront grid size (same rules as ``GET /api/products`` / ``/categories``)."""
...
@router.get("/monitor-catalog")
async def monitor_catalog():
    """Slim real-id storefront listing for Alien Monitor (bounded-latency cache).

    Never synthesizes placeholder product ids — only rows that pass the same
    storefront gates as ``GET /api/products``.
    """
    from web.backend.services.storefront_counts_cache import get_storefront_categories_cached
    ...
    count = len(listings) if listings else (int(total) if total is not None else None)
    return {
        "products": listings,
        "count": count,
        "pending": pending,
...
def _product_has_code(pid: str) -> bool:
    ...
    Manifest ``files: []`` after a no-op patch is not empty product — fall back to
    scanning the tree (see ``product_code_presence``).
    """
    from web.backend.services.product_code_presence import product_has_code

    return product_has_code(code_dir(pid))
...
def is_product_showcase_listable(pid: str, product: dict[str, Any]) -> bool:
    """Whether a product appears on the public storefront grid (alias for ops/scripts)."""
    return _public_storefront_grid_accepts(pid, product)
...
async def list_products(
    ...
    return await asyncio.to_thread(_list_products_sync, query)


def _list_products_sync(query: ProductListQuery) -> dict[str, Any]:
    try:
        category = query.normalized_category()
    except ValueError as e:
        raise HTTPException(status_code=422, detail=client_error_detail(e)) from e
