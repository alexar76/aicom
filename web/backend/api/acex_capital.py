...
def _ipo_overlay() -> tuple[dict, dict]:
    """Merge local ACEX IPO/audit ledgers (and optional Hub listings) into Pulse LIVE."""
    overlay: dict = {}
    audit_overlay: dict = {}
    try:
    ...
    hub_url = os.environ.get(
        "ACEX_HUB_LISTINGS_URL", ""
    ).strip()
    if hub_url and not overlay:
        try:
            ...
            for st in payload.get("listings") or []:
                if isinstance(st, dict) and st.get("listing_id") and not st.get("error"):
                    overlay.setdefault(st["listing_id"], st)
        except Exception:
            pass
    return overlay, audit_overlay
...
async def _compute_pricing_snapshot(
    *,
    ...
    overlay, audit_overlay = await asyncio.to_thread(_ipo_overlay)
