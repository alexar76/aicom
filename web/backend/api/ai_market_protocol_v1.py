...
class PipelineNode(BaseModel):
    ...
    # Which hub the catalogue row came from, so a federated hop is routed to the peer that
    # actually sells it rather than guessed at.
...
async def run_pipeline(
    ...
    """Run a graph. Hops this factory does not host are routed to the federation hub.

    The visitor header is forwarded rather than dropped: the hub meters its free trial per
    visitor, so carrying the caller's own id is what lets a person without a wallet run a
    real graph instead of everyone sharing — and exhausting — one bucket belonging to this
    service.
    """
    ...
    return await execute_pipeline(
        ...
        llm_router=_get_llm_router(request),
        sandbox_visitor=x_sandbox_visitor,
    )


@router.get("/pipelines")
async def recent_pipelines(limit: int = 20):
    """Recent pipeline runs, redacted — cost, hops and hop-level blame.

    A projection, not the signed artefact: the payment channel and the per-hop receipt
    nonces are left out so a feed cannot enumerate who bought what. Each row names the
    path to its own signed bill of materials.
    """
    return {
        "traces": list_traces(limit=limit),
        "note": (
            "Projection. The signed bill of materials for a run is at "
            "/ai-market/pipelines/{trace_id}."
        ),
        "protocol_version": "v1",
    }


@router.get("/pipelines/{trace_id}")
async def pipeline_trace(trace_id: str):
    """The signed bill of materials for one run, verbatim, so it can be verified."""
    row = get_trace(trace_id)
    if not row:
        return JSONResponse(status_code=404, content={"detail": "trace not found"})
    return {**row, "protocol_version": "v1"}
...
async def invoke_capability_prefixed(
    ...
    x_payment_channel_secret: str | None = Header(default=None, alias="X-Payment-Channel-Secret"),
    x_ai_market_license: str | None = Header(default=None, alias="x-ai-market-license"),
    # Declared here because this route CALLS the root handler as a plain function.
    # Every parameter it does not pass keeps that handler's *default*, and a header
    # default is a FastAPI `Header(...)` OBJECT, not None — so omitting `x_api_key`
    # sent a `Header` instance into `reseller_label`, which does `presented.strip()`.
    # Result: this route answered 500 on EVERY invoke, and the API key it is supposed
    # to read could never arrive.
    x_api_key: str | None = Header(default=None, alias="X-API-Key"),
    authorization: str | None = Header(default=None),
):
