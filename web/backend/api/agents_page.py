...
# The webfonts come from /fonts/fonts.css on this same host — nginx sends /agents here and
# everything else to the Next.js app, which serves public/fonts — so 'self' covers them and
# Google's origins no longer need naming.
AGENTS_PAGE_CSP = (
    "default-src 'self'; "
    "base-uri 'self'; "
    "object-src 'none'; "
