...
from web.backend.services.ai_market_protocol.rate_limits import enforce_invoke_limits
...
def _federation_hub_url() -> str:
    from core.aimarket_hub_url import DEFAULT_FEDERATION_HUB_URL, resolve_federation_hub_url

    # fallback_on_invalid=False preserves the original fail-closed behavior: a
    # malformed AIMARKET_FEDERATION_HUB_URL is returned as-is so _federation_url_is_safe
    # rejects it, rather than silently falling back to the public default hub.
    return resolve_federation_hub_url(
        default=DEFAULT_FEDERATION_HUB_URL, fallback_on_invalid=False
    )
...
async def invoke_v2(
    ...
    # Throttle the expensive invoke path per-customer and per-IP before doing any
    # work (LLM call or federated proxy) (A7).
    ...
    if body.source_hub and body.source_hub not in ("local", "") and _widget_federate_search():
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=60.0, limits=limits) as client:
                ...
                # The hub meters its free trial per visitor; dropping this header
                # merges every caller into one bucket and exhausts it for everyone.
                if x_sandbox_visitor:
                    headers["X-AIMarket-Sandbox-Visitor"] = x_sandbox_visitor
                r = await client.post(
    ...
    status, payload, extra_headers = await invoke_capability_v1(
        ...
        # Same reseller rail as the v1 route. A guard that only one of two entry points
        # applies is the shape of half this repo's incidents.
        x_api_key=x_api_key,
