...
This distinction powers the factory **soft hold**: when ``general.factory_on_hold``
is set (but the env emergency stop is *not*), the pipeline worker keeps advancing
on-demand products while pausing autonomous + post-ship improvement work. See
``pipeline_worker._process_cycle`` and ``core.factory_hold``.
"""
