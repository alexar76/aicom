...
def security_store_db_path() -> Path:
    """Persistent store for login rate-limit counters and OIDC nonce replay guard.

    Survives restarts so brute-force counters and used-nonce records are not reset
    by a process bounce. Shared across web workers.
    """
    return Path(
        os.environ.get(
            "AIFACTORY_SECURITY_STORE_DB",
            str(state_dir() / "security_store.db"),
        )
    )
...
def code_dir(product_id: str, *, data_root: str | Path | None = None) -> Path:
    """Return ``<data_root>/code/<product_id>``.

    Optional ``data_root`` lets callers (auto-recovery, storefront refresh) pin
    the tree without relying solely on process-wide env.
    """
    return resolve_data_root(data_root) / "code" / product_id
...
def admin_actions_log_path() -> Path:
    return logs_dir() / "admin_actions.jsonl"
