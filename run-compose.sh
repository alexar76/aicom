...
resolve_compose_files() {
  ...
  if [[ "${AIFACTORY_USE_HOST_DOCKER:-}" == "1" ]]; then
    echo -e "${RED}╔══════════════════════════════════════════════════════════════════╗${NC}" >&2
    echo -e "${RED}║  ⚠  AIFACTORY_USE_HOST_DOCKER=1 — MOUNTING THE HOST DOCKER SOCKET ║${NC}" >&2
    echo -e "${RED}║                                                                  ║${NC}" >&2
    echo -e "${RED}║  This grants the container FULL CONTROL of the host Docker daemon ║${NC}" >&2
    echo -e "${RED}║  — equivalent to UNCONFINED ROOT ON THE HOST. Any sandboxed       ║${NC}" >&2
    echo -e "${RED}║  product can escape and own this machine. Emergency fallback only ║${NC}" >&2
    echo -e "${RED}║  — prefer the default DinD sidecar. NEVER use in production.      ║${NC}" >&2
    echo -e "${RED}╚══════════════════════════════════════════════════════════════════╝${NC}" >&2
