...
All monetary calculations use ``Decimal`` to avoid IEEE 754 float precision loss.
"""
...
DEFAULT_ETH_USD = Decimal(os.environ.get("AIFACTORY_ETH_USD", "3500"))
DEFAULT_SOL_USD = Decimal(os.environ.get("AIFACTORY_SOL_USD", "150"))
...
def _amount_to_approx_usd(amount: Any, currency: str) -> Decimal:
    ...
    if c in ("USDT", "USDC", "DAI"):
        return amt
    if c == "ETH":
        return amt * DEFAULT_ETH_USD
    if c == "SOL":
        return amt * DEFAULT_SOL_USD
...
def compute_financial_metrics(
    ...
    total_revenue_usdt = Decimal("0")
    total_revenue_usdc = Decimal("0")
    total_revenue_eth = Decimal("0")
    total_revenue_sol = Decimal("0")
    ...
    for order in orders.values():
        ...
        amt = _to_decimal(order.get("amount") or 0)
    ...
    def window_sum(seconds: float) -> dict[str, Decimal]:
        cutoff = now - seconds
        usd = Decimal("0")
        for order in orders.values():
    ...
    out = {
        "total_revenue_usdt": float(round(total_revenue_usdt, 6)),
        "total_revenue_usdc": float(round(total_revenue_usdc, 6)),
        "total_revenue_eth": float(round(total_revenue_eth, 8)),
        "total_revenue_sol": float(round(total_revenue_sol, 8)),
        "total_revenue_approx_usd": float(round(total_approx_usd, 2)),
        "transactions_pending": transactions_pending,
        "transactions_completed": transactions_completed,
        "transactions_failed": transactions_failed,
        "revenue_last_24h_approx_usd": float(round(w24["approx_usd"], 2)),
        "revenue_last_7d_approx_usd": float(round(w7["approx_usd"], 2)),
        "revenue_last_30d_approx_usd": float(round(w30["approx_usd"], 2)),
