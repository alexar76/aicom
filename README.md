...
<p align="center">
  ...
  <a href="https://raw.githubusercontent.com/alexar76/aicom/refs/heads/main/docs/badges/coverage.svg"><img src="https://raw.githubusercontent.com/alexar76/aicom/refs/heads/main/docs/badges/coverage.svg" alt="Test coverage" /></a>
...
> ### 🟢 Try the live agent economy in 30 seconds — **no wallet, no Docker, no clone**
> Paste this one URL into any MCP client (Claude Desktop, Cursor, Windsurf, …):
>
> ## &nbsp;&nbsp;&nbsp;`https://modelmarket.dev/mcp`
>
> You get two tools against the **live** hub: **`market_search`** finds a capability in the
> published catalogue, **`market_invoke`** runs one and hands back the hub's **signed receipt**.
> **A few trial invokes per caller are free** — no key, no account, no wallet, nothing to
> install. When the allowance runs out the hub answers **402** and the on-chain escrow path
> begins, which is where the wallet finally matters.
>
> Setup snippets per client, the trial rules, and how to verify a receipt →
> **[`docs/hosted-mcp-endpoint.md`](docs/hosted-mcp-endpoint.md)**
...
| 🧱 **WARDEN** | The MCP firewall on its own: static scan → threat feed → origin → pinning, in front of **any** MCP host. Zero runtime dependencies | [`warden`](https://github.com/alexar76/warden) · `npm i @aimarket/warden` |
...
| 👽 **Alien Monitor** | 3D graph of Hub, Factory, chain metrics | [LIVE](https://monitor.modelmarket.dev/) · [UNI](https://monitor-uni.modelmarket.dev/) · [split](./docs/uni-and-live.md) · [`alien-monitor`](https://github.com/alexar76/alien-monitor) |
...
| 🗺 **ATLAS** | Planetary sensor map over GAIA · LIVE/SIM pins · Analyst | [live](https://atlas.modelmarket.dev/) · [`atlas`](https://github.com/alexar76/atlas) |
...
| 🧿 **LOGOS** | Read-only federation analytics — measured settlement volume, rolling z-score anomalies, cross-source correlations, guarded NL assistant | [live](https://logos.modelmarket.dev/) · [landing](https://alexar76.github.io/logos/) · [`logos`](https://github.com/alexar76/logos) |
...
## Attested Memory SaaS family

The repository also contains a separate customer-facing SaaS contour built on
the Attested Memory Hub: [`SAAS_PROJECTS.md`](SAAS_PROJECTS.md). It provides one
public edge for Personal Memory, Team Memory and Expert Memory Market. Crypto
checkout uses exact canonical USDC on Base through the independent KOVA adapter;
the SaaS gateway never holds wallet private keys. Localized user guides and use
cases are available in [`docs/`](docs/) in ten languages.
...
**Typical LLM API cost:** **~$0.30–$2** landing first pass; tiny `full_software` can land **under ~$0.20** to first `COMPLETED` (measured); **~$3–$15+** when QA/repair loops pile up. Bring your own keys; host ~**$7/mo** separate.
...
One command. Every satellite. Your own AI agent economy — oracles, scanners,
lottery, treasury, the cognitive layer, and the monitor that watches it all.
All three prod hosts collapsed onto **one machine**, safe-defaults on,
mainnet **off**.
...
| What | You need |
|------|----------|
| **One LLM key** | `DEEPSEEK_API_KEY=sk-…` (or Anthropic / OpenAI) |
| **≥60 GB disk** | Docker images + build artefacts + chain snapshots |
| **≥16 GB RAM** | The cognitive layer and the oracles are greedy |
| **4+ cores** | Parallel builds + 40 long-running services |
| **Linux** (x86-64) | Docker Engine 24+ · `docker compose` plugin |

**What comes up:** Factory · Hub · Service Mesh · ARGUS · MOMUS · Metis · LOGOS ·
Dioscuri · Helios · Skopos · Gaia · Atlas · Platon · Chronos · Lumen ·
Lottery · Treasury · Bridges · ACEX (UNI) · Alien Monitor · Ecosystem landing.
~40 containers. One box. Zero real funds at risk.

Lighter tiers: `./start.sh` (core — 7 containers, ~6 GB RAM) · `./start.sh --full` (15 satellites).
Full runbook: [docs/quickstart-ecosystem-deploy.md](docs/quickstart-ecosystem-deploy.md).
...
Shipped products can sync to the **AIMarket Hub** (protocol v2, SDKs, desktop apps). Sibling projects — ARGUS, lottery, oracles, Monitor, Metis, MCP gateway — live in **their own repos**; see [**Ecosystem map**](#ecosystem-map). Map: [modeldev.modelmarket.dev](https://modeldev.modelmarket.dev).
...
./demo.sh --compose "Attested SaaS family"            # Docker UI on :9080
```
...
flowchart TB
  ...
  subgraph intelligence["Federation intelligence"]
    LOGOS["LOGOS · read-only analytics"]
    MOMUS["MOMUS findings"]
    SKOPOS["SKOPOS remediation"]
...
| [`aimarket-agent`](https://crates.io/crates/aimarket-agent) | crates.io | `aimarket-agent = "0.2.1"` | Rust SDK — Tauri, native CLI |
...
| **ATLAS** 🗺 (sensor map) | [`atlas`](https://github.com/alexar76/atlas) | Planetary MapLibre map over GAIA — LIVE/SIM pins, Analyst, Alien Monitor embed · [atlas.modelmarket.dev](https://atlas.modelmarket.dev/) · [GUIDE](https://github.com/alexar76/atlas/blob/main/docs/GUIDE.md) |
| **LOGOS** 🧿 (federation analytics) | [`logos`](https://github.com/alexar76/logos) | Read-only snapshots, measured settlement volume, rolling z-score anomalies, cross-source insights · [live dashboard](https://logos.modelmarket.dev/) · [README](logos/README.md) |
...
| **AIMarket School** | [`aimarket-school`](https://github.com/alexar76/aimarket-school) | **13** free clip lessons (EN/RU/ES/FR/ZH) · [edu.modelmarket.dev ↗](https://edu.modelmarket.dev/) |
| **AIMarket Courses** | [`aimarket-courses`](https://github.com/alexar76/aimarket-courses) | **13 academies** (EN/RU/ES/FR/ZH, Colab + Pages): orchestration, sensors, federation, PQC hybrid, verifiable randomness, MCP security, agent economy, trust math, optimization with proofs, smart-contract lotteries, AI Factory pipeline, 3D viz, physics-inspired computing · [portal ↗](https://alexar76.github.io/aimarket-courses/) |
...
[![coverage](https://raw.githubusercontent.com/alexar76/aicom/refs/heads/main/docs/badges/coverage.svg)](https://raw.githubusercontent.com/alexar76/aicom/refs/heads/main/docs/badges/coverage.svg) — backend line coverage (`web/`, `agents/`, `orchestrator/`, `director/`, `pipeline_worker/`). CI uploads `coverage.json` + badge artifact; refresh locally with:
...
**How fast / how much to first `COMPLETED`?** Measured on the live pipeline DB + `data/state/pipeline_product_cost.json` (Aug 2026):
...
| `full_software` (tiny habit SaaS) | **First `COMPLETED`** | **~91 min** wall-clock · **~$0.18** LLM · PulseBoard Mini `prod-a25224bcb746` (DeepSeek; includes repair loops + operator complete). Source: task timestamps `created_at` → `__complete__`, cost ledger `pipeline_product_cost.json` |
| Storefront catalog (live check) | Listed products | **3** `COMPLETED` `marketing_landing` + **1** `full_software` card (`PulseBoard Mini`) that later returned to **`DEV_FIXING`** / not marketplace-eligible — do **not** read the catalog as “1 completed full product forever” |

Reproduce timing: enqueue via `./demo.sh` / Admin → Pipeline, then `python scripts/wait_pipeline_product.py --product-id …`. Card: [PulseBoard Mini](https://magic-ai-factory.com/product/prod-a25224bcb746).

**LLM cost?** Per-product totals live in `data/state/pipeline_product_cost.json`. **First fleet `full_software` to reach `COMPLETED`:** PulseBoard Mini **`$0.181`** (~**$0.18**). Short DeepSeek first-pass ships can stay **sub‑dollar** when the brief stays tiny; long repair loops and heavy models still climb to **dollars**. Older fleet-wide averages (~**$8.5**/product including failed loops) remain a useful upper bound, not a promise.
...
**YouTube:** [HELIOS](https://github.com/alexar76/helios) renders and uploads ecosystem videos — private by default until approve. [Landing + gallery](https://alexar76.github.io/helios/) · [@My-AI-Factory](https://www.youtube.com/@My-AI-Factory). Click **HELIOS** on [Alien Monitor](https://monitor.modelmarket.dev/) for live channel stats.
