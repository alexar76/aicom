...
def main() -> int:
    ...
    for sat in data.get("satellites") or []:
        ...
        if sat_id == "profile" or repo == org:
            ...
            continue
        if sat.get("optional") or sat.get("factory_embedded"):
            print(f"  skip {sat_id}: no separate GitHub repo (factory / profile / monorepo only)")
            continue
        if sat.get("github_published") is False:
            print(f"  skip {sat_id}: github_published: false (not on GitHub yet)")
            continue
