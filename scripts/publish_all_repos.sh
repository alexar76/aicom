...
Satellite ↔ monorepo map (quick reference):
  ...
  aicom-products       ← aicom-products/ (catalog shell; products/ selective)
  platon               ← platon/ (UMBRAL cave app → alexar76/platon)
  lottery              ← lottery/ (AI-Agent Oracle Lottery → alexar76/lottery)
  aimarket-courses       ← courses/ → alexar76/aimarket-courses (10 academies, Colab + Pages)
  aimarket-school        ← school/ → alexar76/aimarket-school (clip lessons + Colab)
                         · live portal: edu.modelmarket.dev (./scripts/deploy_edu_school.sh)
                         · NOT on alexar76/aicom (Gitea monorepo only + satellite)
  ...
  profile (optional)   ← scripts/profile-readme/README.md → alexar76/alexar76
  ...
  atlas                ← atlas/ → alexar76/atlas (physical sensor map · Pages)
  momus                ← momus/ → alexar76/momus (runtime red team · :9400)
  basanos              ← basanos/ → alexar76/basanos (static EVM assurance)
  dolos                ← dolos/ → alexar76/dolos (dynamic EVM red team · UNI fork)
  attested-memory-hub  ← attested-memory-hub/ → alexar76/attested-memory-hub
  attested-saas-gateway ← attested-saas-gateway/ → alexar76/attested-saas-gateway
...
list_satellites() {
  ...
  # Anchored to $ROOT rather than the cwd — see the note on _python_map in
  # scripts/mirror_satellites.sh for what a relative read costs here.
