...
# Neither `mapfile` (bash 4+) nor `find -printf` (GNU find) exists on a stock
# macOS, and this pushes the wiki from the laptop — it died before listing a page.
PAGES=()
while IFS= read -r _page; do
  [[ -n "$_page" ]] && PAGES+=("$_page")
done < <(find "$SRC" -maxdepth 1 -name '*.md' ! -name 'README.md' -exec basename {} \; | sort)
if [[ ${#PAGES[@]} -eq 0 ]]; then
