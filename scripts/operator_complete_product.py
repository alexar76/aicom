...
def complete_product(pid: str, *, force: bool = False, ignore_live_gate: bool = False) -> int:
    ...
    try:
        ...
        if blocked and not ignore_live_gate:
            print(
                "ERROR: live gate still blocking completion "
                "(demo-auth 401 / failed live URL). Fix the deployed product, "
                "then retry. --force skips storefront only; "
                "use --ignore-live-gate only as a last resort.",
                file=sys.stderr,
            )
            print("live_gate_block", blocked, file=sys.stderr)
            return 1
        if blocked and ignore_live_gate:
            print("WARN: --ignore-live-gate; completing over live-gate defect:", blocked)
...
def main() -> int:
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="Skip storefront eligibility gate (operator recovery only). Does not skip a failed live gate.",
    )
    parser.add_argument(
        "--ignore-live-gate",
        action="store_true",
        help="Allow COMPLETE even when live_gate recorded demo-auth 401 / passed=False",
    )
    parser.add_argument(
        "--no-pause",
        action="store_true",
    ...
    rc = complete_product(
        ...
        ignore_live_gate=bool(args.ignore_live_gate),
    )
    return rc
