...
#   https://pulse.modelmarket.dev/      → :5199 (TLS via certbot.timer)
...
PULSE_DOMAIN="${PULSE_TERMINAL_PUBLIC_DOMAIN:-pulse.modelmarket.dev}"
NGINX_PULSE_SRC="$ROOT/deploy/nginx/pulse.modelmarket.dev.conf"
NGINX_PULSE_AVAIL="/etc/nginx/sites-available/${PULSE_DOMAIN}"
NGINX_PULSE_ENABLED="/etc/nginx/sites-enabled/${PULSE_DOMAIN}"
...
PUBLIC_PULSE_HOST="${PULSE_TERMINAL_PUBLIC_HOST_URL:-https://${PULSE_DOMAIN}/}"
...
# ── Pulse subdomain vhost + TLS (same webroot + certbot.timer as monitor/atlas) ─
install_pulse_subdomain() {
  [[ -f "$NGINX_PULSE_SRC" ]] || { echo "NOTE: missing $NGINX_PULSE_SRC"; return 0; }
  if [[ "$(id -u)" -ne 0 ]] && ! command -v sudo >/dev/null 2>&1; then
    echo "NOTE: not root and no sudo — skip pulse subdomain nginx install"
    return 0
  fi
  local sudo=()
  [[ "$(id -u)" -eq 0 ]] || sudo=(sudo)
  ...
  if [[ ! -f "/etc/letsencrypt/live/${PULSE_DOMAIN}/fullchain.pem" ]]; then
    echo "Issuing TLS for ${PULSE_DOMAIN} (HTTP-only bootstrap)…"
    "${sudo[@]}" tee "$NGINX_PULSE_AVAIL" >/dev/null <<HTTPONLY
upstream pulse_terminal { server 127.0.0.1:5199; keepalive 8; }
server {
    listen 80;
    listen [::]:80;
    ...
    location / {
        proxy_pass http://pulse_terminal;
        proxy_http_version 1.1;
        proxy_set_header Host \$host;
        proxy_set_header X-Real-IP \$remote_addr;
        proxy_set_header X-Forwarded-For \$proxy_add_x_forwarded_for;
...
HTTPONLY
    "${sudo[@]}" ln -sfn "$NGINX_PULSE_AVAIL" "$NGINX_PULSE_ENABLED"
    "${sudo[@]}" nginx -t && "${sudo[@]}" systemctl reload nginx
    if command -v certbot >/dev/null 2>&1; then
      "${sudo[@]}" certbot certonly --webroot -w /var/www/certbot --non-interactive --agree-tos \
        --keep-until-expiring -d "$PULSE_DOMAIN" "${email_args[@]}" || {
          echo "WARNING: certbot failed for ${PULSE_DOMAIN} — is DNS A → this host live?"
          echo "          HTTP vhost is up; re-run after DNS propagates."
          return 0
        }
    else
    ...
    if command -v certbot >/dev/null 2>&1; then
      "${sudo[@]}" certbot certonly --webroot -w /var/www/certbot --non-interactive --agree-tos \
        --keep-until-expiring -d "$PULSE_DOMAIN" "${email_args[@]}" || true
    fi
  fi

  "${sudo[@]}" install -m 644 "$NGINX_PULSE_SRC" "$NGINX_PULSE_AVAIL"
  "${sudo[@]}" ln -sfn "$NGINX_PULSE_AVAIL" "$NGINX_PULSE_ENABLED"
  "${sudo[@]}" systemctl enable --now certbot.timer 2>/dev/null || true
}
...
echo "Pulse host:     $PUBLIC_PULSE_HOST"
