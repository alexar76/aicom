...
# One pair per entry: `for a b in ...` is zsh-only and this runbook declares bash,
# where it is a syntax error that kills the script before the first RPC call.
for entry in "Escrow=$ESCROW_ADDR" "NFT=$NFT_ADDR"; do
  label="${entry%%=*}"
  addr="${entry#*=}"
