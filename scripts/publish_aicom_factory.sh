...
# Resolve publish target. Prefer explicit flag/env, then a GitHub-shaped remote
# named `gh`/`github`, and only then `origin`. Origin in this monorepo is often
# Gitea — using it would force-push a trimmed snapshot over the canon.
if [[ -z "$REMOTE" ]]; then
  for _cand in gh github origin; do
    _url="$(git remote get-url "$_cand" 2>/dev/null || true)"
    if [[ -z "$_url" ]]; then
      continue
    fi
    if [[ "$_cand" == "origin" ]]; then
      # Only accept origin when it already points at GitHub aicom.
      if [[ "$_url" == *github.com*aicom* ]]; then
        REMOTE="$_url"
        break
      fi
      continue
...
# that push would OVERWRITE it and destroy all history + satellites.
_remote_display="$REMOTE"
if [[ "$_remote_display" == *"@"* ]]; then
  _remote_display="${_remote_display##*@}"   # strip any user:token@ prefix
fi
if [[ -z "$REMOTE" ]]; then
  echo "ERROR: no publish target." >&2
  echo "  Set AICOM_FACTORY_REMOTE=git@github.com:<org>/aicom.git (or pass --remote)." >&2
  echo "  Hint: this repo has a 'gh' remote for GitHub; origin is often Gitea." >&2
  exit 1
fi
