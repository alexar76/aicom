...
cd "$ROOT"
# Python helpers (and any accidental relative Path reads) must survive cd into temp clones.
...
_root = Path(os.environ.get("AICOM_ROOT") or Path.cwd())
MAP = yaml.safe_load((_root / "scripts/satellite-map.yaml").read_text())
cmd = sys.argv[1]
...
_commit_and_push() {
  ...
  # All git ops run in a subshell so the parent shell's cwd stays at $ROOT.
  # (A plain `cd "$clone"` + early return used to leave us inside the temp clone
  # and break the next satellite; a RETURN trap was fragile on bash 3.2.)
  # Inner function: bash 3.2 allows `local` only inside functions, not bare `( )`.
  _commit_and_push_body() {
...
EOF
  ...
  ( cd "$clone" || exit 1; _commit_and_push_body )
}
...
if [[ -n "$SINGLE_SATELLITE" ]]; then
  ...
  # Belt-and-suspenders: never leave this script in a temp-clone directory.
  cd "$ROOT" || true

  echo ""
  echo "=============================================================="
  if [[ "$errors" -eq 0 ]]; then
    echo "OK All satellites mirrored successfully."
  else
    echo "WARN ${errors} satellite(s) had errors."
  fi
fi
