...
# ── Python helpers ──────────────────────────────────────────────────────────
# AICOM_ROOT, not a relative path. This read used to be
# Path("scripts/satellite-map.yaml"), resolved against the CURRENT directory —
# and _commit_and_push cd'd into the satellite clone and never came back, so
# every call after the first satellite looked for the map inside that clone.
# There it either did not exist (a traceback, and an EMPTY result) or, worse,
# DID exist because line ~307 copies the map into the clone — in which case the
# read silently succeeded against the wrong file.
#
# The empty result was the dangerous half: callers assign it to extra_excludes,
# the per-satellite list of paths that must NOT be published (helios's
# client_secret.json, skopos's servers.yaml and agent.yaml, argus's
# argus.config.json). An empty list is a valid-looking list.
#
# _commit_and_push now runs in a subshell so the cd cannot leak, but this stays
# anchored regardless: one fix removes the symptom, the other removes the class,
# and a path that only works from one directory is a trap for the next caller.
_python_map() {
  AICOM_ROOT="$ROOT" python3 - "$@" <<'PYEOF'
import os, sys, json, yaml
from pathlib import Path

_map_path = Path(os.environ["AICOM_ROOT"]) / "scripts" / "satellite-map.yaml"
if not _map_path.is_file():
    sys.exit(f"satellite-map.yaml not found at {_map_path} — is AICOM_ROOT wrong?")
MAP = yaml.safe_load(_map_path.read_text(encoding="utf-8"))
cmd = sys.argv[1]

if cmd == "list-ids":
    for s in MAP.get("satellites", []):
        if s.get("optional") or s.get("factory_embedded"):
            continue
        # Intent-only map entries stay off GitHub until flipped after first mirror.
        if s.get("github_published") is False:
            continue
        print(s["id"])
...
PY
  ...
  # Keep documented dotenv templates (.env.example) — the broad .env.* rule would
  # otherwise strip them and break satellite CI / scaffold templates.
  rsync_args+=(--include ".env.example")
  # Read the ONE canonical list rather than keeping a copy. The copy that used to live here
  # had drifted 21 patterns behind scripts/aicom_publish_config.py — missing `*_key`,
  # `conductor_key`, `*_ed25519` and `session_secret` among them — so this path would have
  # rsynced key material that the factory publish path excluded. A public mirror already
  # leaked a real key once through exactly this mechanism.
  local -a base_excludes=()
  while IFS= read -r pat; do
    [[ -n "$pat" ]] && base_excludes+=("$pat")
  done < <(python3 "$ROOT/scripts/aicom_publish_config.py" list-rsync-excludes)
  # Fail closed. An empty list would rsync .git, .env and every *.key into a public repo,
  # so a broken interpreter must stop the mirror, not silently widen it.
  if (( ${#base_excludes[@]} < 20 )); then
    echo "refusing to mirror: expected the canonical rsync exclude list, got ${#base_excludes[@]} patterns" >&2
    return 1
  fi
...
_post_push_satellite_hook() {
  # Pages is dispatched AFTER the push, not before it. It used to run inside
  # _post_rsync_satellite_hook, which fires while the clone is still local — so
  # the workflow deployed the PREVIOUS contents and, whenever the push-triggered
  # run did not fire (a force-push replacing history can evaluate its `paths`
  # filter as no-match), the landing stayed one deploy behind indefinitely. That
  # is exactly how logos published a hero whose 3D could never load: the fix was
  # on main and Pages kept serving the file from before it.
  local sat_id="$1"
  local clone="$2"
  local repo="$3"

  [[ "$DRY_RUN" -eq 1 ]] && return 0
  [[ -f "$clone/docs/landing/index.html" || -f "$clone/index.html" ]] || return 0
...
_post_rsync_satellite_hook() {
  ...
  case "$sat_id" in
    ...
    metis)
      ...
      ;;
    skopos)
      # Economy PQC tests + hybrid signing need oracle-core with require_pq /
      # load_pq_keypair. PyPI aimarket-oracle-core lags the monorepo; CI installs
      # vendor/oracle-core when present (see skopos/.github/workflows/ci.yml).
      ...
      if [[ -d "$ROOT/oracles/core" ]]; then
        ...
        echo "  ✓ vendored oracles/core → vendor/oracle-core (CI + GHCR)"
      else
        echo "  ⚠️  oracles/core missing — PQC CI will fall back to stale PyPI"
      fi
      if [[ -f "$clone/docs/landing/index.html" ]]; then
        : > "$clone/docs/landing/.nojekyll"
        echo "  ✓ docs/landing/ → GitHub Pages (workflow uploads folder)"
    ...
    gaia)
      ...
      ;;
    momus)
      # MOMUS's standalone Docker build needs oracle-core without the monorepo path.
      mkdir -p "$clone/vendor"
      if [[ -d "$ROOT/oracles/core" ]]; then
        rsync -a --delete \
          --exclude '.venv' --exclude '__pycache__' --exclude '.pytest_cache' \
        ...
        echo "  ⚠️  oracles/core missing — standalone Docker build will fail until vendored"
      fi
      # Protocol conformance reference for cross_check + CI parity tests (sibling
      # aimarket-protocol is absent on the GitHub satellite; monorepo Dockerfile
      # COPYs it from the tree — standalone must ship the same script).
      if [[ -d "$ROOT/aimarket-protocol/conformance" ]]; then
        rsync -a --delete \
          --exclude '__pycache__' --exclude '.pytest_cache' \
          "$ROOT/aimarket-protocol/conformance/" "$clone/vendor/protocol-conformance/"
        echo "  ✓ vendored aimarket-protocol/conformance → vendor/protocol-conformance"
      else
        echo "  ⚠️  aimarket-protocol/conformance missing — cross-check CI will fail until vendored"
      fi
      if [[ -f "$clone/docs/landing/index.html" ]]; then
        : > "$clone/docs/landing/.nojekyll"
        echo "  ✓ docs/landing/ → GitHub Pages (workflow uploads folder)"
      ...
      ;;
    treasury)
      # The Treasury depends on BOTH oracle-core AND aimarket-momus; vendor both for standalone.
      mkdir -p "$clone/vendor"
      if [[ -d "$ROOT/oracles/core" ]]; then
        rsync -a --delete \
          --exclude '.venv' --exclude '__pycache__' --exclude '.pytest_cache' \
          --exclude '*.egg-info' --exclude '.mypy_cache' \
          "$ROOT/oracles/core/" "$clone/vendor/oracle-core/"
        echo "  ✓ vendored oracles/core → vendor/oracle-core"
      fi
      if [[ -d "$ROOT/momus" ]]; then
        rsync -a --delete \
          --exclude '.venv' --exclude '__pycache__' --exclude '.pytest_cache' \
          --exclude '*.egg-info' --exclude 'data' --exclude 'frontend/node_modules' \
          --exclude 'frontend/dist' --exclude '.github' \
          "$ROOT/momus/" "$clone/vendor/momus/"
        echo "  ✓ vendored momus → vendor/momus (Treasury imports aimarket-momus)"
      else
        echo "  ⚠️  momus/ missing — Treasury standalone build will fail until vendored"
      fi
      if [[ -f "$clone/docs/landing/index.html" ]]; then
        : > "$clone/docs/landing/.nojekyll"
        echo "  ✓ docs/landing/ → GitHub Pages (workflow uploads folder)"
    ...
    atlas)
      ...
      ;;
    logos)
      # LOGOS's standalone Docker build needs oracle-core without the monorepo path.
      mkdir -p "$clone/vendor"
      if [[ -d "$ROOT/oracles/core" ]]; then
        rsync -a --delete \
          --exclude '.venv' --exclude '__pycache__' --exclude '.pytest_cache' \
    ...
    signal-hunt)
      ...
      ;;
    use-cases-portal)
      # Static portal root is the Pages artifact (see .github/workflows/pages.yml).
      : > "$clone/.nojekyll"
      echo "  ✓ use-cases-portal root → GitHub Pages"
      ;;
    aimarket-mcp)
...
export_desktop_monorepo() {
  ...
  local rsync_base_args=(-a --exclude .git --exclude .claude --exclude .cursor --exclude .venv --exclude venv --exclude node_modules --exclude __pycache__ --exclude .pytest_cache --exclude .dart_tool --exclude build --exclude dist --exclude .DS_Store \
    --include '.env.example' --exclude .env --exclude '.env.*' --exclude '*.key' --exclude '*.pem' --exclude '*.enc' --exclude data/secrets --exclude '*_signing_key' --exclude '*_vrf_sk' --exclude dataset_salt)
...
PY
  ...
  local rsync_base=(-a --exclude .git --exclude .claude --exclude .cursor --exclude .venv --exclude venv
    ...
    --include '.env.example' --exclude .env --exclude '.env.*' --exclude '*.key' --exclude '*.pem' --exclude '*.enc'
...
export_aicom_products() {
  # Catalog shell from monorepo aicom-products/ → alexar76/aicom-products.
  # history:live. Never wipe remote products/ (selective publish owns that tree).
  local sat_id="aicom-products"
  local repo="aicom-products"
  local src_rel="aicom-products"
  local remote_url
  remote_url="$(satellite_remote_url "$repo")"

  echo ""
  echo "━━━ ${sat_id} ━━━"
  echo "  Source:  $src_rel/ (catalog shell; preserve remote products/)"
  echo "  Remote:  ${GITHUB_HOST}/${GITHUB_ORG}/${repo}"
  ...
  if [[ "$DRY_RUN" -eq 1 ]]; then
    echo "  [dry-run] would sync $src_rel/ → $repo/ (keep products/)"
    return 0
  fi

  python3 "$ROOT/scripts/ensure_github_repo.py" "$GITHUB_ORG" "$repo" \
    "Selective catalog of full AI-Factory products (prod-*) — shell from monorepo, trees published on demand." \
    || return 1

  local clone="$WORKDIR/${repo}"

  if git_auth ls-remote "$remote_url" "refs/heads/$BRANCH" &>/dev/null; then
    git_auth clone --depth 50 --branch "$BRANCH" "$remote_url" "$clone" 2>/dev/null || {
      git_auth clone --depth 50 "$remote_url" "$clone"
      (cd "$clone" && git checkout -B "$BRANCH" 2>/dev/null || git checkout -b "$BRANCH")
    }
  else
    mkdir -p "$clone"
  ...
  local products_backup=""
  if [[ -d "$clone/products" ]]; then
    products_backup="$(mktemp -d "${WORKDIR}/aicom-products-products.XXXXXX")"
    rsync -a "$clone/products/" "$products_backup/"
    echo "  ✓ preserved remote products/"
  fi
  ...
  local rsync_args=(-a)
  rsync_args+=(--include ".env.example")
  for pat in .git .claude .cursor .venv venv node_modules __pycache__ .pytest_cache .mypy_cache .DS_Store \
             .env ".env.*" "*.key" "*.pem" data/secrets; do
    rsync_args+=(--exclude "$pat")
  done
  rsync_args+=(--exclude "products/")
  rsync "${rsync_args[@]}" "$src/" "$clone/"

  mkdir -p "$clone/products"
  if [[ -n "$products_backup" && -d "$products_backup" ]]; then
    rsync -a "$products_backup/" "$clone/products/"
    echo "  ✓ restored products/ after shell sync"
  elif [[ -f "$src/products/.gitkeep" ]]; then
    cp "$src/products/.gitkeep" "$clone/products/.gitkeep"
  fi

  _copy_governance "$clone" "mit" "$repo" "aicom-products"
  _inject_mirror_banner "$clone" "$sat_id" "$repo"
  _inject_readme_badges "$sat_id" "$clone"
  _commit_and_push "$clone" "$sat_id" "$remote_url" "$repo" "mit"
}
...
export_lottery() {
  ...
  # Same canonical exclude list as export_simple. The hand-written copy here
  # skipped `broadcast` and a nested `alien-monitor/` checkout, so the working
  # tree (rsync, not git) shipped Foundry run-latest.json plus a private host IP.
  local extra_excludes
  extra_excludes="$(_python_map exclude-paths lottery)"
  local rsync_args=(-a)
  rsync_args+=(--include ".env.example")
  local -a base_excludes=()
  while IFS= read -r pat; do
  ...
  if (( ${#base_excludes[@]} < 20 )); then
    echo "refusing to mirror lottery: expected the canonical rsync exclude list, got ${#base_excludes[@]} patterns" >&2
    return 1
  fi
  for pat in "${base_excludes[@]}"; do
    rsync_args+=(--exclude "$pat")
...
_copy_governance() {
  ...
  # LICENSE
  # A dual-licensed satellite ships its own LICENSE (a pointer to LICENSE-MIT and
  # LICENSE-APACHE beside it). Copying the monorepo's plain MIT over that would silently
  # strip the Apache-2.0 half — including its patent grant — from every mirror.
  if [[ -f "$target/LICENSE-MIT" && -f "$target/LICENSE-APACHE" ]]; then
    echo "  · LICENSE kept from satellite source (dual-licensed)"
  elif [[ "$license" == "apache-2.0" || "$license" == "apache" ]]; then
  ...
  # SECURITY.md — prefer satellite-authored policy (export already copied it).
  # Overwriting with the generic template wiped THEMIS trust-boundary docs and
  # failed CI (`test_security_documentation_matches_implemented_boundaries`).
  if [[ -f "$target/SECURITY.md" ]]; then
    echo "  · SECURITY.md kept from satellite source"
  else
...
export_profile() {
  ...
  # Language portals (EN is README.md; RU/ES/FR/ZH are siblings linked from the bar).
  local lang_copied=0
  local lang_f
  for lang_f in README.ru.md README.es.md README.fr.md README.zh.md; do
    if [[ -f "$ROOT/scripts/profile-readme/$lang_f" ]]; then
      cp -f "$ROOT/scripts/profile-readme/$lang_f" "$clone/$lang_f"
      lang_copied=$((lang_copied + 1))
    fi
  done
  if [[ "$lang_copied" -gt 0 ]]; then
    echo "  ✓ language portals synced (${lang_copied} file(s))"
  fi
...
# Body is ( … ), not { … }, ON PURPOSE: this function cd's into the satellite
# clone and has a dozen `return` paths, so restoring the directory by hand would
# mean getting it right on every one of them. A subshell body makes the cd
# structurally unable to escape — `return N` still sets the function's exit
# status, `local` still works, and the EXIT trap on line ~163 does NOT fire when
# the subshell ends (bash resets inherited traps in subshells), so $WORKDIR
# survives. All nine call sites invoke it plainly, so before this the cd leaked
# into the main loop and every later satellite ran from the previous one's clone.
# Nothing inside is read by the caller: every variable is a local, and msg_file
# is used only here.
_commit_and_push() (
  local clone="$1"
  local sat_id="$2"
  ...
  # desktop and courses monorepos, wiki, wiki_argus, profile — rsync'd their own
  # hand-written exclude lists and pushed with no scan at all. The reasoning given for
  # the live-only placement was that live history is permanent; but a mirror push is
  # just as public, and a later force-push does not un-publish a secret.
  #
  # It has to be here, not in the callers: rsync copies the WORKING TREE, so .gitignore
  # protects nothing, and this is the same mechanism that published a real key once.
  # Every export_* funnels through this function, and this runs before the first
  # `git add -A` on any branch through it.
  if ! bash "$ROOT/scripts/verify_mirror_secrets.sh" "$clone"; then
    echo "  ⛔ ${repo}: secret scan failed — refusing to publish." >&2
    return 1
  fi
...
EOF
  ...
  if [[ "$mode" == "live" ]]; then
    ...
    case "$head_subj" in
      "chore(satellite): sync"* | "Publish products:"* | "") : ;;  # monorepo sync, catalog product push, or empty
      *)
    ...
    # (The hard secret gate now runs once at the top of this function, for every
    # history mode — live history being permanent is a reason it matters MORE here,
    # never a reason it was the only path that needed it.)
...
export_satellite() {
  ...
  # github_published: false = intent-only map entry; skip unless forced.
  if [[ "${FORCE_UNPUBLISHED_SATELLITE:-0}" != "1" ]]; then
    local published
    published="$(
      AICOM_ROOT="$ROOT" SAT_ID="$sat_id" python3 -c "
import os, yaml
from pathlib import Path
m = yaml.safe_load((Path(os.environ['AICOM_ROOT']) / 'scripts/satellite-map.yaml').read_text())
sid = os.environ['SAT_ID']
for s in m.get('satellites') or []:
    if s.get('id') == sid:
        print('no' if s.get('github_published') is False else 'yes')
        break
else:
    print('yes')
")"
    if [[ "$published" == "no" ]]; then
      echo "⏭️  skip $sat_id (github_published: false — flip after first intentional mirror)"
      return 0
    fi
  fi
  ...
  case "$sat_id" in
    ...
    aimarket-courses)
      ...
      ;;
    aimarket-school)
      export_simple "$sat_id" "school" "aimarket-school" "mit"
      ;;
    aimarket-desktop)
      export_desktop_monorepo
      ;;
    ...
    acex)
      local extra_excludes
      extra_excludes="$(_python_map exclude-paths acex)"
      export_simple "$sat_id" "acex" "acex" "apache-2.0" "$extra_excludes"
      ;;
    ai-service-mesh)
    ...
    aimarket-bridges)
      local extra_excludes
      extra_excludes="$(_python_map exclude-paths aimarket-bridges)"
      export_simple "$sat_id" "aimarket-bridges" "aimarket-bridges" "apache-2.0" "$extra_excludes"
      ;;
    aicom-wiki)
      export_wiki
      ;;
    ...
    argus)
      ...
      ;;
    warden)
      # Publish @aimarket/warden (scripts/publish_warden.sh) BEFORE mirroring argus —
      # Argus pins the registry package exactly.
      local extra_excludes
      extra_excludes="$(_python_map exclude-paths warden)"
      export_simple "$sat_id" "warden" "warden" "mit" "$extra_excludes"
      ;;
    dioscuri)
      export_dioscuri
      ;;
    ...
    basanos)
      local extra_excludes
      extra_excludes="$(_python_map exclude-paths basanos)"
      export_simple "$sat_id" "basanos" "basanos" "mit" "$extra_excludes"
      ;;
    metis)
      local extra_excludes
    ...
    atlas)
      local extra_excludes
      extra_excludes="$(_python_map exclude-paths atlas)"
      export_simple "$sat_id" "atlas" "atlas" "mit" "$extra_excludes"
      ;;
    momus)
      local extra_excludes
      extra_excludes="$(_python_map exclude-paths momus)"
      export_simple "$sat_id" "momus" "momus" "mit" "$extra_excludes"
      ;;
    dolos)
      # Same path as momus/basanos: export_simple → _commit_and_push (sanitize_git_commit_meta
      # strips AI/bot Co-authored-by / Assisted-by; human aicom-coauthors stay).
      local extra_excludes
      extra_excludes="$(_python_map exclude-paths dolos)"
      export_simple "$sat_id" "dolos" "dolos" "mit" "$extra_excludes"
      ;;
    attested)
      echo "ERROR: attested/ is Gitea-only (github_published: false). Refuse GitHub mirror." >&2
    ...
    cite-desks)
      local extra_excludes
      extra_excludes="$(_python_map exclude-paths cite-desks)"
      export_simple "$sat_id" "cite-desks" "cite-desks" "mit" "$extra_excludes"
      ;;
    emberline)
      echo "ERROR: emberline is a nested desk, not a top-level GitHub repo." >&2
      echo "        Parent satellite is cite-desks → alexar76/cite-desks." >&2
      echo "        Use: ./scripts/publish_all_repos.sh --satellite cite-desks" >&2
      return 1
      ;;
    tideline|solrecord|seamark|plinth)
      echo "ERROR: $sat_id is a nested desk inside alexar76/cite-desks, not its own GitHub repo." >&2
      echo "        Use: ./scripts/publish_all_repos.sh --satellite cite-desks" >&2
      return 1
      ;;
    attested-memory-hub|attested-saas-gateway)
      echo "ERROR: $sat_id moved under attested/. Use satellite id 'attested' (still GitHub-blocked)." >&2
      return 1
      ;;
    escrow-signer)
      local extra_excludes
      extra_excludes="$(_python_map exclude-paths escrow-signer)"
      export_simple "$sat_id" "escrow-signer" "escrow-signer" "mit" "$extra_excludes"
      ;;
    treasury)
      local extra_excludes
      extra_excludes="$(_python_map exclude-paths treasury)"
      export_simple "$sat_id" "treasury" "treasury" "mit" "$extra_excludes"
      ;;
    logos)
      local extra_excludes
      extra_excludes="$(_python_map exclude-paths logos)"
      export_simple "$sat_id" "logos" "logos" "mit" "$extra_excludes"
      ;;
    signal-hunt)
      local extra_excludes
      extra_excludes="$(_python_map exclude-paths signal-hunt)"
      export_simple "$sat_id" "signal-hunt" "signal-hunt" "mit" "$extra_excludes"
      ;;
    aimarket-playground)
      local extra_excludes
      extra_excludes="$(_python_map exclude-paths aimarket-playground)"
      export_simple "$sat_id" "aimarket-playground" "aimarket-playground" "mit" "$extra_excludes"
      ;;
    create-aimarket-agent)
      local extra_excludes
      extra_excludes="$(_python_map exclude-paths create-aimarket-agent)"
      export_simple "$sat_id" "create-aimarket-agent" "create-aimarket-agent" "mit" "$extra_excludes"
      ;;
    themis)
      local extra_excludes
      extra_excludes="$(_python_map exclude-paths themis)"
      export_simple "$sat_id" "themis" "themis" "mit" "$extra_excludes"
      ;;
    hestia)
      local extra_excludes
    ...
    use-cases-portal)
      ...
      export_simple "$sat_id" "use-cases-portal" "use-cases-portal" "mit" "$extra_excludes"
      ;;
    aimarket-mcp)
      local extra_excludes
    ...
    hephaestus)
      local extra_excludes
      extra_excludes="$(_python_map exclude-paths hephaestus)"
      export_simple "$sat_id" "hephaestus" "hephaestus" "mit" "$extra_excludes"
      ;;
    *)
...
echo "  https://${GITHUB_HOST}/${GITHUB_ORG}/aimarket-bridges"
...
echo "  https://${GITHUB_HOST}/${GITHUB_ORG}/gaia"
echo "  https://${GITHUB_HOST}/${GITHUB_ORG}/atlas"
echo "  https://${GITHUB_ORG}.github.io/atlas/"
echo "  https://forge.modelmarket.dev/"
echo "  https://modelmarket.dev/studio/"
echo "  https://${GITHUB_HOST}/${GITHUB_ORG}/hephaestus"
echo "  https://${GITHUB_ORG}.github.io/hephaestus/"
echo "  https://${GITHUB_HOST}/${GITHUB_ORG}/basanos"
echo "  https://${GITHUB_ORG}.github.io/basanos/"
echo "  https://basanos.modelmarket.dev/"
echo "  https://${GITHUB_HOST}/${GITHUB_ORG}/dolos"
echo "  https://${GITHUB_ORG}.github.io/dolos/"
echo "  https://${GITHUB_HOST}/${GITHUB_ORG}/logos"
echo "  https://${GITHUB_ORG}.github.io/logos/"
echo "  https://${GITHUB_HOST}/${GITHUB_ORG}/signal-hunt"
echo "  https://${GITHUB_ORG}.github.io/signal-hunt/"
echo "  https://${GITHUB_HOST}/${GITHUB_ORG}/aimarket-playground"
echo "  https://${GITHUB_HOST}/${GITHUB_ORG}/create-aimarket-agent"
echo "  https://${GITHUB_HOST}/${GITHUB_ORG}/themis"
...
echo "  https://${GITHUB_HOST}/${GITHUB_ORG}/aicom-products"
