...
Auto-generated by scripts/mirror_satellites.sh"
  ...
  if ! git_auth push origin "HEAD:$BRANCH" 2>&1; then
    echo "  ↻ Remote ahead — rebasing and retrying push …"
    if git_auth pull --rebase origin "$BRANCH" 2>&1 && git_auth push origin "HEAD:$BRANCH" 2>&1; then
      echo "  ✅ Pushed ${sat_id} (after rebase)"
      return 0
    fi
