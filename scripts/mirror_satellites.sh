...
")"
  ...
  case "$sat_id" in
    ...
    attested)
      ...
      ;;
    emberline)
      echo "ERROR: emberline/ (now cite-desks/emberline) is Gitea-only (github_published: false). Refuse GitHub mirror." >&2
      echo "        Use ./scripts/push_gitea_monorepo.sh — flip github_published only when asked." >&2
      return 1
      ;;
    cite-desks|tideline|solrecord|seamark)
      echo "ERROR: cite-desks/ is Gitea-only (github_published: false). Refuse GitHub mirror." >&2
      echo "        Use ./scripts/push_gitea_monorepo.sh — flip github_published only when asked." >&2
      return 1
      ;;
