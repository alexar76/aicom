#!/usr/bin/env bash
# Run ON the Platon server (78.17.126.214) as root.
# DNS: oracles.modelmarket.dev A → 78.17.126.214 (Timeweb).
...
if [[ "$(id -u)" -ne 0 ]]; then
  echo "Run as root on Platon host (78.17.126.214): sudo $0" >&2
  exit 1
fi
