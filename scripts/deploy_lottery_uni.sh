...
# The address above is a snapshot taken at deploy time. Mount the directory the UNI bootstrap
# rewrites on every contract redeploy so the relayer can follow the lottery when it moves —
# without that, a bubble re-bootstrap leaves it calling a code-less address forever.
export UNI_SHARED_DIR="${UNI_SHARED_DIR:-$ROOT/data/alien-monitor/universe}"
export LOTTERY_ADDRESS_FILE="${LOTTERY_ADDRESS_FILE:-/uni-shared/hub.env.snippet}"
