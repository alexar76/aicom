...
# A "deploy tag" is one minted per deploy and safe to reap. `^prod-` alone matched only
# the hub and factory shape; alien-monitor tags itself `20260907-142114` and
# `live-20260907-chainhonesty`, so 21 unreferenced monitor images at 1.66 GB each — 25 GB
# on a 77 GB disk — were reported as "skip (not a deploy tag)" and accumulated until a
# build died with "no space left on device" on 2026-09-11. That is the exact failure this
# script was written to prevent, in a tag shape it did not recognise.
#   prod-…            hub, factory, themis
#   YYYYMMDD-HHMMSS   alien-monitor
#   live-…            alien-monitor's live lineage
#   uni-…             uni lineages
DEPLOY_TAG_RE='^(prod-|live-|uni-|[0-9]{8}-[0-9]{4,6}$)'
