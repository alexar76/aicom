...
#
# Full tree (history + every folder): satellites, independent/, pantheon/, dolos/, …
# GitHub satellites (momus, dolos, …) are published separately via
# publish_all_repos.sh / mirror_satellites.sh — not by this script.
#
...
# Author identity for this monorepo is alexar76 only. A wrong global git config
# (e.g. another GitHub account) once landed 113 tip commits as alexart2025 on
# Gitea; they had to be rewritten and force-pushed. Catch that before push.
# Override: SKIP_AUTHOR_GATE=1 (emergency only).
_ALLOWED_AUTHOR_EMAIL="alexar76@rambler.ru"
_ALLOWED_AUTHOR_NAME="aleksandr.artamokhov"
_assert_alexar76_author() {
  local url="$1"
  if [[ "${SKIP_AUTHOR_GATE:-0}" == "1" ]]; then
    echo "  ⚠️  SKIP_AUTHOR_GATE=1 — not checking commit author identity"
    return 0
  fi
  ...
  if ! git rev-list --max-count=1 "$range" >/dev/null 2>&1; then
    echo "  ✓ alexar76 author gate: nothing to scan"
    return 0
  fi
  local bad
  bad="$(
    git log --format='%H%x09%an%x09%ae' "$range" | python3 -c '
import sys
allowed_name = "'"${_ALLOWED_AUTHOR_NAME}"'"
allowed_email = "'"${_ALLOWED_AUTHOR_EMAIL}"'"
bad = []
for line in sys.stdin:
    sha, name, email = line.rstrip("\n").split("\t", 2)
    if name != allowed_name or email != allowed_email:
        bad.append(f"{sha[:12]}  {name} <{email}>")
if bad:
    print("\n".join(bad[:40]))
    if len(bad) > 40:
...
'
    ...
    echo "ERROR: unpushed commit(s) are not authored as ${_ALLOWED_AUTHOR_NAME} <${_ALLOWED_AUTHOR_EMAIL}>:" >&2
    echo "$bad" | sed 's/^/  /' >&2
    echo "  This repo's author is alexar76 only. Fix with:" >&2
    echo "    git -c user.name='${_ALLOWED_AUTHOR_NAME}' -c user.email='${_ALLOWED_AUTHOR_EMAIL}' commit --amend --reset-author" >&2
    echo "  Or rewrite a range, then force-push to Gitea (branch protection must allow it)." >&2
    echo "  Emergency only: SKIP_AUTHOR_GATE=1" >&2
    exit 1
  }
  echo "  ✓ alexar76 author gate clean (${_ALLOWED_AUTHOR_NAME} <${_ALLOWED_AUTHOR_EMAIL}>)"
}
