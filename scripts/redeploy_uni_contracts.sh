...
for i in $(seq 1 90); do
  if HEALTH="$(curl -sf "http://127.0.0.1:9100/api/health" 2>/dev/null || true)"; then
    OK="$(echo "$HEALTH" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('blockchain_ready') and d.get('contracts',{}).get('evm_escrow',''))" 2>/dev/null || true)"
...
if docker ps --format '{{.Names}}' | grep -qx 'modelmarket-hub'; then
  echo "6/6 Restart modelmarket-hub…"
  docker restart modelmarket-hub
  sleep 3
  curl -sf "http://127.0.0.1:9083/.well-known/ai-market.json" >/dev/null && echo "Hub: OK" || echo "WARNING: Hub not responding yet"
else
  echo "6/6 modelmarket-hub not running — restart Hub manually after merging .env"
fi
