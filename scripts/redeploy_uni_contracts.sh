...
echo ""
echo "=== Restart UNI lottery relayer (binds to fresh evm_lottery) ==="
if [[ -x "$ROOT/scripts/deploy_lottery_uni.sh" ]]; then
  bash "$ROOT/scripts/deploy_lottery_uni.sh" || echo "WARN: lottery relayer restart failed"
fi
