...
#
# Anchored with \b because the prefixes are short enough to occur mid-word. Self-hosted
# webfonts named after their family blocked a publish this way: `spacegrotesk-V8mDoQDjQSk…`
# contains `sk-` followed by base64, which is indistinguishable from an OpenAI key to a
# substring match. A real credential is always preceded by a quote, space, `=` or `:` — never
