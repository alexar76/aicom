...
if [[ -n "$forbidden_hosts" ]]; then
  ...
  if rg -q -F "${rg_host_args[@]}" -g '!.mirror-forbidden-hosts' "$clone" 2>/dev/null; then
    echo "error: private server IP/hostname literal in mirror tree" >&2
    fail=1
  fi
else
  echo "warn: no MIRROR_FORBIDDEN_HOSTS / $forbidden_hosts_file — skipping private-host check" >&2
fi
...
0x5de4111afa1a4b94908f83103eb1f1706367c2e68ca870fc3fb9a804cdab365a # gitleaks:allow
0x7c852118294e51e653712a81e05800f419141751be58f605c371e15141b007a6 # gitleaks:allow
0x47e179ec197488593b187f80a00eb0da91f1b9d0b13f8733639f19c30a34926a # gitleaks:allow
0x8b3a350cf5c34c9194ca85829a2df0ec3153be0318b5e2d3348e872092edffba # gitleaks:allow
0x92db14e403b83dfe3df233f83dfa3a0d7096f21ca9b0d6d6b8d88b2b4ec1564e # gitleaks:allow
0x4bbbf85ce3377467afe5d46f804f221813b2bb87f24d81f60f1fcdbf7cbf4356 # gitleaks:allow
0xdbda1821b80551c9d65939329250298aa3472ba22feea921c0cf5d620ea67b97 # gitleaks:allow
0x2a871d0798f97d79848a013d4936a73bf4cc922c825d33c1cf7073dff6d409c6 # gitleaks:allow
ANVIL_KEYS
)
