...
def badge_block(sat: dict[str, Any], *, use_raw_coverage: bool = False) -> str:
    ...
    factory_embedded = bool(sat.get("factory_embedded"))
    if factory_embedded:
        # Monorepo subrepo (e.g. hephaestus/) — no alexar76/<repo> satellite mirror.
        # Badge assets live beside this README under docs/badges/.
        ci_img = "docs/badges/ci.svg"
        ci = f'<img src="{ci_img}" alt="CI" />'
        html_parts: list[str] = [ci]
        extras = [
            _extra_badge_html(e, "")
            for e in (sat.get("extra_badges") or [])
            if e and e.strip()
        ]
        html_parts.extend(extras)
        license_src = "docs/badges/license.svg"
        license_href = sat.get("license_path") or "../LICENSE"
        license_badge = (
            f'<a href="{license_href}">'
    ...
    # Use refs/heads/main, not the /main/ shortcut: GitHub's CDN has been returning HTTP 400
    # for alexar76/aicom/main/docs/badges/coverage.svg while the same blob via refs/heads/main
    # (and via commit SHA) serves 200. The README then shows the alt text "Test coverage".
    raw = f"https://raw.githubusercontent.com/{org}/{repo}/refs/heads/main"
