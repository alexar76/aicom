...
DEFAULT_RSYNC_EXCLUDES = [
    ...
    "pantheon",
    # Cite-desks family stays out of every *other* satellite and the factory.
    # Do not list "emberline" here: that name is the fire sub-desk inside
    # cite-desks/, and the parent satellite must ship it. Factory trim still
...
FACTORY_LOCAL_EXCLUDES = [
    ...
    "emberline",
    "cite-desks",
    # Attested Memory family + SaaS deploy contour — Gitea only until
    # github_published is flipped. Never ship host/deploy runbooks, compose
...
# Everything at the monorepo ROOT that the public factory mirror is allowed to carry.
# The exclusion lists above are a DENY-list, and a deny-list has one failure mode: a
# private directory created tomorrow is public by default until somebody remembers to
# add it — plans/, promo/ and .momus/ reached alexar76/aicom exactly that way.
# `check_roots()` closes the gap: every TRACKED root entry must be either excluded above,
# a satellite root from satellite-map.yaml, or named here. A new root that is none of the
# three fails the publish preflight and the test suite, and whoever adds it decides which
...
PUBLIC_FACTORY_ROOTS = frozenset([
    ...
    'docker-compose.dind-rootless.yml',
    'docker-compose.dind.yml',
    'docker-compose.everything.yml',
    'docker-compose.host-docker.yml',
    'docker-compose.host-gateway.yml',
    'docker-compose.pg.yml',
    'docker-compose.prod.yml',
    'docker-compose.remediation-fix.yml',
...
def classify_root(name: str) -> str:
    """'excluded' | 'satellite' | 'public' | 'unclassified' for one root entry."""
    patterns = [*DEFAULT_RSYNC_EXCLUDES, *FACTORY_LOCAL_EXCLUDES]
    if any(fnmatch.fnmatch(name, pat) or name == pat.rstrip("/") for pat in patterns):
        return "excluded"
    if name in set(factory_exclude_paths()):
        return "satellite"
    if name in PUBLIC_FACTORY_ROOTS:
        return "public"
    return "unclassified"
...
def main() -> int:
    ...
    if cmd == "check-roots":
        stray = check_roots()
        if stray:
            print("unclassified repo roots — add each to an exclude list, to satellite-map.yaml, "
                  "or to PUBLIC_FACTORY_ROOTS:", file=sys.stderr)
            for name in stray:
                print(f"  {name}", file=sys.stderr)
            return 4
        print("all tracked repo roots are classified")
        return 0
    if cmd == "rsync-args":
