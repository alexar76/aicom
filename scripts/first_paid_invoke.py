...
def main() -> int:
    ...
    # Federated catalogue rows must be invoked with source_hub=peer URL. Defaulting to
    # "local" (InvokeRequest) sends them down the factory path → 502 capability not found.
    source_hub = (cap_meta.get("source_hub") if cap_meta else None) or "local"
    if price <= 0:
        print("\ncould not resolve a positive price — pass --price")
        return 2
    print(f"\ninvoke target   : {product} / {args.capability} @ ${price}  source_hub={source_hub}")
    ...
    # The hub quotes the TOTAL it will debit — list price plus its routing fee, rounded up
    # to a whole cent, because that is the granularity both the ledger and the on-chain
    # converter bill in. A buyer that signs the capability's list price instead is refused
    # for the difference, which is exactly what happened on the first real purchase: the
    # deposit was on chain, the signature was for $0.01, and the invoke wanted $0.02. Sign
    # what the seller asked for, once, rather than making a human re-run this.
    if status == 402 and result.get("error") == "payment_authorization_required" and (
        payment_authorization is not None and args.private_key
    ):
        quoted = result.get("needed")
        quoted_units = usd_to_base_units(float(quoted)) if quoted else 0
        if quoted_units and quoted_units != int(payment_authorization.get("amount", 0)):
            print(f"\nre-signing for the hub's quoted total: ${quoted} "
                  f"({quoted_units} base units; was {payment_authorization.get('amount')})")
            payment_authorization = build_payment_authorization(
                escrow_channel=args.escrow_channel,
                hub_addr=args.hub_address,
                token=args.token_address,
            ...
            show(f"invoke (re-signed) → {status}", result)
