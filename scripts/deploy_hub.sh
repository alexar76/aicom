...
# Keep in sync with aimarket-hub/aimarket_hub/federation_seeds.json — this env var REPLACES that
# file, so a seed missing here is a seed the deployed hub never crawls. The oracle URL is the
# /family aggregate (42 capabilities from 17 oracles); the bare oracles.modelmarket.dev
# well-known that used to be here is Platon alone (11), which is why prod listed 11 for a month.
SEED_LIST="${AIMARKET_SEED_LIST:-https://magic-ai-factory.com/.well-known/ai-market.json,https://oracles.modelmarket.dev/family/.well-known/ai-market.json,https://iot.modelmarket.dev/.well-known/ai-market.json,https://atlas.modelmarket.dev/.well-known/ai-market.json}"
