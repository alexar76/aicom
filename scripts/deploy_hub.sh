...
# THEMIS has no netns of its own — it joins this container's via `--network container:modelmarket-hub`,
# because the hub's SSRF guard only exempts loopback. So recreating the hub strands THEMIS in the
# deleted netns: it keeps reporting healthy (its healthcheck runs inside that dead namespace) while
# nginx gets connection refused on 9460. This script omitted both the port above and this reattach,
# and 502'd themis.modelmarket.dev on 2026-09-12 exactly that way — the same failure the script's
# own docs already blamed on 2026-09-08.
if [[ -x "$ROOT/themis/deploy/themis-reattach.sh" ]]; then
  FORCE=1 "$ROOT/themis/deploy/themis-reattach.sh" || echo "WARN: THEMIS reattach failed — themis.modelmarket.dev will 502"
fi
