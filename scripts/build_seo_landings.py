...
_EN_UI: dict[str, str] = {
    ...
    "oracle_hub_lede": (
        "Each oracle sells a specific verifiable computation — not trust-me APIs. "
...
def _build_learn(base: str, cfg: dict[str, Any]) -> list[tuple[str, str]]:
    ...
    for lang in LANGS:
        ...
        hub_body = (
            ...
            f'{html.escape(U("course_hub_full_portal"))}</a></div></div>'
    ...
    for lang in LANGS:
        ...
        for o in oracles:
            ...
            cards.append(
                ...
                f'<p>{html.escape(skill)}</p></article>'
            )
...
def _build_guides(base: str, cfg: dict[str, Any]) -> list[tuple[str, str]]:
    ...
    for g in guides_cfg:
        ...
        prepared.append({"g": g, "prose": _md_to_html(md_body)})
