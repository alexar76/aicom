...
Outputs:
  ecosystem-landing/learn/          — course hub + per-course landings
  ...
  ecosystem-landing/shared/learn.js — learn-hub galaxy + filters
...
_EN_UI: dict[str, str] = {
    ...
    "chip_labs": "labs",
    "course_hub_eyebrow": "{n} academies · EN / RU / ES / FR / ZH · Colab",
    "course_hub_h1": "Learn the agent economy hands-on",
    "course_hub_lede": (
        "Python courses with live labs on real AIMarket infrastructure — GAIA/ATLAS "
        "sensors, Hub federation, hybrid signatures (PQC), oracles, MCP security, "
        "USDC payment channels, and 3D visualization."
    ),
    "course_hub_full_portal": "Full course portal ↗",
    "course_hub_school_clips": "School · 5-min clips",
    "course_hub_all": "All courses",
    "course_hub_tracks": "{n} academies · four tracks",
    "course_hub_filter_all": "All tracks",
    "course_hub_filter_builder": "Agent builder",
    "course_hub_filter_security": "Security & crypto",
    "course_hub_filter_physical": "Physical world",
    "course_hub_filter_proofs": "Trust & proofs",
    "course_hub_track_builder": "Track · Agent builder",
    "course_hub_track_security": "Track · Security & crypto",
    "course_hub_track_physical": "Track · Physical world",
    "course_hub_track_proofs": "Track · Trust & proofs",
    "course_hub_meta_desc": (
        "{n} hands-on Python academies on the live AIMarket ecosystem — GAIA/ATLAS "
        "sensors, Hub federation, hybrid signatures (PQC), verifiable randomness, "
        "agent economy, MCP security, and trust math."
    ),
    "course_hub_stat_academies": "academies",
    "course_hub_stat_tracks": "tracks",
    "course_hub_stat_langs": "langs",
...
def _md_to_html(md: str, src_rel: str | None = None) -> str:
    ...
    while i < len(lines):
        ...
        if line.startswith("# "):
            ...
            flush_list()
            # A hard-wrapped blockquote is one quote, not one per source line:
            # gather the run, and treat a bare ">" as a paragraph break inside it.
            quote: list[list[str]] = [[]]
            while i < len(lines) and lines[i].startswith(">"):
                ...
                if text:
                    ...
                    quote.append([])
                i += 1
            paragraphs = "".join(f"<p>{inline(' '.join(part))}</p>" for part in quote if part)
            out.append(f"<blockquote>{paragraphs}</blockquote>")
            continue
        else:
            flush_list()
            # Same for prose: consecutive lines are one paragraph, so a source
            # file wrapped at 100 columns does not render as a stack of stubs.
            paragraph: list[str] = []
            while i < len(lines):
                candidate = lines[i]
                ...
                i += 1
            if not paragraph:
                # The break conditions are broader than the branches above (a
                # "+ " bullet reaches this else), so always consume one line —
                # an empty paragraph here would spin forever.
...
def _meta_head(
    *,
    ...
    if hreflang:
        ...
        alt_links = "\n".join(rows)
    css_link = (
        f'<link rel="stylesheet" href="{html.escape(base)}/shared/seo.css?v=3" />'
        if include_seo_css
        else ""
    )
...
def _page_shell(
    *,
    ...
    extra_head: str = "",
    extra_foot: str = "",
    hide_bg_glow: bool = False,
) -> str:
    foot = cfg.get("external", {})
    U = _ui(lang)
    body_attrs = f' class="{html.escape(body_class)}"' if body_class else ""
    glow = "" if hide_bg_glow else '<div class="bg-glow"></div>\n'
    return (
...
# Learn-hub tracks (keep in sync with courses/_tooling/build_monorepo_pages.py).
LEARN_TRACKS: list[tuple[str, str, str, list[str]]] = [
    (
        "builder",
        "course_hub_track_builder",
        "🚀",
        [
...
def _build_learn(base: str, cfg: dict[str, Any]) -> list[tuple[str, str]]:
    ...
    if not courses:
        return []
    by_folder = {c["folder"]: c for c in courses if c.get("folder")}
    ...
    for lang in LANGS:
        ...
        # Per-course landing pages (flat cards — hub gets the wow treatment).
        for c in courses:
            folder = c["folder"]
        ...
        # Hub — galaxy canvas + track filters + animated cards.
        track_blocks: list[str] = []
        for track_id, label_key, icon, folders in LEARN_TRACKS:
            cards: list[str] = []
            for folder in folders:
                c = by_folder.get(folder)
                if not c:
                    continue
                en_title = c.get("title") or folder
                en_tagline = c.get("tagline") or c.get("description") or ""
                ...
                ic = COURSE_ICONS.get(folder, "✦")
                cards.append(
                    f'<article class="learn-card">'
                    f'<div class="ic" aria-hidden="true">{ic}</div>'
                    f'<h3><a href="{html.escape(folder)}/">{html.escape(title)}</a></h3>'
            ...
            if not cards:
                continue
            track_blocks.append(
                f'<section class="learn-track" data-track="{html.escape(track_id)}">'
                f'<div class="learn-track-head"><h2>{html.escape(icon + " " + U(label_key))}</h2></div>'
                f'<div class="learn-grid">{"".join(cards)}</div></section>'
            )

        eyebrow = _fmt(U("course_hub_eyebrow"), n=n)
        tracks_label = _fmt(U("course_hub_tracks"), n=n)
        meta_desc = _fmt(U("course_hub_meta_desc"), n=n)
        hub_body = (
            '<header class="learn-hero-stage">'
            '<canvas id="learn-galaxy" aria-hidden="true"></canvas>'
            '<div class="learn-nebula" aria-hidden="true"></div>'
            f'<div class="wrap hero"><p class="eyebrow"><span class="pip" aria-hidden="true"></span>'
            ...
            f'{html.escape(U("course_hub_school_clips"))}</a></div>'
            f'<div class="learn-meta"><span><b>{n}</b> {html.escape(U("course_hub_stat_academies"))}</span>'
            f'<span><b>4</b> {html.escape(U("course_hub_stat_tracks"))}</span>'
            f'<span><b>5</b> {html.escape(U("course_hub_stat_langs"))}</span></div></div>'
            "</header>"
            f'<div class="wrap section"><h2>{html.escape(tracks_label)}</h2>'
            f'<div class="learn-filter" role="group" aria-label="Filter by track">'
            f'<button type="button" class="active" data-filter="all">{html.escape(U("course_hub_filter_all"))}</button>'
            f'<button type="button" data-filter="builder">{html.escape(U("course_hub_filter_builder"))}</button>'
            f'<button type="button" data-filter="security">{html.escape(U("course_hub_filter_security"))}</button>'
            f'<button type="button" data-filter="physical">{html.escape(U("course_hub_filter_physical"))}</button>'
            f'<button type="button" data-filter="proofs">{html.escape(U("course_hub_filter_proofs"))}</button>'
            f"</div>{''.join(track_blocks)}</div>"
        )
        fonts = (
            '<link href="https://fonts.googleapis.com/css2?family=Orbitron:wght@500;700;800'
            '&family=Rajdhani:wght@500;600;700&display=swap" rel="stylesheet" />'
        )
        script = f'<script src="{html.escape(base)}/shared/learn.js?v=3" defer></script>\n'
        hub = _page_shell(
            base=base,
            path="/learn/",
            lang=lang,
...
def _build_encyclopedia(base: str, cfg: dict[str, Any]) -> list[tuple[str, str]]:
    ...
    missing = [rel for rel in titles if rel != "index.html" and not (dest / rel).is_file()]
    if missing:
        raise SystemExit(
            "encyclopedia locales missing after copy from docs/encyclopedia/: "
            + ", ".join(missing)
            + " — run: node docs/encyclopedia/scripts/generate-encyclopedia.mjs all"
        )
...
def _copy_shared_assets() -> None:
    ...
    learn_js = SEO_ROOT / "shared" / "learn.js"
    if learn_js.is_file():
        shutil.copy2(learn_js, dest / "learn.js")
