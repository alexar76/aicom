...
Typical automation:
  ...
  - ``ALIEN_PUBLIC_READ=1`` — same host: public Alien Monitor graph at ``/monitor/`` (no Bearer token in browser).
...
def main() -> int:
    ...
    if public_url:
        ...
        if "magic-ai-factory.com" in host and "ALIEN_PUBLIC_READ" not in keys:
            additions.append(("ALIEN_PUBLIC_READ", "1"))
    ...
    if "POSTGRES_PASSWORD" not in keys:
        additions.append(("POSTGRES_PASSWORD", secrets.token_urlsafe(24)))
    if "REDIS_PASSWORD" not in keys:
        additions.append(("REDIS_PASSWORD", secrets.token_urlsafe(24)))
