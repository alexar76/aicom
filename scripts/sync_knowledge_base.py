...
FILE_MIRRORS = (
    ...
    # The monitor's in-container fallback map. Hand-copied until it drifted three satellites
    # behind (DOLOS shipped, and the assistant inside the container had never heard of it).
    ("scripts/satellite-map.yaml", "alien-monitor/scripts/satellite-map.yaml"),
)

MIRROR_BLOB_BASE = "https://github.com/alexar76/aicom/blob/main/"

_GENERATED_HEADER = (
    "# GENERATED COPY — do not hand-edit. `scripts/sync_knowledge_base.py --write` mirrors\n"
    "# scripts/satellite-map.yaml here, minus satellites that are not published on GitHub.\n"
)

# `  - id: <name>` opens a satellite entry; its body is indented deeper.
_SAT_ENTRY = re.compile(r"^  - id: (\S+)\s*$")


def satellite_map_mirror_text(src_rel: str, text: str) -> str:
    """The monitor's copy of the map: the source verbatim, minus what GitHub does not have.

    Same rule as the knowledge bases — a component the reader cannot open is worse than one
    they never heard of, because the agent then hands a stranger a dead link. `github_published:
    false` entries are therefore dropped here too, while every published one is copied through
    unchanged (comments and all), so this file can never lag the central map again.
    """
    unpublished = {
        s.get("id")
        for s in (yaml.safe_load(text) or {}).get("satellites", [])
        if isinstance(s, dict) and s.get("github_published") is False
    }
    out: list[str] = [_GENERATED_HEADER]
    skipping = False
    for line in text.splitlines(keepends=True):
        m = _SAT_ENTRY.match(line)
        if m:
            skipping = m.group(1) in unpublished
        elif skipping and line.strip() and not line.startswith("    "):
            skipping = False  # dedented out of the entry body
        if not skipping:
            out.append(line)
    return "".join(out)


# Per-source mirror transform; anything unlisted is a markdown file whose relative links are
# absolutised for the destination repo.
MIRROR_TRANSFORMS = {
    "scripts/satellite-map.yaml": satellite_map_mirror_text,
}

DEFAULT_MIRROR_NOTE = "relative links absolutised"
MIRROR_NOTES = {
    "scripts/satellite-map.yaml": "satellites not published on GitHub dropped",
}


def mirror_for(src_rel: str):
    return MIRROR_TRANSFORMS.get(src_rel, mirror_text)
...
def run(mode: str) -> int:
    ...
    for src, dst in FILE_MIRRORS:
        ...
        if not src_p.is_file() or not dst_p.is_file():
            ...
            worst = max(worst, 2)
            continue
        want = mirror_for(src)(src, src_p.read_text(encoding="utf-8"))
        if dst_p.read_text(encoding="utf-8") == want:
            print(f"  mirror ok {dst}")
            continue
        if mode == "write":
            dst_p.write_text(want, encoding="utf-8")
            print(f"  mirrored  {src} → {dst}  ({MIRROR_NOTES.get(src, DEFAULT_MIRROR_NOTE)})")
        else:
