...
# ── Pre-flight: model ids ────────────────────────────────────────────────────
# One definition of the rule, in scripts/lib/common.sh, shared with start.sh — this
# repo has been bitten before by a rule living on two surfaces with only one fixed.
# Asks each provider with a one-token call; a dead id blocks, an unreachable
# provider does not. AIFACTORY_SKIP_MODEL_ID_GATE=1 ships anyway.
if [[ -f "$ROOT/scripts/lib/common.sh" ]]; then
  # shellcheck source=scripts/lib/common.sh
  . "$ROOT/scripts/lib/common.sh"
  echo "--- Pre-flight: model ids ---"
  model_id_gate "$ROOT"
  echo ""
fi
...
echo "  Monitor LIVE: https://monitor.modelmarket.dev/  · UNI: https://monitor-uni.modelmarket.dev/ (factory /monitor* → redirect)"
