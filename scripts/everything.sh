...
# Monitor, Pulse, Landing, GAIA, ATLAS, THEMIS, BASANOS, the oracle family, Platon, MOMUS,
...
themis|THEMIS admission|9420|/health|240|http
...
basanos|BASANOS touchstone|9470|/health|240|http
...
logos|LOGOS analytics|9460|/health|240|http
...
if [[ "$BIND_ADDR" != "127.0.0.1" && "$BIND_ADDR" != "localhost" ]]; then
  ...
  say "${R}${B}╔══════════════════════════════════════════════════════════════════════════╗${N}"
  printf '%s║%-74s║%s\n' "${R}${B}" "  PUBLISHING 34 PORTS ON ${BIND_ADDR} - 40 CONTAINERS" "$N"
...
say "${DIM}Three prod hosts on one box · ~42 containers · no edge, no TLS, IP:PORT${N}"
...
# Pin the THEMIS provider pubkey before Hub starts — admission is advisory by
# default and needs AIMARKET_SUPPLY_CHAIN_AUDITOR_PUBKEY in .env. The key lives
# in the themis_data volume (same path prod uses) so redeploys keep identity.
bootstrap_themis_auditor_pubkey() {
  if env_has AIMARKET_SUPPLY_CHAIN_AUDITOR_PUBKEY; then
    return 0
  fi
  step "THEMIS auditor key (Hub admission pin)"
  local img="themis:${AICOM_IMAGE_TAG:-local}"
  if [[ "$BUILD" -eq 1 ]]; then
    "${COMPOSE[@]}" build themis >/dev/null
  fi
  local vol="${PROJECT}_themis_data"
  local pub
  pub="$(docker run --rm -v "${vol}:/data" "$img" \
    python -c "from provider_signing import ProviderSigner; print(ProviderSigner().public_key_b64)" 2>/dev/null || true)"
  [[ -n "$pub" ]] || die "Could not read THEMIS provider pubkey — run without --no-build once, or set AIMARKET_SUPPLY_CHAIN_AUDITOR_PUBKEY in .env"
  ensure_secret AIMARKET_SUPPLY_CHAIN_AUDITOR_PUBKEY "$pub"
  if ! env_has AIMARKET_SUPPLY_CHAIN_ADMISSION_MODE; then
    printf 'AIMARKET_SUPPLY_CHAIN_ADMISSION_MODE=advisory\n' >> .env
    say "  ${DIM}set AIMARKET_SUPPLY_CHAIN_ADMISSION_MODE=advisory${N}"
  fi
...
if [[ "$BUILD" -eq 1 ]]; then
  step "Starting ~42 containers (building — a first run is 30-60 min on 8 cores)"
...
say "    THEMIS admission ${C}${BASE}:9420${N}      Mesh dashboard ${C}${BASE}:8091${N}"
say "    Landing          ${C}${BASE}:3847${N}"
say "    Prometheus       ${C}${BASE}:9090/prometheus/${N}   Grafana ${C}${BASE}:9082${N}"
say ""
say "  ${B}SATELLITES${N}"
say "    GAIA   ${C}${BASE}:9320${N} / ${C}:5185${N}      ATLAS    ${C}${BASE}:9330${N}"
say "    BASANOS ${C}${BASE}:9470${N}              Oracles ${C}${BASE}:9400${N} / ${C}:5180${N}"
say "    CHRONOS ${C}${BASE}:9300${N}              Platon ${C}${BASE}:9200${N} / ${C}:9201${N}"
say "    Metis    ${C}${BASE}:9111${N}              LOGOS    ${C}${BASE}:9460${N}"
...
say "  ${DIM}  (prod's own precedents), plus platon-frontend 8080→9201, Metis 8080→9111,${N}"
say "  ${DIM}  and THEMIS console 8080→9420 on the Hub netns mapping.${N}"
say ""
