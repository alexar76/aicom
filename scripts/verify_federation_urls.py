...
def main() -> int:
    ...
    for seed in load_seeds(args.seeds):
        ...
        advertised = int(wk.get("capabilities_count") or 0)
        check(
            advertised == count or count == 0,
            "seed's advertised count matches its manifest",
            f".well-known says {advertised}, manifest lists {count} — the hub writes the peer row from "
