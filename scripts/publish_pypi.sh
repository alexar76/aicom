...
for archive in dist/*.tar.gz dist/*.whl; do
  ...
  bad="$(printf '%s\n' "$listing" | grep -Ei '(^|/)(data)/|_signing_key|signing_key$|(^|/)\.env($|\.)|\.pem$|\.key$|(^|/)id_rsa|\.sqlite3?$' || true)"
  if [[ -n "$bad" ]]; then
    gate_hits+="  $archive:"$'\n'"$(printf '%s\n' "$bad" | sed 's/^/    /')"$'\n'
  fi
  ...
  if [[ "$archive" == *.tar.gz ]] && printf '%s\n' "$listing" | grep -qE '(^|/)\.gitignore$'; then
    shipped="$(tar -xzOf "$archive" --include='*/.gitignore' 2>/dev/null || true)"
    own=""
