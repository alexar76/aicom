...
# Monorepo path when the PyPI project name differs from the folder (gaia/ → aimarket-gaia-gateway).
resolve_package_dir() {
  local arg="$1"
  if [[ -f "$ROOT/$arg/pyproject.toml" ]]; then
    PKG="$arg"
    return 0
  fi
  case "$arg" in
...
[[ -n "$PKG" ]] || { usage; exit 1; }
PKG_ARG="$PKG"
if ! resolve_package_dir "$PKG"; then
  echo "error: no pyproject.toml for package '$PKG_ARG' (path or PyPI project name)" >&2
  exit 2
fi
...
if [[ "$PKG" != "$PKG_ARG" ]]; then
  echo "Resolved '$PKG_ARG' → monorepo path $PKG/"
fi
