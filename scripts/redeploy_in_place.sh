...
#
# Capture-and-replay is faithful, and that is also its failure mode: a variable that got
# into a container once stays there forever, through every rebuild, whether or not the
# service reads it. `alien-monitor` was carrying 11 secrets it never mentions anywhere in
# its source — ATLAS_SIGNING_SEED_B64, MESH_ADMIN_TOKEN, POSTGRES_PASSWORD and friends,
# inherited from an `env_file: ../.env` that no longer exists in the compose file. So the
# capture consults a per-container drop list checked into the repo, next to this script,
# where it can be reviewed: scripts/redeploy-drop/<container>.txt, one NAME per line.
umask 077
export ENV_FILE
EXTRA_ENV="/root/.${NAME}.extra.env"
DROP_LIST="$ROOT/scripts/redeploy-drop/${NAME}.txt"
export DROP_LIST
...
drop_path = os.environ.get("DROP_LIST", "")
drop = set()
if drop_path and os.path.isfile(drop_path):
...
with open(env_path, "w", encoding="utf-8") as fh:
    for entry in spec["Config"]["Env"]:
        ...
        if name not in skip:
            fh.write(entry + "\n")
if dropped:
    print("dropped   : %d variable(s) per %s — %s" % (
        len(dropped), os.path.basename(drop_path), ", ".join(sorted(dropped))))
