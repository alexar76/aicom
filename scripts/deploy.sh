...
if [[ "$NO_BUILD" -eq 0 ]]; then
  ...
  echo "deploy.sh: image tag $AICOM_IMAGE_TAG"
  # bash 3.2 + set -u: empty "${arr[@]}" is unbound — use ${arr[@]+…}
  docker compose ${PASS_THROUGH[@]+"${PASS_THROUGH[@]}"} "${COMPOSE_FILES[@]}" build --pull "${COMPOSE_SERVICES[@]}"
else
  echo "deploy.sh: skipping docker compose build (--no-build)"
fi

docker compose ${PASS_THROUGH[@]+"${PASS_THROUGH[@]}"} "${COMPOSE_FILES[@]}" up -d "${COMPOSE_SERVICES[@]}"

# Post-up: LLM + sandbox previews on bind-mounted data (idempotent)
docker compose ${PASS_THROUGH[@]+"${PASS_THROUGH[@]}"} "${COMPOSE_FILES[@]}" exec -T app \
