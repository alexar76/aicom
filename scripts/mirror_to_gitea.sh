...
m = yaml.safe_load(_map_path.read_text(encoding="utf-8")) or {}
_targets_path = Path(os.environ["AICOM_ROOT"]) / "scripts" / "gitea-targets.yaml"
# Absence is legitimate here — gitea-targets.yaml is mirror-excluded, so a trimmed
# checkout has none. Anchored, but not required: `or {}` is the intended fallback.
g = (yaml.safe_load(_targets_path.read_text(encoding="utf-8")) if _targets_path.is_file() else None) or {}
