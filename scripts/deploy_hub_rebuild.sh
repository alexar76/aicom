...
# An environment-only rollout — a changed address, a corrected seller list — has no reason
# to rebuild. Rebuilding it anyway is a real risk on this host: the build tree is rsynced,
# not a checkout, so `docker build` can quietly ship code that never went through the
...
for entry in json.load(sys.stdin) or []:
    ...
    if "\n" in value or "\r" in value:
        ...
        continue
    kept.append(key)
    # Last assignment wins, because that is what the container process sees: docker passes
    # duplicates through in order and the C library keeps the final one. Emitting them all
    # would round-trip the ambiguity into the next container, and reading the FIRST one — as
    # this script and tests/test_hub_payment_env.py both did until 2026-08-24 — reports a
    # value the hub is not using. That is how AIMARKET_SELLS_FOR came to list atlas on a
    # line that eleven later lines overrode, and atlas was served free while every check
    # that looked at the first line passed.
    last[key] = entry
for key, entry in last.items():
    print(entry)
dupes = sorted({k for k in kept if kept.count(k) > 1})
if dupes:
    sys.stderr.write("collapsed duplicate assignments (kept the LAST of each): %s\n" % ", ".join(dupes))
if bad:
...
# ── Apply --set overrides ───────────────────────────────────────────────────────
# ${a[@]+"${a[@]}"} — expands to nothing when empty (set -u safe) and keeps each element
# intact when not; a bare ${a[@]} would word-split a value containing a space.
for kv in ${OVERRIDES[@]+"${OVERRIDES[@]}"}; do
  key="${kv%%=*}"; value="${kv#*=}"
  [[ "$key" =~ ^[A-Za-z_][A-Za-z0-9_]*$ ]] || die "--set: '$key' is not a variable name"
  [[ "$kv" == *=* ]] || die "--set: '$kv' is not KEY=VALUE"
  before="$(grep -E "^${key}=" "$ENV_CAPTURE" | tail -1 | cut -d= -f2- || true)"
  grep -vE "^${key}=" "$ENV_CAPTURE" > "$ENV_CAPTURE.tmp" || true
  printf '%s=%s\n' "$key" "$value" >> "$ENV_CAPTURE.tmp"
  mv "$ENV_CAPTURE.tmp" "$ENV_CAPTURE"; chmod 600 "$ENV_CAPTURE"
  # Names, not values: an override can legitimately carry a secret.
  if [[ -n "$before" ]]; then log "override $key (was set, changed)"; else log "override $key (was unset)"; fi
done

# ── The studio's run path ───────────────────────────────────────────────────────
# `/studio/run` forwards a blueprint to the pipeline executor, and its destination comes
# from the hub's own environment — never from the request. Once the variable is in the
# container, the capture above carries it forward on every later redeploy; this branch
# exists only for the deploy that introduces it. Unset stays unset: the route then refuses
# with 503 and names the variable, which is the correct behaviour for a hub with no
# executor, not something to paper over with a guessed localhost port.
if [[ -n "${AIMARKET_PIPELINE_EXECUTOR_URL:-}" ]]; then
  grep -v '^AIMARKET_PIPELINE_EXECUTOR_URL=' "$ENV_CAPTURE" > "$ENV_CAPTURE.tmp" || true
  echo "AIMARKET_PIPELINE_EXECUTOR_URL=${AIMARKET_PIPELINE_EXECUTOR_URL}" >> "$ENV_CAPTURE.tmp"
  mv "$ENV_CAPTURE.tmp" "$ENV_CAPTURE"
  chmod 600 "$ENV_CAPTURE"
  log "studio run target: ${AIMARKET_PIPELINE_EXECUTOR_URL}"
elif grep -q '^AIMARKET_PIPELINE_EXECUTOR_URL=.\+' "$ENV_CAPTURE"; then
  log "studio run target carried over from the running container"
else
  log "no studio run target configured — /studio/run will refuse with 503 (page still works)"
fi
...
# ── 2. Build (or reuse the live image) ──────────────────────────────────────────
if (( NO_BUILD )); then
  # An explicit image wins. This is how a single-file fix ships without a full rebuild:
  # derive an image FROM the running one (`FROM <live>` + `COPY one/file`), which is the only
  # way to be sure you shipped "exactly what is live, plus this change" — on this host the
  # build tree's api.py does NOT match the running image (verified 2026-08-24), so building
  # from it would silently mix in whatever else drifted.
  if [[ -n "${AIMARKET_HUB_IMAGE:-}" ]]; then
    IMAGE="$AIMARKET_HUB_IMAGE"
    docker image inspect "$IMAGE" >/dev/null 2>&1 || die "image $IMAGE not found locally"
    log "--no-build: deploying the explicit image $IMAGE"
  else
    IMAGE="$(docker inspect "$NAME" --format '{{.Config.Image}}')"
    [[ -n "$IMAGE" ]] || die "could not read the running container's image for --no-build"
    log "--no-build: reusing the live image $IMAGE (environment-only rollout)"
  fi
else
...
# The capture is deduped above, so there is exactly one assignment per key and this reads
# the value the hub will actually run with. `head -1` on a raw capture read the FIRST of
# several and reported a list the container had overridden.
SELLS_FOR="${AIMARKET_SELLS_FOR:-$(grep -E '^AIMARKET_SELLS_FOR=' "$ENV_CAPTURE" | tail -1 | cut -d= -f2-)}"
...
# The studio is built by a Node stage in the image, so a broken frontend build is
# invisible until someone opens the page. `503` is the honest "bundle missing" answer the
# route serves on purpose, so accept only a real page.
studio_code="$(curl -s -o /tmp/studio-check.html -w '%{http_code}' "http://127.0.0.1:${PORT}/studio/")"
if [[ "$studio_code" != "200" ]] || ! grep -q 'HEPHAESTUS' /tmp/studio-check.html; then
...
# THEMIS lives in this container's network namespace (the Hub's SSRF guard exempts loopback
# and nothing else), so a swap leaves it pinned to a container ID that no longer exists: it
# keeps reporting healthy and stops being reachable. Found in exactly that state on
# 2026-08-24, degraded since the previous swap. Re-home it here or the gate rots silently.
if docker inspect "${THEMIS_CONTAINER:-themis}" >/dev/null 2>&1; then
  log "Re-homing THEMIS into the new container's namespace"
  if ! "$(dirname "$0")/attach_themis_to_hub.sh"; then
    echo "WARN: THEMIS could not be re-homed — supply-chain admission is degraded until it
     is. The Hub itself is healthy; fix with scripts/attach_themis_to_hub.sh" >&2
  fi
fi
