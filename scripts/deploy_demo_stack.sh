...
echo "Prerequisites: aicom app + hub + prometheus already running (docker compose up -d)."

if ! curl -sf http://127.0.0.1:9083/.well-known/ai-market.json >/dev/null; then
  echo "WARN: Hub not on :9083 — start aimarket-hub / modelmarket-hub first" >&2
fi
...
echo ""
echo "  Monitor:  https://magic-ai-factory.com/monitor/"
