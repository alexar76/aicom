...
tool's `description` and against the serialised `inputSchema`. Record `{code, severity, tool,
where}` per match.
...
- Every entry **MUST** carry the matched rule's `tier`, and a consumer **MUST NOT** present an
  ...
  `version` and `digest` alongside each rule's tier. Regenerate it rather than editing it.
...
matches on ordinary benign definitions. Executed against `argus/dist/warden/static-scan.js` over
...
property, and a `list_files` tool whose description contains the words "instead of":

| Tool set | Matches | Gate score |
|---|---|---|
| the two benign tools above | **3** — `TOOL_DEF_SECRET_REQUEST` (`high`, description), `TOOL_DEF_SECRET_REQUEST` (`high`, input schema), `TOOL_DEF_INJECTION` (`low`, description) | **0.40** |
| one tool, neutral description, `api_key` property only | 1 — `TOOL_DEF_SECRET_REQUEST` (`high`) | 0.4 |
| one tool, "instead of" in the description only | 1 — `TOOL_DEF_INJECTION` (`low`) | 0.9 |
| two tools, plain weather/city definitions | 0 | 1 |

At that implementation's default blocking threshold (`blockAtSeverity: "high"`) the first row is
refused outright. Neither tool is malicious. **An `api_key` property alone is sufficient**, as row
two shows measured rather than asserted: any server whose schema legitimately accepts an API key
trips this gate at severity `high`.

A pattern match is therefore a reason to read the definition, not a finding of fact, and the
only honest non-`pass` outcome is `inconclusive`. SPEC §3.4's requirement that `inconclusive`
not be treated as a failure is what makes this expressible.
...
records (counted in `argus/src/warden/threat-feed.ts`), three are typosquat patterns over names,
...
### 8.1 Network reputation score — excluded: it never executes

The reputation gate asks the LUMEN trust oracle for the server's score by calling
...
any network call. `grep -rn scoreEntity argus/src` returns four matches — an interface
declaration, the definition, and **two** call sites (`cli/commands/passport.ts:22`,
`warden/reputation.ts:28`) — and neither call site passes edges. The early return
precedes the `fetch`, so the oracle invocation path — including the graph commitment it would
read — is unreachable in production, and the gate's own message that the oracle is "unreachable"
is false: nothing was attempted.
...
The allow/block outcome is a function of the caller's configured blocking threshold, not of the
server: at the default threshold the benign two-tool server of §7.3 is blocked. It is a local
policy decision, correct to make locally and meaningless to publish about a server. **MUST
...
re-verified at 2026-07-31 19:07 UTC against the current bytes (`valid=true`, `profile=L0`, 0
reasons, 0 warnings, exit 0 each), and `awr digest examples/pass-01-subject-descriptor.json`
reproduces
...
2. **The pattern matches are transcribed, not produced.** `generate.py` hardcodes
   `PATTERN_HIT_FINDINGS` as literals; the generator itself never executes the gate. Separately,
   `pattern-set-argus-warden-static-scan.json` is a **hand transcription** of
   `argus/src/warden/static-scan.ts` — 22 patterns and 2 heuristics, counts re-verified on both
   sides while revising this document. §7.3 makes the pattern-set digest the thing that makes a
   scan label falsifiable, so a digest over a hand copy binds the label to the copy, not to the
   code that ran.

   `examples/test_examples.py` now closes the second half of that gap: it re-extracts all 22
   patterns and both heuristics from the TypeScript and requires the JSON to match source and
   flags, and it compares the hardcoded `patternMatches` against a live run of
   `argus/dist/warden/static-scan.js`. Executed while revising this document: **18 passed.** Note
   the residual limits — the drift test skips visibly when node or the ARGUS build is absent, and
   `generate.py` still emits literals rather than gate output, so the *generator* remains a
   fixture rather than an issuer.

The three matches the examples record *were* reproduced against the shipping gate while revising
this document (§7.3's table), so the numbers are right.

| File | Method | Verdict |
|---|---|---|
| `pass-01-pattern-scan.awr.json` | `tool-def-pattern-scan` | `pass` — zero patterns matched |
| `inconclusive-01-pattern-scan.awr.json` | `tool-def-pattern-scan` | `inconclusive` — three matches, all on benign text |
...
| Claim | Source |
|---|---|
| Gate chain and its order | `argus/src/warden/index.ts` — `Warden.create` |
| Composite score is the product of gate scores | `argus/src/warden/index.ts` — `vet` |
| Block decision is threshold-driven; default threshold `high` | `argus/src/warden/index.ts`, `argus/src/config.ts` |
| 22 patterns in four groups plus two heuristics; scans `description` and `inputSchema` only | `argus/src/warden/static-scan.ts` |
| Gate score is `1 −` penalty for the worst severity, i.e. one of {1, 0.9, 0.7, 0.4, 0} | `argus/src/warden/static-scan.ts` |
| **Executed:** benign two-tool server → 3 matches (2 `high`, 1 `low`), gate score 0.40 | `argus/dist/warden/static-scan.js` |
| **Executed:** clean two-tool server → 0 matches, gate score 1 | `argus/dist/warden/static-scan.js` |
| **Executed:** one tool with a neutral description and an `api_key` property → 1 match (`TOOL_DEF_SECRET_REQUEST`, `high`), gate score 0.4 | `argus/dist/warden/static-scan.js` |
| **Executed:** one tool whose description contains "instead of" → 1 match (`TOOL_DEF_INJECTION`, `low`), gate score 0.9 | `argus/dist/warden/static-scan.js` |
| Pinning hash sorts with `localeCompare` and serialises with `JSON.stringify` | `argus/src/warden/pinning.ts` — `canonicalToolsHash` |
| **Executed:** `"Z_tool".localeCompare("a_tool") === 1` vs code-unit `−1`; `résumé`/`rezume` inversion | Node in this environment |
| Reputation gate calls `scoreEntity(id)` with no edges | `argus/src/warden/reputation.ts` |
...
| Degraded gate contributes 0.6 permissive / fatal strict; unpinned contributes 0.9 → ceiling 0.540 | `argus/src/warden/reputation.ts`, `argus/src/warden/pinning.ts` |
| Threat haystack is `id`/`name`/`url`/`command`/`args`; tool defs never examined | `argus/src/warden/threat-feed.ts` — `match` |
| 11 built-in records: 3 name typosquats; 3 wallet/crypto keyword, of which **two** carry the reason string "Crypto-drainer keyword in server identity" and one carries "Server references wallet seed phrases"; 5 command-line | `argus/src/warden/threat-feed.ts` — `BUILTIN` (`grep -n 'reason:'` → 11 records) |
| `timestamp` is parsed and never used; no staleness check exists | `argus/src/warden/threat-feed.ts` |
| Feed signature is over `JSON.stringify` of parsed wire JSON | `argus/src/warden/threat-feed.ts` — `load` |
| No default feed URL or feed public key ships | `argus/src/config.ts` |
| No model is used by any warden gate | `argus/src/warden/*.ts` |
| Host posture scoring has MCP servers nowhere in its subject | `skopos/security/posture.py` |
| **Executed:** a standalone label verifies `valid: true`, `profile: "L0"` | `awr/reference/python`, `awr verify` |
...
| **Executed:** all four shipped labels → `valid=true`, `profile=L0`, 0 reasons, 0 warnings, exit 0 | `awr verify examples/*.awr.json` |
...
| 22 patterns + 2 heuristics in `pattern-set-argus-warden-static-scan.json` match the 22 regex literals in 4 groups + `BASE64_BLOB`/`HIDDEN_UNICODE` in the source — counted on both sides, **and now asserted by a drift test** (Appendix A, item 2) | `argus/src/warden/static-scan.ts`, `examples/pattern-set-argus-warden-static-scan.json` |
| **Executed:** `pytest examples/test_examples.py -q` → 18 passed | `examples/test_examples.py` |
