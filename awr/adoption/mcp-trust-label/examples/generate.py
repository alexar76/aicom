...
#: A server whose *benign* schema trips the pattern set: an ``api_key`` property, a mention of
#: a personal access token, and the phrase "instead of".  Under the shipping ARGUS gate
#: (ruleset v3) all three matches are ``advise``-tier, so the gate score stays 1.0 and the
#: connection is refused at no ``blockAtSeverity`` setting.  Ruleset v1 scored exactly this
#: tool set 0.40 and blocked it at the default threshold, which is the calibration bug v2
#: fixed -- and the reason MTL/1 never renders a pattern match as ``fail``.
...
# warden/dist/static-scan.js over exactly these tool definitions with node and
...
#
# ``tier`` is REQUIRED by PROFILE 7.3 on every match entry. The gate reports the tier as
# ``advisory: true`` on the finding and as ``tier: "advise"`` in the rule table; the label
# carries the table's spelling, because that is the value a reader can look up in the
# published pattern set.
# ---------------------------------------------------------------------------

PATTERN_HIT_FINDINGS = [
    {"code": "TOOL_DEF_CREDENTIAL_PARAM", "severity": "low", "tier": "advise",
     "tool": "create_issue", "where": "description"},
    {"code": "TOOL_DEF_CREDENTIAL_PARAM", "severity": "low", "tier": "advise",
     "tool": "create_issue", "where": "inputSchema"},
    {"code": "TOOL_DEF_IMPERATIVE", "severity": "info", "tier": "advise",
     "tool": "list_files", "where": "description"},
]

#: What the same run reports for the gate's own composite contribution. Recorded next to the
#: findings because the two numbers are the whole argument of PROFILE 7.3: three matches, and
#: nothing blocked.
PATTERN_HIT_GATE_SCORE = 1.0
...
def pattern_set_digest() -> str:
    ...
    The table is GENERATED from the shipping gate (``staticScanRuleset()`` in
    ``@aimarket/warden``), not transcribed -- regenerate it with
    ``tools/regen_pattern_set.mjs``.  The digest is taken over the whole published file, so a
    label naming this pattern set names these exact 25 rules with their tiers; two scan labels
    are comparable only when this value is equal.
    """
    ...
    inconclusive = label(
        ...
        detail={
            ...
            "note": (
                ...
                "an API key and documents a personal access token, and a description "
                "containing the words 'instead of'. All three are advise-tier, so the "
                "scanner's own gate score for this server is 1.0 and it refuses the "
                "connection at no threshold. MTL/1 therefore reports inconclusive and never "
