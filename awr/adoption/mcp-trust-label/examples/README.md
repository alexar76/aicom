...
re-run on 2026-08-24 — after the `demonstration` marker below was added — with
`python -m awr verify` on each file, reading `valid`, `profile`, `len(reasons)` and
`len(warnings)` out of the JSON result:

```
exit 0  pass-01-pattern-scan.awr.json              valid=True profile=None reasons=0 warnings=0
exit 0  inconclusive-01-pattern-scan.awr.json      valid=True profile=None reasons=0 warnings=0
exit 0  pass-02-tool-set-observation.awr.json      valid=True profile=None reasons=0 warnings=0
exit 0  pass-03-tool-set-continuity.awr.json       valid=True profile=None reasons=0 warnings=0
```

`profile: null` is the correct result, not a downgrade: SPEC §10.4 settles that the profile of a
document which is not a `WorkReceipt` is null, because L1/L2 are levels of assurance *about a unit
of work* and a `VerificationVerdict` is not one. `profilesEvaluated` says so explicitly, with
`AWR-PROFILE-001` under both L1 and L2.
...
- the hardcoded `patternMatches` are compared against a live run of
  `warden/dist/static-scan.js` over exactly these tool definitions (three matches, all
  `advise`-tier, gate score `1.0`); the test skips, visibly, if node or that build is absent;
- `pattern-set-argus-warden-static-scan.json` is **generated** from the shipping gate by
  `tools/regen_pattern_set.mjs` calling `staticScanRuleset()`, so the drift check is exact:
  regenerate and compare bytes. A digest over a hand copy would bind to the copy rather than to
  the code that ran, which is why the transcription this file used to be was replaced. Two
  cheaper checks run without a build: the rule *codes* in the table must be exactly those in
  `warden/src/static-scan.ts`, and the table's own `digest` must be the gate's
  `rulesets.staticScan.digest`.
...
`valid=True profile=None reasons=0 warnings=0` — the table above is the run *with* the marker
...
| `inconclusive-01-pattern-scan.awr.json` | **inconclusive** label — three matches, all `advise`-tier, all on benign text |
| `pattern-set-argus-warden-static-scan.json` | The pattern table, digested so labels are comparable (`PROFILE.md` §7.3). **Generated** from the gate by `../tools/regen_pattern_set.mjs`; a drift test regenerates it and compares bytes |
...
| `test_examples.py` | Fails unless the demonstration marker is present and costs no warning, the generator's DID is declared in `own-keys.txt`, the quoted digests are stable, and the pattern table and the hardcoded findings (tiers included) still match the shipping gate. 21 tests, all passing as of 2026-08-24 |
...
ARGUS static-scan gate (`warden/dist/static-scan.js`) over exactly these definitions —
re-run 2026-08-24 — produces:

| Match | Severity | Tier | Where |
|---|---|---|---|
| `TOOL_DEF_CREDENTIAL_PARAM` | `low` | `advise` | `create_issue` description |
| `TOOL_DEF_CREDENTIAL_PARAM` | `low` | `advise` | `create_issue` input schema |
| `TOOL_DEF_IMPERATIVE` | `info` | `advise` | `list_files` description |

Three matches, gate score **1.0**, and the connection is refused at **no** `blockAtSeverity`
setting: every match is `advise`-tier, and an advisory hit never blocks and never affects the
score. Isolated in the same run, each of the three tells on its own — the `api_key` property, the
"personal access token" phrase, the words "instead of" — produces exactly one `advise` match and
leaves the score at 1.0. The clean two-tool server in `pass-01` produces 0 matches and a score
of 1.

Ruleset **v1** scored this same tool set `0.40` and blocked it outright at the default threshold,
with two of the matches at `high`. That was the calibration bug v2 fixed by giving every rule a
tier, and it is the reason the numbers above are re-measured here rather than quoted: a document
that still claimed 0.40 would be describing a scanner nobody runs.

Neither tool is malicious. This is why `PROFILE.md` §7.3 forbids `fail` for the pattern-scan
method and requires `inconclusive` instead, why every match entry must carry its tier, and why
§9.2 requires registries to render `inconclusive` as neutral rather than as failure.
