...
| Only `inconclusive` labels for a popular server | normal | An `api_key` property alone does it: executed here against `argus/dist/warden/static-scan.js`, a single tool with a neutral description and an `api_key` property produces one `TOOL_DEF_SECRET_REQUEST` match at severity `high` and a gate score of 0.4. Not a red flag |
