...
def test_counts_by_document_type_and_profile(verifier):
    ...
    # differs from the receipt's (SPEC.md 10.2). The plain receipt is L0. The verdict and the
    # blame attestation are NOT receipts, so SPEC.md 10.4 gives them a null profile, which the
    # report buckets as "none" -- valid, counted, and not a level of assurance about a unit of
    # work. Counting them as L0 (as this assertion once did) claimed a level they never had.
    assert profiles == {"L0": 1, "L1": 1, "none": 2}
...
def test_demo_corpus_reproduces_the_readme_transcript(demo, verifier):
    ...
    # and neither does having none: the two foreign verdicts are not receipts, so SPEC 10.4
    # gives them a null profile and the report buckets them as "none".
    assert report["context"]["byProfile"]["foreign"] == {"L0": 40, "none": 2, "L2": 1}
    assert report["context"]["byProfile"]["all"] == {"L0": 41, "none": 3, "L1": 1, "L2": 1}
