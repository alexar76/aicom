...
def _mandated_charter_sections(charter: str) -> str:
    """The charter sections the operator marked as requirements, repeated verbatim.

    Same marker the charter-fidelity gate keys on, so the prompt asks for exactly what the
    gate will check. A gate demanding something the prompt never mentioned is just a wall.
    """
    try:
        from core.charter_fidelity import marked_sections
    except ImportError:
        return ""
    parts = [f"--- {title} ---\n{body.strip()}" for title, body in marked_sections(charter)]
    return "\n\n".join(parts)
...
class PMAgent(BaseAgent):
    ...
    async def execute(self, agent_input: AgentInput) -> AgentOutput:
        ...
        research_file = (Path(self.data_root) / "state" / pid / "market_research.json").resolve()
        state_root = (Path(self.data_root) / "state").resolve()
        if state_root not in research_file.parents and research_file != state_root:
            raise ValueError(f"product_id escapes state root: {product_id!r}")
        ...
        try:
            ...
            for attempt in range(max_rounds):
                ...
                # The charter itself, verbatim. It used to reach the model ONLY through
                # compile_product_brief, which distils any charter — 9 000 characters of
                # engineering direction in the case that exposed this — down to a `domain`
                # matched from a seven-word list, a few keyword flags, and the first six
                # sentences over 20 characters. Everything past those sentences was discarded
                # before generation: an operator appended two explicitly marked requirements
                # and the PM stage dropped both, five attempts running, while feedback named
                # the exact identifiers to include. It was not ignoring them — it never saw
                # them. The charter is the operator's only lever on a product; it goes in whole.
                if admin:
                    prompt += (
                        "\n=== OPERATOR CHARTER (verbatim, authoritative) ===\n"
                        f"{admin}\n"
                        "This is the operator's direction and outranks inference from the idea "
                        "text or the market research below.\n"
                    )
                    mandated = _mandated_charter_sections(admin)
                    if mandated:
                        prompt += (
                            "\n=== MANDATORY REQUIREMENTS (gated — the pipeline will refuse a "
                            "spec that omits these) ===\n"
                            f"{mandated}\n"
                            "Each section above MUST appear as core_features with testable "
                            "acceptance_criteria, naming the identifiers it mentions (env vars, "
                            "headers, JSON fields, capability ids) so the build can be verified "
                            "against them. Do NOT invent prices or quotas the charter does not "
                            "state.\n"
                        )
                prompt += (
