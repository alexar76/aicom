...
Use passed=false if alignment_score < 55 or critical gaps exist."""
    ...
    async def execute(self, agent_input: AgentInput) -> AgentOutput:
        ...
        try:
            ...
            try:
                ...
                code_root = product_code_dir(product_id)
                apply_visual_gate_autofix(code_root)
                # Factory-owned: login/session salt and UUID PK coerce land in data/code.
                apply_live_auth_autofix(code_root)
                from web.backend.services.github_house_assets import apply_github_house_asset_autofix

                apply_github_house_asset_autofix(code_root)
            ...
            try:
                salt_issues = self._assess_session_token_salt(product_id, code_files)
                all_bugs.extend(salt_issues)
            except Exception as e:
                self._log("WARNING", f"Session token salt scan failed: {e}")

            try:
                uuid_pk_issues = self._assess_uuid_pk_lookup(product_id, code_files)
                all_bugs.extend(uuid_pk_issues)
            except Exception as e:
                self._log("WARNING", f"UUID PK lookup scan failed: {e}")

            try:
                pinned_issues = self._assess_relay_pinned_compat(product_id, code_files)
                all_bugs.extend(pinned_issues)
            except Exception as e:
                self._log("WARNING", f"Relay pinned compat scan failed: {e}")

            try:
                from core.github_catalog import github_house_gate_active

                if github_house_gate_active():
                    ...
                    self._log(
                        "INFO",
                        "GitHub-house gate skipped (catalog off or GH_PAT not configured)",
                    )
            except Exception as e:
            ...
            if llm_review:
                ...
                for _llm_bug in (llm_review.get("bugs_found") or []):
                    if isinstance(_llm_bug, dict):
                        ...
                        from web.backend.services.qa_llm_finding_filters import (
                            is_test_hygiene_llm_finding,
                        )

                        if is_test_hygiene_llm_finding(_llm_bug):
                            self._log(
                                "INFO",
                                "Dropping LLM test-hygiene finding (does not block ship): "
                                f"{str(_llm_bug.get('title') or '')[:80]}",
                            )
                            continue
            ...
            # Bound before the try because the charter-contract gates below read it too, and an
            # import failure here must not turn into a NameError three checks later.
            _charter_text = ""
            try:
            ...
            # A charter may pin a dependency the product must import rather than rewrite, and
            # state properties the product must actually satisfy. Both are enforced here rather
            # than trusted to the prose, because every other gate in this stage is structural:
            # they establish that the code is well-formed, and none of them can see that a
            # function is wrong while being well-formed. That is the shape of all three
            # verifiable-output bugs this ecosystem has shipped — platon's vacuous proof (twice),
            # sortes' prover/verifier wire-format split, and a signed bill of materials no buyer
            # can verify. Folded into module_health so the findings travel the same path as the
            # rest; the charter comes from the same store fallback the unchartered-subsystem
            # check uses, for the same reason (the task payload does not reliably carry it).
            for _label, _runner in (
                ("pinned dependency", "pinned_dependency_gate.run_pinned_dependency_check"),
                ("executable property", "property_contract_gate.run_property_contract_check"),
            ):
                try:
                    _mod_name, _fn_name = _runner.split(".")
                    _mod = __import__(
                        f"web.backend.services.{_mod_name}", fromlist=[_fn_name]
                    )
                    _contract = await asyncio.to_thread(
                    ...
                    self._log("WARNING", f"{_label} check failed to run: {_ce}")
                    continue
                # Unconditional, for the reason stated above the unchartered-subsystem log:
                # "the gate ran and found nothing" and "the gate never ran" have already
                # produced identical logs in this pipeline, and both times it cost a night.
                self._log(
                    "INFO",
                    f"{_label} check for {product_id}: "
                    f"{_contract.get('declared', 0)} declared, "
                    f"{len(_contract.get('issues') or [])} finding(s)"
                    + (" (skipped)" if _contract.get("skipped") else ""),
                )
                for _result in _contract.get("results") or []:
                    self._log(
                        "INFO",
                        f"  property '{_result.get('name')}' ({_result.get('kind')}): "
                        f"{'ok' if _result.get('ok') else 'FAILED'} — {_result.get('detail', '')}",
                    )
                if _contract.get("issues"):
                    module_health["issues"] = list(_contract["issues"]) + list(
                        module_health.get("issues") or []
                    )
                    if not _contract.get("passed", True):
                        module_health["passed"] = False
                    module_health["skipped"] = False
            ...
            if not journey.get("skipped") and not journey.get("passed", True):
                for line in journey.get("issues") or []:
                    ...
                    # A timeout caused by a dependency that does not answer is a CONSEQUENCE,
                    # not an independent defect — and scoring it as one is what selected for the
                    # wrong fix on Sentinel for 38 rounds. Its advisory endpoint carried two
                    # ATLAS addresses (services/aimarket_participant.py:29 pointed at the real
                    # service, app/config.py:9 at localhost:8001 where nothing listens), so every
                    # round that restored the real call scored 9 -> 12 on one high finding and was
                    # reverted; the only edit the gate would accept was a static placeholder,
                    # which is the one the developer finally made. Reported as work, no vote —
                    # the same treatment auth_rejected already gets, for the same reason.
                    _dead_deps = journey.get("external_unreachable") or []
                    _frontier = (
                        "auth_rejected" in str(line)
                        or (bool(_dead_deps) and "demo_journey_unreachable" in str(line))
                    )
            ...
            # The dead dependency itself, named once with the file and line to edit. This is the
            # finding the developer can actually act on in one round, and without it the only
            # signal was "your endpoint times out", whose cheapest cure is deleting the call.
            for _dep in (journey.get("external_unreachable") or []):
                all_bugs.append(
                    {
                        "severity": "high",
                        "title": (
                            "Config: declared external dependency does not answer — "
                            f"{_dep.get('url')}"
                        ),
                        "description": (
                            f"{_dep.get('source')} declares {_dep.get('url')} and nothing answers "
                            f"there from the sandbox ({_dep.get('detail')}). Every endpoint that "
                            "calls it will time out, and those timeouts are reported separately "
                            "and do NOT count against the round. Fix the address — do not remove "
                            "the call, and do not replace the response with a placeholder: check "
                            "the other places in this product that name the same service, because "
                            "a second, correct address usually already exists somewhere in it."
                        ),
                        "file": f"code/{product_id}/{str(_dep.get('source') or '').split(':')[0]}",
                    }
                )
            ...
            # Live soft mesh (UNKNOWN / payment_authorization / escrow) outranks
            # undeclared_dep noise — otherwise developer thrash never opens atlas_client.
            try:
                from pathlib import Path as _Path
                import json as _json
                import urllib.request as _urlreq

                _ap = self.data_root / "state" / product_id / "auto_publish.json"
                _live_url = ""
                if _ap.is_file():
                    _rec = _json.loads(_ap.read_text(encoding="utf-8"))
                    _live_url = str(
                        _rec.get("vercel_url")
                        or (_rec.get("vercel") or {}).get("published_url")
                        or _rec.get("published_url")
                        or ""
                    ).strip()
                    _lg = _rec.get("live_gate") if isinstance(_rec.get("live_gate"), dict) else {}
                    for _iss in _lg.get("issues") or []:
                        _txt = str(_iss)
                        if "live_mesh" in _txt.lower() or "payment_authorization" in _txt.lower() or "mesh response" in _txt.lower():
                            all_bugs.insert(
                                0,
                                {
                                    "severity": "critical",
                ...
                if _live_url:
                    _probe = _live_url.rstrip("/") + "/api/advisory?lat=34.05&lon=-118.25"
                    try:
                        with _urlreq.urlopen(_probe, timeout=20) as _resp:
                            _body = _json.loads(_resp.read().decode("utf-8", errors="replace"))
                        _reason = str(((_body.get("overall") or {}) if isinstance(_body, dict) else {}).get("reason") or "")
                        if any(
                            x in _reason.lower()
                            for x in (
                                "mesh response unavailable",
                            ...
                            all_bugs.insert(
                                ...
                                {
                                    "severity": "critical",
                                    "title": f"live_mesh_unreachable:{_reason[:160]}",
                                    "detail": (
                                        "Deployed /api/advisory soft-fails mesh. Prefer "
                                        "atlas_client / aimarket_participant / escrow reopen "
                                        f"({_reason[:200]})"
                                    ),
                                    "file": "backend/app/services/atlas_client.py",
                                },
                            ...
                            for _mesh_rel in (
                                ...
                                if _mesh_rel not in blocking_files:
                                    blocking_files.insert(0, _mesh_rel)
                    except Exception:
                        pass
            except Exception:
                pass
    ...
    def _assess_github_house(
        ...
        *,
        ...
        if readme_text:
            try:
                from web.backend.services.github_house_assets import extract_readme_local_assets

                for rel in extract_readme_local_assets(readme_text):
                    if not has_suffix(rel):
                        issues.append(
                            {
                                "severity": "high",
                                "title": f"GitHub house: dead README image {rel}",
                                "description": (
                                    "GitHub renders a 404 <img> as the alt text (a blue link), "
                                    "not a badge or hero. Commit the file under the path the "
                                    "README already names — do not leave coverage/tests/hero as "
                                    "caption-only links."
                                ),
                                "file": f"code/{product_id}/{rel}",
                            }
                        )
            except Exception as exc:
                self._log("WARNING", f"README asset scan failed: {exc}")

        if not has_suffix("contributing.md"):
            issues.append(
                {
                    "severity": "high",
                    "title": "GitHub house: missing CONTRIBUTING.md",
                    "description": (
                        "Ship a root CONTRIBUTING.md (how to run tests, PR expectations, no secrets). "
                        "Required for the factory product catalog / GitHub house bar."
                    ),
                    "file": f"code/{product_id}/CONTRIBUTING.md",
                }
            )
        ...
        # Product packaging: admin + user guide + use-cases, and badges that resolve.
        if delivery_profile != MARKETING_LANDING and has_ui:
            for rel, title, why in (
                (
                    "docs/admin.md",
                    "GitHub house: missing docs/admin.md",
                    "Ship an admin/deploy guide (env, funding, republish, failure table).",
                ),
                (
                    "docs/user-guide.md",
                    "GitHub house: missing docs/user-guide.md",
                    "Ship a short end-user / operator UI walkthrough.",
                ),
                (
                    "docs/use-cases.md",
                    "GitHub house: missing docs/use-cases.md",
                    "Ship who-it-is-for / not-for and 2–3 concrete journeys.",
                ),
            ):
                if not has_suffix(rel):
                    issues.append(
                        {
                            "severity": "high",
                            "title": title,
                            "description": why + " Required for factory product-catalog packaging.",
                            "file": f"code/{product_id}/{rel}",
                        }
                    )
            for badge in ("ci.svg", "license.svg"):
                if not has_suffix(f"docs/badges/{badge}"):
                    issues.append(
                        {
                            "severity": "high",
                            "title": f"GitHub house: missing docs/badges/{badge}",
                            "description": (
                                "README badge links must resolve to committed SVG files under "
                                "docs/badges/ (dead badge images fail catalog review)."
                            ),
                            "file": f"code/{product_id}/docs/badges/{badge}",
                        }
                    )
            if readme_text and "docs/gallery/" in readme_text.lower():
                # Only the lines that actually reference the gallery count. Looking for
                # `![`/`<img` anywhere in the README meant a single badge `<img>` at the
                # top satisfied this check, so a gallery table listing bare
                # `docs/gallery/01.svg` paths passed — which is precisely the shape the
                # finding is named for, and every README the factory writes has badges.
                _gallery_lines = [
                    line
                    for line in readme_text.splitlines()
                    if "docs/gallery/" in line.lower()
                ]
                _gallery_text = "\n".join(_gallery_lines)
                if "![" not in _gallery_text and "<img" not in _gallery_text.lower():
                    issues.append(
                        {
                            "severity": "medium",
                            "title": "GitHub house: gallery table has no embedded images",
                            "description": (
                                "Gallery rows must use markdown images "
                                "![caption](docs/gallery/…) or <img src=…>, not bare paths."
                            ),
                            "file": f"code/{product_id}/README.md",
                        }
                    )
            docs_links_ok = all(
                needle in readme_text.lower()
                for needle in ("docs/admin.md", "docs/user-guide.md", "docs/use-cases.md")
            )
            if readme_text and not docs_links_ok:
                issues.append(
                    {
                        "severity": "medium",
                        "title": "GitHub house: README Docs section incomplete",
                        "description": (
                            "README must link docs/admin.md, docs/user-guide.md, and "
                            "docs/use-cases.md so the repo explains what the product is for."
                        ),
                        "file": f"code/{product_id}/README.md",
                    }
                )
    ...
    def _assess_session_token_salt(self, product_id: str, code_files: list[dict]) -> list[dict]:
        """Login must sign with the same salt/secret the protected routes verify.

        Measured on Factory Relay: UI login 200 with salt=relay-access-token, then
        GET /api/handoffs 401 because verify uses relay-session. Vercel publish used
        to patch only the bundle copy. This finding is measured (API contract:) so
        an LLM cannot drop it.
        """
        issues: list[dict] = []
        login_salt = False
        protected_salt = False
        cookie_session = False
        cookie_relay = False
        auth_file = f"code/{product_id}"
        for info in code_files:
            path = str(info.get("path") or "").replace("\\", "/")
            content = str(info.get("content") or "")
            name = Path(path).name
            if name == "auth.py" and "relay-access-token" in content:
                login_salt = True
                auth_file = path
            if name in {"handoffs.py", "security.py"} and "relay-session" in content:
                protected_salt = True
            if name == "auth.py" and re.search(r'key=["\']session["\']', content):
                cookie_session = True
            if name == "deps.py" and "SESSION_COOKIE_NAME" in content and "relay_session" in content:
                cookie_relay = True
        if login_salt and protected_salt:
            issues.append(
                {
                    "severity": "high",
                    "title": "API contract: login token salt does not match protected routes",
                    "description": (
                        "Login signs with salt=relay-access-token while handoffs/security verify "
                        "relay-session. The visitor sees a successful login; business routes return "
                        "401. Factory must rewrite login to create_session_token (session_secret + "
                        "relay-session) in data/code — not only the Vercel bundle."
                    ),
                    "file": auth_file,
                }
            )
        if cookie_session and cookie_relay:
            issues.append(
                {
                    "severity": "high",
                    "title": "API contract: login cookie name does not match session reader",
                    "description": (
                        "Login Set-Cookie uses key=session but deps SESSION_COOKIE_NAME is "
                        "relay_session. Align the cookie name in data/code."
                    ),
                    "file": auth_file,
                }
            )
    ...
    def _assess_uuid_pk_lookup(self, product_id: str, code_files: list[dict]) -> list[dict]:
        """Token ``sub`` / path ids are str; SQLAlchemy UUID columns need uuid.UUID.

        Measured on Factory Relay: login 200, then GET /api/handoffs 500 because
        ``Operator.id == operator_id`` passes a string into a UUID column. Prefix
        ``API contract:`` so an LLM cannot drop it.
        """
        issues: list[dict] = []
        hit_file = f"code/{product_id}"
        raw = False
        for info in code_files:
            path = str(info.get("path") or "").replace("\\", "/")
            ...
            content = str(info.get("content") or "")
            if re.search(
                r"\b(?:Operator|Handoff)\.id\s*==\s*(?:operator_id|handoff_id)\b",
                content,
            ):
                raw = True
                hit_file = path
        ...
        if raw:
            issues.append(
                {
                    "severity": "high",
                    "title": "API contract: string UUID compared to UUID column",
                    "description": (
                        "verify_access_token succeeds, then Operator.id == operator_id "
                        "(or Handoff.id == handoff_id) raises because the token/path id is "
                        "str and the SQLAlchemy column is UUID. Factory must coerce with "
                        "uuid.UUID(str(...)) in data/code — not only a Docker sed."
                    ),
                    "file": hit_file,
                }
            )
    ...
    def _assess_relay_pinned_compat(self, product_id: str, code_files: list[dict]) -> list[dict]:
        """Enum `.value` and raw UUID JSON must be coerced — not only in an AEGIS Docker pin.

        Measured on Factory Relay: receipt/audit dump ``handoff.id`` / ``e.action.value``
        and ORM models hit Pydantic UUID columns → 500. Prefix ``API contract:``.
        """
        from web.backend.services.vercel_fullstack_adapter import _RELAY_PINNED_FRAGMENTS

        issues: list[dict] = []

        def _file_for_rel(rel: str) -> dict | None:
            needle = rel.replace("\\", "/")
            for info in code_files:
                path = str(info.get("path") or "").replace("\\", "/")
        ...
        for rel, old, new in _RELAY_PINNED_FRAGMENTS:
            info = _file_for_rel(rel)
            if not info:
                continue
            content = str(info.get("content") or "")
            if old in content and new not in content:
                raw = True
                hit_file = str(info.get("path") or hit_file)
                break
        if raw:
            issues.append(
                {
                    "severity": "high",
                    "title": "API contract: Relay enum/ORM fragment not coerced",
                    "description": (
                        "Receipt/audit/handoff_service dump Enum.value or UUID objects, or "
                        "Pydantic ORM models lack UUID stringify. Factory must apply the "
                        "pinned exact fragments in data/code — not an AEGIS Docker sed."
                    ),
                    "file": hit_file,
                }
            )
            return issues
        has_handoffs = _file_for_rel("routers/handoffs.py") is not None
        has_receipt = _file_for_rel("services/receipt.py") is not None
        if has_handoffs and has_receipt:
            for rel, old, new in _RELAY_PINNED_FRAGMENTS:
                info = _file_for_rel(rel)
                if info is None:
                    issues.append(
                        {
                            "severity": "high",
                            "title": "API contract: Relay pinned fragment file missing",
                            "description": (
                                f"Expected {rel} for the pinned Relay heal; structure changed "
                                "so the factory will not guess a neighbour."
                            ),
                            "file": f"code/{product_id}/{rel}",
                        }
                    )
                ...
                if old not in content and new not in content:
                    issues.append(
                        {
                            "severity": "high",
                            "title": "API contract: Relay pinned fragment moved",
                            "description": (
                                f"{rel} no longer contains the pinned bytes; fail-fast rather "
                                "than patching the wrong site. Factory must not guess."
                            ),
                            "file": str(info.get("path") or rel),
                        }
                    )
                    break
        return issues
    ...
    def _run_static_analysis(self, code_files: list[dict]) -> list[dict]:
        ...
        """
        issues = []
        # Pylint and flake8 can load project-local plugins/configuration. Treat them
        # as execution of generated code and never expose the factory environment.
    ...
    def _run_tests(self, code_files: list[dict]) -> dict:
        ...
        # Run each test file
        child_env = scrub_child_env(os.environ)
        child_env.setdefault("DATABASE_URL", "sqlite:///:memory:")
        child_env.setdefault("ENVIRONMENT", "test")
