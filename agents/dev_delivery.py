...
5. **One HTML ↔ one CSS (sandbox-critical):** each HTML file MUST link the stylesheet that defines **its** class names. Root `index.html` uses `./style.css` (classes like `.navbar` / `.hero-section` live there). `frontend/index.html` uses `./style.css` or `frontend/style.css` (classes like `.landing-nav` / `.hero`). **Never** point root HTML at `frontend/style.css` or the nested page at a root sheet that styles different classes. Prefer a single marketing landing, not two mismatched trees. Factory preview often serves static HTML without uvicorn — a class/CSS mismatch paints naked browser-default text.
6. **CSS variables** in `:root` for background, surface, text, accent, radius, shadow — then use them everywhere (no scattered magic hex except rare one-offs).
7. **Visual depth**: at least one of — layered gradient background, soft glass card (`backdrop-filter`), subtle mesh/blur, or refined dual shadow on cards. Flat `#cccccc` boxes filling the screen are **not** acceptable as the whole design.
8. **Motion**: transitions on buttons/links/cards (150–300ms); at least one tasteful `@keyframes` OR scroll reveal via small vanilla JS (`IntersectionObserver`). Honor `prefers-reduced-motion`.
9. **Responsive**: flex/grid, fluid type with `clamp()`, readable line length, comfortable tap targets on mobile.
10. **Page shell**: every app route view gets horizontal inset — `.page-shell` or `.container` with `padding-inline: 16–24px`, `max-width`, `margin-inline: auto`. Never ship bare `<main>` with only vertical padding (fields glued to the screen edge).
...
def validate_saved_files(
    ...
    existing_paths: list[str] | set[str] | None = None,
) -> tuple[bool, str]:
    """
    Verify the PRODUCT matches the mandated delivery mode.

    Returns (ok, error_message).

    ``existing_paths`` is what is already on disk. Without it this checked only the round's
    write list, which made every focused repair round illegal by construction: a round that
    edits three backend files writes no ``.html``, so a web product was told "Web stack requires
    at least one .html file" and the whole round was discarded. Measured on production — eight
    such rejections in two hours, including every batched round, each one after three attempts.
    The instruction "return only the files you actually modified" and this check were in direct
    contradiction, and this check won: only a round that rewrote the entire tree could pass, so
    the loop was *forced* into the sprawling rounds that then measured worse and were reverted.

    PRESENCE checks (an .html exists, a manifest exists) therefore consider the write list plus
    the tree. FORBIDDEN checks still consider the write list alone: a round must not introduce a
    banned file, and holding it responsible for one that was already there would make an
    unrelated repair unlandable — the same trap one level down.
    """
    rel = [p.replace("\\", "/").strip() for p in relative_paths if p.strip()]
    lower = [x.lower() for x in rel]
    # Presence is a property of the product, not of this round.
    present = set(lower) | {
    ...
    if mode == DeliveryMode.PYTHON_CLI:
        py_ok = any(x.endswith(".py") for x in present)
        if not py_ok:
    ...
    if mode == DeliveryMode.DESKTOP_APP:
        norm = [p.replace("\\", "/") for p in present]
    ...
    # WEB_APP
    has_html = any(p.endswith((".html", ".htm")) for p in present)
    if not has_html:
...
**Static preview CSS pairing:** sandbox may show `index.html` before FastAPI/compose is up. Root landing and `frontend/` landing must each link **their own** theme CSS. Do not generate a root `index.html` that `<link>`s `frontend/style.css` while using different class names (`.navbar` vs `.landing-nav`) — that renders as unstyled HTML in the storefront iframe.
"""
