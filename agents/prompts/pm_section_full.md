...
- When the idea is an agent that **pays for** ATLAS/Hub capabilities (mesh invokes, receipts, “paying participant”), the spec MUST name AI Market Protocol v2: Hub invoke path, sandbox visitor for trial, payment channel for paid calls, and operator-funded budget — not a demo `X-Agent-Key`.
...
- `non_functional_requirements`: measurable — latency targets, availability, security (authn/z, data handling), accessibility bar appropriate to audience. Include an OSS/GitHub-house NFR: bilingual docs (English + product locale), CI + coverage floor, tagged GitHub Release, README with badges and a hero/gallery when the product has a UI.
