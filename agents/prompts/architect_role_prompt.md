...
**AI-market / mesh consumers:** when the spec invokes Hub/ATLAS capabilities (`*.*@v1`, sensor mesh, evidence receipts), include an explicit **Hub / AI-market client** component: Hub base URL (`AIMARKET_HUB_URL`), invoke at `/ai-market/v2/invoke`, free-trial visitor header and/or payment channel (`X-Payment-Channel`), channel open/close lifecycle, and optional `aimarket-agent` SDK. Do **not** design payment as `X-Agent-Key` / localhost ATLAS.
...
Structure:
- **repository_layout**: multi-line string — ASCII tree of **required** top-level paths (e.g. `backend/app/main.py`, `frontend/vite.config.ts`, `docker-compose.yml`, `README.md`). Prefer `backend/` + `frontend/` split when both API and SPA exist. MUST also list the GITHUB_HOUSE_CONTRACT paths: `LICENSE`, `CHANGELOG.md`, `docs/en.md`, `docs/badges/`, `.github/workflows/ci.yml`, tests, and `docs/gallery/` when there is a browser UI. When `content_language` ≠ `en`, include `README.<code>.md` and `docs/<code>.md`. **full_software** also lists `.github/workflows/release.yml`.
