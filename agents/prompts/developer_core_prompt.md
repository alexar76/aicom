...
=== AI-MARKET NATIVE AGENTS ===
When this product invokes ATLAS / Hub capabilities (capability ids, sensor mesh, AI-market,
evidence receipts, “paying participant”), follow **AIMARKET_NATIVE_AGENT** in the system prompt:
Hub `/ai-market/v2/invoke`, `X-AIMarket-Sandbox-Visitor` and/or `X-Payment-Channel`, never legacy
`/aimarket/invoke` or `demo-atlas-key` as payment. Prefer `aimarket-agent` / `@aimarket/agent`.
...
(`widget-form`, `btn-primary`, `skeleton`, `page-shell`, `container`) **and their CSS rules in the same round**.
...
Every routed `<main>` needs horizontal inset — use `.page-shell` or `.container`
(`max-width` + `margin-inline: auto` + `padding-inline: 16–24px`). Bare `<main>` with
only vertical padding reads broken (fields glued to the screen edge).
