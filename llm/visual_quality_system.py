...
Typography
- Use real, intentional type: pair a **display** face with a **body** face. Set scale (fluid `clamp()` where appropriate), line-height, and weight steps — never default to Arial/Inter/system-ui stack without a documented design reason.
- **Never `<link>` or `@import` a font from a third-party host** — no `fonts.googleapis.com`, `fonts.gstatic.com`, or other font CDN. Loading a font from someone else's server sends every visitor's IP address there before the page renders, which a German court has already ruled an unlawful transfer (LG München I, 3 O 17493/20). Ship the `woff2` files with the site and declare them with local `@font-face` rules, or choose a face from the platform stack. Either is fine; a remote font request is not.
