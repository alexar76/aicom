...
class LLMRouter:
    ...
    async def generate(
        ...
        if self._cache_enabled and not getattr(config, "no_cache", False):
            cached = self._cache_get(cache_key)
            if cached is not None:
                logger.debug("LLM cache hit for task_type=%s", task_type)
                # A hit means no provider runs, so nothing would set this. Say plainly that the
                # answer came whole from cache rather than leaving the field at "", which a
                # caller cannot tell apart from "the provider did not say".
                try:
                    config.finish_reason = "cache"
                ...
                with span("llm.generate", attributes=span_attrs) as llm_span:
                    ...
                    if llm_span is not None:
                        try:
                            ...
                            pass
                if (self._cache_enabled and not getattr(config, "was_truncated", False)
                        and not getattr(config, "no_cache", False)):
                    # A reply the provider cut off at the output limit is never cached. Caching
                    # it turned a transient into a permanent: for the whole TTL every retry got
                    # the same half-written bytes back WITHOUT a provider call, so
                    # `finish_reason` was never set and the truncation guard saw nothing to
                    # guard. The remediation loop's three attempts were three cache hits.
