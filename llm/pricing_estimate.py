...
_BUILTIN_MODEL_USD_PER_MTOK: dict[str, float] = {
    ...
    "llama3-8b-8192": 0.05,
    # Blended, for callers that only know a total. Roughly 1:2 in:out, which is the shape
    # measured on this ecosystem's own traffic — remediation runs output-heavy because the
    # model returns whole files.
    "minimax/minimax-m3": 0.90,
    "moonshotai/kimi-k3": 11.00,
    "deepseek/deepseek-v4-pro": 0.76,
    "deepseek/deepseek-v4-flash": 0.13,
    "qwen/qwen3-coder": 0.77,
    "qwen/qwen3.6-35b-a3b": 0.63,
}
...
_BUILTIN_MODEL_IN_OUT_PER_MTOK: dict[str, tuple[float, float]] = {
    ...
    # The models this ecosystem actually runs. Not one of them was in this table, so every
    # real call was priced at the generic fallback below and every recorded figure was a
    # guess wearing four decimal places. Read from the provider's live price list on
    # 2026-08-30; the source is named so the next person knows where to re-read it.
...
# ── what we are NOT accounting for ────────────────────────────────────────────────
#
# A model with no rate silently received the generic fallback, so every spend figure in the
# system looked precise and was a guess. Measured on one day's remediation traffic: recorded
# $0.3862, honest range $0.028 to $1.42, depending on which of three disagreeing tables you
# believed. A number nobody can bill against is worse than no number, because it stops people
# asking.
#
# The fallback stays — refusing to estimate would break every caller — but an unpriced model
# is now named, once, and countable. "We spent $X, plus N calls on models we have no rate for"
# is a sentence an operator can act on.

_unpriced_models: dict[str, int] = {}


def note_unpriced_model(provider: str, model: str) -> None:
    """Record that a call was priced by fallback rather than by a real rate."""
    key = f"{normalize_llm_provider_id(provider)}:{(model or '').strip().lower()}".strip(":")
    if not key:
        return
    first = key not in _unpriced_models
    _unpriced_models[key] = _unpriced_models.get(key, 0) + 1
    if first:
        logger.warning(
            "no price for %s — its cost is a generic estimate, not a rate. Add it to "
            "_BUILTIN_MODEL_IN_OUT_PER_MTOK or data/config/llm_pricing.yaml.", key)


def unpriced_models() -> dict[str, int]:
    """Models charged at the fallback, and how many calls each. Empty is the goal."""
    return dict(_unpriced_models)


def reset_unpriced_models() -> None:
    ...
    *,
    ...
    if use_split and pt is not None and ct is not None:
        ...
        if blended is not None:
            ...
            return round(usd, 6)
        # Past this point there is no rate for this model, only a rate for something like it.
        note_unpriced_model(provider, model)
