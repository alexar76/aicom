...
class GenerationConfig:
    ...
    #: Set BY the provider on the way back: how the completion ended, in the provider's own
    #: words ("stop", "length", "max_tokens", …). Empty when the provider did not say.
    #:
    #: `generate()` returns a plain string, so a completion cut off by the output limit was
    #: indistinguishable from a finished one. Measured in production: the remediation fixer
    #: received a Python file whose last triple-quoted string was never closed, committed it,
    #: built an image and launched a container that died at import. The config object already
    #: travels from caller to provider and back, so it is where the answer belongs — no
    #: signature changes, and callers that do not care are unaffected.
    finish_reason: str = ""
    #: Never serve this call from the response cache. A retry ladder asks the same question
    #: again ON PURPOSE and needs a different answer; a cache turns the second attempt into a
    #: replay of the first, which is not a retry at all.
    no_cache: bool = False

    @property
    def was_truncated(self) -> bool:
        """True when the provider said it stopped because it ran out of output budget."""
        return str(self.finish_reason or "").lower() in ("length", "max_tokens", "max_output_tokens")

    @property
    def produced_no_text(self) -> bool:
        """True when the provider returned nothing usable — a refusal, a filter, a tool-only turn.

        An empty string recorded as a success is indistinguishable from "the model had nothing
        to add"; every one of these used to arrive that way."""
        reason = str(self.finish_reason or "").lower()
        return reason.startswith("no_text") or reason in ("refusal", "no_content", "content_filter")
