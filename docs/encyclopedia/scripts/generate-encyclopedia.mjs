...
function renderEcosystemDiagram(caption, esc, locale) {
  const simCaptions = {
    ...
    fr: "Alien Monitor — mode simulation avec métriques et flux d'activité",
    zh: 'Alien Monitor——带指标与活动流的模拟模式',
  };
...
function buildHtml(c, locale) {
  const langAttr = { en: 'en', ru: 'ru', es: 'es', fr: 'fr', zh: 'zh' }[locale];
...
<body>
  ...
  <header class="cover">
    ...
    <p class="subtitle">A storybook for geek children of the distant future — ideology, planets, oracles, and practical magic in five tongues.</p>
