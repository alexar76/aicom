...
> Locales marked **not built** have no committed artefact in this checkout. Build them
> with the scripts in [`scripts/`](./scripts/) — a link to a file that was never generated
> is worse than an honest gap.

| Language | HTML | PDF |
|----------|------|-----|
| English | build via `generate-encyclopedia.mjs` (gitignored) | build via `generate-pdf.mjs` |
| Русский | build via `generate-encyclopedia.mjs` (gitignored) | build via `generate-pdf.mjs` |
| Español | build via `generate-encyclopedia.mjs` (gitignored) | build via `generate-pdf.mjs` |
| Français | build via `generate-encyclopedia.mjs` (gitignored) | — |
| 中文 | build via `generate-encyclopedia.mjs` (gitignored) | — |

> Locale `*/index.html` and PDFs are **generated** (gitignored). Host deploy and
> GitHub Pages must run `node docs/encyclopedia/scripts/generate-encyclopedia.mjs all`
> before copying into `ecosystem-landing/` — otherwise nginx SPA fallback used to
> silently serve the landing page at `/encyclopedia/ru/` etc.
