...
> 🌐 Languages: [English](metis-integration.md) · **Русский** · [Español](metis-integration.es.md)
...
Фабрика работает **без присутствия Metis**, а Metis работает **без присутствия фабрики**. Каждая связь между ними опциональна и деградирует до no-op.
...
| hub отсутствует | Metis этого не замечает (регистрация + webhook являются opt-in) |
...
**Почему auto-detect, а не режим «включено-по-умолчанию-с-блокировкой»?** Потому что независимость никогда не должна быть теоретической. Отсутствующий Metis обходится в одну быструю, кэшируемую пробу health — никогда в потаймаут на стадию — и никогда в крах. Блокировка является opt-in, чтобы непроверенное развёртывание Metis не могло молча застопорить конвейер.
...
| Бейдж | Значение |
|---|---|
| **Без проверки Метис** | Результат шлюза ещё не записан (`metis_gate` отсутствует или нет поля `at`). Обычно до завершения architect/methodologist или когда gate выключен и Metis для этого продукта не вызывался. |
...
**Дашборд экосистемы:** **Админ → Дашборд** — карточка **Metis в экосистеме** (зелёный **Активен**, если Metis развёрнут и фабрика использует gate; серый **Неактивен** иначе) с показателями развёртывания, использования фабрикой и сводкой одобрений/замечаний по продуктам.
...
| Route | Вызывающая сторона | Body → Response |
|---|---|---|
