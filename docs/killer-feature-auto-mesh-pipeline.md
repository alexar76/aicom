...
1. **Network effect** — every shipped product adds capabilities others can mesh into the next run.
2. **Unit economics** — channel model makes multi-agent runs affordable (one deposit, N invokes).
3. **Moat vs. Copilot/Cursor** — they generate files; you generate **connected products** with payments and federation baked in.
...
| [`ai-service-mesh/`](../ai-service-mesh/) | Mesh API reference (discovery → escrow → invoke) |
| [`aimarket-hub/`](../aimarket-hub/) | Federation + invoke surface |
