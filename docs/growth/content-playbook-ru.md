...
> 🌐 Языки: [English](./content-playbook.md) · **Русский** · [Français](./content-playbook-fr.md) · [中文](./content-playbook-zh.md)

Инструкция для коллег: что писать на лендингах, в README и доках. Плейбук для агентов и людей, которые работают с репозиториями экосистемы AICOM.

**Публичный контент — на английском.** Русский — только если есть явная RU-версия (как `README-ru.md` у DIOSCURI).

**См. также:** [THEOXENIA content plan (EN)](../../dioscuri/docs/content-plan.md) · [content-playbook (EN)](./content-playbook.md) · [knowledge base](../ecosystem/knowledge-base-ru.md)
...
Минимальный скелет (как у [`landing/index.html`](../../dioscuri/landing/index.html) DIOSCURI):
...
Сверяться с [`docs/content-plan.md`](../../dioscuri/docs/content-plan.md):
...
Запрещённые корпоративные фразы — см. `styleCharter()` в [`src/personas/index.ts`](../../dioscuri/src/personas/index.ts) (если пишете текст, который близнецы будут цитировать — не используйте их).
...
- Mastodon: аккаунт прогревать вручную до API; детали — [`docs/use-cases.md`](../../dioscuri/docs/use-cases.md) §9
...
Загрузка видео — это **больше не** ручной `upload_youtube.py`, используйте [HELIOS](../../helios/README.md):
