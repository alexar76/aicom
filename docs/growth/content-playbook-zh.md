...
> 🌐 语言： [English](./content-playbook.md) · [Русский](./content-playbook-ru.md) · [Français](./content-playbook-fr.md) · **中文**

面向所有接触 AICOM 生态系统仓库的 agent 与人类的 playbook。**面向公众的文案仅用英语。** 俄语应放在明确的 RU 配套文件里（例如 DIOSCURI 上的 `README-ru.md`）— 不要混进主 README 或 landing。

**另见：** [THEOXENIA content plan](../../dioscuri/docs/content-plan.md) · [seeding playbook](./seeding-playbook.md) · [knowledge base](../ecosystem/knowledge-base.md)
...
最小骨架（见 [DIOSCURI `landing/index.html`](../../dioscuri/landing/index.html)）：
...
与 [`docs/content-plan.md`](../../dioscuri/docs/content-plan.md) 对齐：
...
被禁的 corporate 短语 — 见 [`dioscuri/src/personas/index.ts`](../../dioscuri/src/personas/index.ts) 中的 `styleCharter()`。如果孪生会引用你的文本，就不要使用这些短语。
...
- Mastodon：在使用 API 之前手动养号；细节见 [`docs/use-cases.md`](../../dioscuri/docs/use-cases.md) §9
...
视频上传**不再是**手动的 `upload_youtube.py` — 请使用 [HELIOS](../../helios/README.md)：
