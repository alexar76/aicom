...
**Metis** ([`metis/`](https://github.com/alexar76/metis)) is the ecosystem's **cognition & verification tier** — a
...
> 🌐 Languages: **English** · [Русский](metis-integration.ru.md) · [Español](metis-integration.es.md) · [Français](metis-integration.fr.md) · [中文](metis-integration.zh.md)
> 📖 Metis-side view: [`metis/docs/en/ECOSYSTEM.md`](https://github.com/alexar76/metis/blob/main/docs/en/ECOSYSTEM.md)
...
when Metis is unreachable), [`metis/tests/test_ecosystem_api.py`](https://github.com/alexar76/metis/blob/main/tests/test_ecosystem_api.py)
...
**Ecosystem dashboard:** **Admin → Dashboard** shows a **Metis in the ecosystem** card with deployment
status, factory usage, and aggregate approval/flag counts across products. It has **three** states,
because a failed health probe has two very different causes:

| Badge | Card says | Cause |
|---|---|---|
| green **Active** | Metis deployed: **Yes** | Metis answered `/health` and the factory gate is on |
| gray **Inactive** | Metis deployed: **Not responding** | A URL *is* configured, but nothing answered at it — the card prints the address it probed |
| amber **Not configured** | Metis deployed: **Not configured** | Neither `AIFACTORY_METIS_URL` nor `METIS_URL` is set, so the probe hit the `http://127.0.0.1:8080` fallback |

The third state matters: with the variable unset, the probe fails on a localhost address that in most
deployments has nothing behind it, and in `auto` mode that one failure also zeroes **Factory uses** —
two "No"s from a single cause. Rendering that as "Metis is not deployed" is false whenever Metis is
live at its real address (`https://metis.modelmarket.dev` in the reference deployment, which is what
`aimarket-mcp`, `alien-monitor` and `docker-compose.core.yml` already default to, while the factory's
own compose leaves `AIFACTORY_METIS_URL` empty). The card now names the variable to set, and reports
*why* the factory is not using Metis: `gate-disabled`, `metis-unconfigured` or `metis-unreachable`.

Payload fields: `ecosystem.state` (`deployed` | `unreachable` | `unconfigured`), `ecosystem.configured`,
`ecosystem.url_source`, `ecosystem.url_env_vars`, `factory.blocked_reason`.
Derivation: [`web/frontend/lib/metisCardState.ts`](../web/frontend/lib/metisCardState.ts) (unit-tested) ·
backend: [`web/backend/services/metis_status.py`](../web/backend/services/metis_status.py).
...
[`metis/metis/api/ecosystem.py`](https://github.com/alexar76/metis/blob/main/metis/api/ecosystem.py), optional & self-contained):
...
[`alien-monitor/backend/metis_status.py`](https://github.com/alexar76/alien-monitor/blob/main/backend/metis_status.py)) so the Metis
API key never reaches the browser, and a dead Metis yields a readable message instead of an error.
Node/topology: [`alien-monitor/backend/metis_layers.py`](https://github.com/alexar76/alien-monitor/blob/main/backend/metis_layers.py).
