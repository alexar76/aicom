...
> **English:** [security-persistence.md](./security-persistence.md) · **Español:** [security-persistence.es.md](./security-persistence.es.md)
...
| Метод | Поведение |
|-------|-----------|
| `check_login_attempts(ip)` | Счёт в окне ban из SQLite |
| `record_login_attempt(ip, success=False)` | Запись попытки |
| `reset_login_attempts(ip)` | Сброс после успешного входа |

In-memory dict — fallback при недоступности стора.

---

## H-3 — Replay OIDC nonce

После проверки nonce в `verify_id_token` nonce **погашается** в сторе (TTL = min(время жизни id_token, 1 ч)).

| Ситуация | Поведение |
|----------|-----------|
| Первое использование | Вход OK |
| Повтор в TTL | `ValueError("nonce already used")` |
| Стор недоступен | **Fail-open** — предупреждение в лог, логины не блокируются |
