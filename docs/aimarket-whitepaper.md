...
the streak threshold cost trust only; a Metis **verified failure** first refunds the buyer (escrow)
and dings trust, escalating to stake only on repeat offenses within the window — with the signed
...
`0x3Df85a639EAB8B50DD14f09bdeB46D5FeF163017` has settled real USDC (see
...
single-EOA owner controls only the hub/token whitelist, not user deposits.
