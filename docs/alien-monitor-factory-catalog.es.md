...
| **UNIVERSE (UNI)** | **Sí** — sincroniza desde `GET /api/products` |
| **LIVE (real)** | **Sí** — mismo catálogo + RPC prod |

UNI levanta Anvil local **y** consulta Factory / Hub / Mesh / Prometheus en vivo. Sincronización en el arranque y cada ~60 s (`ALIEN_FACTORY_SYNC_TICKS`, 40 ticks por defecto).

Solo aparecen los productos **listados en storefront** (mismo filtro que la tienda pública), no todos los estados del pipeline.

---

## Configuración

| Variable | Por defecto (prod) | Rol |
|----------|----------------|-----|
...
2. **Modo TEST** — sin sincronización de catálogo.
...
- [alien-monitor/README.md](https://github.com/alexar76/alien-monitor/blob/main/README.md) — despliegue
- [uni-troubleshooting.md](./uni-troubleshooting.md) §16 — solución de problemas ampliada
- [funnel-growth.es.md](./funnel-growth.es.md) — lead público → pipeline (lado Factory)
