...
| **Hub LIVE** | Федеративный маркетплейс [modelmarket.dev](https://modelmarket.dev) | `modelmarket-hub` |
| **Hub UNI** | Герметичный параллельный каталог [uni.modelmarket.dev](https://uni.modelmarket.dev) — [uni-and-live.ru.md](./uni-and-live.ru.md) | `deploy/uni-hub.sh` |
...
| **Alien Monitor UNI** | 3D-граф пузыря — [monitor.modelmarket.dev](https://monitor.modelmarket.dev/) · `:9100` · `ALIEN_MODE=universe` | `alien-monitor` |
| **Alien Monitor LIVE** | 3D-граф живых денег — [`/monitor-live/`](https://monitor.modelmarket.dev/) · `:9101` · `ALIEN_MODE=real` | `alien-monitor-live` |
...
**Не входит в `deploy_ecosystem.sh`:** Metis, DIOSCURI, HELIOS — отдельно; см. [§Чего нет на одном VPS](#9-чего-нет-на-одном-vps). UNI-хаб + шесть лабораторий — **второй флот**: `deploy/uni-hub.sh` и `deploy/uni-satellites.sh`. Не направляйте UNI-монитор на живой хаб. Разрез: [uni-and-live.ru.md](./uni-and-live.ru.md).
...
| **L2** | Разместить **core fleet** на своём сервере (одном VPS) | `./scripts/quickstart_ecosystem.sh` или `./scripts/deploy_ecosystem.sh` |
...
- **DNS-записи A/AAAA**, указывающие на хост, на котором вы запускаетесь (уровень 3+):
  ...
  - `oracles.modelmarket.dev` → **хост оракулов** (`oracles.modelmarket.dev`), напрямую (без проксирования через factory)
...
Публичный Alien Monitor отдаётся по адресу `https://monitor.modelmarket.dev/` (nginx проксирует
...
Оракулы работают на **отдельной машине** (`oracles.modelmarket.dev`). **`deploy_ecosystem.sh` НЕ
...
### 6.1 На хосте Platon (`oracles.modelmarket.dev`, от root)
...
описаны в [`oracles/docs/en.md`](https://github.com/alexar76/oracles/blob/main/docs/en.md).
...
`deploy_ecosystem.sh` / `quickstart_ecosystem.sh` — **левый блок**, шаги 1–7. Оракулы — Уровень 4,
...
|-----------|--------|--------------|
| **17 оракулов** | Уровень 4 | `setup-oracles-platon-on-host.sh` + `announce-platon-oracles.sh` |
| **On-chain Base** | опционально | `deploy_ecosystem_base.sh broadcast` |
| **Metis** | не в fleet-скрипте | отдельный deploy |
| **DIOSCURI / HELIOS** | спутники | отдельные репо |
...
- [`oracles/docs/en.md`](https://github.com/alexar76/oracles/blob/main/docs/en.md) — семнадцать оракулов и экономический цикл
- [Документация AIMarket SDK](https://github.com/alexar76/aimarket-sdks/blob/main/docs/en.md) · [Python-агент](https://github.com/alexar76/aimarket-agent/blob/main/docs/en.md) — потребление Hub
