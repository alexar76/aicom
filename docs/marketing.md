...
Summary: SEO metadata, blog content hub, launch-kit and embeddable badge pages, referral attribution, customer referral dashboard, checkout attribution, and first-party marketing event logs.
...
**Generated pipeline landings (static `*.html` under `data/code/<product_id>/`):** extra `<head>` markup for **those** pages is **Admin → Settings → Head snippet on generated sites** (`general.published_site_head_html`). Google Analytics / gtag in that field is stripped and is not loaded on the Next.js storefront. See **[admin-guide.md](./admin-guide.md)** → Settings.
