...
Attested Memory products are **GitHub satellites** (not factory-only, not
Gitea-only): `attested-memory-hub` → `alexar76/attested-memory-hub`,
`attested-saas-gateway` → `alexar76/attested-saas-gateway`. Mirror with
`./scripts/publish_all_repos.sh --satellite <id>`.
