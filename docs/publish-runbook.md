...
`pantheon.modelmarket.dev`). Both are listed in `aicom_publish_config.py` rsync
excludes and fail-closed in `publish_aicom_factory.sh`. Deploy Pantheon via
`./scripts/deploy_pantheon.sh`.

Attested Memory products live under **`attested/`** and are **Gitea-only**
(monorepo push via `./scripts/push_gitea_monorepo.sh`). They are excluded from
the GitHub factory mirror and blocked as a satellite (`github_published: false`)
until explicitly flipped. Layout: `attested/attested-memory-hub`,
`attested/attested-saas-gateway`.
...
## 2. Push to Gitea#2 (canonical monorepo)

```bash
./scripts/push_gitea_monorepo.sh --dry-run   # preview: remote + pending commits
./scripts/push_gitea_monorepo.sh             # push to Gitea#2 alexar76/aicom only
```
