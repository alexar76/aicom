...
| HESTIA | HESTIA | HESTIA | HESTIA | HESTIA | Hosting control plane for AIMarket capability providers. Named after Ἑστία, but the hearth metaphor is **retired from copy** (2026-09-17, owner decision) — the product speaks plain technical terms; see `host (HESTIA)` and `agent (on a HESTIA host)`. Product name stays Latin. **Not** Hub, not Factory, not a job board. |
| host (HESTIA) | хост | host | hôte | 主机 | The operator’s machines that actually run seller processes. **Replaces `hearth (HESTIA)`, retired 2026-09-17** — do not write очаг / hogar / âtre / 炉灶 in prose. Latin identifiers are untouched: the path `/v1/hearth`, the field `hearth_url`, the module `hearth.js`. |
| agent (on a HESTIA host) | агент | agente | agent | 智能体 | One isolated agent process on a host — what a deploy starts. **Replaces `tenant (HESTIA)`, retired 2026-09-17** — prose no longer says тенант / inquilino / locataire / 租户. Same rendering as `agent` above, deliberately: one word, not two. Identifiers and API keep `tenant`: `/v1/tenants`, `TenantRow`, `tenant_payments`, `payout_address` on the tenant row. |
| deploy (onto a host) | деплой (на хост) | desplegar (en el host) | déployer (sur l’hôte) | 部署（到主机） | Explicit start of a signed bundle on HESTIA. Factory finishing a pipeline is **not** a deploy. |
...
| roster (host) | roster / список хоста | roster / lista del host | roster / liste de l’hôte | 名册 | Public list of agents **this** host is running. Empty ≠ empty market. The path stays `/v1/hearth`. |
...
| layer (map) | слой | capa | couche | 图层 | ATLAS layer key: `weather` · `air` · `tide` · `river` · `marine` · `grid` · `quake` · `energy` · `fire` · `radiation` · `jamming` · `traffic` · `ais` · `tsunami` · `aviation` · `road` · `rail` · `drought` · …. Labels localized EN/RU/ES/FR/ZH in `LAYER_META.labels`. |
| METAR | METAR (сводка погоды аэродрома) | METAR (informe meteorológico de aeródromo) | METAR (message d’observation d’aérodrome) | METAR（机场例行天气报告） | ICAO/WMO aerodrome routine report — keep `METAR`. Not a land ASOS pin and not a TAF. Layer `aviation`. |
| TAF | TAF (прогноз по аэродрому) | TAF (pronóstico de aeródromo) | TAF (prévision d’aérodrome) | TAF（机场预报） | Terminal Aerodrome Forecast — keep `TAF`. Forecast product; our METAR SKU is observation, not TAF. |
| ICAO (airport code) | код ИКАО / ICAO | código OACI / ICAO | code OACI / ICAO | ICAO 机场代码 | Four-letter aerodrome id (`KJFK`). Keep `ICAO` in catalogs. |
| PEGELONLINE | PEGELONLINE | PEGELONLINE | PEGELONLINE | PEGELONLINE | WSV Germany hydrological open-data system — brand stays Latin. Gloss once: RU «гидрологическая система WSV», ES «sistema hidrológico de la WSV», FR «système hydrologique de la WSV», ZH «德国联邦航道水文系统». |
| WSV | WSV | WSV | WSV | WSV | Wasserstraßen- und Schifffahrtsverwaltung des Bundes — keep `WSV`. |
| Digitraffic | Digitraffic | Digitraffic | Digitraffic | Digitraffic | Fintraffic open traffic data brand — keep Latin. Attribution: Fintraffic / Digitraffic, CC BY 4.0. |
| Fintraffic | Fintraffic | Fintraffic | Fintraffic | Fintraffic | Finnish traffic authority brand — keep Latin. |
| road weather | дорожная погода | clima vial / meteorología vial | météo routière | 道路气象 | In-situ highway sensors (temp, wind, …). Layer `road`. Prefer glossary form over «дорожный климат». |
| rail traffic (map) | железнодорожный трафик | tráfico ferroviario | trafic ferroviaire | 铁路交通 | Live train locations. Layer `rail`. |
| U.S. Drought Monitor / USDM | U.S. Drought Monitor / USDM (монитор засухи США) | U.S. Drought Monitor / USDM (monitor de sequía de EE. UU.) | U.S. Drought Monitor / USDM (observatoire de la sécheresse aux É.-U.) | U.S. Drought Monitor / USDM（美国干旱监测） | Keep `USDM` / full English product name. Weekly map + state stats; attribute NDMC / USDA / NOAA / NASA. Layer `drought`. |
| drought (map layer) | засуха | sequía | sécheresse | 干旱 | ATLAS layer `drought` — USDM classification, not soil-moisture in-situ. |
| NDMC | NDMC | NDMC | NDMC | NDMC | National Drought Mitigation Center — keep Latin. |
| GeoShake | GeoShake | GeoShake | GeoShake | GeoShake | Community earthquake catalog brand — keep Latin. Data **CC BY 4.0**; not a USGS/EMSC replacement. |
| NAAD | NAAD | NAAD | NAAD | NAAD | National Alert Aggregation and Dissemination System (Canada) — keep `NAAD`. Public brand **Alert Ready** / FR **En Alerte**. |
| Alert Ready | Alert Ready | Alert Ready | En Alerte / Alert Ready | Alert Ready | Canada public alerting brand. FR official: **En Alerte**; keep English once. |
| CAP-CP | CAP-CP | CAP-CP | CAP-CP | CAP-CP | Canadian Profile of CAP — keep Latin. |
| DONKI | DONKI | DONKI | DONKI | DONKI | Space Weather Database Of Notifications, Knowledge, Information (NASA CCMC) — keep `DONKI`. Cite NASA/CCMC; research product, not NOAA SWPC official forecast. |
| SWPC | SWPC | SWPC | SWPC | SWPC | NOAA Space Weather Prediction Center — keep Latin. |
| solar wind | солнечный ветер | viento solar | vent solaire | 太阳风 | Space-weather plasma flow. |
| GOES X-ray | рентген GOES / GOES X-ray | rayos X GOES / GOES X-ray | rayons X GOES / GOES X-ray | GOES X 射线 | Soft X-ray irradiance from GOES; keep `GOES`. |
...
| build order | приказ на сборку | orden de compilación | ordre de compilation | 构建指令 | Conductor-signed, names a COMMIT (never inline source). Type stays `BuildOrder`. |
| rollback order | приказ на откат | orden de reversión | ordre de restauration | 回滚指令 | Carries no image: the target comes from the agent's own journal. Type stays `RollbackOrder`. |
| candidate build / candidate container | кандидатная сборка / кандидатный контейнер | compilación candidata / contenedor candidato | build candidat / conteneur candidat | 候选构建 / 候选容器 | The freshly built image the gate probes BEFORE promotion. Container is `<service>-candidate`. |
| pre-promotion gate | гейт до промоушена | puerta previa a la promoción | porte de pré-promotion | 晋级前闸门 | MOMUS probing the candidate. The signed verdict records which it examined (`gated`). |
| circuit breaker (remediation) | предохранитель | cortacircuitos | coupe-circuit | 熔断器 | Refuses to sign further deploy orders once the loop's own numbers look wrong. Only a human clears it. |
| health gate | health-гейт | puerta de salud | porte de santé | 健康门控 | The agent's own check that the container really came up. `compose up` exiting 0 is not a verdict. RU keeps «гейт». |
| fix branch | ветка фикса | rama de corrección | branche de correctif | 修复分支 | `momus/fix-<finding_id>`. Both the transport to the builder AND the artifact a human reviews. |
| deploy hand | деплойная рука | mano de despliegue | main de déploiement | 部署之手 | The installed executor. Deliberately a hand: it carries out one order and cannot author anything. |
| heap census | перепись кучи | censo del montículo | recensement du tas | 堆普查 | GAIA's leak hunter: exact container lengths + allocated blocks, no profiler. |
...
## Prepaid credit terms (ATLAS credit accounts)

Added with [`atlas/docs/CREDIT-ACCOUNTS.md`](https://github.com/alexar76/atlas/blob/main/docs/CREDIT-ACCOUNTS.md).
Env vars (`ATLAS_CREDITS_ENABLED`, `ATLAS_CREDITS_DB_PATH`, `ATLAS_CREDITS_LOW_BALANCE_USD`),
headers (`X-API-Key`, `X-Atlas-Token`, `X-Atlas-Credit-Balance-Usd`, `X-Atlas-Credit-Charged-Usd`,
`X-Atlas-Credit-Low`), routes, the `atls_` key prefix, JSON fields (`grant_usd`, `amount_usd`,
`reference`, `insufficient_credit`, `recognized`) and the ledger verbs `hold` / `capture` /
`release` are **identifiers and never translated** — an operator copies them into a config file or
greps a log for them.
...
## HORKOS terms (escrow policy signer)

Product name `HORKOS` (ὅρκος) stays Latin in all locales. Repo id `escrow-signer` and contract field names (`authorizedHubs`, `debitChannel`, `hub`) are identifiers — never translate.

| EN | RU | ES | FR | ZH | Notes |
|----|----|----|----|----|-------|
| escrow policy signer | подписант политики эскроу | firmante de política de depósito en garantía | signeur de politique de séquestre | 托管策略签名服务 | The process, not the Hub key. |
| authorized hub (on-chain) | авторизованный hub | hub autorizado | hub autorisé | 获授权枢纽 | Field `authorizedHubs` in `AIMarketEscrow` — English id in prose. |
| debit channel (call) | debit канала | débito de canal | débit de canal | 通道扣款 | Canonical selector `0xf7becd80` only. |
| amount authority | полномочие на сумму | autoridad del importe | autorité sur le montant | 金额授权 | Depositor EIP-712, not Hub bearer token. |
| reverse SSH tunnel | reverse SSH tunnel | túnel SSH inverso | tunnel SSH inverse | 反向 SSH 隧道 | Host boundary — skopos box, not Hub host. |

## HEPHAESTUS terms (composing and costing capability graphs)

Added with [`hephaestus-studio.md`](./hephaestus-studio.md). The product name `HEPHAESTUS`, the field
names (`reputation_basis`, `observations_30d`, `trust_basis`, `input_from`, `depends_on`,
`capability_id`, `product_id`), the basis values (`measured`, `unobserved`, `self`, `unknown`) and the
route paths are **identifiers and never translated**.

`bill of materials` is the protocol's name for the signed record of a run: keep it in English and
gloss it once per document (RU «спецификация работ», ES «lista de materiales», FR « nomenclature »,
ZH「物料清单」). Do not shorten to `BoM` in translated prose.

| EN | RU | ES | FR | ZH | Notes |
|----|----|----|----|----|-------|
| blueprint | чертёж | plano | plan | 蓝图 | The graph a person draws. Not «схема» / «esquema» — those are JSON Schema. |
| cost estimate | смета | presupuesto | devis | 成本预估 | Computed before spending. Never «quote/оферта»: it is not binding. |
| critical path (latency) | самый длинный путь | camino más largo | chemin le plus long | 最长路径 | Reported as a floor, not a forecast. |
| composable | соединяемый | componible | composable | 可组合 | Declares input fields AND a non-empty output schema. |
| reputation basis | основание репутации | base de reputación | base de réputation | 信誉依据 | Whether a published rate is a measurement. Field name stays `reputation_basis`. |
| observed / unobserved | наблюдалось / не наблюдалось | observado / no observado | observé / non observé | 已观察 / 未观察 | «unobserved» is not «bad» — keep the distinction. |
| placeholder (rate) | заглушка | marcador de posición | substitut | 占位值 | A neutral value standing in for missing evidence. Never render it as a score. |
| at-fault hop | виновный хоп | salto culpable | saut fautif | 归责的跳 | Hop-level blame. Sits with `hop` in Core terms. |
| cleared (not at fault) | снята вина | exonerado | disculpé | 已免责 | Upstream hops that did their work. |
| projection (redacted) | вычищенная проекция | proyección redactada | projection expurgée | 脱敏投影 | A listing view that is deliberately NOT the signed object. |
| studio | студия | estudio | studio | 工坊 | The construction surface. Product name `HEPHAESTUS` stays Latin. |
| field reference | ссылка на поле | referencia a campo | référence de champ | 字段引用 | `${hop.field}`. The syntax itself never translates. |
| free trial / allowance | пробный тариф / остаток | prueba gratuita / saldo | offre d'essai / solde | 试用 / 额度 | Per-visitor, renewing. Not an account. |
| visitor id | идентификатор посетителя | id de visitante | identifiant de visiteur | 访客 id | Opaque, browser-local. Header name `X-AIMarket-Sandbox-Visitor` never translates. |
| payer (per hop) | плательщик | pagador | payeur | 付费方 | BoM field `payer`: `local` / `trial` / `channel` / `unpaid`. Values never translate. |
| federated hop | федеративный хоп | salto federado | saut fédéré | 联邦跳 | A hop the executor does not host, routed to the hub. |


## Contract-security terms (BASANOS static assurance · DOLOS contract red team)

Added with [`basanos/README.md`](https://github.com/alexar76/basanos/blob/main/README.md) and
[`dolos/README.md`](https://github.com/alexar76/dolos/blob/main/README.md). The product names
`BASANOS`, `DOLOS`, `DolosCanary`, `Foundry`, `Anvil`, `forge`, `Solidity`, `EVM`, the outcome enums
(`finding` / `no_finding` / `inconclusive`), the verdict enums (`PASS` / `REVIEW` / `FAIL`), the
document tags (`BY DESIGN`, `ADVISORY`), the probe ids (`escrow_channel_hijack`, …), the cheat codes
(`anvil_impersonateAccount`, `anvil_setBalance`, `evm_snapshot`, `evm_revert`), `chainId`, env vars
and route paths are **identifiers and never translated**.

The shared vocabulary lives in the tables above and must not be re-invented here: `finding`
(RU находка · ES hallazgo · FR **constat** · ZH 发现), `red team`, `probe`, `bounty`, `verdict`,
`canary (fixture)`, `dedup key`, `inconclusive`, `deploy gate`, `separation of duties`,
`fail closed`. In particular, FR uses **constat**, never «découverte»: the two were mixed in the
first DOLOS landing.

| EN | RU | ES | FR | ZH | Notes |
|----|----|----|----|----|-------|
| attack harness | стенд атак | banco de ataque | banc d'attaque | 攻击平台 | DOLOS's role one-liner. A harness, not a daemon: it has no standing verdict to poll. |
| fork (of a chain) | форк | fork | fork | 分叉 | RU/ES/FR keep the English root; ZH 分叉. Never «ответвление» / «bifurcación» / « embranchement ». |
| throwaway / disposable fork | одноразовый форк | fork desechable | fork jetable | 一次性分叉 | The whole safety argument. Say *disposable*, never merely «test». |
| sandbox chain | песочная цепочка | cadena sandbox | chaîne sandbox | 沙盒链 | `chainId` in `DOLOS_SANDBOX_CHAIN_IDS`, default `31337`. Keep `sandbox` Latin in ES/FR. |
| copy-on-write clone | copy-on-write-клон | clon copy-on-write | clone copy-on-write | 写时复制副本 | Keep `copy-on-write` Latin everywhere; ZH translates. |
| invariant | инвариант | invariante | invariant | 不变量 | The property a contract claims. An attack is an invariant plus a sequence that would break it. |
| held (attack outcome) | устоял | resistió | a tenu | 守住 | The contract refused the attack. Enum stays `no_finding`. |
| exploited (attack outcome) | пробит | vulnerado | percé | 被攻破 | The invariant broke. Enum stays `finding`. |
| honest negative | честный отрицательный результат | negativo honesto | négatif honnête | 诚实的否定结果 | A *held* result that refutes a static flag. Load-bearing — it is a product, not a non-event. |
| static-analysis flag | флаг статического анализа | marca del análisis estático | signalement de l'analyse statique | 静态分析标记 | What BASANOS raises and DOLOS confirms or refutes. |
| false positive | ложное срабатывание | falso positivo | faux positif | 误报 | |
| reproducer | воспроизводящая последовательность | secuencia que lo reproduce | cas de reproduction | 复现步骤 | The exact transaction sequence. Field name stays `reproducer`. |
| impersonation (chain cheat) | подмена отправителя | suplantación de dirección | usurpation d'adresse | 冒充身份 | Transacting AS an address without its key. Possible only on a fork. |
| advisory only | исключительно рекомендательный характер | solo consultivo | à titre consultatif uniquement | 仅作建议 | Reported, and it stops at the report. **Never** shorten to RU «только advisory» / «рекомендательная» — pick this one form. |
| by design (bubble affordance) | предусмотрено дизайном | por diseño | par conception | 设计使然 | An exploit that works and is intended inside the sealed bubble. Tag stays `BY DESIGN`. |
| safety boundary | граница безопасности | límite de seguridad | limite de sécurité | 安全边界 | The sandbox-only rule for auto-fix-and-redeploy. |
| auto-fix cycle | цикл автоисправления | ciclo de auto-corrección | cycle d'auto-correction | 自动修复闭环 | attack → fix → re-test → redeploy → re-attack. |
| patcher (built-in / Factory) | патчер | parcheador | correcteur | 补丁器 | Produces the patched source. Distinct from the *fixer* role in the bounty split. |
| redeploy (one contract) | передеплой | redespliegue | redéploiement | 重新部署 | Of the single contract only — never of the infrastructure. |
| touchstone | пробирный камень | piedra de toque | pierre de touche | 试金石 | BASANOS's metaphor (βάσανος, the Lydian stone). Product name stays Latin. |
| assurance pack | пакет гарантий | paquete de garantía | dossier d'assurance | 保障包 | BASANOS's signed deliverable. **Not** insurance: it makes no promise of payout. |
| pinned commit | закреплённый коммит | commit fijado | commit épinglé | 固定提交 | Commit SHA + tree digest. The signature is worthless without the pin. |
| operator playbook | инструкция оператора | manual del operador | manuel de l'opérateur | 运维手册 | ISO/IEC 29147 / 30111 next steps shipped with FAIL and REVIEW. |
| detector | детектор | detector | détecteur | 检测器 | BASANOS's static rule. The set is CLOSED: intel may reorder it, never extend it. |
| address book (realm) | адресная книга | libreta de direcciones | carnet d'adresses | 地址簿 | `universe_config.json`. Not a wallet address book. |

---

## Post-quantum migration terms (hybrid Ed25519 + ML-DSA-65)

See [docs/pqc-migration.md](pqc-migration.md). `pinned` follows the existing rendering used for
`pinned commit` — закреплённый / fijado / épinglé / 固定 — because it is the same idea applied to a
key: trusted because it was recorded out of band, not because the document says so.

| EN | RU | ES | FR | ZH | Notes |
| --- | --- | --- | --- | --- | --- |
| post-quantum cryptography (PQC) | постквантовая криптография (ПКК) | criptografía post-cuántica | cryptographie post-quantique | 后量子密码学 | Expand the abbreviation on first use per document. |
| hybrid signature | гибридная подпись | firma híbrida | signature hybride | 混合签名 | Two signatures over ONE canonical string. Not «двойная»/«doble»/«double». |
| ML-DSA-65 | ML-DSA-65 | ML-DSA-65 | ML-DSA-65 | ML-DSA-65 | FIPS 204 algorithm name. Never translate, never write «Dilithium» in prose — that is the library, not the standard. |
| downgrade attack | атака понижения (downgrade) | ataque de degradación | attaque par rétrogradation | 降级攻击 | Stripping the `pq_*` fields so a classical-only document is accepted. Gloss English once in RU. |
| downgrade guard | защита от понижения | protección contra degradación | protection contre la rétrogradation | 降级防护 | The rule that refuses a document with no `pq_value` once PQ is required. |
| pinned PQ key | закреплённый PQ-ключ | clave PQ fijada | clé PQ épinglée | 固定的 PQ 密钥 | Recorded out of band. An unpinned PQ key authenticates the document, **not** the peer — do not soften this. |
...
- METAR / TAF: [NWS — What's a METAR?](https://www.weather.gov/asos/METAR.html), [SKYbrary — Meteorological Aerodrome Report](https://skybrary.aero/articles/meteorological-aerodrome-report-metar), [Википедия — METAR](https://ru.wikipedia.org/wiki/METAR), [Wikipedia — METAR (es)](https://es.wikipedia.org/wiki/METAR), [Wikipédia — METAR](https://fr.wikipedia.org/wiki/METAR), [维基百科 — METAR](https://zh.wikipedia.org/zh-hans/METAR).
- PEGELONLINE / WSV: [ITZBund — PEGELONLINE](https://www.itzbund.de/DE/itloesungen/fachverfahren/pegelonline/pegelonline.html), [GovData — PEGELONLINE](https://data.gov.de/suche/daten/pegelonline-gewasserkundliches-informationssystem-der-wsv).
- Digitraffic / Fintraffic: [Digitraffic terms of service (CC BY 4.0)](https://www.digitraffic.fi/en/terms-of-service/), [Fintraffic — how to use data](https://www.fintraffic.fi/en/digitalservices/data-services/how?language_content_entity=en).
- USDM / NDMC: [What is the USDM?](https://droughtmonitor.unl.edu/About/WhatistheUSDM.aspx), [Drought.gov — USDM](https://www.drought.gov/data-maps-tools/us-drought-monitor).
- GeoShake: [GeoShake Open Data — developers (CC BY 4.0)](https://api.geoshake.org/developer).
- NAAD / Alert Ready / CAP-CP: [Public Safety Canada — NPAS](https://www.publicsafety.gc.ca/cnt/mrgnc-mngmnt/mrgnc-prprdnss/ntnl-pblc-lrtng-sstm-en.aspx), [Alert Ready / En Alerte](https://www.alertready.ca/), [CAP-CP](https://npas.ca/cap-cp/).
- DONKI / SWPC: [NASA CCMC DONKI](https://kauai.ccmc.gsfc.nasa.gov/DONKI/), [NOAA SWPC](https://www.swpc.noaa.gov/).
