...
| `GAIA_CORS_ORIGINS` | (empty) | Comma-separated browser origins. Empty = fail closed: no cross-origin reads. Server-to-server callers send no `Origin` and are unaffected. |
