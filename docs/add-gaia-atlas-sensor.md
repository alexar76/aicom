...
| `openaq` | LIVE | `--device-id` + host **free** `GAIA_OPENAQ_API_KEY` from [explore.openaq.org](https://explore.openaq.org) |
...
python3 -m pytest tests/test_om_mesh_catalog_sync.py tests/test_sc_mesh_catalog_sync.py tests/test_extra_sensors_catalog_sync.py -q
```

Catalogs synced: `om_mesh_cities.yaml`, `sc_mesh_cities.yaml`, `extra_sensors.yaml`.
...
| Alien Monitor | `atlas` on [monitor.modelmarket.dev/](https://monitor.modelmarket.dev/) |
