...
| «Pipeline долго пишет Connecting / try N of 8» | **Pipeline** (подождать до 5 мин на попытку) | Полоса *Connection phase* — это **ретраи HTTP**, не % каталога; после ответа — *N / total* | ![Pipeline](./assets/screenshots/admin-pipeline.png) |
...
| «COMPLETED, но нет на витрине» | **Pipeline** → карточка → storefront gates | `storefront_gate_reasons`, quality, код на диске | ![Pipeline](./assets/screenshots/admin-pipeline.png) |
...
| «Откуда брать идеи автоматически» | **Discovery** | Очередь ranked ideas, auto-enqueue в Settings | ![Discovery](./assets/screenshots/admin-discovery.png) |
