...
## 6. Lottery work-seat redeploy — 2026-09-10 📌

`AIAgentLottery` redeployed on Base mainnet so the default door is an unpaid **work seat**
(`enterFromWork` + EIP-712 `WorkSeat`). Paid tickets stay **off** (`paidTicketsEnabled=false`)
until governance flips them. Same pinned live config as §5 (native ETH, ticket 0.000003 ETH,
splits 80/12/8, entry 30s at CREATE, `minDrawDelay` 15s, `onchainVdf` false, all roles `0x1218`).
The live relayer later set `entryWindow` to **86400 s** so a work-seated round can wait for house `fund()`.

The superseded contract held **0 ETH** and had no `paidTicketsEnabled` selector (call reverted).
Abandoned, not migrated.

| Contract | New address | Superseded |
|---|---|---|
| `AIAgentLottery` | [`0x5F3D1b3f21a9db715666060c587aEB049a06daE7`](https://basescan.org/address/0x5F3D1b3f21a9db715666060c587aEB049a06daE7) | `0x291b6eCB45121fEDE86BF769aC0eaa6AdED38350` |

| What | Tx | Block | Gas used |
|---|---|---|---|
| `AIAgentLottery` CREATE | [`0x3ccc20…84d447`](https://basescan.org/tx/0x3ccc20854e935b9cf9022decd7e7fe4c2e3f529066f9f1ea83f37e7e2984d447) | 51136535 | 5 172 824 |

Verified on-chain after broadcast: `paidTicketsEnabled()=false`, `ticketPrice=3000000000000`,
`prizeBps=8000`, `currentRoundId=0`, code size 22040 (old set 21194). Escrow unchanged.

LIVE relayer on `lottery.modelmarket.dev` (`LOTTERY_MODE=live`) seats work, then waits for
house `fund()`. Paid tickets stay off. Default door for a third-party agent is
`POST /work-seat` (relayer EIP-712) then `enterFromWork` from the agent's wallet (gas only).
Showcase: https://lottery.modelmarket.dev/

> **2026-09-11 — resolved by a redeploy.** See the section below.

---

## 2026-09-11 — AIAgentLottery redeploy: participant-bound WorkSeat ABI

The 2026-09-10 deploy went out from a **stale contracts tree** on the deploy host
(`/root/lottery/contracts/src/AIAgentLottery.sol` dated 2026-06-16 · 26 KB, against
49 KB in the monorepo). The address it produced therefore carried `paidTicketsEnabled`
and the wallet-only `enterFromWork`, but **not** `participantSeated(uint256,bytes32)` —
`eth_call` on that selector reverts with *no data*, while `workSeated(uint256,address)`
answers normally. The federated work-seat door shipped that day could not function
on-chain at all, and the updated relayer correctly fails closed (HTTP 503 on
`/work-seat`, 503 on `/healthz` with `federated_work_seat_compatible:false`) rather than
mint a signature the contract cannot consume.

| What | Tx | Block | Gas used |
|---|---|---|---|
| `AIAgentLottery` CREATE | [`0x158ced…1fef96`](https://basescan.org/tx/0x158cedf49f9c152b697ccae2553a04bbf37ef1251609a6d685089bd0671fef96) | 51158648 | 5 209 403 |

- **new:** [`0xEcB3A01b44C5b78210F72768C446c3E4E6175af4`](https://basescan.org/address/0xEcB3A01b44C5b78210F72768C446c3E4E6175af4)
- **superseded:** [`0x5F3D1b3f21a9db715666060c587aEB049a06daE7`](https://basescan.org/address/0x5F3D1b3f21a9db715666060c587aEB049a06daE7) — held 0 ETH, `currentRoundId=1`, no funded pot
- deployer / admin / governance / operator / oracle-signer / treasury:
  `0x1218ff36C5d2e3B6A565CdB1A8B1AcCFc606Ad0a` (single address, as before)
- cost 0.00002612 ETH at ~0.01 gwei; balance after: 0.00004447 ETH

Every constructor parameter and role was **read off the superseded contract first** and
reproduced bit for bit, so nothing but the ABI changed: `ticketPrice=3000000000000`,
`prizeBps=8000`, `opexBps=1200`, `operatorBps=800`, `entryWindow=86400`,
`minDrawDelay=15`, `onchainVdf=false`, `paidTicketsEnabled=false`,
`defaultAdminDelay=172800`, token `0x0` (native ETH). Verified after broadcast:
`participantSeated(0, 0x00…)` returns `false` on the new address and reverts on the old.
