...
- [alexar76/argus](https://github.com/alexar76/argus) [![alexar76/argus3 MCP server](https://glama.ai/mcp/servers/alexar76/argus3/badges/score.svg)](https://glama.ai/mcp/servers/alexar76/argus3) 🎖️ 📇 🏠 ☁️ 🍎 🪟 🐧 - **ARGUS-3 as a stdio MCP server** (`argus mcp` → `argus_ask`, `argus_status`). **WARDEN** vets third-party MCP servers before any tool runs (LUMEN-scored firewall, tool-def pinning, drift sentinel). Distinct from `aimarket-oracle-gateway` (oracle tools) and `aimarket-plugins` (hub packager). npm `@alexar76/argus3` · [live](https://magic-ai-factory.com/argus/).
...
- Glama: https://glama.ai/mcp/servers/alexar76/argus3 (after listing)
