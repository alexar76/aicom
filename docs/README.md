...
> ### 🟢 Try it in 30 seconds — paste **`https://modelmarket.dev/mcp`** into any MCP client
> No wallet, no Docker, no clone: you get `market_search` + `market_invoke` against the live
> hub, a few free trial invokes per caller, a signed receipt for each, then **402**.
> → **[`hosted-mcp-endpoint.md`](hosted-mcp-endpoint.md)**
...
| [`aimarket-hub`](https://pypi.org/project/aimarket-hub/) | PyPI | `pip install aimarket-hub` | Reference federation hub server — **v3.2.x** |
| [`@aimarket/agent`](https://www.npmjs.com/package/@aimarket/agent) | npm | `npm install @aimarket/agent` | TypeScript SDK — **v0.2.x line** (see [`sdk-version-policy.md`](./sdk-version-policy.md)) |
| [`aimarket-agent`](https://crates.io/crates/aimarket-agent) | crates.io | `aimarket-agent = "0.2"` | Rust SDK — v0.2.x |
| [`aimarket-agent`](https://pub.dev/packages/aimarket_agent) | pub.dev | `dart pub add aimarket_agent` | Dart SDK — v0.2.x |
...
**AWR work receipts** — the two sides are deliberately separate packages, because a component that
both issues receipts and judges them is not evidence of anything. Guide:
[`awr-receipts.md`](./awr-receipts.md). All four are zero-dependency on the JavaScript side.

| Package | Registry | Install | Purpose |
|---|---|---|---|
| [`awr`](https://pypi.org/project/awr/) | PyPI | `pip install awr` | **Verifier** + reference implementation, §17 CLI — v2.0.0 |
| [`@alexar76/awr-verify`](https://www.npmjs.com/package/@alexar76/awr-verify) | npm | `npm i @alexar76/awr-verify` | **Verifier** for Node and the browser; the code that runs at verify.modelmarket.dev — v2.0.0 |
| [`awr-emitter`](https://pypi.org/project/awr-emitter/) | PyPI | `pip install awr-emitter` | **Emitter** + LangChain/LangGraph callback — v2.0.0 |
| [`@alexar76/awr-emit`](https://www.npmjs.com/package/@alexar76/awr-emit) | npm | `npm i @alexar76/awr-emit` | **Emitter** + MCP tool-call wrapper — v2.0.0 |

**Hub plugins (PyPI):** `aimarket-tee`, `aimarket-channels`, `aimarket-reputation`, `aimarket-safety`, `aimarket-mcp-packager` — [`plugins/docs/install.md`](https://github.com/alexar76/aimarket-plugins/blob/main/plugins/docs/install.md). The canonical in-hub plugin is [`aimarket-provenance`](https://pypi.org/project/aimarket-provenance/) (v2.0.0), which issues AWR/2 receipts on hub invokes.

**Version policy:** [`sdk-version-policy.md`](./sdk-version-policy.md) — why Python is 2.x and Dart/TS/Rust are 0.2.x.
...
| **ARGUS wiki** (install, WARDEN, channels) | **EN** — [github.com/alexar76/argus/wiki](https://github.com/alexar76/argus/wiki) · source: [`scripts/wiki-argus/`](https://github.com/alexar76/argus/wiki) |
...
Operator-facing UI strings may be **en / ru / es** in Admin; that is product i18n, not documentation locale. Wiki language policy: [`scripts/wiki-gitea/Languages.md`](https://github.com/alexar76/aicom/wiki/Languages).

**In-app docs** at `/docs` (Next.js) use JSON language packs in [`web/frontend/language-packs/docs/`](../web/frontend/language-packs/docs/) — regenerate with `python3 scripts/generate_docs_i18n.py`. Wiki: [`Ecosystem`](https://github.com/alexar76/aicom/wiki/Ecosystem) · [`Oracles`](https://github.com/alexar76/aicom/wiki/Oracles) ([RU](https://github.com/alexar76/aicom/wiki/Oracles-RU) · [ES](https://github.com/alexar76/aicom/wiki/Oracles-ES)).
...
- **`observability-prometheus.md`** (+ `.ru` / `.es` / `.fr` / `.zh`) — fleet **Prometheus scrape**, Hub `/metrics`, Grafana ecosystem dashboards, and Skopos **Observability** page (`SKOPOS_PROMETHEUS_URL`).
...
- **`hosted-mcp-endpoint.md`** — **`https://modelmarket.dev/mcp`**: the zero-install way in. Client setup snippets, the per-caller trial and why its identity must not be shared, how the same gateway serves peers at `/ai-market/mcp`, plus deploying it (`scripts/deploy_hub_rebuild.sh`) and watching it (`scripts/payment_canary.py`).
...
- **[`ecosystem/hub-vps-competing.md`](./ecosystem/hub-vps-competing.md)** — Competing lab VPS + federation mesh + second Monitor galaxy (**[RU](./ecosystem/hub-vps-competing-ru.md)** · **[ES](./ecosystem/hub-vps-competing-es.md)** · **[FR](./ecosystem/hub-vps-competing-fr.md)** · **[ZH](./ecosystem/hub-vps-competing-zh.md)**).
...
- **[`awr-receipts.md`](./awr-receipts.md)** — **AWR work receipts**: what a signed receipt does and does not prove, emit and verify in Node or Python, the MCP wrapper, profiles L0–L2, and the three implementations that agree on 354 conformance vectors. Normative definition: [`awr/SPEC.md`](../awr/SPEC.md) (EN · [RU](./awr-receipts.ru.md) · [ES](./awr-receipts.es.md) · [FR](./awr-receipts.fr.md) · [ZH](./awr-receipts.zh.md)).
