...
| **Hub LIVE** | Marketplace federado en [modelmarket.dev](https://modelmarket.dev) | `modelmarket-hub` |
| **Hub UNI** | Catálogo paralelo sellado en [uni.modelmarket.dev](https://uni.modelmarket.dev) — [uni-and-live.es.md](./uni-and-live.es.md) | `deploy/uni-hub.sh` |
...
| **Alien Monitor UNI** | Grafo 3D de la burbuja — [monitor.modelmarket.dev](https://monitor.modelmarket.dev/) · `:9100` · `ALIEN_MODE=universe` | `alien-monitor` |
| **Alien Monitor LIVE** | Grafo 3D del dinero en vivo — [`/monitor-live/`](https://monitor.modelmarket.dev/) · `:9101` · `ALIEN_MODE=real` | `alien-monitor-live` |
...
**No incluido en `deploy_ecosystem.sh`:** Metis, DIOSCURI, HELIOS — por separado; ver [§Qué no incluye un VPS](#9-qué-no-incluye-un-vps). El hub UNI + seis laboratorios son una **segunda flota**: `deploy/uni-hub.sh` y `deploy/uni-satellites.sh`. No apuntes el monitor UNI al hub en vivo. División: [uni-and-live.es.md](./uni-and-live.es.md).
...
El Alien Monitor público se sirve en `https://monitor.modelmarket.dev/` (nginx hace de proxy de
