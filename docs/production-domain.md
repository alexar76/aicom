...
### Alien Monitor (LIVE + UNI subdomains)

| Map | URL | Upstream |
|-----|-----|----------|
| **LIVE** | https://monitor.modelmarket.dev/ | `:9101` `ALIEN_MODE=real` |
| **UNI** | https://monitor-uni.modelmarket.dev/ | `:9100` `ALIEN_MODE=universe` |

Factory bookmarks redirect: `/monitor/` → UNI subdomain, `/monitor-live/` → LIVE subdomain.

```bash
./scripts/deploy_alien_monitor.sh
```

Nginx: `deploy/nginx/monitor.modelmarket.dev.conf`, `deploy/nginx/monitor-uni.modelmarket.dev.conf`, factory redirects in `deploy/nginx/snippets/alien-monitor.conf`. TLS via certbot webroot; renews with `certbot.timer`.
