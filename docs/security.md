...
> **Disclaimer:** [magic-ai-factory.com](https://magic-ai-factory.com) is a **shared demonstration**. Admin login is **passwordless** (`admin`, empty password). The instance is **not** a private factory.
...
The admin UI reads `public_demo` from `GET /api/admin/auth/me` and shows a banner on **Settings**; backup/restore controls are hidden. Login page uses `GET /api/public/demo-config` to hide the password field when `passwordless_admin` is true.
...
| **Login rate limit persistence (H-5)** | Failed admin logins stored in SQLite **`data/state/security_store.db`** (`core/persistent_security_store.py`). Survives container restart; in-memory fallback if DB unavailable. Details: **[security-persistence.md](./security-persistence.md)** (RU/ES companions). |
| **OIDC nonce replay (H-3)** | After JWT `id_token` nonce check, nonce is atomically claimed in the same SQLite store (TTL ≤ 1h). Replay → login rejected; store failure → fail-open (logins not blocked). Override path: `AIFACTORY_SECURITY_STORE_DB`. See **[security-persistence.md](./security-persistence.md)**. |
...
| `tests/test_persistent_security_store.py` | Rate-limit persistence, nonce replay, SecurityManager integration |
| `tests/test_oidc_nonce_replay.py` | OIDC `verify_id_token` first-use vs replay |
