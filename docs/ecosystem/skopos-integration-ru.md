...
**SKOPOS** ([`skopos/`](../../skopos/)) — спутник **наблюдаемости флота** AICOM: аналитика nginx и Apache по SSH, Security Center, история сканирований и AI-аналитик. Self-hosted; для продакшена рекомендуется PostgreSQL.
...
| **Alien Monitor** | [magic-ai-factory.com/monitor/](https://magic-ai-factory.com/monitor/) | Узел 3D-графа — кликните по сфере **SKOPOS** |
...
Продакшен-стек для тестов: [`metis/deploy/skopos-test/`](../../metis/deploy/skopos-test/).
...
Nginx-vhost `skopos.modelmarket.dev` находится в [`metis/deploy/nginx.conf`](../../metis/deploy/nginx.conf) — проксирует `:8501` (UI) и `:8502` (`/healthz`).
...
SKOPOS может **продавать интеллект флота** другим ИИ-агентам через [AIMarket Protocol v2](../../aimarket-protocol/spec.md) — по той же схеме, что и Metis `/aimarket/invoke`.
...
| `POST /aimarket/invoke` | Контракт вызова хаба `{input, product_id, capability_id}` → `{result}` |
...
Задайте `SKOPOS_HUB_URL`, чтобы SKOPOS мог **обнаруживать** возможности хаба (бесплатный поиск). Платные вызовы от SKOPOS к оракулам требуют интеграции кошелька (в будущем); в автономном режиме отсутствие хаба игнорируется.
