...
| **GitHub** `alexar76/dioscuri` | Публичное зеркало — доки, CI, справочник для самостоятельного размещения |
...
## Развёртывание на oracle-хосте (oracles.modelmarket.dev)

DIOSCURI работает на **oracle VPS**, а не на фабричном флоте (`modeldev.modelmarket.dev`). Factory Monitor опрашивает только `http://oracles.modelmarket.dev:8790/health`.
...
Требуется беспарольный `ssh root@oracles.modelmarket.dev` (или `ssh oracle` — см. `~/.ssh/config`). Аутентификация для **git push** использует `data/secrets/git-credentials` (как и `mirror_to_gitea.sh`), а не SSH.
...
Те, кто размещает у себя, задают приглашения в `dioscuri.config.json`:
...
Castor/Pollux анонсируют главы; они **не** выдают себя за Theoros. Полное руководство: **[dioscuri/docs/theoros.md](https://github.com/alexar76/dioscuri/blob/main/docs/theoros.md)** (архитектура, voice charter, конфиг, чек-лист).
