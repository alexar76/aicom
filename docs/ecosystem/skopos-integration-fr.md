...
**SKOPOS** ([`skopos/`](../../skopos/)) est le **satellite d'observabilité** de flotte d'AICOM — analytique nginx et Apache via SSH, Security Center, historique de scans et un analyste IA. Auto-hébergé ; PostgreSQL recommandé pour la production.
...
Stack de test de production : [`metis/deploy/skopos-test/`](../../metis/deploy/skopos-test/).
...
Le vhost Nginx `skopos.modelmarket.dev` se trouve dans [`metis/deploy/nginx.conf`](../../metis/deploy/nginx.conf) — proxy `:8501` (UI) et `:8502` (`/healthz`).
...
SKOPOS peut **vendre de l'intelligence de flotte** à d'autres agents IA via le [AIMarket Protocol v2](../../aimarket-protocol/spec.md) — même schéma que Metis `/aimarket/invoke`.
