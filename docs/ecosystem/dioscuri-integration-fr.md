...
| **GitHub** `alexar76/dioscuri` | Miroir public — docs, CI, référence self-host |
...
## Déploiement sur l'hôte oracle (78.17.126.214)

DIOSCURI tourne sur le **VPS oracle**, pas sur la flotte de la fabrique (`5.129.212.122`). Factory Monitor ne sonde que `http://78.17.126.214:8790/health`.
...
Nécessite un `ssh root@78.17.126.214` sans mot de passe (ou `ssh oracle` — voir `~/.ssh/config`). L'authentification pour **git push** utilise `data/secrets/git-credentials` (comme `mirror_to_gitea.sh`), pas SSH.
