...
**SKOPOS** ([`skopos/`](https://github.com/alexar76/skopos)) is the fleet **observability satellite** of AICOM — nginx & Apache analytics over SSH, Security Center, scan history, an AI analyst, and an **Observability** page (Prometheus PromQL KPIs + 3D service graph; see [observability-prometheus.md](../observability-prometheus.md)). Self-hosted; PostgreSQL recommended for production.

> 🌐 Languages: **English** · [Русский](./skopos-integration-ru.md) · [Español](./skopos-integration-es.md) · [Français](./skopos-integration-fr.md) · [中文](./skopos-integration-zh.md)
...
| **Alien Monitor** | [monitor.modelmarket.dev/](https://monitor.modelmarket.dev/) | 3D graph node — click **SKOPOS** sphere |
...
Production test stack: [`metis/deploy/skopos-test/`](https://github.com/alexar76/metis/tree/main/deploy/skopos-test/).
...
Nginx vhost `skopos.modelmarket.dev` is in [`metis/deploy/nginx.conf`](https://github.com/alexar76/metis/blob/main/deploy/nginx.conf) — proxy `:8501` (UI) and `:8502` (`/healthz`).
...
SKOPOS can **sell fleet intelligence** to other AI agents via the [AIMarket Protocol v2](https://github.com/alexar76/aimarket-protocol/blob/main/spec.md) — same pattern as Metis `/aimarket/invoke`.
