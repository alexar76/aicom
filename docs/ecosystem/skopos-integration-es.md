...
**SKOPOS** ([`skopos/`](https://github.com/alexar76/skopos)) es el **satélite de observabilidad de flota** de AICOM: analítica nginx y Apache vía SSH, Security Center, historial de escaneos y un analista IA. Autoalojado (self-hosted); se recomienda PostgreSQL para producción.
...
| **Alien Monitor** | [monitor.modelmarket.dev/](https://monitor.modelmarket.dev/) | Nodo del grafo 3D — haz clic en la esfera **SKOPOS** |
...
Stack de prueba de producción: [`metis/deploy/skopos-test/`](https://github.com/alexar76/metis/tree/main/deploy/skopos-test/).
...
El vhost de Nginx `skopos.modelmarket.dev` está en [`metis/deploy/nginx.conf`](https://github.com/alexar76/metis/blob/main/deploy/nginx.conf) — proxy `:8501` (UI) y `:8502` (`/healthz`).
...
SKOPOS puede **vender inteligencia de flota** a otros agentes IA vía el [AIMarket Protocol v2](https://github.com/alexar76/aimarket-protocol/blob/main/spec.md) — el mismo patrón que Metis `/aimarket/invoke`.
...
Configura `SKOPOS_HUB_URL` para que SKOPOS pueda **descubrir** capacidades del hub (búsqueda gratuita). Las invocaciones de pago de SKOPOS a los oráculos requieren una integración de cartera (wallet) (futuro); el modo autónomo ignora la ausencia del hub.
