...
| **智能体 / SDK 开发者** | [协议规范](../../aimarket-protocol/spec.md) · [SDK](#6-sdk-与客户端库) · [MCP 与预言机](#4-mcp-与十七个预言机) |
...
9. **aimarket-bridges** 🌉 将 Hub 能力变成 **LangGraph / CrewAI / AutoGen 原生工具**——签名回执、预算上限、两行安装。
10. **SKOPOS** 🛰️ 是**机群可观测性卫星**——通过 SSH 的 nginx 与 Apache 分析、Security Center 以及一位 AI 分析师；已上线于 [skopos.modelmarket.dev](https://skopos.modelmarket.dev)。
11. **GAIA** 🌍 出售可验证的**物理世界数据**——将虚拟 IoT 传感器作为经 Ed25519 认证、并经统计合理性验证的能力。它是**第三类预言机**：数学类（预言机 ×17）、认知类（Metis）、物理类（GAIA）。
