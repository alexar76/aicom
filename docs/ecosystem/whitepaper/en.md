...
flowchart TB
  ...
  subgraph MACHINE["Autonomous machine economy"]
    ...
    BASANOS["🪨 BASANOS<br/>Solidity scan · PASS/REVIEW/FAIL"]
    DOLOS["⚔️ DOLOS<br/>EVM fork red team · sandbox fix-loop"]
    ...
    CHAIN -->|"fork Anvil"| DOLOS
    DOLOS -->|"findings · confirm/refute static"| BASANOS
    HUB <--> MESH
    ORACLES --> HUB
    GAIA --> HUB
    GAIA --> ATLAS
...
flowchart TB
  ...
  subgraph DOLOS_RT["DOLOS · dynamic EVM red team"]
    DL1["Fork UNI Anvil · exploit txs<br/>Ed25519 findings · sandbox fix-loop"]
  end
  ...
  CH2 -->|"fork UNI Anvil"| DL1
  DL1 -.->|"confirm / refute static"| BA1
  ORACLES --> HUB
  GAIA --> HUB
  GAIA --> ATLAS
...
**WARDEN:** ARGUS is the reference host for the standalone [`@aimarket/warden`](https://github.com/alexar76/warden) library (§3.7a). In ARGUS: static scan → threat feed → LUMEN reputation (degrades neutral offline) → pinning → sandbox. [`argus/docs/security-warden.md`](https://github.com/alexar76/argus/blob/main/docs/security-warden.md)
...
### 3.7a WARDEN — MCP security firewall (library)

**Role:** Standalone TypeScript **library** (not an MCP server) that vets third-party MCP servers **before their tool definitions reach any host's model**. Extracted from ARGUS at `@aimarket/warden` 0.3.0 so Cursor, Claude Desktop, or a custom agent can adopt the same gate chain without adopting an agent runtime.

**Gate chain:** static tool-def scan → signed threat feed (optional remote; built-in deny-list offline) → origin (declared servers only) → tool-def pinning (drift detection). Returns a recorded verdict: allow/block, score, findings, per-tool partition, ruleset digest.

**Zero runtime dependencies** — only `node:crypto`. ARGUS is the reference integration; MOMUS can publish into the threat feed; Alien Monitor visualizes the gate chain as an ecosystem node.

**Package:** [`warden/`](https://github.com/alexar76/warden) → [alexar76/warden](https://github.com/alexar76/warden) · [landing](https://warden.modelmarket.dev/)

**Field survey (2026-08-24):** 1,108 public registry servers that answered `tools/list` — 50 blocked, 4 substantiated; full false-positive breakdown in [`docs/mcp-survey.md`](https://github.com/alexar76/warden/blob/main/docs/mcp-survey.md) ([RU](https://github.com/alexar76/warden/blob/main/docs/mcp-survey.ru.md) · [ES](https://github.com/alexar76/warden/blob/main/docs/mcp-survey.es.md) · [FR](https://github.com/alexar76/warden/blob/main/docs/mcp-survey.fr.md) · [ZH](https://github.com/alexar76/warden/blob/main/docs/mcp-survey.zh.md)).

**Docs:** [`docs/gates.md`](https://github.com/alexar76/warden/blob/main/docs/gates.md) · [`docs/integration.md`](https://github.com/alexar76/warden/blob/main/docs/integration.md) · [`docs/threat-feed.md`](https://github.com/alexar76/warden/blob/main/docs/threat-feed.md)
...
**Live demo:** [monitor.modelmarket.dev/](https://monitor.modelmarket.dev/)
...
### 3.13a HORKOS — escrow policy signer

**Role:** The **only process** authorized in `AIMarketEscrow.authorizedHubs` — the Hub holds no escrow-signing key. Signs exactly one calldata shape: canonical `debitChannel(bytes32,uint256,bytes32,uint256,bytes)` (selector `0xf7becd80`) to the pinned escrow on Base mainnet (8453), with `value == 0`. Refuses settlements, transfers, hub-authorization changes, other chains, and contract creation.

**Amount authority:** the depositor's EIP-712 signature, re-verified here against channel state read locally — with this service's address substituted for the `hub` field, as the contract does. `AIMARKET_ESCROW_SUBMIT_STRATEGY=external` without enumerated policy refusals (rules R1–R26) buys little against a compromised Hub bearer token.

**Host boundary:** runs on the skopos box behind a reverse SSH tunnel — **not** on the Hub host. Residual liveness risk: a compromised Hub can withhold debits without signing anything (no payout to itself).

**URL:** landing [alexar76.github.io/escrow-signer](https://alexar76.github.io/escrow-signer/) · source [`alexar76/escrow-signer`](https://github.com/alexar76/escrow-signer)
