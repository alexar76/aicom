...
>
> **Languages:** [English](./en.md) · Русский · [Español](./es.md) · **См. также:** [Экономика протокола AIMarket](../../aimarket-whitepaper.md) · [Архитектура экосистемы](../../ecosystem-architecture.md) · [Руководство оператора Factory](../../USER_GUIDE.md)
...
| [`aimarket-protocol/`](../../../aimarket-protocol/) | **Протокол v2** spec + схемы | — (нормативная документация) | `aimarket-protocol` |
| [`plugins/`](../../../plugins/) | **14× плагинов Hub** | загружаются Hub | один репозиторий на плагин |
| [`ai-service-mesh/`](../../../ai-service-mesh/) | **AI Service Mesh** | `:8090` | `ai-service-mesh` |
| [`oracles/`](../../../oracles/) | **17 оракулов** + портал | [oracles.modelmarket.dev](https://oracles.modelmarket.dev) | `oracles` |
| [`gaia/`](../../../gaia/) | **GAIA физические оракулы** | `:9320` | `gaia` |
| [`argus/`](../../../argus/) | **ARGUS-3** | установка через лендинг Factory | `argus` |
| [`alien-monitor/`](../../../alien-monitor/) | **Alien Monitor** | `/monitor/` · `:9100` | `alien-monitor` |
| [`apps/pulse-terminal/`](../../../apps/pulse-terminal/) | **Pulse Terminal** | `/pulse/` · `:5199` | вместе с `acex` |
| [`acex/`](../../../acex/) | **ACEX** капитальный слой | контракты + Pulse API | `acex` |
...
**Интеграция с протоколом:** Поставляет шлюз протокола v1 (402, MCP, прямой invoke) и экспортирует `/.well-known/ai-market.json`. `factory_bridge` Hub зеркалирует продукты конвейера в федеративный каталог ([`aimarket-hub/aimarket_hub/factory_bridge.py`](https://github.com/alexar76/aimarket-hub/blob/main/aimarket_hub/factory_bridge.py)).
...
**Роль:** Федеративный хаб — индексирует возможности от Factory, пиринговых хабов и автономных провайдеров; маршрутизирует `POST /ai-market/v2/invoke`; запускает конвейер плагинов (safety, channels, reputation, TEE, ZK); рассчитывает платёжные каналы ончейн при включённом блокчейне.
...
| Контроль | Механизм |
|----------|----------|
| **Stake** | `POST /ai-market/v2/supply/stake` — минимальный депозит (по умолчанию $10) до публикации |
| **Anti-spam** | Лимиты публикаций на издателя |
| **LUMEN trust** | `lumen.reputation@v1` оценивает издателей по stake и графу invoke |
| **Подписанные ответы** | Провайдер подписывает объект `result`; хаб проверяет `X-Provider-Signature` (Ed25519) |
| **Пороги discover/invoke** | Низкий trust и дубликаты `invoke_url` отфильтровываются в search; invoke ниже `AIMARKET_SUPPLY_MIN_TRUST_INVOKE` блокируется |
| **Slash** | Неудачные invoke могут слэшить stake и эмитировать федеративные slash-аттестации |

ARGUS фильтрует discover по `ARGUS_MIN_HUB_TRUST` (по умолчанию `0.25`). Гайд разработчика: [`argus/docs/developer-guide/`](https://github.com/alexar76/argus/tree/main/docs/developer-guide/) (20 языков). Справка: [`aimarket-hub/docs/supply-security.md`](https://github.com/alexar76/aimarket-hub/blob/main/docs/supply-security.md).
...
**MCP и экономика:** ARGUS — MCP **сервер** (`argus mcp`) и **клиент** (сторонние MCP через WARDEN). 17 оракулов нативными tools; **продажа возможностей** — `argus economy register` + `argus serve`. [`mcp-oracles-capabilities.md`](../../../argus/docs/mcp-oracles-capabilities.md) · [wiki ARGUS](https://github.com/alexar76/argus/wiki)
...
sequenceDiagram
  ...
  Hub->>Escrow: депозит USDC (истечение 24ч)
  ...
  Hub->>Escrow: дебет (EIP-712 DebitAuthorization)
  Agent->>Hub: POST /ai-market/v2/channel/close
  Hub->>Escrow: расчёт — комиссия хаба + возврат неиспользованного
  Escrow-->>Agent: возврат остатка
```

### 4.2 Правила канала эскроу
...
- **Расчёт** выплачивает хабу `usedAmount`, возвращает остаток. **Истечение** разрешено без разрешений и экономически идентично — вкладчик не может уклониться от оплаты ожиданием.
...
4. Пиринговые хабы подтягивают логи аттестаций; кросс-хабовый слэш требует **proof-of-misbehavior** (спор с подписью потребителя) — хабы не могут подделать обе половины.
...
| **MCP, 17 оракулов и продажа** | [`mcp-oracles-capabilities.md`](../../../argus/docs/mcp-oracles-capabilities.md) |
| **Юмор + мульт** | [`humor/`](../../../argus/docs/user-guide/humor/) · [cartoon](https://magic-ai-factory.com/argus/humor-cartoon.html) |
| **Установка** | `curl -fsSL https://magic-ai-factory.com/install \| bash` |
...
- Закрыть **Known Issues** ([`docs/known-issues.md`](../../known-issues.md)), блокирующие mainnet TVL:
  - **KI-2** — внешний аудит смарт-контрактов (Escrow, NFT, программа Solana, ZK-схема).
