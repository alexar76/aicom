...
flowchart TB
  ...
  subgraph MACHINE["Автономная машинная экономика"]
    ...
    BASANOS["🪨 BASANOS<br/>скан Solidity · PASS/REVIEW/FAIL"]
...
flowchart TB
  ...
  aicom -->|деревья Solidity| basanos
...
**WARDEN:** ARGUS — эталонный хост для отдельной библиотеки [`@aimarket/warden`](https://github.com/alexar76/warden) (§3.7a). В ARGUS: static scan → threat feed → репутация LUMEN (деградирует до нейтральной оффлайн) → pinning → sandbox. [`argus/docs/security-warden.md`](https://github.com/alexar76/argus/blob/main/docs/security-warden.md)
...
### 3.7a WARDEN — MCP-файрвол (библиотека)

**Роль:** Отдельная TypeScript-**библиотека** (не MCP-сервер), которая проверяет сторонние MCP-серверы **до того, как их tool-def попадут в контекст модели любого хоста**. Вынесена из ARGUS в `@aimarket/warden` 0.3.0 — Cursor, Claude Desktop или свой агент могут подключить ту же цепочку шлюзов без runtime агента.

**Цепочка:** static scan tool-def → подписанный threat feed (опционально удалённый; встроенный deny-list оффлайн) → origin (только объявленные серверы) → pinning (детект дрейфа). Возвращает записываемый вердикт: allow/block, score, findings, разбиение по tools, digest ruleset.

**Ноль runtime-зависимостей** — только `node:crypto`. ARGUS — эталонная интеграция; MOMUS может публиковать в threat feed; Alien Monitor показывает цепочку как узел экосистемы.

**Пакет:** [`warden/`](https://github.com/alexar76/warden) → [alexar76/warden](https://github.com/alexar76/warden) · [лендинг](https://warden.modelmarket.dev/)

**Полевой обзор (2026-08-24):** 1 108 публичных серверов реестра, ответивших на `tools/list` — 50 заблокировано, 4 подтверждено; разбор false positive в [`docs/mcp-survey.ru.md`](https://github.com/alexar76/warden/blob/main/docs/mcp-survey.ru.md).

**Доки:** [`docs/gates.ru.md`](https://github.com/alexar76/warden/blob/main/docs/gates.ru.md) · [`docs/integration.ru.md`](https://github.com/alexar76/warden/blob/main/docs/integration.ru.md)
...
**Живое демо:** [monitor.modelmarket.dev/](https://monitor.modelmarket.dev/)
...
### 3.13a HORKOS — подписант политики эскроу

**Роль:** **Единственный процесс**, авторизованный в `AIMarketEscrow.authorizedHubs` — Hub не держит ключ подписи эскроу. Подписывает ровно одну форму calldata: канонический `debitChannel(bytes32,uint256,bytes32,uint256,bytes)` (селектор `0xf7becd80`) на закреплённый escrow в Base mainnet (8453), с `value == 0`. Отказывает settlement, transfer, смену hub-authorization, другие сети и создание контрактов.

**Полномочие на сумму:** EIP-712 подпись депозитора, повторно проверенная здесь по состоянию канала, прочитанному локально — с подстановкой адреса этого сервиса вместо поля `hub`, как в контракте. `AIMARKET_ESCROW_SUBMIT_STRATEGY=external` без перечисленных отказов политики (правила R1–R26) почти ничего не даёт против скомпрометированного bearer-токена Hub.

**Граница хоста:** работает на skopos box за reverse SSH tunnel — **не** на хосте Hub. Остаточный риск liveness: скомпрометированный Hub может не подписывать debits (без выплаты себе).

**URL:** лендинг [alexar76.github.io/escrow-signer](https://alexar76.github.io/escrow-signer/) · исходники [`alexar76/escrow-signer`](https://github.com/alexar76/escrow-signer)
