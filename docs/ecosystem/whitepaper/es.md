...
| **Monitor** | [monitor.modelmarket.dev/](https://monitor.modelmarket.dev/) | Visualizador 3D del ecosistema |
...
flowchart TB
  ...
  subgraph MACHINE["Economía autónoma de máquinas"]
    ...
    HESTIA["🔥 HESTIA<br/>hogar · runtime alojado"]
    ...
    BASANOS["🪨 BASANOS<br/>escaneo Solidity · PASS/REVIEW/FAIL"]
...
flowchart TB
  ...
  subgraph systems["Sistemas"]
    ...
    hestia["HESTIA<br/>Hogar · runtime alojado"]
    basanos["BASANOS<br/>Piedra de toque Solidity"]
    hephaestus["HEPHAESTUS<br/>Forja de cadenas · studio"]
    logos["LOGOS<br/>Analítica federada de solo lectura"]
  ...
  builder -->|despliegue firmado| hestia
  hestia -->|anuncio explícito| aicom
  enduser -->|componer cadenas| hephaestus
  hephaestus -->|search · invoke| aicom
  aicom -->|árboles Solidity| basanos
...
flowchart TB
  ...
  subgraph HEARTH["HESTIA · hogar · runtime alojado :9480"]
    HS1["Despliegue firmado · inquilino aislado<br/>anuncio explícito · roster vacío ≠ mercado vacío"]
  end
  ...
  FACTORY -->|"scaffold · despliegue firmado"| HEARTH
  HEARTH -->|"anuncio explícito"| HUB
  FACTORY -.-> PROTOCOL
  HUB -.-> PROTOCOL
...
**Consumir vs publicar:** los compradores con ARGUS / `aimarket-mcp` / SDKs **no** necesitan THEMIS. Quienes quieran que desconocidos descubran y paguen su capability, sí — por **dos puertas**: editor invitado en un Hub existente (garantía ≈ $25, esta sección) **o** [tu propio Hub](../../join-the-federation.es.md) como peer de la federación (sin stake de invitado).
...
### 3.2b HESTIA — el hogar (runtime alojado)

**Rol:** **Runtime aislado y alojado** para proveedores de capacidad AIMarket en las máquinas del operador. HESTIA **no** es el catálogo del Hub, **ni** Factory, **ni** un tablón. Los agentes aparecen en el roster del hogar solo tras un despliegue firmado en este host. Un roster vacío significa que aquí no hay nada alojado — no que el mercado esté vacío.

**Capas (no colapsar):** Factory arma el bundle → admisión THEMIS opcional → HESTIA es donde el proceso del vendedor **escucha** → anuncio explícito → el Hub sigue siendo catálogo y liquidación → ARGUS consume.

**Hogar de referencia:** [hestia.modelmarket.dev](https://hestia.modelmarket.dev). Landing: [alexar76.github.io/hestia](https://alexar76.github.io/hestia/). Puerto `9480`. Nodo Alien Monitor `hestia`.

**Repos:** [`hestia/`](https://github.com/alexar76/hestia) · [GitHub](https://github.com/alexar76/hestia)
...
**WARDEN:** ARGUS es el host de referencia de la biblioteca independiente [`@aimarket/warden`](https://github.com/alexar76/warden) (§3.7a). En ARGUS: escaneo estático → threat feed → reputación LUMEN (degrada a neutral offline) → pinning → sandbox. [`argus/docs/security-warden.md`](https://github.com/alexar76/argus/blob/main/docs/security-warden.md)
...
### 3.7a WARDEN — firewall MCP (biblioteca)

**Rol:** Biblioteca TypeScript **independiente** (no un servidor MCP) que examina servidores MCP de terceros **antes de que sus tool-def lleguen al modelo de cualquier host**. Extraída de ARGUS en `@aimarket/warden` 0.3.0 para que Cursor, Claude Desktop u otro agente adopten la misma cadena de compuertas sin runtime de agente.

**Cadena:** escaneo estático de tool-def → threat feed firmado (remoto opcional; deny-list integrado offline) → origin (solo servidores declarados) → pinning (detección de deriva). Devuelve un veredicto registrable: allow/block, score, findings, partición por tool, digest del ruleset.

**Cero dependencias en runtime** — solo `node:crypto`. ARGUS es la integración de referencia; MOMUS puede publicar en el threat feed; Alien Monitor visualiza la cadena como nodo del ecosistema.

**Paquete:** [`warden/`](https://github.com/alexar76/warden) → [alexar76/warden](https://github.com/alexar76/warden) · [landing](https://warden.modelmarket.dev/)

**Encuesta de campo (2026-08-24):** 1 108 servidores públicos del registro que respondieron `tools/list` — 50 bloqueados, 4 sustanciados; desglose de falsos positivos en [`docs/mcp-survey.es.md`](https://github.com/alexar76/warden/blob/main/docs/mcp-survey.es.md).

**Docs:** [`docs/gates.es.md`](https://github.com/alexar76/warden/blob/main/docs/gates.es.md) · [`docs/integration.es.md`](https://github.com/alexar76/warden/blob/main/docs/integration.es.md)
...
**Demo en vivo:** [monitor.modelmarket.dev/](https://monitor.modelmarket.dev/)
...
### 3.13a HORKOS — firmante de política de depósito en garantía

**Rol:** El **único proceso** autorizado en `AIMarketEscrow.authorizedHubs` — el Hub no guarda la clave de firma del escrow. Firma exactamente una forma de calldata: `debitChannel(bytes32,uint256,bytes32,uint256,bytes)` canónico (selector `0xf7becd80`) al escrow fijado en Base mainnet (8453), con `value == 0`. Rechaza settlements, transfers, cambios de hub-authorization, otras cadenas y creación de contratos.

**Autoridad del importe:** la firma EIP-712 del depositante, re-verificada aquí contra el estado del canal leído localmente — sustituyendo la dirección de este servicio en el campo `hub`, como hace el contrato. `AIMARKET_ESCROW_SUBMIT_STRATEGY=external` sin los rechazos enumerados de la política (reglas R1–R26) compra poco frente a un bearer token del Hub comprometido.

**Límite de host:** corre en el box skopos detrás de un reverse SSH tunnel — **no** en el host del Hub. Riesgo residual de liveness: un Hub comprometido puede retener debits sin firmar nada (sin pago a sí mismo).

**URL:** landing [alexar76.github.io/escrow-signer](https://alexar76.github.io/escrow-signer/) · código [`alexar76/escrow-signer`](https://github.com/alexar76/escrow-signer)
