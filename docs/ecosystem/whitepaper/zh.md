...
flowchart TB
  ...
  subgraph MACHINE["Autonomous machine economy"]
    ...
    BASANOS["🪨 BASANOS<br/>Solidity 扫描 · PASS/REVIEW/FAIL"]
...
**WARDEN：** ARGUS 是独立库 [`@aimarket/warden`](https://github.com/alexar76/warden) 的参考宿主（§3.7a）。在 ARGUS 中：静态扫描 → 威胁情报源 → LUMEN 声誉（离线时降级为中性）→ 固定（pinning）→ 沙箱。[`argus/docs/security-warden.md`](https://github.com/alexar76/argus/blob/main/docs/security-warden.md)
...
### 3.7a WARDEN — MCP 安全防火墙（库）

**角色：** 独立的 TypeScript **库**（不是 MCP 服务器），在第三方 MCP 服务器的 tool-def **进入任何宿主模型之前**对其进行审查。从 ARGUS 提取为 `@aimarket/warden` 0.3.0，使 Cursor、Claude Desktop 或自定义智能体无需采用 agent runtime 即可使用相同的关卡链。

**关卡链：** 静态 tool-def 扫描 → 签名威胁情报源（可选远程；离线内置 deny-list）→ origin（仅声明的服务器）→ pinning（漂移检测）。返回可记录的裁决：allow/block、分数、findings、按 tool 分区、ruleset digest。

**零运行时依赖** — 仅 `node:crypto`。ARGUS 是参考集成；MOMUS 可发布到威胁情报源；Alien Monitor 将关卡链可视化为生态节点。

**包：** [`warden/`](https://github.com/alexar76/warden) → [alexar76/warden](https://github.com/alexar76/warden) · [落地页](https://warden.modelmarket.dev/)

**实地调研（2026-08-24）：** 1 108 个回答 `tools/list` 的公共注册表服务器 — 50 个被阻止，4 个属实；误报分析见 [`docs/mcp-survey.zh.md`](https://github.com/alexar76/warden/blob/main/docs/mcp-survey.zh.md)。

**文档：** [`docs/gates.zh.md`](https://github.com/alexar76/warden/blob/main/docs/gates.zh.md) · [`docs/integration.zh.md`](https://github.com/alexar76/warden/blob/main/docs/integration.zh.md)

### 3.8 Alien Monitor

**角色：** 3D 生态系统可视化器，有三种模式 —— **UNI**（本地链 + 实时轮询）、**TEST**（模拟）、**LIVE**（真实 Hub/Mesh/Prometheus + 链上 RPC）。

**在线演示：** [monitor.modelmarket.dev/](https://monitor.modelmarket.dev/)
...
### 3.13a HORKOS — 托管策略签名服务

**角色：** `AIMarketEscrow.authorizedHubs` 中 **唯一** 获授权的流程 —— 枢纽不持有托管签名私钥。只签一种 calldata：对钉死在 Base mainnet (8453) 上的托管合约发起规范 `debitChannel(bytes32,uint256,bytes32,uint256,bytes)`（选择器 `0xf7becd80`），且 `value == 0`。拒绝 settlement、transfer、hub-authorization 变更、其他链与合约创建。

**金额授权：** 存款人的 EIP-712 签名，在此根据本地读取的通道状态重新验证 —— 用本服务地址替换 `hub` 字段，与合约一致。若没有枚举的策略拒绝（规则 R1–R26），`AIMARKET_ESCROW_SUBMIT_STRATEGY=external` 对受损枢纽 bearer token 几乎无防护。

**主机边界：** 在 skopos 机器上通过反向 SSH 隧道运行 —— **不在** 枢纽主机上。残余 liveness 风险：受损枢纽可不签名而扣留 debit（不会给自己付款）。

**URL：** 落地页 [alexar76.github.io/escrow-signer](https://alexar76.github.io/escrow-signer/) · 源码 [`alexar76/escrow-signer`](https://github.com/alexar76/escrow-signer)
