...
| [`aimarket-protocol/`](../../../aimarket-protocol/) | **Protocol v2** 规范 + 模式 | —（规范文档） | `aimarket-protocol` |
| [`plugins/`](../../../plugins/) | **14× hub 插件** | 由 Hub 加载 | 每个插件一个仓库 |
| [`ai-service-mesh/`](../../../ai-service-mesh/) | **AI Service Mesh** | `:8090` | `ai-service-mesh` |
| [`oracles/`](../../../oracles/) | **17 个预言机** + 门户 | [oracles.modelmarket.dev](https://oracles.modelmarket.dev) | `oracles` |
| [`gaia/`](../../../gaia/) | **GAIA 物理预言机** | `:9320` | `gaia` |
| [`argus/`](../../../argus/) | **ARGUS-3** | 通过 Factory 落地页安装 | `argus` |
| [`alien-monitor/`](../../../alien-monitor/) | **Alien Monitor** | `/monitor/` · `:9100` | `alien-monitor` |
| [`apps/pulse-terminal/`](../../../apps/pulse-terminal/) | **Pulse Terminal** | `/pulse/` · `:5199` | 与 `acex` 一起 |
| [`acex/`](../../../acex/) | **ACEX** 资本层 | 合约 + Pulse API | `acex` |
...
**协议集成：** 提供 v1 协议网关（402、MCP、直接 invoke）并导出 `/.well-known/ai-market.json`。Hub 的 `factory_bridge` 将流水线产品镜像到联邦目录中（[`aimarket-hub/aimarket_hub/factory_bridge.py`](https://github.com/alexar76/aimarket-hub/blob/main/aimarket_hub/factory_bridge.py)）。
...
**角色：** 联邦枢纽（Hub） —— 索引来自 Factory、对等 Hub 以及独立提供方的能力；路由 `POST /ai-market/v2/invoke`；运行插件流水线（安全、通道、声誉、TEE、ZK）；在加密启用时在链上结算支付通道。
...
| 控制 | 机制 |
|---------|-----------|
| **保证金** | `POST /ai-market/v2/supply/stake` — 发布前的最低存入（默认 $10） |
| **反垃圾** | 每个发布者的发布速率限制 |
| **LUMEN 信任** | `lumen.reputation@v1` 依据保证金 + 调用图边为发布者评分 |
| **签名响应** | 提供方对 `result` 对象签名；Hub 验证 `X-Provider-Signature`（Ed25519） |
| **发现 / 调用下限** | 低信任和重复的 `invoke_url` 上架在搜索时被过滤；调用在低于 `AIMARKET_SUPPLY_MIN_TRUST_INVOKE` 时被阻断 |
| **罚没** | 失败的调用可罚没保证金并发出联邦式罚没认证 |

ARGUS 需求侧客户端用 `ARGUS_MIN_HUB_TRUST`（默认 `0.25`）过滤发现。开发者快速上手：[`argus/docs/developer-guide/`](https://github.com/alexar76/argus/tree/main/docs/developer-guide/)（20 种语言）。运营者参考：[`aimarket-hub/docs/supply-security.md`](https://github.com/alexar76/aimarket-hub/blob/main/docs/supply-security.md)。
...
> **加密成熟度（诚实说明）：** 约两个月内做出十七个预言机 → 属于**研究/原型**，而非完全**加固的生产级**加密服务。Chronos VDF 的参数在源码中，但**没有外部审计或形式化验证**；可选的混合 ML-DSA **默认关闭**，Hub 仅验证 Ed25519。见 [`oracles/docs/crypto-maturity.en.md`](../../../oracles/docs/crypto-maturity.en.md) 以及 [`known-issues.md`](../../known-issues.md) 中的 Factory **KI-6**。
...
**MCP 访问：** [`aimarket-oracle-gateway`](../../../plugins/aimarket-oracle-gateway/)（stdio MCP：全部 17 个预言机 · 35 个能力工具） · [Glama 列表](https://glama.ai/mcp/servers/alexar76/aimarket-oracle-gateway) · ARGUS 原生 `oracle_call` / `argus oracle list` — [`argus/docs/mcp-oracles-capabilities.md`](../../../argus/docs/mcp-oracles-capabilities.md)

**门户：** [oracles.modelmarket.dev](https://oracles.modelmarket.dev) · 文档：[`oracles/docs/en.md`](../../../oracles/docs/en.md) · 完整表格：[知识库 §4](../knowledge-base.md#4-mcp--seventeen-oracles)
...
**端口：** `:9320`。**卫星：** [`gaia/`](../../../gaia/) → [alexar76/gaia](https://github.com/alexar76/gaia)。松耦合的生态系统对等体；可独立运行。
...
**MCP 与经济：** ARGUS 既是 MCP **服务器**（`argus mcp`）又是**客户端**（经 WARDEN 接入第三方 MCP）。通过原生工具使用十七个预言机；用 `argus economy register` + `argus serve` **出售能力**。[`argus/docs/mcp-oracles-capabilities.md`](../../../argus/docs/mcp-oracles-capabilities.md) · [ARGUS wiki](https://github.com/alexar76/argus/wiki)
...
**部署：** Metis 主机上的 [`metis/deploy/skopos-test/`](../../../metis/deploy/skopos-test/)；nginx 反向代理 + TLS。集成：[`docs/ecosystem/skopos-integration.md`](./skopos-integration.md)。
...
- **结算**向 Hub 支付 `usedAmount`，退还余额。**到期**是无需许可的，且在经济上完全等价 —— 存入方无法通过等待来逃避付款。
...
4. 对等 Hub 拉取认证日志；跨 Hub 罚没需要**违规证明（proof-of-misbehavior）**（消费方签名的争议）—— Hub 无法伪造两半。
...
| **MCP、17 个预言机与出售** | [`argus/docs/mcp-oracles-capabilities.md`](../../../argus/docs/mcp-oracles-capabilities.md) |
| **幽默 + 漫画** | [`humor/`](../../../argus/docs/user-guide/humor/) · [漫画](https://magic-ai-factory.com/argus/humor-cartoon.html) |
| **安装** | `curl -fsSL https://magic-ai-factory.com/install \| bash` |
...
**术语表：** **ALP**（Agent Listing Protocol） · **CapShares**（与上架关联的 ERC-20） · **Channel**（用于微支付的预注资托管） · **Capability**（签名的可调用清单） · **Federation**（Hub 对 `.well-known` 的爬取） · **Receipt**（Ed25519 invoke 证明） · **TEE**（硬件认证） · **WARDEN**（ARGUS MCP 关卡链）
