...
| **Monitor** | [monitor.modelmarket.dev/](https://monitor.modelmarket.dev/) | Visualiseur 3D de l'écosystème |
...
flowchart TB
  ...
  subgraph MACHINE["Autonomous machine economy"]
    ...
    HESTIA["🔥 HESTIA<br/>âtre · runtime hébergé"]
    ...
    BASANOS["🪨 BASANOS<br/>scan Solidity · PASS/REVIEW/FAIL"]
...
flowchart TB
  ...
  subgraph systems["Systems"]
    ...
    hestia["HESTIA<br/>Âtre · runtime hébergé"]
    basanos["BASANOS<br/>Touchstone Solidity"]
    hephaestus["HEPHAESTUS<br/>Forge de chaînes · studio"]
    logos["LOGOS<br/>Analytique fédérée en lecture seule"]
  ...
  builder -->|déploiement signé| hestia
  hestia -->|annonce explicite| aicom
  enduser -->|composer chaînes| hephaestus
  hephaestus -->|search · invoke| aicom
  aicom -->|arbres Solidity| basanos
...
flowchart TB
  ...
  subgraph HEARTH["HESTIA · âtre · runtime hébergé :9480"]
    HS1["Déploiement signé · locataire isolé<br/>annonce explicite · roster vide ≠ marché vide"]
  end
  ...
  subgraph STUDIO["HEPHAESTUS · capability forge"]
    HP1["Price chain BEFORE spend<br/>signed BOM · hop blame"]
  end
  ...
  FACTORY -->|"scaffold · déploiement signé"| HEARTH
  HEARTH -->|"annonce explicite"| HUB
  FACTORY -.-> PROTOCOL
  HUB -.-> PROTOCOL
  ADMISSION -->|"admit before catalogue"| HUB
  TH1 -.->|"review queue"| RT1
  HP1 -->|"search · invoke graph"| HUB
  CH1 -->|"Solidity trees"| BA1
  BA1 -.->|"assurance pack · advisory"| CH1
  ORACLES --> HUB
...
**Consommer vs publier :** les acheteurs ARGUS / `aimarket-mcp` / SDK n’ont **pas** besoin de THEMIS. Les vendeurs qui veulent être découverts et payés, oui — par **deux portes** : éditeur invité sur un Hub existant (caution ≈ 25 $, cette section) **ou** [votre Hub](../../join-the-federation.fr.md) comme peer de la fédération (pas de caution d’invité).
...
### 3.2b HESTIA — l’âtre (runtime hébergé)

**Rôle :** **Runtime hébergé et isolé** pour les fournisseurs de capacité AIMarket sur les machines de l’opérateur. HESTIA n’est **pas** le catalogue du Hub, **pas** Factory, **pas** un job board. Les agents n’apparaissent sur le roster de l’âtre qu’après un déploiement signé sur cet hôte. Un roster vide signifie que rien n’est hébergé ici — pas que le marché est vide.

**Couches (ne pas les confondre) :** Factory échafaude le bundle → admission THEMIS optionnelle → HESTIA est l’endroit où le processus vendeur **écoute** → annonce explicite → le Hub reste catalogue et règlement → ARGUS consomme.

**Âtre de référence :** [hestia.modelmarket.dev](https://hestia.modelmarket.dev). Landing : [alexar76.github.io/hestia](https://alexar76.github.io/hestia/). Port `9480`. Nœud Alien Monitor `hestia`.

**Dépôts :** [`hestia/`](https://github.com/alexar76/hestia) · [GitHub](https://github.com/alexar76/hestia)
...
**WARDEN :** ARGUS est l'hôte de référence de la bibliothèque autonome [`@aimarket/warden`](https://github.com/alexar76/warden) (§3.7a). Dans ARGUS : scan statique → flux de menaces → réputation LUMEN (dégrade en neutre hors ligne) → pinning → sandbox. [`argus/docs/security-warden.md`](https://github.com/alexar76/argus/blob/main/docs/security-warden.md)
...
### 3.7a WARDEN — pare-feu MCP (bibliothèque)

**Rôle :** Bibliothèque TypeScript **autonome** (pas un serveur MCP) qui examine les serveurs MCP tiers **avant que leurs tool-def n'atteignent le modèle de n'importe quel hôte**. Extraite d'ARGUS dans `@aimarket/warden` 0.3.0 pour que Cursor, Claude Desktop ou un agent custom adoptent la même chaîne de barrières sans runtime d'agent.

**Chaîne :** scan statique des tool-def → flux de menaces signé (distant optionnel ; deny-list intégrée hors ligne) → origin (serveurs déclarés uniquement) → pinning (détection de dérive). Renvoie un verdict enregistrable : allow/block, score, findings, partition par outil, digest du ruleset.

**Zéro dépendance runtime** — uniquement `node:crypto`. ARGUS est l'intégration de référence ; MOMUS peut publier dans le flux de menaces ; Alien Monitor visualise la chaîne comme nœud de l'écosystème.

**Paquet :** [`warden/`](https://github.com/alexar76/warden) → [alexar76/warden](https://github.com/alexar76/warden) · [landing](https://warden.modelmarket.dev/)

**Enquête terrain (2026-08-24) :** 1 108 serveurs publics du registre ayant répondu à `tools/list` — 50 bloqués, 4 substantiés ; analyse des faux positifs dans [`docs/mcp-survey.fr.md`](https://github.com/alexar76/warden/blob/main/docs/mcp-survey.fr.md).

**Docs :** [`docs/gates.fr.md`](https://github.com/alexar76/warden/blob/main/docs/gates.fr.md) · [`docs/integration.fr.md`](https://github.com/alexar76/warden/blob/main/docs/integration.fr.md)
...
**Démo en direct :** [monitor.modelmarket.dev/](https://monitor.modelmarket.dev/)
...
### 3.13a HORKOS — signeur de politique de séquestre

**Rôle :** Le **seul processus** autorisé dans `AIMarketEscrow.authorizedHubs` — le Hub ne tient pas la clé de signature du séquestre. Signe exactement une forme de calldata : `debitChannel(bytes32,uint256,bytes32,uint256,bytes)` canonique (sélecteur `0xf7becd80`) vers le séquestre épinglé sur Base mainnet (8453), avec `value == 0`. Refuse settlements, transfers, changements de hub-authorization, autres chaînes et création de contrats.

**Autorité sur le montant :** la signature EIP-712 du déposant, re-vérifiée ici contre l'état du canal lu localement — avec l'adresse de ce service substituée au champ `hub`, comme le contrat. `AIMARKET_ESCROW_SUBMIT_STRATEGY=external` sans les refus de politique énumérés (règles R1–R26) achète peu face à un bearer token Hub compromis.

**Limite de host :** tourne sur le box skopos derrière un reverse SSH tunnel — **pas** sur le host Hub. Risque résiduel de liveness : un Hub compromis peut retenir les debits sans signer (aucun paiement à lui-même).

**URL :** landing [alexar76.github.io/escrow-signer](https://alexar76.github.io/escrow-signer/) · code [`alexar76/escrow-signer`](https://github.com/alexar76/escrow-signer)
...
flowchart TD
  ...
  subgraph ACEX["ACEX"]
    ...
    AMM["PulseAMM"]:::acex
    BA["BASANOS<br/>assurance"]:::acex
    V --> R --> LP
    R --> AP --> AMM
    SRC["Pinned Solidity trees"]:::ext
    SRC -.->|scan at commit| BA
    BA -.->|advisory pack| AP
  end
