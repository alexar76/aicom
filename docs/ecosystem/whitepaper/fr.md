...
| [`aimarket-protocol/`](../../../aimarket-protocol/) | Spéc + schémas **Protocol v2** | — (docs normatifs) | `aimarket-protocol` |
| [`plugins/`](../../../plugins/) | **14× plugins hub** | chargés par le Hub | un repo par plugin |
| [`ai-service-mesh/`](../../../ai-service-mesh/) | **AI Service Mesh** | `:8090` | `ai-service-mesh` |
| [`oracles/`](../../../oracles/) | **17 oracles** + portail | [oracles.modelmarket.dev](https://oracles.modelmarket.dev) | `oracles` |
| [`gaia/`](../../../gaia/) | **oracles physiques GAIA** | `:9320` | `gaia` |
| [`argus/`](../../../argus/) | **ARGUS-3** | installer via le landing Factory | `argus` |
| [`alien-monitor/`](../../../alien-monitor/) | **Alien Monitor** | `/monitor/` · `:9100` | `alien-monitor` |
| [`apps/pulse-terminal/`](../../../apps/pulse-terminal/) | **Pulse Terminal** | `/pulse/` · `:5199` | avec `acex` |
| [`acex/`](../../../acex/) | couche capital **ACEX** | contrats + Pulse API | `acex` |
...
**Rôle :** Hub de fédération — indexe les capacités de Factory, des hubs pairs et des fournisseurs autonomes ; route `POST /ai-market/v2/invoke` ; exécute le pipeline de plugins (sécurité, canaux, réputation, TEE, ZK) ; règle les canaux de paiement on-chain quand le crypto est activé.
...
| Contrôle | Mécanisme |
|----------|-----------|
| **Caution / dépôt de garantie** | `POST /ai-market/v2/supply/stake` — dépôt minimum (défaut 10 $) avant publication |
| **Anti-spam** | Limites de débit de publication par éditeur |
| **Confiance LUMEN** | `lumen.reputation@v1` note les éditeurs à partir de la caution + des arêtes du graphe d'invoke |
| **Réponses signées** | Les fournisseurs signent l'objet `result` ; le hub vérifie `X-Provider-Signature` (Ed25519) |
| **Planchers de découverte / invoke** | Les annonces à faible confiance et à `invoke_url` dupliquée sont filtrées à la recherche ; l'invoke est bloqué en dessous de `AIMARKET_SUPPLY_MIN_TRUST_INVOKE` |
| **Slash** | Les invokes échoués peuvent slasher la caution et émettre des attestations de slash fédérées |

Les clients de demande ARGUS filtrent la découverte avec `ARGUS_MIN_HUB_TRUST` (défaut `0.25`). Quickstart développeur : [`argus/docs/developer-guide/`](https://github.com/alexar76/argus/tree/main/docs/developer-guide/) (20 langues). Référence opérateur : [`aimarket-hub/docs/supply-security.md`](https://github.com/alexar76/aimarket-hub/blob/main/docs/supply-security.md).
...
**Accès MCP :** [`aimarket-oracle-gateway`](../../../plugins/aimarket-oracle-gateway/) (MCP stdio : les 17 oracles · 35 outils de capacité) · [listing Glama](https://glama.ai/mcp/servers/alexar76/aimarket-oracle-gateway) · `oracle_call` natif ARGUS / `argus oracle list` — [`argus/docs/mcp-oracles-capabilities.md`](../../../argus/docs/mcp-oracles-capabilities.md)

**Portail :** [oracles.modelmarket.dev](https://oracles.modelmarket.dev) · Docs : [`oracles/docs/en.md`](../../../oracles/docs/en.md) · Tableau complet : [base de connaissances §4](../knowledge-base.md#4-mcp--seventeen-oracles)
...
**Port :** `:9320`. **Satellite :** [`gaia/`](../../../gaia/) → [alexar76/gaia](https://github.com/alexar76/gaia). Pair de l'écosystème faiblement couplé ; tourne en autonomie.
...
**Déploiement :** [`metis/deploy/skopos-test/`](../../../metis/deploy/skopos-test/) sur l'hôte Metis ; reverse proxy nginx + TLS. Intégration : [`docs/ecosystem/skopos-integration.md`](./skopos-integration.md).
...
### 4.2 Règles des canaux de séquestre
...
- Le **règlement** paie au hub le `usedAmount` et rembourse le reste. L'**expiration** est sans permission et économiquement identique — le déposant ne peut pas esquiver le paiement en attendant.
...
4. Les hubs pairs récupèrent les journaux d'attestation ; un slash inter-hub requiert une **preuve de faute (proof-of-misbehavior)** (litige signé par le consommateur) — les hubs ne peuvent pas falsifier les deux moitiés.
...
| **MCP, 17 oracles & vente** | [`argus/docs/mcp-oracles-capabilities.md`](../../../argus/docs/mcp-oracles-capabilities.md) |
| **Humour + dessin animé** | [`humor/`](../../../argus/docs/user-guide/humor/) · [dessin animé](https://magic-ai-factory.com/argus/humor-cartoon.html) |
| **Installation** | `curl -fsSL https://magic-ai-factory.com/install \| bash` |
...
**Glossaire :** **ALP** (Agent Listing Protocol) · **CapShares** (ERC-20 liés à une annonce) · **Channel** (séquestre préfinancé pour micropaiements) · **Capability** (manifeste invocable signé) · **Federation** (crawl de `.well-known` par le hub) · **Receipt** (preuve d'invoke Ed25519) · **TEE** (attestation matérielle) · **WARDEN** (chaîne de barrières MCP d'ARGUS)
