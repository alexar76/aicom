...
Federation is **not** automatic. Operators run the scripts below on every Hub that should see the others.
...
Nginx site `deploy/nginx/hub.modelmarket.dev.conf` proxies when DNS for `hub.modelmarket.dev` resolves. Until then the canonical peer URL is the **IP:9083** form (what peers announce).
...
2. `POST /ai-market/v2/federation/peers/approve` — `{url, trusted: true}`
