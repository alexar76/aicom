...
`--write` also copies two whole files into the Monitor, which ships its own fallbacks because the
monorepo is not mounted in its container:

| Source | Mirror | Transform |
|---|---|---|
| [`docs/ecosystem/knowledge-base.md`](knowledge-base.md) | [`alien-monitor/docs/ecosystem/knowledge-base.md`](https://github.com/alexar76/alien-monitor/blob/main/docs/ecosystem/knowledge-base.md) | relative links absolutised (they resolve to nothing in the other repo) |
| [`scripts/satellite-map.yaml`](../../scripts/satellite-map.yaml) | [`alien-monitor/scripts/satellite-map.yaml`](https://github.com/alexar76/alien-monitor/blob/main/scripts/satellite-map.yaml) | satellites with `github_published: false` dropped |

Reading a file at runtime is the better pattern and the one the sync generalises: the monitor's bot
builds its prompt context from `satellite-map.yaml` on every request. But the copy it reads inside the
container was hand-maintained, and it fell three satellites behind — DOLOS shipped and the in-container
assistant had never heard of it — so that copy is generated now too. Prefer runtime loading for
anything new that can do it; injection is for prompts that must ship as a static string.
