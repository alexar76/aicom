...
| **GitHub** `alexar76/dioscuri` | 公共镜像 — docs、CI、self-host 参考 |
...
## 在 oracle 主机上部署 (78.17.126.214)

DIOSCURI 运行在 **oracle VPS** 上，而非工厂机群（`5.129.212.122`）。Factory Monitor 仅轮询 `http://78.17.126.214:8790/health`。
...
需要免密码的 `ssh root@78.17.126.214`（或 `ssh oracle` — 见 `~/.ssh/config`）。**git push** 的认证使用 `data/secrets/git-credentials`（与 `mirror_to_gitea.sh` 相同），而非 SSH。
...
| 文件 | 用途 |
|------|------|
| `.env` | 机器人令牌、LLM 密钥、syndication 密钥 |
