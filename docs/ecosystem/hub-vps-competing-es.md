...
El **cableado del mesh** en esta máquina no es automático: hay que ejecutar los
scripts para que cada Hub conocido vea a los demás. Tras un golpe, la admisión de
producto es otro camino: el sandbox admite un `pass` solo (ver
[`join-the-federation.es.md`](../join-the-federation.es.md)). Los scripts siguen
haciendo `approve` explícito para que un peer de laboratorio sea trusted aunque no
tenga un SKU sandbox gratuito.
