...
**Допуск сторонних компонентов:** не open signup — токен оператора, стейк ≈ $25, манифест, подписи, trust floors; опционально THEMIS (`off` по умолчанию). **Две двери продавца:** гостевой залог ≈ $25 на чужом Hub, или [свой Hub](../join-the-federation.ru.md) как peer (гостевого стейка нет). **Потребление** через ARGUS / `aimarket-mcp` без THEMIS. Стейкинг + подписи — [`supply-security.md`](https://github.com/alexar76/aimarket-hub/blob/main/docs/supply-security.md); таблица + шаги — [supply-chain-admission-ru.md](./supply-chain-admission-ru.md) ([EN](./supply-chain-admission.md) · [ES](./supply-chain-admission-es.md) · [FR](./supply-chain-admission-fr.md) · [ZH](./supply-chain-admission-zh.md)). THEMIS = можно ли в каталог; WARDEN = можно ли invoke сейчас; Metis = совет; MOMUS = review; Monitor = история; Hub = применение.
...
| gaia.spacewx.read@v1 | spacewx (Космическая погода) | swpc-01, swpc-solarwind-01, swpc-xray-01, donki-01 | NOAA SWPC Kp; пин Boulder, планетарный индекс |
...
| gaia.aviation.read@v1 | aviation (Авиа METAR) | metar-kjfk-01, metar-klga-01, metar-kbos-01, metar-kord-01 +10 | NOAA AWC METAR (U.S. PD); in-situ ASOS, не TAF |
| gaia.road.read@v1 | road (Дорожная погода) | fintraffic-road-01 | Fintraffic Digitraffic дорожная погода CC BY 4.0; только Финляндия |
| gaia.rail.read@v1 | rail (Железная дорога) | fintraffic-rail-01 | Fintraffic Digitraffic поезда CC BY 4.0; только финская ж/д |
| gaia.drought.read@v1 | drought (Засуха) | usdm-01 | U.S. Drought Monitor, недельная статистика штатов; атрибуция NDMC/USDA/NOAA/NASA |
| gaia.uv.read@v1 | uv (UV forecast) | epa-uv-01 | якорь device_id оператора; LIVE только с provenance source |
| gaia.energy.read@v1 | energy (Энергия) | em-01 | якорь device_id оператора; LIVE только с provenance source |
| gaia.reservoir.read@v1 | reservoir (Reservoirs) | usace-keys-01, usace-pine-01, usace-deni-01, usace-huds-01 | якорь device_id оператора; LIVE только с provenance source |
...
- cite-desks: Cite desks — независимые evidence-столы на рельсах AIMarket. Один родительский репозиторий: kernel + Emberline / Tideline / Solrecord / Seamark / Plinth, у каждого свой README. · https://desk.modelmarket.dev/
