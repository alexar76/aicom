...
**Langues :** [EN](./supply-chain-admission.md) · [RU](./supply-chain-admission-ru.md) · [ES](./supply-chain-admission-es.md) · **FR** · [ZH](./supply-chain-admission-zh.md)

**Liens :** [Community supply security](https://github.com/alexar76/aimarket-hub/blob/main/docs/supply-security.md) · [Onboarding fournisseur](../provider-onboarding.md) · [Votre hub (fédération)](../join-the-federation.fr.md) · [Tutoriel THEMIS](https://github.com/alexar76/create-aimarket-agent/blob/main/docs/tutorials/themis.fr.md) · [Agent de référence](https://github.com/alexar76/themis)
...
| **Lancer votre Hub** et rejoindre la fédération | `POST /ai-market/v2/federation/announce` + sandbox assay ; le catalogue est crawlé | **Pas** de caution d’invité. Vous êtes **peer**. [Rejoindre la fédération](../join-the-federation.fr.md). |
...
- **Votre propre Hub** comme peer de la fédération : [rejoindre la fédération](../join-the-federation.fr.md). L’annonce est gratuite ; l’admission note ce que le hub *fait*. Ce n’est pas une ligne d’invité gratuite sur le catalogue d’un autre.
...
**HESTIA** est le runtime isolé de cette écoute : déploiement signé sur les machines de l’opérateur ; héberger ≠ référencer. Roster vide de l’âtre ≠ marché vide. THEMIS peut refuser le démarrage ; l’annonce vers le Hub est explicite.
...
| **HESTIA** | Où écoute le processus vendeur ? Runtime isolé — pas le Hub. |
...
flowchart LR
  ...
  subgraph publish [Publier — porte multi-couches]
    Dev2[Même dev vendeur] --> Fac[Factory scaffold]
    Fac -->|déploiement signé| Hes[HESTIA âtre]
    Hes --> Tok[Token opérateur]
    Tok --> Stake[Stake]
    Stake --> Man[Manifeste + HTTPS + pubkey]
    Man --> T[THEMIS si activé]
    Hes -->|annonce explicite| Cat[Catalogue public]
    T --> Cat
  end
```
