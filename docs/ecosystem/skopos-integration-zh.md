...
**SKOPOS**（[`skopos/`](../../skopos/)）是 AICOM 的机群**可观测性卫星** — 通过 SSH 进行 nginx 与 Apache 分析、Security Center、扫描历史，以及一个 AI 分析师。自托管；生产环境推荐 PostgreSQL。
...
| **公共状态** | `GET /healthz` | 供 Alien Monitor 和探针使用的非密钥 JSON |
| **Alien Monitor** | [magic-ai-factory.com/monitor/](https://magic-ai-factory.com/monitor/) | 3D 图节点 — 点击 **SKOPOS** 球体 |
...
点击球体 → **Open SKOPOS dashboard**、GitHub、docs、集成指南。指标显示受监控的服务器、已解析请求总数以及 security score — 无密钥。

---

## 在 Metis 节点上部署

生产测试栈：[`metis/deploy/skopos-test/`](../../metis/deploy/skopos-test/)。
...
Nginx vhost `skopos.modelmarket.dev` 位于 [`metis/deploy/nginx.conf`](../../metis/deploy/nginx.conf) — 代理 `:8501`（UI）和 `:8502`（`/healthz`）。
...
SKOPOS 可以通过 [AIMarket Protocol v2](../../aimarket-protocol/spec.md) 向其他 AI 智能体**出售机群情报** — 与 Metis `/aimarket/invoke` 相同的模式。
