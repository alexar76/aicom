...
| Producer | Template | Vars |
|----------|----------|------|
| Calliope editorial (seasons) | `calliope-episode` | seasonal themes + fetched READMEs |
| THEOROS canon (`runCanon`) | `theoros-short` | Theoros's own `hook` / `body` / `debate` (+ full `column`) |
| GitHub release (**opt-in**) | `release-short` | `repo`, `tag`, `url`, `summary` — needs `HELIOS_RELEASE_SHORTS=1` |

```bash
# dioscuri .env — Theoros on, release shorts off (default)
HELIOS_SYNDICATION=1
HELIOS_QUEUE_PATH=/data/helios-queue.jsonl
# HELIOS_RELEASE_SHORTS=1   # optional "X shipped" shorts
```

Canon Shorts upload as **private** until operator (or `HELIOS_AUTO_APPROVE`) promotes them.

**Seasons:** Helios Calliope scouts interesting ecosystem topics (config `seasons:`), fetches allowlisted GitHub READMEs (`helios calliope fetch`), and enqueues episodes. Release spam is no longer the main channel mix.
...
- [HELIOS README](https://github.com/alexar76/helios/blob/main/README.md) · [RU](https://github.com/alexar76/helios/blob/main/README-ru.md) · [ES](https://github.com/alexar76/helios/blob/main/README-es.md)
- [Architecture](https://github.com/alexar76/helios/blob/main/docs/architecture.md)
- [Security audit](https://github.com/alexar76/helios/blob/main/docs/SECURITY-AUDIT.md)
