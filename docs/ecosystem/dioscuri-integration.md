...
**Landing:** [alexar76.github.io/dioscuri](https://alexar76.github.io/dioscuri/) · **Languages:** **EN** · **[RU](./dioscuri-integration-ru.md)** · **[ES](./dioscuri-integration-es.md)** · **[FR](./dioscuri-integration-fr.md)** · **[ZH](./dioscuri-integration-zh.md)**
...
## Deploy on oracle host (oracles.modelmarket.dev)

DIOSCURI runs on the **oracle VPS**, not the factory fleet (`modeldev.modelmarket.dev`). Factory Monitor polls `http://oracles.modelmarket.dev:8790/health` only.
...
Requires passwordless `ssh root@oracles.modelmarket.dev` (or `ssh oracle` — see `~/.ssh/config`). Auth for **git push** uses `data/secrets/git-credentials` (same as `mirror_to_gitea.sh`), not SSH.
...
Castor/Pollux announce chapters; they do **not** impersonate Theoros. Full guide: **[dioscuri/docs/theoros.md](https://github.com/alexar76/dioscuri/blob/main/docs/theoros.md)** (architecture, voice charter, config, checklist).
...
When `HELIOS_SYNDICATION=1`, DIOSCURI appends jobs to `HELIOS_QUEUE_PATH` (fail-soft):

- THEOROS canon column → `theoros-short` (Theoros prose passed through)
- GitHub release → `release-short` only if `HELIOS_RELEASE_SHORTS=1` (opt-in; off by default)

Main YouTube mix is Helios Calliope **seasons** (interesting ecosystem themes + GitHub fetch), not release spam. See [helios-integration.md](./helios-integration.md).
...
- [DIOSCURI README (EN)](https://github.com/alexar76/dioscuri/blob/main/README.md) · [RU](https://github.com/alexar76/dioscuri/blob/main/README-ru.md) · [ES](https://github.com/alexar76/dioscuri/blob/main/README-es.md)
- [Setup](https://github.com/alexar76/dioscuri/blob/main/docs/setup.md)
