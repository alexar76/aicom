...
| **GitHub** `alexar76/dioscuri` | Espejo público — docs, CI, referencia self-host |
...
## Despliegue en el host oracle (78.17.126.214)

DIOSCURI corre en el **VPS oracle**, no en la flota de la fábrica (`5.129.212.122`). Factory Monitor solo sondea `http://78.17.126.214:8790/health`.
...
Requiere `ssh root@78.17.126.214` sin contraseña (o `ssh oracle` — ver `~/.ssh/config`). La autenticación para **git push** usa `data/secrets/git-credentials` (igual que `mirror_to_gitea.sh`), no SSH.
...
Los self-hosters definen las invitaciones en `dioscuri.config.json`:
