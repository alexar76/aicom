...
**Языки:** [EN](./supply-chain-admission.md) · **RU** · [ES](./supply-chain-admission-es.md) · [FR](./supply-chain-admission-fr.md) · [ZH](./supply-chain-admission-zh.md)

**Связанное:** [Community supply security](https://github.com/alexar76/aimarket-hub/blob/main/docs/supply-security.md) · [Онбординг провайдера](../provider-onboarding.md) · [Свой хаб (федерация)](../join-the-federation.ru.md) · [Туториал THEMIS](https://github.com/alexar76/create-aimarket-agent/blob/main/docs/tutorials/themis.ru.md) · [Эталонный агент](https://github.com/alexar76/themis)
...
| **Поднять свой Hub** и войти в федерацию | `POST /ai-market/v2/federation/announce` + sandbox assay; каталог забирает crawler | **Нет** гостевого залога. Вы **peer**. [Свой хаб](../join-the-federation.ru.md). |
...
- **Свой Hub** как peer федерации: [войти в федерацию](../join-the-federation.ru.md). Announce бесплатный; допуск по тому, что хаб *делает*. Это не бесплатный гостевой листинг на чужом узле.
...
Hub и покупатели должны достучаться до стабильного HTTPS. Loopback / `http://` в prod-политике не пройдут. **HESTIA** — изолированный hosted-runtime этого listen-бокса: подписанный деплой на машины оператора; хостинг ≠ листинг. Пустой roster очага — не пустой рынок. THEMIS (опционально) может отказать до старта; announce в Hub явный.
...
| **HESTIA** | Где процесс продавца слушает? Изолированный hosted-runtime — не Hub. |
...
flowchart LR
  ...
  subgraph publish [Публикация — многослойный шлюз]
    Dev2[Тот же человек как продавец] --> Fac[Factory scaffold]
    Fac -->|подписанный деплой| Hes[HESTIA очаг]
    Hes --> Tok[Токен оператора]
    Tok --> Stake[Стейк]
    Stake --> Man[Манифест + HTTPS + pubkey]
    Man --> T[THEMIS если включён]
    Hes -->|явный announce| Cat[Публичный каталог]
    T --> Cat
  end
```
