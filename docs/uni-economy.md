...
4. Creates `EcosystemEntity` node in 3D graph — flagged `simulated`

**A spawned hub is a fixture, not a service.** The announce carries a loopback URL
(`127.0.0.1:9083+n`) to exercise the real hub's federation path, but no process ever listens
there. So the entity carries **no address and no measured metrics** — only the declared
capability count — and its node ships `simulated: true`, which the panel paints as a
`SIMULATED` badge with the reason. It used to advertise that loopback URL as its "internal
address" and sit `peers: 1` / `invocations_24h: 0` in the metrics grid; both were literals that
could never move, and `peers` fed the reputation graph's activity term.
