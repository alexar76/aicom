...
C4Context
  ...
  Person(analyst, "Federation analyst", "Reads live cross-system telemetry and reports")
  ...
  Rel(hephaestus, aicom, "Search · invoke composed graph")
  Rel(enduser, aicom, "Discover, invoke, review")
  Rel(analyst, aicom, "Inspect LOGOS read-only analytics")
...
| [`logos/`](https://github.com/alexar76/logos/tree/main/) | Read-only federation intelligence: source snapshots, rolling z-score anomalies, cross-system correlations, Metis-assisted reports | `logos` |
...
**Live loop today:** human storefront on the factory → oracles & IoT on the hub → consume from desktop/widget.

The factory ships **demo products for people** on its public storefront / admin. It does **not** currently list those SKUs as hub capabilities (factory peer reports **0** caps). The live federated catalog is **verifiable oracles + IoT (GAIA)**.

**Architecture path (not live for factory SKUs):** `factory_bridge` / `sync_pipeline_mirror_and_hub.py` exist to mirror pipeline products into the hub catalog when enabled.
...
flowchart TB
  subgraph factory["AI-Factory (magic-ai-factory.com)"]
    ...
    STORE["Human storefront · /api/products"]
  ...
  subgraph hub["AIMarket Hub (modelmarket.dev)"]
    BRIDGE["factory_bridge · auto_listing (code path)"]
    ORACLES["Oracles + IoT caps · live catalog"]
    ...
    ORACLES --> IDX
    BRIDGE -.->|"not currently mirroring factory SKUs"| IDX
    SRCH --> IDX
    INV --> PLG
    IDX --> WELL2
  end
  ...
  subgraph assurance["BASANOS · Solidity touchstone"]
    BA["scan at pin · PASS/REVIEW/FAIL pack"]
  end

  subgraph studio["HEPHAESTUS · chain forge"]
    HF["price graph · run · signed BOM"]
  end
  ...
  subgraph clients["Consumers"]
    ...
    HUMANS["Humans · factory storefront"]
    ANALYSTS["Operators · federation analysts"]
  end

  subgraph intelligence["LOGOS · read-only federation intelligence"]
    SNAP["Real source snapshots"]
    ANOM["Rolling z-score anomalies"]
  ...
  CORR -->|"dashboard · report · guarded assistant"| ANALYSTS
```
...
flowchart LR
  ...
  subgraph host["Single VPS or k8s cluster"]
    ...
    LOGOS["LOGOS edge :5199<br/>FastAPI :9460"]
  ...
  NGINX -->|"logos.modelmarket.dev"| LOGOS
  FAC --> DATA
  HUB --> DATA
  HUB -->|"crawl peers"| PEERS["Other hubs"]
  LOGOS -->|"poll public stats · manifest · peers"| HUB
```
...
| LOGOS edge | 5199 (loopback-published) | 5199 | React dashboard + nginx proxy to internal FastAPI `:9460` (`LOGOS_PORT`) |
...
| **LOGOS** read-only federation intelligence | [logos/README.md](https://github.com/alexar76/logos/blob/main/README.md) · [live dashboard](https://logos.modelmarket.dev/) |
