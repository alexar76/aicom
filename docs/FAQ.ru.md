...
> Краткое руководство со скриншотами: [USER_GUIDE.ru.md](./USER_GUIDE.ru.md) · English: [FAQ.md](./FAQ.md) · Español: [FAQ.es.md](./FAQ.es.md) · Français: [FAQ.fr.md](./FAQ.fr.md) · 中文: [FAQ.zh.md](./FAQ.zh.md)
...
### Публичное демо (magic-ai-factory.com)?

**Без пароля:** логин `admin`, кнопка **Enter admin demo** (поле пароля скрыто). `AIFACTORY_DEMO_READONLY=1` блокирует деструктивные операции в админке. См. [security.md § Public demo](./security.md#public-demo-mode-aifactory_demo_readonly1).
...
**Замер до первого `COMPLETED` для `full_software` (авг 2026):** PulseBoard Mini `prod-a25224bcb746` — **~91 мин** wall-clock, **~$0.18** LLM (`$0.181` в `pipeline_product_cost.json`). Позже продукт снова ушёл в `DEV_FIXING` после QA reopen; на витрине сейчас **3** `COMPLETED` лендинга, а не устойчивый eligible `full_software`. Подробности: [README FAQ](../README.md#faq--scope).
...
Зависит от состояния и политики воркера. См. admin-guide и API. Часто проще оставить `FAILED` / не доводить до конца, чем физически удалять.
