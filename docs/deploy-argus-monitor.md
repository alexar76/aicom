...
public at `https://monitor.modelmarket.dev/`). Read this before deploying
...
- **Important honest caveat:** the deployed Argus process did **not** yet
  ...
  `https://monitor.modelmarket.dev` (nginx → `:9100`). Avoid
...
- **Argus today (fleet roster):** `"argus"` remains **one** graph ball (label
  `ARGUS · N`). Connected installs heartbeat / push runs into a multi-slot
  registry (`POST /api/argus/heartbeat`, `POST /api/argus/run` with
  `instance_id`). Click the ball → **Fleet** tab: searchable, sortable,
  cursor-paginated list (Factory `Products · N` pattern). Thousands of agents
  do **not** become thousands of 3D nodes.
- True Hub-federation category `agent` is still optional future work; the
  Monitor roster is enough for presence + discovery on the map.
