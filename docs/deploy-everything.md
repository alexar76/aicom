...
`docker-compose.everything.yml` already redeclares the core Hub, Mesh, and
Monitor services. It is a sibling of `docker-compose.core.yml`; never load both
overlays in the same Compose project.
...
| 9083 | `hub` ↔ mesh's embedded hub | `docker-compose.everything.yml#hub` | the other two definitions stay down (see below) |
...
> **`ALIEN_API_TOKEN` is server-side only.** It must arrive via `env_file` **only**. Setting `${ALIEN_API_TOKEN:-}` in an `environment:` block overrides the `env_file` value with an empty string and silently breaks Monitor auth. Never expose it through a `VITE_*` build argument: the browser now exchanges an operator-entered token once at `/api/auth/session` for a signed HttpOnly cookie. Service clients continue to use Bearer authentication.
