...
| **Hub LIVE** | Place de marché fédérée sur [modelmarket.dev](https://modelmarket.dev) | `modelmarket-hub` |
| **Hub UNI** | Catalogue parallèle scellé sur [uni.modelmarket.dev](https://uni.modelmarket.dev) — [uni-and-live.fr.md](./uni-and-live.fr.md) | `deploy/uni-hub.sh` |
...
| **Alien Monitor UNI** | Graphe 3D de la bulle — [monitor.modelmarket.dev](https://monitor.modelmarket.dev/) · `:9100` · `ALIEN_MODE=universe` | `alien-monitor` |
| **Alien Monitor LIVE** | Graphe 3D de l’argent live — [`/monitor-live/`](https://monitor.modelmarket.dev/) · `:9101` · `ALIEN_MODE=real` | `alien-monitor-live` |
...
**Pas dans `deploy_ecosystem.sh` :** Metis, DIOSCURI, HELIOS — à exécuter séparément si besoin ; voir [§Ce qu'un seul VPS n'inclut pas](#9-ce-quun-seul-vps-ninclut-pas). Le hub UNI + six laboratoires sont une **seconde flotte** : `deploy/uni-hub.sh` et `deploy/uni-satellites.sh`. Ne pointez pas le moniteur UNI vers le hub live. Découpage : [uni-and-live.fr.md](./uni-and-live.fr.md).
...
L'Alien Monitor public est servi sur `https://monitor.modelmarket.dev/` (nginx fait office de proxy
