...
**Остаётся открытым (обновлено 2026-07-25):** S-H5 demo admin, C-M3 Prometheus, P2 docs/hardening.
~~S-H4 stake verify~~ — закрыто: ончейн-верификация одноразового депозита на **каждое** пополнение
в production. Новое: **KI-11** — каналы хаба офчейн и кастодиальные, `AIMarketEscrow.debitChannel`
не вызывается из рантайма (см. `docs/known-issues.md`).
...
| **Статус** | **Исправлено** (2026-07) |

Было: `stake()` записывал `tx_hash` в БД без проверки транзакции. Вместе с слабым publish gate — Sybil на listing supply.

**Что сделано:** в production (`AIFACTORY_PROD`, без `AIMARKET_SUPPLY_SECURITY_RELAXED=1`) `stake()`
требует `tx_hash` и верифицирует его ончейн (получатель платформы, сумма, подтверждения) для
**каждого** пополнения любого размера — прежнее условие `amount_usd >= min_stake_usd` было полным
обходом гейта (пополнения по $9.99 против минимума $25 не проверялись вовсе). Хеш депозита
«сжигается» отдельной строкой `supply_stakes` под зарезервированным `publisher_id`, поэтому повторно
зачесть его нельзя даже после следующего stake того же издателя. Кредиты, выданные в dev/relaxed,
помечаются сентинелом; `publish` и регистрация self-bond отказывают, пока такой баланс не списан в
ноль (иначе достаточно было накопить залог в dev и включить `AIFACTORY_PROD`).

**Осталось (ops):** `ACEX_MIN_LISTING_REVENUE_USD` > 0 на mainnet.
...
7. ~~On-chain verify для `/supply/stake`~~ — закрыто (верификация + single-use burn на каждое пополнение в production).
