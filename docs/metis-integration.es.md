...
**Metis** ([`metis/`](../metis/)) es el **nivel de cognición y verificación** del ecosistema — una
...
> 🌐 Idiomas: [English](metis-integration.md) · [Русский](metis-integration.ru.md) · **Español**
> 📖 Vista del lado de Metis: [`metis/docs/en/ECOSYSTEM.md`](../metis/docs/en/ECOSYSTEM.md)
...
cuando Metis es inalcanzable), [`metis/tests/test_ecosystem_api.py`](../metis/tests/test_ecosystem_api.py)
...
mocks, wallets), pero una única llamada al LLM no le da **ninguna señal legible por máquina de "no estoy seguro"** sobre el
...
| `AIFACTORY_METIS_GATE_MIN_SCORE` | `0.7` | umbral de verificación para el flag `verified` |
| `AIFACTORY_METIS_GATE_TIMEOUT` | `300` | timeout de la llamada de verificación (s) — debe igualar el límite del servidor Metis (300 s) |
...
en la fila de acciones (junto a pausa / prototipo). Refleja el último snapshot de `product["metis_gate"]` del
**pipeline de la factory** — no si el agente entregado llama a Metis en runtime.

| Insignia | Significado |
|---|---|
| **Factory Metis —** | Aún no hay resultado de la puerta (`metis_gate` ausente o sin `at`). Típico antes de que architect/methodologist terminen, o cuando la puerta está off y Metis no se contactó para este producto. |
| **Factory Metis ✓** | La puerta corrió en una etapa de alto riesgo y devolvió `ok: true` (comprensión verificada). |
| **Factory Metis ⚠** | La puerta corrió y devolvió `ok: false` (score bajo, `needs_clarification`, etc.). Consultivo por defecto — el pipeline sigue salvo que `AIFACTORY_METIS_GATE_BLOCK=1` haya puesto `blocked: true`. |

Pase el cursor sobre la insignia para ver stage, route, score y status cuando hay veredicto. La API del pipeline
...
[`metis/metis/api/ecosystem.py`](../metis/metis/api/ecosystem.py), opcional y autocontenida):
...
[`alien-monitor/backend/metis_status.py`](../alien-monitor/backend/metis_status.py)) para que la clave de la
API de Metis nunca llegue al navegador, y un Metis caído produce un mensaje legible en lugar de un error.
Nodo/topología: [`alien-monitor/backend/metis_layers.py`](../alien-monitor/backend/metis_layers.py).
