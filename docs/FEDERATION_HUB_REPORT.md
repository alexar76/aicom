...
**"Done" here means the module exists and its unit tests pass — not that the hub reaches it.**
Re-checked on 2026-08-28 by grepping for importers outside each module's own file and tests:
eight of these had none, so nothing a buyer or a publisher could do over HTTP touched them.
The ones still in that state are marked ⚠️ *unwired*, which is what a reader deciding whether
to rely on one needs to know.

`data_capability.py` was the sharpest case and has since been rebuilt: its `query()` returned
a formatted string made from the caller's own words, and its 70/30 revenue split was
arithmetic on a dict no money passed through. The corpus and the search were always the
publisher's to run, so the module is now a manifest builder for `/supply/register`, and the
split is enforced for real — `AIMARKET_PUBLISHER_SHARE_BPS`, credited to the publisher's
account on every completed sale.
...
| 4 | Spot auction mode | B (Wow) | `spot_auction.py` | ⚠️ Unwired (no caller) |
| 5 | Agent personas | B (Wow) | `agent_personas.py` | ⚠️ Unwired (no caller) |
| 6 | Live AI Economy stream | B (Wow) | `live-stream.html` | ✅ Done |
| 7 | Data-as-capability | C (Rev) | `data_capability.py` | ✅ Manifest builder + enforced 70/30 split |
| 8 | Capability NFT | C (Rev) | `capability_nft.py` | ⚠️ Partially wired |
| 9 | MCP-server-as-a-product | C (Rev) | `mcp_packager.py` | ⚠️ Unwired (no caller) |
| 10 | Orchestrator-as-capability | C (Rev) | `orchestrator_capability.py` | ⚠️ Unwired (no caller) |
| 11 | Streaming + per-chunk billing | D (Polish) | `streaming.py` | ✅ Done |
| 12 | Time-locked offers/promo | D (Polish) | `time_locked_promo.py` | ⚠️ Unwired (no caller) |
...
| 15 | Open dataset (anonymized) | D (Polish) | `dataset_exporter.py` | ⚠️ Unwired (no caller) |
