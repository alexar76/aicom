...
| **Hub LIVE** | [modelmarket.dev](https://modelmarket.dev) 上的联邦市场 | `modelmarket-hub` |
| **Hub UNI** | [uni.modelmarket.dev](https://uni.modelmarket.dev) 上的密封平行目录 — [uni-and-live.zh.md](./uni-and-live.zh.md) | `deploy/uni-hub.sh` |
| **Mesh** | 将产品互相接线的服务网格 API | `aicom-mesh-api` |
| **ARGUS-3** | 个人智能体 + WARDEN MCP 防火墙（参考客户端） | `argus` / `:8787` |
| **Alien Monitor UNI** | 气泡 3D 图谱 — [monitor.modelmarket.dev](https://monitor.modelmarket.dev/) · `:9100` · `ALIEN_MODE=universe` | `alien-monitor` |
| **Alien Monitor LIVE** | 实况资金 3D 图谱 — [`/monitor-live/`](https://monitor.modelmarket.dev/) · `:9101` · `ALIEN_MODE=real` | `alien-monitor-live` |
...
**不在 `deploy_ecosystem.sh` 中的：** Metis、DIOSCURI、HELIOS — 需要时单独运行；参见 [§一台 VPS 不包含什么](#9-一台-vps-不包含什么)。UNI Hub + 六个实验室是**第二套舰队**：`deploy/uni-hub.sh` 与 `deploy/uni-satellites.sh`。不要把 UNI 监视器指向实况 Hub。划分：[uni-and-live.zh.md](./uni-and-live.zh.md)。
...
- **DNS A/AAAA 记录** 指向你运行所在的主机（层级 3+）：
  ...
  - `oracles.modelmarket.dev` → **预言机主机**（`oracles.modelmarket.dev`），直接指向（不经 factory 代理）
...
公开的 Alien Monitor 在 `https://monitor.modelmarket.dev/` 提供服务（nginx 将
...
预言机运行在一台 **独立机器** 上（`oracles.modelmarket.dev`）。**`deploy_ecosystem.sh` 不会
部署预言机或 Platon** — 本 monorepo 中的 `oracles/` 和 `platon/` 是外部栈的归档镜像。
请在 Platon 主机上把它们搭建起来，然后从 Factory 主机联邦（federate）过去。

### 6.1 在 Platon 主机上（`oracles.modelmarket.dev`，以 root）
...
经济循环记录在 [`oracles/docs/en.md`](https://github.com/alexar76/oracles/blob/main/docs/en.md) 中。
...
- [`oracles/docs/en.md`](https://github.com/alexar76/oracles/blob/main/docs/en.md) — 十七个预言机与经济循环
- [AIMarket SDK 文档](https://github.com/alexar76/aimarket-sdks/blob/main/docs/en.md) · [Python agent](https://github.com/alexar76/aimarket-agent/blob/main/docs/en.md) — 消费 Hub
