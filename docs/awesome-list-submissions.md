...
| 4 | [vonzosten/awesome-LangGraph](https://github.com/vonzosten/awesome-LangGraph) | `aimarket-bridges` | PR — **📦 Packages**. Live list. `NirDiamant/awesome-LangGraph` is a stale fork. LangGraph-only copy; no monorepo tour. |
...
| awesome-LangGraph (vonzosten) | `aimarket-bridges` satellite + PyPI | Factory monorepo, CrewAI/AutoGen tour, unpublished Dev.to drafts |
...
# 4) LangGraph (live list is vonzosten, not the NirDiamant fork)
GH_TOKEN=$GH_PAT gh repo fork vonzosten/awesome-LangGraph --clone
# ... insert Packages row for aimarket-bridges
```
