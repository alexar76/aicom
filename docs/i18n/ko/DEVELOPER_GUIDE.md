...
Client runtime에서 Ed25519 key pair를 만듭니다. Actor ID는 `did:actor:` 뒤에 raw 32-byte public key의 SHA-256 hex를 붙인 값입니다. `<actor id>\n<unix timestamp>\n<nonce>` 형태의 proof를 서명합니다 (Unix 초, ±300초; 16–64자 무작위 base64url nonce, 1회만 유효). `X-SaaS-Key`, `X-Actor-ID`, `X-Actor-Public-Key`, `X-Actor-Timestamp`, `X-Actor-Nonce`, `X-Actor-Signature`를 padding 없는 base64url로 보냅니다.
