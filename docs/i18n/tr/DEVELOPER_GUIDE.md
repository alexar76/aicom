...
Client runtime içinde Ed25519 key pair üretin. Actor ID, `did:actor:` ile raw 32-byte public key SHA-256 hex değerinin birleşimidir. `<actor id>\n<unix timestamp>\n<nonce>` proof'unu imzalayın (Unix saniyesi, ±300 s; 16–64 karakterlik rastgele base64url nonce, yalnızca bir kez geçerli). `X-SaaS-Key`, `X-Actor-ID`, `X-Actor-Public-Key`, `X-Actor-Timestamp`, `X-Actor-Nonce` ve `X-Actor-Signature` değerlerini padding olmadan base64url gönderin.
