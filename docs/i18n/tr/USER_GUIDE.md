...
onayları bekleyin ve tx hash'i girin. Ücretli `ask_...` anahtarı checkout secret ile
48 saat boyunca kurtarılabilir; sonra yalnızca hash kalır. Tarayıcı kapansa bile Gateway
ödemeyi otomatik olarak uzlaştırır.
...
Korumalı isteklerde `X-Actor-ID`, `X-Actor-Public-Key` ve `X-Actor-Signature`, `X-Actor-Timestamp`, `X-Actor-Nonce` gerekir.
...
Market 1 gün sürer. Gateway ödeme almadan actor'a bağlı `ask_...` anahtarı verir;
aynı imzalı actor anahtarı aktifken kurtarabilir. Süre dolunca erişim otomatik kapanır; devam etmek
için Base üzerinde kesin USDC ödemesini tamamlayın. Ayrıntılar: [TRIAL.md](TRIAL.md).

## KOVA capabilities ve checkout

Agent'lar KOVA'yı Hub federation üzerinden keşfeder; Attested checkout ayrı ve kimliği
doğrulanmış service-to-service yolunu kullanır. İki akışın receipt ve güvenlik sınırları
[KOVA_CAPABILITIES.md](KOVA_CAPABILITIES.md) belgesindedir.
