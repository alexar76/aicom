...
| API key | A secret pass that identifies a paid SaaS entitlement. | An `ask_...` bearer credential; only a hash and prefix are stored. Paid checkout can re-derive it during a bounded 48-hour recovery window. |
...
| Trial | A short free period to test a product before paying. | An actor-bound entitlement issued once per product by the Gateway without a KOVA invoice; the same signed actor can recover its active key. |
