...
Gere Ed25519 no runtime. O actor ID é `did:actor:` mais SHA-256 hexadecimal da chave pública raw de 32 bytes. Assine a prova `<actor id>\n<unix timestamp>\n<nonce>` (segundos Unix, ±300 s; nonce base64url aleatório de 16–64 caracteres, válido uma única vez). Envie `X-SaaS-Key`, `X-Actor-ID`, `X-Actor-Public-Key`, `X-Actor-Timestamp`, `X-Actor-Nonce` e `X-Actor-Signature` em base64url sem padding.
