...
在 client runtime 中生成 Ed25519 key pair。Actor ID 是 `did:actor:` 加 raw 32-byte public key 的 SHA-256 hex。对 proof `<actor id>\n<unix timestamp>\n<nonce>` 签名（Unix 秒，±300 秒；16–64 字符的随机 base64url nonce，仅接受一次）。使用无 padding 的 base64url 发送 `X-SaaS-Key`、`X-Actor-ID`、`X-Actor-Public-Key`、`X-Actor-Timestamp`、`X-Actor-Nonce` 与 `X-Actor-Signature`。
