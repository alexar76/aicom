...
Générez Ed25519 dans le runtime client. L’actor ID vaut `did:actor:` suivi du SHA-256 hex de la clé publique brute de 32 octets. Signez la preuve `<actor id>\n<unix timestamp>\n<nonce>` (secondes Unix, ±300 s ; nonce base64url aléatoire de 16 à 64 caractères, valable une seule fois). Envoyez `X-SaaS-Key`, `X-Actor-ID`, `X-Actor-Public-Key`, `X-Actor-Timestamp`, `X-Actor-Nonce` et `X-Actor-Signature` en base64url sans padding.
