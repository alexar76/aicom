...
Передавайте `X-SaaS-Key`, `X-Actor-ID`, `X-Actor-Public-Key`, `X-Actor-Timestamp`, `X-Actor-Nonce` и `X-Actor-Signature`. Подпись покрывает строку `<actor id>\n<unix timestamp>\n<nonce>`: время в секундах Unix (окно ±300 с) и случайный nonce из 16–64 символов base64url; каждый proof принимается один раз. Никогда не передавайте private key, seed phrase или checkout token в API продукта. Actor identity можно использовать повторно, но политика хранения каждого секрета должна быть явной.
