...
Erzeugen Sie Ed25519 in der Client-Runtime. Die Actor-ID ist `did:actor:` plus SHA-256-Hexdigest des rohen 32-Byte Public Keys. Signieren Sie den Proof `<actor id>\n<unix timestamp>\n<nonce>` (Unix-Sekunden, ±300 s; zufälliger base64url-Nonce mit 16–64 Zeichen, einmal gültig). Senden Sie `X-SaaS-Key`, `X-Actor-ID`, `X-Actor-Public-Key`, `X-Actor-Timestamp`, `X-Actor-Nonce` und `X-Actor-Signature` als base64url ohne Padding.
