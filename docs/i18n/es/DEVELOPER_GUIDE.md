...
Genera Ed25519 dentro del runtime. El actor ID es `did:actor:` más SHA-256 hexadecimal de la clave pública raw de 32 bytes. Firma la prueba `<actor id>\n<unix timestamp>\n<nonce>` (segundos Unix, ±300 s; nonce base64url aleatorio de 16–64 caracteres, válido una sola vez). Envía `X-SaaS-Key`, `X-Actor-ID`, `X-Actor-Public-Key`, `X-Actor-Timestamp`, `X-Actor-Nonce` y `X-Actor-Signature` en base64url sin padding.
