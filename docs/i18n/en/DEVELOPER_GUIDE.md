...
Generate an Ed25519 key pair inside the client runtime. Encode the raw 32-byte public key with unpadded base64url. The actor ID is `did:actor:` followed by the SHA-256 hex digest of that raw public key. Sign the proof `<actor id>\n<unix timestamp>\n<nonce>` — a newline-joined string of the actor ID, the current Unix time in seconds and a random 16–64 character base64url nonce — and send the signature as unpadded base64url. A proof is accepted within ±300 seconds and exactly once; the signature never covers the request itself, so it survives the product proxies.

Send `X-SaaS-Key`, `X-Actor-ID`, `X-Actor-Public-Key`, `X-Actor-Timestamp`, `X-Actor-Nonce` and `X-Actor-Signature`. Never send the private key, seed phrase or checkout token to a product API. Reuse the actor identity, but create a fresh request policy around every secret.
