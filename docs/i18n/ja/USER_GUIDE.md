...
tx hash を入力します。有料の `ask_...` キーは checkout secret により48時間
再取得できます。その後は hash のみが残ります。ブラウザを閉じても Gateway が
支払い状態を自動照合します。キー管理には
...
`X-Actor-Signature`, `X-Actor-Timestamp`, `X-Actor-Nonce` が必要です。秘密鍵はクライアント内に保持します。
...
Expert Market は1日間です。支払いなしで actor に紐づく `ask_...` キーが発行され、
同じ署名済み actor は有効期間中に再取得できます。期間終了後は自動失効し、継続には Base の正確な USDC
決済が必要です。詳しくは [TRIAL.md](TRIAL.md) を参照してください。

## KOVA capabilities と checkout

Agent は Hub federation から KOVA を発見します。Attested checkout は別の認証済み
service-to-service 経路を利用します。2つの経路と security boundary は
[KOVA_CAPABILITIES.md](KOVA_CAPABILITIES.md) にあります。
