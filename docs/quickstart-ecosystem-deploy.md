...
| **Hub LIVE** | Federated marketplace at [modelmarket.dev](https://modelmarket.dev) | `modelmarket-hub` |
| **Hub UNI** | Sealed parallel catalogue at [uni.modelmarket.dev](https://uni.modelmarket.dev) — [uni-and-live.md](./uni-and-live.md) | `deploy/uni-hub.sh` |
...
| **Alien Monitor UNI** | 3D graph of the bubble — [monitor.modelmarket.dev](https://monitor.modelmarket.dev/) · `:9100` · `ALIEN_MODE=universe` | `alien-monitor` |
| **Alien Monitor LIVE** | 3D graph of live money — [`/monitor-live/`](https://monitor.modelmarket.dev/) · `:9101` · `ALIEN_MODE=real` | `alien-monitor-live` |
...
**Not in `deploy_ecosystem.sh`:** Metis, DIOSCURI, HELIOS — run separately if needed; see [§What one VPS does not include](#9-what-one-vps-does-not-include). UNI hub + six labs are a **second fleet**: `deploy/uni-hub.sh` and `deploy/uni-satellites.sh`. Do not point the UNI monitor at the live hub. Split: [uni-and-live.md](./uni-and-live.md).
...
- **DNS A/AAAA records** pointing at the host you run on (Level 3+):
  ...
  - `oracles.modelmarket.dev` → **Oracle host** (`oracles.modelmarket.dev`), directly (no factory proxy)
...
The public Alien Monitor is served at `https://monitor.modelmarket.dev/` (nginx proxies
...
The oracles run on a **separate machine** (`oracles.modelmarket.dev`). **`deploy_ecosystem.sh` does NOT
...
### 6.1 On the Platon host (`oracles.modelmarket.dev`, as root)
...
economy loop are documented in [`oracles/docs/en.md`](https://github.com/alexar76/oracles/blob/main/docs/en.md).
...
- [`oracles/docs/en.md`](https://github.com/alexar76/oracles/blob/main/docs/en.md) — the seventeen oracles and the economy loop
- [AIMarket SDK docs](https://github.com/alexar76/aimarket-sdks/blob/main/docs/en.md) · [Python agent](https://github.com/alexar76/aimarket-agent/blob/main/docs/en.md) — consume the Hub
