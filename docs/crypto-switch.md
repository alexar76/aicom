...
## Crypto on is not the same as selling

The master switch enables the on-chain machinery. It does **not** by itself put a
price on a federated capability — that is a second, separate switch:

> [!WARNING]
> **`AIMARKET_SELLS_FOR` is unset by default, and setting it makes 42 currently
> free capabilities paid at once.** With it unset the hub charges nothing for a
> federated invoke: no channel required, no hold taken, no routing fee. With a
> peer URL prefix listed, every capability from that peer becomes priced and an
> invoke without `X-Payment-Channel` is answered `402 payment_required` — so
> existing free callers, including any agent, bridge or MCP client already using
> them, start failing that same minute. There is no partial rollout and no grace
> period.

Selling on a peer's behalf has to be **declared** rather than inferred: a peer
that invoices out of band also answers `200`, so treating "the peer didn't charge
us" as "we may charge" would bill the buyer twice.

The oracle side has its own switch (`ORACLE_PAID_TIER_SECRET`), and it must be set
**first** — otherwise the hub demands payment for work the oracle would still
refuse above its free ceiling. Full sequence and rationale:
[free-and-paid-tiers](free-and-paid-tiers.md).
...
fully functional on the free tier — see
[free-and-paid-tiers](free-and-paid-tiers.md) for exactly what that tier includes
and what bounds it.
