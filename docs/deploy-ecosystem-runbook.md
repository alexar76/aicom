...
**Прочитать до первого деплоя:** [`operations-traps.md`](./operations-traps.md) — измеренные на живом проде случаи, когда проверка зелёная, деплой отчитался об успехе, а сломано. В частности: рестарт хаба уносит THEMIS и сам её не поднимает (T-2), сборка из рабочего дерева даёт другое приложение (T-6), дубликаты env — docker берёт последний (T-5).
