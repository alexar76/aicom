...
> 🌐 语言： [English](./product-pnl.md) · [Русский](./product-pnl.ru.md) · [Français](./product-pnl.fr.md) · [Español](./product-pnl.es.md) · **中文**

自主工厂的逐产品盈亏（P&L）。它把工厂本就记录的两半 — **收入**（已付款订单）与
**推理 COGS**（每个产品的 LLM 花费）— 合并为一个实时、只读的单位经济视图：逐产品的
收入、成本、毛利/毛利率、ROI 和成本回收，外加一份组合汇总。

> 诚实优先于精确。成本是一个*估算值*（按令牌定价），FX 由 env 配置。这是运营层面的
...
- **是估算，不是计费。** `inference_cost_usd` 使用令牌定价
