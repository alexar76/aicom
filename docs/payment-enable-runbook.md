...
Everything below was executed. Live state now:
...
- [`aimarket_hub/config.py`](../aimarket-hub/aimarket_hub/config.py) →
  `is_dev_chain_address()`, used by `payment_readiness()` and by the standalone
  fallback in [`cli.py`](../aimarket-hub/aimarket_hub/cli.py), which previously
...
[`api_models.py`](../aimarket-hub/aimarket_hub/api_models.py):
...
path in [`channels.py`](../aimarket-hub/aimarket_hub/channels.py) verifies on-chain only
