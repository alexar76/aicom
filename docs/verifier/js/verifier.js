...
async function verifyProvenanceReceipt(receipt) {
  ...
  if (!receipt || typeof receipt !== 'object') {
    errors.push('Invalid receipt: not a JSON object');
  ...
  if (!subject) errors.push('Missing credentialSubject');
  if (!proof) errors.push('Missing proof block');
  if (!issuer) errors.push('Missing issuer block');
  ...
  if (!jwk) {
    errors.push('Missing issuer.publicKeyJwk');
  } else {
    try {
      const publicKeyBytes = extractPublicKeyFromJWK(jwk);
      if (!publicKeyBytes) {
        errors.push('Invalid JWK: must be OKP Ed25519');
      } else {
  ...
  if (subject.timestamp) {
    ...
    if (isNaN(ts)) {
      ...
      let tsValid = true;
      if (ts > now + 300000) {
        errors.push('Timestamp is in the future (clock skew > 5 min)');
        tsValid = false;
      }
      if (ts < now - MAX_AGE_MS) {
        errors.push('Timestamp is too old (> 90 days)');
        tsValid = false;
      }
  ...
  if (subject.inputHash?.value && hashHex.test(subject.inputHash.value)) {
    ...
    errors.push('Invalid input hash format (expected 64-char hex)');
    hashOk = false;
  } else {
    errors.push('Missing inputHash');
    hashOk = false;
  }
  if (subject.outputHash?.value && hashHex.test(subject.outputHash.value)) {
    // ok
  } else if (subject.outputHash?.value) {
    errors.push('Invalid output hash format (expected 64-char hex)');
    hashOk = false;
  } else {
    errors.push('Missing outputHash');
    hashOk = false;
  }
  ...
  if (subject.parentReceipts && subject.parentReceipts.length > 0) {
    ...
    if (bad.length > 0) {
      errors.push('Invalid parent receipt format: ' + bad.slice(0, 3).join(', '));
    }
  }
  ...
  if (subject.teeAttestation) {
    ...
    if (hasSig && teeValid) {
      try {
        ...
        if (issuerKeyBytes) {
          ...
          teeValid = sigValid;
          if (!sigValid) {
            errors.push('TEE attestation signature verification failed');
          }
        }
      } catch(e) {
        teeValid = false;
        errors.push('TEE attestation verification error');
      }
    }
    checks.push({
    ...
    if (!teeValid && !errors.some(e => e.includes('TEE'))) {
      errors.push('TEE attestation is invalid or incomplete');
    }
  }
