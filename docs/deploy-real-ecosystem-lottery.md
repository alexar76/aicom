...
1. **Deploy the lottery contract.**
   ...
   - The scripts default `OWNER` to the demonstration wallet
     ...
     The recorded `0x5F3D…` deployment has the wallet-only 2026-09-10 WorkSeat ABI;
     deploy the current source before enabling federated work seats because it adds
     the stable Hub `participantId` anti-Sybil binding.
