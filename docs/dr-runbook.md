...
2. Immediate: `./scripts/prune_pipeline_db_backups.sh` removes all but the newest
   `AIFACTORY_SQLITE_BACKUP_KEEP` (default 2) `pipeline.db.bak.*` snapshots.
   *(This step used to name `cleanup_old_backups.sh`, which has never existed in the repo —
   an on-call operator following it hit "no such file" at the one moment it mattered.)*
