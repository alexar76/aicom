...
//!
//! LIVENESS: `cancel_round` is authority-only, and `refund_ticket` requires `Cancelled`, so
//! every exit from a stuck round once depended on one key. `cancel_expired_round` is the
//! permissionless counterpart: past the fulfill window (or past the authority's grace period
//! on a still-Open round) anyone may flip the round to `Cancelled` so participants can refund
//! themselves. A lost authority key can no longer strand lamports in the vault.
//!
//! RESIDUAL — ORACLE ABORT BIAS (not fixed, by design decision): the oracle learns the seed
//! and the pinned slot hash before it must submit, so it can compute the winner privately and
//! then simply NOT submit when it dislikes the result. The window lapses, the round cancels,
//! everyone is refunded, and a fresh round is played. It cannot CHOOSE a winner — H17 and the
//! `seed_commitment` prevent that — but it can veto outcomes at no cost, which biases the
//! distribution in its favour whenever it has a stake in who wins. Closing this needs an
//! economic or cryptographic commitment that this program deliberately does not invent:
//! either a posted oracle bond slashed on a missed window (decide who funds it and where the
//! slash goes), or a VRF whose output no single party can withhold (Switchboard / ORAO).
...
/// How long after `entries_close` the authority keeps the exclusive right to run the draw.
/// Past it, an Open round is abandoned and anyone may cancel it so participants can refund.
/// Deliberately generous: inside this window only the authority can end the round, so a
/// griefer cannot cancel a live round out from under a draw that is about to happen.
const OPEN_ABANDON_GRACE_SECS: i64 = 7 * 86_400;
...
pub mod aimarket_lottery {
    ...
    /// Cancel a round the authority has stopped ending — **permissionless**, so refunds never
    /// depend on one key staying alive.
    ///
    /// `cancel_round` above is authority-only and `refund_ticket` requires `Cancelled`, so
    /// every path out of a stuck round used to run through the authority: an oracle that
    /// missed its window left participants' lamports in the vault with no way to reclaim them,
    /// and a lost authority key locked them permanently. The module header already promises
    /// "miss the window → cancel + refunds"; this is what makes that true without an operator.
    ///
    /// Two demonstrably-abandoned shapes qualify, both proven by on-chain clocks rather than
    /// by anyone's assertion:
    ///   - `Drawing` past `draw_slot + DRAW_FULFILL_WINDOW` — the oracle's slot hash is gone,
    ///     so `fulfill_draw` can no longer succeed for this round by construction (H17).
    ///   - `Open` past `entries_close + OPEN_ABANDON_GRACE_SECS` — entries shut long ago and
    ///     the authority never closed the round.
    ///
    /// Settling is still impossible from here: this only flips the status, and the vault is
    /// emptied by each participant's own `refund_ticket`. There is no admin drain (anti-rug).
    pub fn cancel_expired_round(ctx: Context<CancelExpiredRound>) -> Result<()> {
        let clock = Clock::get()?;
        let round = &mut ctx.accounts.round;
        let s = round.status();
        require!(
            s == RoundStatus::Open || s == RoundStatus::Drawing,
            LotteryError::WrongStatus
        );
        require!(
            round_is_abandoned(
                s,
            ...
            LotteryError::DrawWindowNotExpired
        );
        round.status = RoundStatus::Cancelled as u8;
        emit!(RoundCancelled { round_id: round.round_id });
        Ok(())
...
/// Whether a round is demonstrably abandoned and may therefore be cancelled by anyone.
///
/// Pure, so the window arithmetic is unit-testable without a validator. `Drawing` uses the
/// slot clock because that is what `fulfill_draw` is bound to; `Open` uses the wall clock
/// because that is what `entries_close` is expressed in. Any other status is never abandoned:
/// `Settled` still has a claimable prize and `Cancelled` is already there.
fn round_is_abandoned(
    status: RoundStatus,
    draw_slot: u64,
    entries_close: i64,
    now_slot: u64,
    ...
    match status {
        // Strictly past the window fulfill_draw itself enforces, so a cancel can never race
        // a draw that would still have been accepted.
        RoundStatus::Drawing => now_slot > draw_slot.saturating_add(DRAW_FULFILL_WINDOW),
        RoundStatus::Open => match entries_close.checked_add(OPEN_ABANDON_GRACE_SECS) {
            Some(deadline) => now_ts > deadline,
            // A round whose grace deadline overflows i64 is ~292 billion years out; treat it
            // as never abandoned rather than wrapping into "cancellable immediately".
            None => false,
        },
        _ => false,
    }
...
/// No `authority` and no `Signer` at all — the transaction's fee payer is the only signature
/// involved, exactly like `RefundTicket`. Whether the cancel is allowed is decided purely by
/// the round's own state against the on-chain clock.
#[derive(Accounts)]
pub struct CancelExpiredRound<'info> {
    #[account(mut, seeds = [b"round", round.round_id.to_le_bytes().as_ref()], bump = round.bump)]
    pub round: Account<'info, Round>,
}
...
pub enum LotteryError {
    ...
    TooLate,
    #[msg("round is not abandoned yet — the authority may still end it")]
    DrawWindowNotExpired,
...
mod tests {
    ...
    #[test]
    fn drawing_round_is_abandoned_only_strictly_after_the_fulfill_window() {
        let draw_slot = 1_000u64;
        let last_ok = draw_slot + DRAW_FULFILL_WINDOW; // fulfill_draw still accepts this slot
        // Must not be cancellable while the oracle could still legitimately fulfill —
        // otherwise a cancel could race a valid draw.
        assert!(!round_is_abandoned(RoundStatus::Drawing, draw_slot, 0, draw_slot, 0));
        assert!(!round_is_abandoned(RoundStatus::Drawing, draw_slot, 0, last_ok, 0));
        // One slot past it the draw is impossible by construction, so anyone may cancel.
        assert!(round_is_abandoned(RoundStatus::Drawing, draw_slot, 0, last_ok + 1, 0));
    }

    #[test]
    fn open_round_is_abandoned_only_after_the_authority_grace_period() {
        let entries_close = 1_700_000_000i64;
        let deadline = entries_close + OPEN_ABANDON_GRACE_SECS;
        // Inside the grace period only the authority may end the round — no griefing cancels.
        assert!(!round_is_abandoned(RoundStatus::Open, 0, entries_close, 0, entries_close));
        assert!(!round_is_abandoned(RoundStatus::Open, 0, entries_close, 0, deadline));
        assert!(round_is_abandoned(RoundStatus::Open, 0, entries_close, 0, deadline + 1));
        // No overflow-driven "cancellable immediately" for an absurd entries_close.
        assert!(!round_is_abandoned(RoundStatus::Open, 0, i64::MAX, 0, i64::MAX));
    }

    #[test]
    fn settled_and_cancelled_rounds_are_never_abandoned() {
        // A settled round still owes a claimable prize; re-cancelling it would strand it.
        for status in [RoundStatus::Settled, RoundStatus::Cancelled, RoundStatus::None] {
            assert!(!round_is_abandoned(status, 0, 0, u64::MAX, i64::MAX));
        }
    }
