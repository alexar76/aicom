...
//!
//! MAINNET HARDENING (deliberately deferred — needs a validator to test, not localnet/UNI
//! play-money): the round vault is a bare lamport PDA. Between `claim_prize` and
//! `withdraw_residual` it can be left holding dust below the rent-exempt minimum and be
//! reaped. For a value deployment, fund the vault's rent-exempt minimum at `open_round`
//! (tracked as non-distributable) and close the vault after the final withdrawal so it
//! never sits sub-rent-exempt. An unclaimed prize is NOT lost — like the EVM contract it
//! stays claimable by the winner indefinitely.
...
pub mod aimarket_lottery {
    ...
    pub fn close_round(ctx: Context<CloseRound>, seed_commitment: [u8; 32]) -> Result<()> {
        ...
        round.close_slot = Clock::get()?.slot;
    ...
    /// with a recent slot hash unknown at commit time, then `target = random_word %
    /// total_weight` must fall inside the supplied ticket's cumulative weight range — so
    /// the only valid winner is the actual participant who owns that range. A malicious
    /// oracle can neither grind the seed nor name an arbitrary winner.
    pub fn fulfill_draw(ctx: Context<FulfillDraw>, seed: [u8; 32]) -> Result<()> {
        ...
        // Mix a recent slot hash (finalized AFTER the commit) so neither party controls
        // the result. The draw must be on a later slot than the close.
        let now_slot = Clock::get()?.slot;
        require!(now_slot > round.close_slot, LotteryError::TooEarly);
...
/// (layout: [u64 num_entries][(u64 slot, [u8;32] hash) ...], newest first).
fn recent_slot_hash(slot_hashes: &AccountInfo) -> Result<[u8; 32]> {
    ...
    let mut out = [0u8; 32];
    out.copy_from_slice(&data[16..48]); // skip count(8) + newest entry's slot(8) → its hash(32)
    Ok(out)
}
...
pub struct FulfillDraw<'info> {
    ...
    /// CHECK: the SlotHashes sysvar — address-checked in `recent_slot_hash`; supplies a
    /// recent slot hash (unknown at commit) to make the draw unbiasable.
...
pub enum LotteryError {
    ...
    BadReveal,
    #[msg("draw too early — must be a later slot than close")]
    TooEarly,
