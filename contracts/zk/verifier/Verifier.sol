...
contract PlonkVerifier {
    ...
    uint256 constant Qmx  = 12070235208530808673063424470310882359269953129623736749558698205487675644091;
    uint256 constant Qmy  = 15474922588620781245471908391483258476072485229609505289166483852369008072215;
    uint256 constant Qlx  = 5819677224141316740038678733827287684230417011203393970057409010868896949939;
    uint256 constant Qly  = 140909198739596980098291446979244318146465074454530609873211306403546133772;
    uint256 constant Qrx  = 9007360723284823063436617381651265048385079742780375063205343296800905526776;
    uint256 constant Qry  = 3575770479630807866317718127596240814614015114896870356637753998841479021553;
    uint256 constant Qox  = 19165940851926946578503265726105900353974056795656413337221782829573823901879;
    uint256 constant Qoy  = 4144129708651588539871891506814817669419802106655452349111719359226603334451;
    uint256 constant Qcx  = 21789164717279730721310358113427992789276445762211520991477098820400701075452;
    uint256 constant Qcy  = 14616277636418995675470957673419439553147076444697772033177082294673466347488;
    uint256 constant S1x  = 4434301952866548083814442956889931802863742874574636042684634666735928309716;
    uint256 constant S1y  = 20139471127131595252815738128582728847153193605045860445048049652610301135472;
    uint256 constant S2x  = 11037476048906396626148587340068466959346573779912270280779881245456911083404;
    uint256 constant S2y  = 16960377117457295628682040697688553266128763646309800989689123075989727807547;
    uint256 constant S3x  = 19737283067814119697426398121715563191569598573409131030883316792261098734042;
    uint256 constant S3y  = 11474721772566111455273910099602728646026593522648266042526030928468784424299;
    ...
    // `_pubSignals` carries exactly `nPublic` (3) public inputs, in the order the
    // `input_validity` circuit declares them (see contracts/zk/circuits/
    // input_validity.circom `main {public [...]}` and contracts/zk/README.md):
    //   [0] inputCommitment  — Poseidon(inputValue, nullifierSalt)
    //   [1] nullifier        — Poseidon(inputValue, schemaHash)
    //   [2] schemaHash       — schema identifier
    // Each must be a reduced field element (< q); this is enforced in
    // calculateChallenges via checkField before the values enter Fiat-Shamir.
    function verifyProof(uint256[24] calldata _proof, uint256[3] calldata _pubSignals) public view returns (bool) {
        assembly {
            ...
            // Reject malformed calldata before doing any verification work.
            // The two fixed-size arrays are ABI-encoded inline, so the only
            // valid payload size is: 4 (selector) + 24*32 (proof) + nPublic*32
            // (public signals) = 4 + 768 + 96 = 868 bytes. Anything else means a
            // truncated proof or an extra/missing public input; bail out instead
            // of reading attacker-controlled garbage past the declared arrays.
            function checkInput() {
                if iszero(eq(calldatasize(), add(772, mul(nPublic, 32)))) {
                    mstore(0, 0)
                    return(0, 0x20)
                }
            }
            ...
            function calculateChallenges(pMem, pPublic) {
                ...
                // Public inputs must be reduced field elements (< q) before they
                // are absorbed into the Fiat-Shamir transcript and used in
                // calculatePI. An unreduced signal would let a prover grind two
                // distinct calldata encodings to the same challenge / public
                // input, breaking soundness, so validate every public signal here.
                checkField(calldataload(add(pPublic, 0)))
                checkField(calldataload(add(pPublic, 32)))
                checkField(calldataload(add(pPublic, 64)))
