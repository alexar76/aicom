...
p = make_zk_prover()
schema = {"type":"object"}
proof = p.prove_input("test-cap", schema, {"hello":"world"})
print("Proof generated:", proof.input_commitment[:20], "…")
# The schema is REQUIRED at verification: `schemaHash` is a public signal the prover
# picks, and the nullifier is derived from it, so the verifier must re-derive the
# expected value or replay protection cannot bite. Omit it and the proof is refused.
r = p.verify_input_proof(proof, "test-cap", p.signer.public_key_b64, schema)
print("Verify:", r)
...
The on-chain verifier (`verifier/Verifier.sol` → `PlonkVerifier`) is deployed on Basescan
(chainId 8453):
[`0x1914D8a04dd65c6d8C888B98A31757F79B8e85c5`](https://basescan.org/address/0x1914D8a04dd65c6d8C888B98A31757F79B8e85c5),
owned/deployed by the demo wallet `0x1218ff36C5d2e3B6A565CdB1A8B1AcCFc606Ad0a` (redeployed
2026-07-26). Full context + every transaction: [../../docs/onchain-journal.md](../../docs/onchain-journal.md).
