...
> **Scope:** this applies **only to the optional Groth16 backend** (`npm run setup:groth16`). The **default backend is PLONK**, which uses the universal Hermez powers-of-tau SRS — **no per-circuit ceremony is required**. Follow this guide only if you deliberately chose the Groth16 path.
