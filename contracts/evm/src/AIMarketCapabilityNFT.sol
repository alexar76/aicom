 ...
 *
 *      `mint` uses `_safeMint` (receiver callback) and `consumeCall` mutates
 *      entitlement state — both are `nonReentrant` (audit M16).
 */
contract AIMarketCapabilityNFT is ERC721, ERC721Burnable, Ownable2Step, Pausable, ReentrancyGuard {
    struct Entitlement {
