...
class DirectorWorker:
    ...
    async def _benchmark_league_loop(self):
        ...
        while self._running:
            try:
                ...
                if self._benchmark_enabled and not is_factory_on_hold():
                    now = time.time()
    ...
    async def run_full_analysis(self) -> dict:
        ...
        if is_factory_on_hold():
            logger.info(
                "Factory on hold — skipping Director analysis cycle (no LLM / auto-actions)"
            )
            return {
                "success": True,
                "skipped": True,
                "reason": "factory_on_hold",
                "timestamp": time.time(),
            }
        ...
        try:
            ...
            # `analysis['alerts']` does not exist: the analyzer nests them under
            # `pipeline_analysis.alerts` and `resource_analysis.alerts`. So this line printed
            # `alerts=0` unconditionally, for as long as it has existed — verified by running
            # the analyzer with completion_rate=40 and timeout_tasks=2, which produces three
            # real alerts and still logged zero. A health line that reports "critical" next to
            # "alerts=0" tells the operator there is a problem and nothing about what it is.
            _alerts = [
                *(analysis.get("pipeline_analysis", {}).get("alerts") or []),
                *(analysis.get("resource_analysis", {}).get("alerts") or []),
            ]
            logger.info(
                "Analysis complete: health=%s, alerts=%d%s",
                analysis.get("overall_health"),
                len(_alerts),
                (" — " + ", ".join(
                    f"{a.get('metric')}={a.get('value')}" for a in _alerts[:5]
                )) if _alerts else "",
            )
    ...
    async def run_discovery_cycle(self) -> dict:
        ...
        if is_factory_on_hold():
            logger.info(
                "Factory on hold — skipping discovery cycle (no market_research LLM)"
            )
            return {
                "success": True,
                "skipped": True,
        ...
        try:
            ...
            from orchestrator.autonomy_bridge import resolve_director_pending

            if is_full_autonomy():
                n = await resolve_director_pending(data_root(), self._get_llm_router())
                if n:
                    logger.info("Full autonomy resolved %s pending Director decision(s)", n)
        except Exception as exc:
            logger.debug("resolve_director_pending skipped: %s", exc)
        try:
            from core.paths import data_root
            from core.playbook import llm_refine_enabled, refine_playbook_claims

            if llm_refine_enabled():
                n = await refine_playbook_claims(data_root(), llm_router=self._get_llm_router())
                if n:
                    logger.info("Playbook: refined %s rule claim(s) via LLM", n)
        except Exception as exc:
            logger.debug("playbook refine skipped: %s", exc)
        try:
            ...
            now = time.time()
            if self._discovery_auto_enqueue and not is_factory_on_hold():
        ...
        while self._running:
            if signal_path.exists():
                try:
                    ...
                    # Run analysis (skipped when factory on hold — see run_full_analysis)
                    ...
                    if (trigger_data.get("auto_pipeline") or self._auto_pipeline_enabled) and not is_factory_on_hold():
                        await self._auto_create_product()
                    if trigger_data.get("benchmark_now") is True and not is_factory_on_hold():
                        await self._run_benchmark_league_once()
    ...
    async def _auto_create_product(self):
        ...
        if is_factory_on_hold():
            logger.info(
                "Factory on hold — skipping auto product creation (discovery/market_research)"
            )
            return
        try:
            from director.discovery_pipeline import DiscoveryPipeline
            ...
            try:
                from core.learning_objective import assign_learning_cohort

                assign_learning_cohort(product)
            except Exception:
                pass
